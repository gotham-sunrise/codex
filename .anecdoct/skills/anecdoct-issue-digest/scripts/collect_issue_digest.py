#!/usr/bin/env python3
"""Collect recent lambogenius/anecdoct issue activity for owner-focused digests."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a Anecdoct owner digest."
    )
    parser.add_argument(
        "--repo", default="lambogenius/anecdoct", help="OWNER/REPO, default lambogenius/anecdoct"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(anecdoct(?: desktop| app|\.app| cli)?|desktop|windows anecdoct app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
