...
def _installed_anecdoct_path() -> Path:
    try:
        from codex_cli_bin import bundled_codex_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Anecdoct runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set AppServerConfig.anecdoct_bin "
            "explicitly."
        ) from exc
...
class AnecdoctBinResolverOps:
    installed_anecdoct_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_anecdoct_bin_resolver_ops() -> AnecdoctBinResolverOps:
    return AnecdoctBinResolverOps(
        installed_anecdoct_path=_installed_anecdoct_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_anecdoct_bin(config: "AppServerConfig", ops: AnecdoctBinResolverOps) -> Path:
    if config.anecdoct_bin is not None:
        anecdoct_bin = Path(config.anecdoct_bin)
        if not ops.path_exists(anecdoct_bin):
            raise FileNotFoundError(
                f"Anecdoct binary not found at {anecdoct_bin}. Set AppServerConfig.anecdoct_bin "
                "to a valid binary path."
            )
        return anecdoct_bin

    return ops.installed_anecdoct_path()


def _resolve_anecdoct_bin(config: "AppServerConfig") -> Path:
    return resolve_anecdoct_bin(config, _default_anecdoct_bin_resolver_ops())


@dataclass(slots=True)
class AppServerConfig:
    anecdoct_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "anecdoct_python_sdk"
    client_title: str = "Anecdoct Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class AppServerClient:
    """Synchronous typed JSON-RPC client for `anecdoct app-server` over stdio."""
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            anecdoct_bin = _resolve_anecdoct_bin(self.config)
            args = [str(anecdoct_bin)]
...
def default_anecdoct_home() -> str:
    return str(Path.home() / ".anecdoct")
