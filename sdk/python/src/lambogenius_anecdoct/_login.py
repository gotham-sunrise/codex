...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncAnecdoct needed by async login handles."""
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Anecdoct.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Anecdoct.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncAnecdoct.login_chatgpt()`."""

    _anecdoct: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._anecdoct._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._anecdoct._ensure_initialized()
        return await self._anecdoct._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncAnecdoct.login_chatgpt_device_code()`."""
