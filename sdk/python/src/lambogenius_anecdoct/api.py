...
class Anecdoct:
    ...
    # BEGIN GENERATED: Anecdoct.flat_methods
    def thread_start(
        self,
        *,
    ...
    # END GENERATED: Anecdoct.flat_methods
...
class AsyncAnecdoct:
    """Async mirror of :class:`Anecdoct`.

    Prefer ``async with AsyncAnecdoct()`` so initialization and shutdown are
    ...
    async def __aenter__(self) -> "AsyncAnecdoct":
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncAnecdoct is not initialized yet. Prefer `async with AsyncAnecdoct()`; "
    ...
    # BEGIN GENERATED: AsyncAnecdoct.flat_methods
    ...
    # END GENERATED: AsyncAnecdoct.flat_methods
...
class AsyncThread:
    ...
    async def turn(
        ...
        *,
        ...
        await self._anecdoct._ensure_initialized()
        wire_input = _to_wire_input(_normalize_run_input(input))
        approval_policy, approvals_reviewer = _approval_mode_override_settings(approval_mode)
        params = TurnStartParams(
            thread_id=self.id,
        ...
        turn = await self._anecdoct._client.turn_start(
            self.id,
            wire_input,
            params=params,
        )
        return AsyncTurnHandle(self._anecdoct, self.id, turn.turn.id)

    # END GENERATED: AsyncThread.flat_methods

    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        await self._anecdoct._ensure_initialized()
        return await self._anecdoct._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._anecdoct._ensure_initialized()
        return await self._anecdoct._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._anecdoct._ensure_initialized()
        return await self._anecdoct._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: RunInput) -> TurnSteerResponse:
        await self._anecdoct._ensure_initialized()
        return await self._anecdoct._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        await self._anecdoct._ensure_initialized()
        return await self._anecdoct._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        """Yield only notifications routed to this async turn handle."""
        await self._anecdoct._ensure_initialized()
        self._anecdoct._client.register_turn_notifications(self.id)
        try:
            while True:
                event = await self._anecdoct._client.next_turn_notification(self.id)
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._anecdoct._client.unregister_turn_notifications(self.id)
