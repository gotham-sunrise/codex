#!/usr/bin/env python3
"""Install Anecdoct native binaries (Rust CLI, bwrap, and ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/lambogenius/anecdoct/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. anecdoct-<target>.zst)
...
BINARY_COMPONENTS = {
    "bwrap": BinaryComponent(
        ...
        dest_dir="anecdoct-resources",
        binary_basename="bwrap",
        targets=LINUX_TARGETS,
    ),
    "anecdoct": BinaryComponent(
        artifact_prefix="anecdoct",
        dest_dir="anecdoct",
        binary_basename="anecdoct",
    ),
    "anecdoct-responses-api-proxy": BinaryComponent(
        artifact_prefix="anecdoct-responses-api-proxy",
        dest_dir="anecdoct-responses-api-proxy",
        binary_basename="anecdoct-responses-api-proxy",
    ),
    "anecdoct-windows-sandbox-setup": BinaryComponent(
        artifact_prefix="anecdoct-windows-sandbox-setup",
        dest_dir="anecdoct",
        binary_basename="anecdoct-windows-sandbox-setup",
        targets=WINDOWS_TARGETS,
    ),
    "anecdoct-command-runner": BinaryComponent(
        artifact_prefix="anecdoct-command-runner",
        dest_dir="anecdoct",
        binary_basename="anecdoct-command-runner",
        targets=WINDOWS_TARGETS,
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native Anecdoct binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to bwrap, anecdoct, anecdoct-windows-sandbox-setup,"
            " anecdoct-command-runner, and rg."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    anecdoct_cli_root = (args.root or ANECDOCT_CLI_ROOT).resolve()
    vendor_dir = anecdoct_cli_root / VENDOR_DIR_NAME
    vendor_dir.mkdir(parents=True, exist_ok=True)

    components = args.components or [
        "bwrap",
        "anecdoct",
        "anecdoct-windows-sandbox-setup",
        "anecdoct-command-runner",
        "rg",
    ]
    ...
    with _gha_group(f"Download native artifacts from workflow {workflow_id}"):
        with tempfile.TemporaryDirectory(prefix="anecdoct-native-artifacts-") as artifacts_dir_str:
