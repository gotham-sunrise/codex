...
impl StateRuntime {
    /// Initialize the state runtime using the provided Anecdoct home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `anecdoct_home`,
    ...
    pub(crate) async fn init_with_telemetry_for_tests(
        ...
        Self::init_inner(anecdoct_home, default_provider, Some(telemetry_override)).await
    }
    ...
    async fn init_inner(
        ...
        let state_path = state_db_path(anecdoct_home.as_path());
        let logs_path = logs_db_path(anecdoct_home.as_path());
    ...
    /// Return the configured Anecdoct home directory for this runtime.
    pub fn anecdoct_home(&self) -> &Path {
...
pub fn state_db_path(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(state_db_filename())
}
...
pub fn logs_db_path(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(logs_db_filename())
}
...
mod tests {
    ...
    async fn sqlite_integrity_check_reports_ok_for_valid_db() {
        ...
        tokio::fs::create_dir_all(&anecdoct_home)
            .await
            .expect("create anecdoct home");
        let path = state_db_path(anecdoct_home.as_path());
