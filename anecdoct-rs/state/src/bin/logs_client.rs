...
use anecdoct_state::LogQuery;
use anecdoct_state::LogRow;
...
#[derive(Debug, Parser)]
#[command(name = "anecdoct-state-logs")]
#[command(about = "Tail Anecdoct logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to ANECDOCT_HOME. Defaults to $ANECDOCT_HOME or ~/.anecdoct.
    #[arg(long, env = "ANECDOCT_HOME")]
    anecdoct_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --anecdoct-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let anecdoct_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(anecdoct_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let anecdoct_home = args
        .anecdoct_home
        .clone()
        .unwrap_or_else(default_anecdoct_home);
    Ok(anecdoct_state::logs_db_path(anecdoct_home.as_path()))
}

fn default_anecdoct_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".anecdoct");
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["anecdoct-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["anecdoct-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["anecdoct-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["anecdoct-state-logs", "--level", "WARN"])
