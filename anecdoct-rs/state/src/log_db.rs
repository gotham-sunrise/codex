...
//!
//! # async fn example(state_db: std::sync::Arc<anecdoct_state::StateRuntime>) {
...
mod tests {
    ...
    fn temp_anecdoct_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("anecdoct-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let anecdoct_home = temp_anecdoct_home();
