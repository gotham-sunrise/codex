...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(
            metadata
                .thread_source
                .map(anecdoct_protocol::protocol::ThreadSource::as_str),
        )
...
fn thread_spawn_parent_thread_id_from_source_str(source: &str) -> Option<ThreadId> {
    ...
    match parsed_source.ok() {
        Some(SessionSource::SubAgent(
            anecdoct_protocol::protocol::SubAgentSource::ThreadSpawn {
                parent_thread_id, ..
            },
...
mod tests {
    ...
    use anecdoct_protocol::protocol::GitInfo;
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        let anecdoct_home = unique_temp_dir();
        let runtime = StateRuntime::init(anecdoct_home.clone(), "test-provider".to_string())
            .await
            .expect("state db should initialize");
        let thread_id =
            ThreadId::from_string("00000000-0000-0000-0000-000000000123").expect("valid thread id");
        let mut metadata = test_thread_metadata(&anecdoct_home, thread_id, anecdoct_home.clone());
    ...
    async fn list_threads_updated_after_returns_oldest_changes_first() {
        ...
        for (thread_id, updated_at) in [
            ...
            let mut metadata =
                test_thread_metadata(&anecdoct_home, thread_id, anecdoct_home.clone());
            metadata.updated_at = updated_at;
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = anecdoct_home.join("first");
        let second_cwd = anecdoct_home.join("second");
        let other_cwd = anecdoct_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&anecdoct_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&anecdoct_home, thread_id, anecdoct_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(anecdoct_git_utils::GitSha::new("rollout-sha")),
                branch: Some("rollout-branch".to_string()),
                repository_url: Some("git@example.com:lambogenius/anecdoct.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:lambogenius/anecdoct.git")
        );
    }
    ...
    async fn upsert_thread_preserves_existing_git_fields_atomically() {
        ...
        metadata.git_branch = Some("sqlite-branch".to_string());
        metadata.git_origin_url = Some("git@example.com:lambogenius/anecdoct.git".to_string());
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:lambogenius/anecdoct.git")),
            )
            .await
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&anecdoct_home, thread_id, anecdoct_home.clone());
        ...
        let mut fallback = test_thread_metadata(&anecdoct_home, thread_id, anecdoct_home.clone());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&anecdoct_home, first_id, anecdoct_home.clone());
        first.updated_at = updated_at;
        let mut second = test_thread_metadata(&anecdoct_home, second_id, anecdoct_home.clone());
        ...
        let mut older = test_thread_metadata(&anecdoct_home, older_id, anecdoct_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            anecdoct_protocol::protocol::TokenCountEvent {
                info: Some(anecdoct_protocol::protocol::TokenUsageInfo {
                    total_token_usage: anecdoct_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: anecdoct_protocol::protocol::TokenUsage::default(),
    ...
    async fn thread_spawn_edges_track_directional_status() {
        let anecdoct_home = unique_temp_dir();
        let runtime = StateRuntime::init(anecdoct_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
