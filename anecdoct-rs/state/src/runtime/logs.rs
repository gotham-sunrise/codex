...
mod tests {
    ...
    async fn insert_logs_use_dedicated_log_database() {
        ...
        let logs_count = log_row_count(logs_db_path(anecdoct_home.as_path()).as_path()).await;
    ...
    async fn init_configures_logs_db_with_incremental_auto_vacuum() {
        let anecdoct_home = unique_temp_dir();
        let _runtime = StateRuntime::init(anecdoct_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");

        let pool = open_db_pool(logs_db_path(anecdoct_home.as_path()).as_path()).await;
