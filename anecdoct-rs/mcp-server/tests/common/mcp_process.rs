...
impl McpProcess {
    pub async fn new(anecdoct_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(anecdoct_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = anecdoct_utils_cargo_bin::cargo_bin("anecdoct-mcp-server")
            .context("should find binary for anecdoct-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = anecdoct_login::default_client::originator().value;
    ...
    pub async fn send_anecdoct_tool_call(
        &mut self,
        params: AnecdoctToolCallParam,
    ) -> anyhow::Result<i64> {
        let anecdoct_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "anecdoct".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(anecdoct_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "anecdoct/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "anecdoct/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `anecdoct-mcp-server` child process.
        //
