...
use anecdoct_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use anecdoct_mcp_server::AnecdoctToolCallParam;
use anecdoct_mcp_server::ExecApprovalElicitRequestParams;
use anecdoct_mcp_server::ExecApprovalResponse;
use anecdoct_mcp_server::PatchApprovalElicitRequestParams;
use anecdoct_mcp_server::PatchApprovalResponse;
use anecdoct_protocol::protocol::FileChange;
use anecdoct_protocol::protocol::ReviewDecision;
use anecdoct_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Anecdoct sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "anecdoct" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let anecdoct_request_id = mcp_process
        .send_anecdoct_tool_call(AnecdoctToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            anecdoct_request_id.to_string(),
            params.anecdoct_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `anecdoct` tool call completes and that the file was created.
    let anecdoct_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(anecdoct_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(anecdoct_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    anecdoct_mcp_tool_call_id: String,
    anecdoct_event_id: String,
    thread_id: anecdoct_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Anecdoct to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let anecdoct_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        anecdoct_elicitation: "exec-approval".to_string(),
        anecdoct_mcp_tool_call_id,
        anecdoct_event_id,
        anecdoct_command: command,
        anecdoct_cwd: workdir.to_path_buf(),
        anecdoct_call_id: "call1234".to_string(),
        anecdoct_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "anecdoct" tool request that will trigger the apply_patch command
    let anecdoct_request_id = mcp_process
        .send_anecdoct_tool_call(AnecdoctToolCallParam {
    ...
    // Verify the original `anecdoct` tool call completes
    let anecdoct_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(anecdoct_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_anecdoct_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = anecdoct_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn anecdoct_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `anecdoct mcp` with a specific config.toml.
    let anecdoct_home = TempDir::new()?;
    create_config_toml(anecdoct_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(anecdoct_home.path()).await?;
    ...
    .await??;
    assert_eq!(anecdoct_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(anecdoct_response.id, RequestId::Number(anecdoct_request_id));
    assert_eq!(
        anecdoct_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": anecdoct_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("anecdoct tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
/// Create a Anecdoct config that uses the mock server as the model provider.
