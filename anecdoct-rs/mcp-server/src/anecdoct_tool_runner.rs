//! Asynchronous worker that executes a **Anecdoct** tool-call inside a spawned
...
use anecdoct_core::config::Config as AnecdoctConfig;
...
/// To adhere to MCP `tools/call` response format, include the Anecdoct
...
/// Run a complete Anecdoct session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_anecdoct_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: AnecdoctConfig,
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!(
                    "Failed to start Anecdoct session: {e}"
                ))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the Anecdoct submission so that
    ...
    run_anecdoct_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_anecdoct_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_anecdoct_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<AnecdoctThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(anecdoct_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("Anecdoct runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;
