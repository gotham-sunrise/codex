...
use anecdoct_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "anecdoct-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/anecdoct-backend/agent-identity";
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "anecdoct-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "anecdoct-cli".to_string(),
        },
...
mod tests {
    ...
    use anecdoct_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    #[test]
    fn agent_identity_jwks_url_uses_anecdoct_api_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/anecdoct"),
            "http://localhost:8080/api/anecdoct/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/anecdoct/"),
            "http://localhost:8080/api/anecdoct/agent-identities/jwks"
