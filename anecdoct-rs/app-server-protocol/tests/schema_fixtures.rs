use anecdoct_app_server_protocol::generate_json_with_experimental;
use anecdoct_app_server_protocol::generate_typescript_schema_fixture_subtree_for_tests;
use anecdoct_app_server_protocol::read_schema_fixture_subtree;
use anyhow::Context;
use anyhow::Result;
...
fn schema_root() -> Result<PathBuf> {
    ...
    let typescript_index = anecdoct_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    let json_bundle = anecdoct_utils_cargo_bin::find_resource!(
        "schema/json/anecdoct_app_server_protocol.schemas.json"
    )
    .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/anecdoct_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
