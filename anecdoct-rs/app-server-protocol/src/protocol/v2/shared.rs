use anecdoct_experimental_api_macros::ExperimentalApi;
use anecdoct_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use anecdoct_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use anecdoct_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
...
/// This translation layer make sure that we expose anecdoct error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `anecdoctErrorInfo` variant.
...
impl From<CoreAnecdoctErrorInfo> for AnecdoctErrorInfo {
    fn from(value: CoreAnecdoctErrorInfo) -> Self {
        match value {
            CoreAnecdoctErrorInfo::ContextWindowExceeded => {
                AnecdoctErrorInfo::ContextWindowExceeded
            }
            CoreAnecdoctErrorInfo::UsageLimitExceeded => AnecdoctErrorInfo::UsageLimitExceeded,
            CoreAnecdoctErrorInfo::ServerOverloaded => AnecdoctErrorInfo::ServerOverloaded,
            CoreAnecdoctErrorInfo::CyberPolicy => AnecdoctErrorInfo::CyberPolicy,
            CoreAnecdoctErrorInfo::HttpConnectionFailed { http_status_code } => {
                AnecdoctErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreAnecdoctErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                AnecdoctErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreAnecdoctErrorInfo::InternalServerError => AnecdoctErrorInfo::InternalServerError,
            CoreAnecdoctErrorInfo::Unauthorized => AnecdoctErrorInfo::Unauthorized,
            CoreAnecdoctErrorInfo::BadRequest => AnecdoctErrorInfo::BadRequest,
            CoreAnecdoctErrorInfo::ThreadRollbackFailed => AnecdoctErrorInfo::ThreadRollbackFailed,
            CoreAnecdoctErrorInfo::SandboxError => AnecdoctErrorInfo::SandboxError,
            CoreAnecdoctErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                AnecdoctErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreAnecdoctErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                AnecdoctErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreAnecdoctErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                AnecdoctErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreAnecdoctErrorInfo::Other => AnecdoctErrorInfo::Other,
        }
    }
}
