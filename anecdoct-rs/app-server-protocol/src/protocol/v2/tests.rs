use super::*;
use anecdoct_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use anecdoct_protocol::items::AgentMessageContent;
use anecdoct_protocol::items::AgentMessageItem;
use anecdoct_protocol::items::FileChangeItem;
use anecdoct_protocol::items::ImageViewItem;
use anecdoct_protocol::items::McpToolCallItem;
use anecdoct_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use anecdoct_protocol::items::ReasoningItem;
use anecdoct_protocol::items::TurnItem;
use anecdoct_protocol::items::UserMessageItem;
use anecdoct_protocol::items::WebSearchItem;
use anecdoct_protocol::mcp::CallToolResult;
use anecdoct_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use anecdoct_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use anecdoct_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use anecdoct_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use anecdoct_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use anecdoct_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use anecdoct_protocol::protocol::AgentStatus as CoreAgentStatus;
use anecdoct_protocol::protocol::AskForApproval as CoreAskForApproval;
use anecdoct_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use anecdoct_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use anecdoct_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use anecdoct_protocol::user_input::UserInput as CoreUserInput;
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        anecdoct_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        anecdoct_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        anecdoct_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn network_requirements_deserializes_legacy_fields() {
    let requirements: NetworkRequirements = serde_json::from_value(json!({
        "allowedDomains": ["api.lambogenius.com"],
    ...
    assert_eq!(
        ...
        NetworkRequirements {
            ...
            allowed_domains: Some(vec!["api.lambogenius.com".to_string()]),
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.anecdoct/skills/skill-creator/SKILL.md"),
            },
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(anecdoct_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_source_serializes_local_git_and_remote_variants() {
    ...
    assert_eq!(
        serde_json::to_value(PluginSource::Git {
            url: "https://github.com/lambogenius/example.git".to_string(),
        ...
        json!({
            "type": "git",
            "url": "https://github.com/lambogenius/example.git",
...
fn plugin_read_params_serialization_uses_install_source_fields() {
    ...
    assert_eq!(
        serde_json::from_value::<PluginReadParams>(json!({
            "remoteMarketplaceName": "lambogenius-curated",
            "pluginName": "gmail",
        }))
        .unwrap(),
        PluginReadParams {
            marketplace_path: None,
            remote_marketplace_name: Some("lambogenius-curated".to_string()),
            plugin_name: "gmail".to_string(),
        },
    );
}
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@anecdoct-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "anecdoct-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@anecdoct-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "anecdoct-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
fn plugin_uninstall_params_serialization_omits_force_remote_sync() {
    assert_eq!(
        serde_json::to_value(PluginUninstallParams {
            plugin_id: "gmail@lambogenius-curated".to_string(),
        })
        .unwrap(),
        json!({
            "pluginId": "gmail@lambogenius-curated",
        }),
    );
...
#[test]
fn anecdoct_error_info_serializes_http_status_code_in_camel_case() {
    let value = AnecdoctErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn anecdoct_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(AnecdoctErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn anecdoct_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = AnecdoctErrorInfo::ActiveTurnNotSteerable {
