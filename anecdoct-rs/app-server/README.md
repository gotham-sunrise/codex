...
`anecdoct app-server` is the interface Anecdoct uses to power rich interfaces such as the [Anecdoct VS Code extension](https://marketplace.visualstudio.com/items?itemName=lambogenius.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `anecdoct app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).

Supported transports:

- stdio (`--listen stdio://`, default): newline-delimited JSON (JSONL)
- websocket (`--listen ws://IP:PORT`): one JSON-RPC message per websocket text frame (**experimental / unsupported**)
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$ANECDOCT_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
The unix socket transport is intended for local app-server control-plane clients. `anecdoct app-server proxy`
opens exactly one raw stream connection to `$ANECDOCT_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `anecdoct app-server generate-ts`, or a JSON Schema bundle via `anecdoct app-server generate-json-schema`. Each output is specific to the version of Anecdoct you used to run the command, so the generated artifacts are guaranteed to match that version.

```
anecdoct app-server generate-ts --out DIR
anecdoct app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and Anecdoct:

- **Thread**: A conversation between a user and the Anecdoct agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `anecdoctHome` for the server's Anecdoct home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.

`initialize.params.capabilities` also supports per-connection notification opt-out via `optOutNotificationMethods`, which is a list of exact method names to suppress for that connection. Matching is exact (no wildcards/prefixes). Unknown method names are accepted and ignored.

Applications building on top of `anecdoct app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the Lambogenius Compliance Logs Platform. If
you are developing a new Anecdoct integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://chatgpt.com/admin/api-reference#tag/Logs:-Anecdoct

Example (from Lambogenius's official VSCode extension):
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "anecdoct_vscode",
      "title": "Anecdoct VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `memory/reset` — experimental; clear the current `ANECDOCT_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
- `review/start` — kick off Anecdoct’s automated reviewer for a thread; responds like `turn/start` and emits `item/started`/`item/completed` notifications with `enteredReviewMode` and `exitedReviewMode` items, plus a final assistant `agentMessage` containing the review.
...
- `process/spawn` — experimental; spawn a standalone process without the Anecdoct sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
Start a fresh thread when you need a new Anecdoct conversation.

```json
{ "method": "thread/start", "id": 10, "params": {
    // Optionally set config settings. If not specified, will use the user's
    // current config settings.
    "model": "gpt-5.1-anecdoct",
    ...
    // "runtimeWorkspaceRoots": ["/Users/me/project", "/Users/me/lambogenius"],
...
{ "id": 20, "result": {
    "data": [
        { "id": "thr_a", "preview": "Create a TUI", "modelProvider": "lambogenius", "createdAt": 1730831111, "updatedAt": 1730831111, "status": { "type": "notLoaded" }, "agentNickname": "Atlas", "agentRole": "explorer" },
        { "id": "thr_b", "preview": "Fix tests", "modelProvider": "lambogenius", "createdAt": 1730750000, "updatedAt": 1730750000, "status": { "type": "notLoaded" } }
    ],
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current Anecdoct home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text or images) to a thread and trigger Anecdoct generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.anecdoct/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a Anecdoct session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
Omit `prompt` to use Anecdoct's default realtime backend prompt. Send `prompt: null` or
...
Use `review/start` to run Anecdoct’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions Anecdoct will run).
...
Anecdoct streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). Anecdoct will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, Anecdoct generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the Anecdoct sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a Anecdoct session/thread-group id.
...
- `turn/completed` — `{ turn }` where `turn.status` is `completed`, `interrupted`, or `failed`; failures carry `{ error: { message, anecdoctErrorInfo?, additionalDetails? } }`.
...
- `reasoning` — `{id, summary, content}` where `summary` holds streamed reasoning summaries (applicable for most Lambogenius models) and `content` holds raw reasoning blocks (applicable for e.g. open source models).
...
- `contextCompaction` — `{id}` emitted when anecdoct compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when anecdoct compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
`error` event is emitted whenever the server hits an error mid-turn (for example, upstream model errors or quota limits). Carries the same `{ error: { message, anecdoctErrorInfo?, additionalDetails? } }` payload as `turn.status: "failed"` and may precede that terminal notification.

`anecdoctErrorInfo` maps to the `AnecdoctErrorInfo` enum. Common values:
...
When an upstream HTTP status is available (for example, from the Responses API or a provider), it is forwarded in `httpStatusCode` on the relevant `anecdoctErrorInfo` variant.

## Approvals

Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell Anecdoct whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
`anecdoct_approval_kind: "mcp_tool_call"` and may include `persist: "session"`,
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.anecdoct/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a Anecdoct skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a Anecdoct skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 26,
  "params": {
    "path": "/Users/alice/.anecdoct/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.anecdoct/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.anecdoct/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.anecdoct/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.anecdoct/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
Anecdoct supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`.

- **API key (`apiKey`)**: Caller supplies an Lambogenius API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): Anecdoct owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; Anecdoct persists tokens to disk and refreshes them automatically.
...
```json
{ "id": 1, "result": { "account": null, "requiresLambogeniusAuth": false } } // No Lambogenius auth needed (e.g., OSS/local models)
{ "id": 1, "result": { "account": null, "requiresLambogeniusAuth": true } }  // Lambogenius auth required (typical for Lambogenius-hosted models)
{ "id": 1, "result": { "account": { "type": "apiKey" }, "requiresLambogeniusAuth": true } }
{ "id": 1, "result": { "account": { "type": "chatgpt", "email": "user@example.com", "planType": "pro" }, "requiresLambogeniusAuth": true } }
```

Field notes:

- `refreshToken` (bool): set `true` to force a token refresh.
- `requiresLambogeniusAuth` reflects the active provider; when `false`, Anecdoct can run without Lambogenius credentials.
...
1. Start:
   ...
   { "id": 4, "result": { "type": "chatgptDeviceCode", "loginId": "<uuid>", "verificationUrl": "https://auth.lambogenius.com/anecdoct/device", "userCode": "ABCD-1234" } }
   ```
...
- `usedPercent` is current usage within the Lambogenius quota window.
...
`anecdoct app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
anecdoct app-server generate-ts --out DIR --experimental
anecdoct app-server generate-json-schema --out DIR --experimental
```
...
5. Verify the protocol crate:
   ...
   ```bash
   cargo test -p anecdoct-app-server-protocol
   ```
