...
//!
//! # Relationship to `anecdoct-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `anecdoct-app-server-client`,
...
use anecdoct_analytics::AppServerRpcTransport;
...
use anecdoct_app_server_protocol::Result;
...
pub use anecdoct_rollout::StateDbHandle;
pub use anecdoct_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `anecdoct-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_anecdoct_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.anecdoct_home).await?;
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let auth_manager =
            AuthManager::shared_from_config(args.config.as_ref(), args.enable_anecdoct_api_key_env)
                .await;
        ...
        let config_manager = ConfigManager::new(
            args.config.anecdoct_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(anecdoct_home.path()).await);
        let state_db = anecdoct_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "anecdoct-in-process-test".to_string(),
                    title: None,
        ...
        client._test_anecdoct_home = Some(anecdoct_home);
        client
    }
