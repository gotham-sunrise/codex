use anecdoct_config::types::PluginConfig;
...
use anecdoct_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use anecdoct_core_plugins::marketplace::find_marketplace_manifest_path;
use anecdoct_core_plugins::marketplace_add::MarketplaceAddRequest;
use anecdoct_core_plugins::marketplace_add::add_marketplace;
use anecdoct_core_plugins::marketplace_add::is_local_marketplace_source;
use anecdoct_external_agent_migration::build_mcp_config_from_external;
use anecdoct_external_agent_migration::count_missing_commands;
use anecdoct_external_agent_migration::count_missing_subagents;
use anecdoct_external_agent_migration::hook_migration_event_names;
use anecdoct_external_agent_migration::import_commands;
use anecdoct_external_agent_migration::import_hooks;
use anecdoct_external_agent_migration::import_subagents;
use anecdoct_external_agent_migration::missing_command_names;
use anecdoct_external_agent_migration::missing_subagent_names;
use anecdoct_external_agent_sessions::ExternalAgentSessionMigration;
use anecdoct_external_agent_sessions::detect_recent_sessions;
...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "anecdoct.external_agent_config.detect";
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "anecdoct.external_agent_config.import";
...
impl ExternalAgentConfigService {
    pub(crate) fn new(anecdoct_home: PathBuf) -> Self {
    ...
    #[cfg(test)]
    fn new_for_test(anecdoct_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            anecdoct_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.anecdoct_home.join("config.toml"),
            |repo_root| repo_root.join(".anecdoct").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.anecdoct_home.join("hooks.json"),
            |repo_root| repo_root.join(".anecdoct").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            || self.anecdoct_home.join("agents"),
            |repo_root| repo_root.join(".anecdoct").join("agents"),
        );
        ...
        let target_agents_md = repo_root.map_or_else(
            || self.anecdoct_home.join("AGENTS.md"),
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                .anecdoct_home(self.anecdoct_home.clone())
                .fallback_cwd(Some(self.anecdoct_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.anecdoct_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.anecdoct_home)?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.anecdoct_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome =
                add_marketplace(self.anecdoct_home.clone(), request).await;
    ...
    fn import_subagents(&self, cwd: Option<&Path>) -> io::Result<usize> {
        let (source_agents, target_agents) = if let Some(repo_root) = find_repo_root(cwd)? {
            (
                ...
                repo_root.join(".anecdoct").join("agents"),
            )
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(0);
        } else {
        ...
        let (source_external_agent_dir, target_hooks) =
            if let Some(repo_root) = find_repo_root(cwd)? {
                (
                    ...
                    repo_root.join(".anecdoct").join("hooks.json"),
                )
            } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
                return Ok(());
            } else {
            ...
            marketplace
                ...
                .filter(|plugin| {
                    plugin.policy.products.as_deref().is_none_or(|products| {
                        Product::Anecdoct.matches_product_restriction(products)
                    })
                })
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "Anecdoct");
    }
    rewritten
}
...
fn emit_migration_metric(
    ...
    let Some(metrics) = anecdoct_otel::global() else {
        return;
    };
