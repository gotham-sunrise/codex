...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let anecdoct_home = root.path().join(".anecdoct");
    (root, external_agent_home, anecdoct_home)
}
...
fn service_for_paths(
    ...
    ExternalAgentConfigService::new_for_test(anecdoct_home, external_agent_home)
}
...
async fn detect_home_lists_config_skills_and_agents_md() {
    let (_root, external_agent_home, anecdoct_home) = fixture_paths();
    let agents_skills = anecdoct_home
        .parent()
    ...
    let items = service_for_paths(external_agent_home.clone(), anecdoct_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                anecdoct_home.join("config.toml").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                anecdoct_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, anecdoct_home) = fixture_paths();
    ...
    let items = service_for_paths(external_agent_home.clone(), anecdoct_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    .expect("write invalid anecdoct config");
    fs::write(
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), anecdoct_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root
                        ...
                        .join("settings.json")
                        .display(),
                    repo_root.join(".anecdoct").join("config.toml").display()
                ),
                cwd: Some(repo_root.clone()),
                details: None,
            },
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".anecdoct").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".anecdoct").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
async fn import_repo_migrates_mcp_hooks_commands_and_subagents() {
    ...
    let config: TomlValue = toml::from_str(
        &fs::read_to_string(repo_root.join(".anecdoct").join("config.toml")).expect("read config"),
    )
    .expect("parse config");
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        anecdoct_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
        .expect("migrated MCP config should be supported");

    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".anecdoct").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    let _supported_hooks: anecdoct_config::HooksFile =
...
Research with Anecdoct carefully."""
"#,
    )
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    fs::write(
            external_agent_home.join("settings.json"),
            format!(r#"{{"model":"{SOURCE_EXTERNAL_AGENT_NAME}","permissions":{{"ask":["git push"]}},"env":{{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"anecdoct","IGNORED":null,"LIST":["a","b"],"MAP":{{"x":1}}}},"sandbox":{{"enabled":true,"network":{{"allowLocalBinding":true}}}}}}"#),
        )
        .expect("write settings");
    fs::write(
        external_agent_home
    ...
    service_for_paths(external_agent_home, anecdoct_home.clone())
        .import(vec![
            ExternalAgentConfigMigrationItem {
                item_type: ExternalAgentConfigMigrationItemType::AgentsMd,
                description: String::new(),
    ...
    assert_eq!(
        fs::read_to_string(anecdoct_home.join("AGENTS.md")).expect("read agents"),
        "Anecdoct guidance"
    );

    assert_eq!(
        fs::read_to_string(anecdoct_home.join("config.toml")).expect("read config"),
        "sandbox_mode = \"workspace-write\"\n\n[shell_environment_policy]\ninherit = \"core\"\n\n[shell_environment_policy.set]\nCI = \"false\"\nFOO = \"bar\"\nMAX_RETRIES = \"3\"\nMY_TEAM = \"anecdoct\"\n"
    );
    assert_eq!(
        fs::read_to_string(agents_skills.join("skill-a").join("SKILL.md"))
            .expect("read copied skill"),
        "Use Anecdoct and Anecdoct utilities."
    );
}
...
async fn import_home_skips_empty_config_migration() {
    ...
    assert!(!anecdoct_home.join("config.toml").exists());
}
...
async fn import_local_plugins_returns_completed_status() {
    ...
    fs::create_dir_all(plugin_root.join(".anecdoct-plugin")).expect("create plugin manifest dir");
    fs::create_dir_all(&anecdoct_home).expect("create anecdoct home");
    ...
    fs::write(
        plugin_root.join(".anecdoct-plugin").join("plugin.json"),
        r#"{"name":"cloudflare","version":"0.1.0"}"#,
    )
    .expect("write plugin manifest");

    let outcome = service_for_paths(external_agent_home, anecdoct_home.clone())
        .import(vec![ExternalAgentConfigMigrationItem {
    ...
    let config = fs::read_to_string(anecdoct_home.join("config.toml")).expect("read config");
...
async fn detect_home_skips_config_when_target_already_has_supported_fields() {
    ...
    let items = service_for_paths(external_agent_home, anecdoct_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            cwds: None,
        })
...
#[tokio::test]
async fn import_repo_hooks_preserves_disabled_anecdoct_hooks_feature() {
    let root = TempDir::new().expect("create tempdir");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
    fs::create_dir_all(repo_root.join(".anecdoct")).expect("create anecdoct dir");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
    ...
    .expect("write hooks");
    fs::write(
        repo_root.join(".anecdoct").join("config.toml"),
        "[features]\nanecdoct_hooks = false\n",
    )
    .expect("write config");
...
async fn import_repo_mcp_uses_home_settings_toggles_when_repo_settings_missing() {
    ...
    service_for_paths(external_agent_home, root.path().join(".anecdoct"))
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::McpServerConfig,
            description: String::new(),
            cwd: Some(repo_root.clone()),
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_already_configured_in_anecdoct() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let anecdoct_home = root.path().join(".anecdoct");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_disabled_in_anecdoct() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let anecdoct_home = root.path().join(".anecdoct");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_without_explicit_enabled_in_anecdoct() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let anecdoct_home = root.path().join(".anecdoct");
    let repo_root = root.path().join("repo");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, anecdoct_home)
...
#[tokio::test]
async fn detect_repo_does_not_skip_plugins_only_configured_in_project_anecdoct() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let anecdoct_home = root.path().join(".anecdoct");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".anecdoct")).expect("create repo anecdoct dir");
    fs::create_dir_all(&anecdoct_home).expect("create anecdoct home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
"#,
    ...
    .expect("write project anecdoct config");
...
async fn detect_repo_filters_plugins_against_installed_marketplace() {
    ...
    let anecdoct_home = root.path().join(".anecdoct");
    let repo_root = root.path().join("repo");
    let marketplace_root = anecdoct_home
        .join(".tmp")
        .join("marketplaces")
        .join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, anecdoct_home)
        .import_plugins(
            /*cwd*/ None,
...
fn import_skills_returns_only_new_skill_directory_count() {
    ...
    let copied_count = service_for_paths(external_agent_home, anecdoct_home)
