...
use anecdoct_app_server_protocol::TextPosition as AppTextPosition;
use anecdoct_app_server_protocol::TextRange as AppTextRange;
use anecdoct_config::ConfigLoadError;
use anecdoct_config::TextRange as CoreTextRange;
use anecdoct_core::ExecPolicyError;
...
use anecdoct_rollout::state_db as rollout_state_db;
...
const OTEL_SERVICE_NAME: &str = "anecdoct-app-server";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        disabled_folders.push((
            dot_anecdoct_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
        ...
        .await
    {
        Ok(config) => {
            ...
            let auth_manager = AuthManager::shared_from_config(
                &config, /*enable_anecdoct_api_key_env*/ false,
            )
            .await;
    ...
    let otel = anecdoct_core::otel_init::build_provider(
        &config,
    ...
    anecdoct_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    anecdoct_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let state_db = match rollout_state_db::try_init(&config).await {
        Ok(state_db) => Some(state_db),
        Err(err) => {
            let state_db_path = anecdoct_state::state_db_path(config.sqlite_home.as_path());
    ...
    if should_run_personality_migration {
        ...
        match effective_toml.try_into() {
            Ok(config_toml) => {
                match anecdoct_core::personality_migration::maybe_migrate_personality(
                ...
                .await
                {
                    Ok(anecdoct_core::personality_migration::PersonalityMigrationStatus::Applied) => {
                    ...
                    Ok(
                        anecdoct_core::personality_migration::PersonalityMigrationStatus::SkippedMarker
                        | anecdoct_core::personality_migration::PersonalityMigrationStatus::SkippedExplicitPersonality
                        | anecdoct_core::personality_migration::PersonalityMigrationStatus::SkippedNoSessions,
                    ) => {}
                    Err(err) => {
    ...
    let feedback = AnecdoctFeedback::new();
