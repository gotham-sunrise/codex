...
use anecdoct_app_server_protocol::CommandMigration;
...
use anecdoct_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
...
use anecdoct_app_server_protocol::ExternalAgentConfigMigrationItemType;
use anecdoct_app_server_protocol::HookMigration;
use anecdoct_app_server_protocol::JSONRPCErrorError;
use anecdoct_app_server_protocol::McpServerMigration;
use anecdoct_app_server_protocol::MigrationDetails;
use anecdoct_app_server_protocol::PluginsMigration;
...
use anecdoct_external_agent_sessions::ExternalAgentSessionMigration as CoreSessionMigration;
use anecdoct_external_agent_sessions::ImportedExternalAgentSession;
use anecdoct_external_agent_sessions::PendingSessionImport;
use anecdoct_external_agent_sessions::prepare_validated_session_imports;
use anecdoct_external_agent_sessions::record_imported_session;
...
impl ExternalAgentConfigRequestProcessor {
    pub(crate) fn new(
        ...
        Self {
            outgoing,
            migration_service: ExternalAgentConfigService::new(anecdoct_home.clone()),
            anecdoct_home,
                ...
                .map(|migration_item| ExternalAgentConfigMigrationItem {
                    ...
                    details: migration_item.details.map(|details| MigrationDetails {
                        ...
                        sessions: details
                            .sessions
                            .into_iter()
                            .map(|session| anecdoct_app_server_protocol::SessionMigration {
                                path: session.path,
                                cwd: session.cwd,
                                title: session.title,
                            })
                        ...
                        subagents: details
                            .subagents
                            .into_iter()
                            .map(|subagent| anecdoct_app_server_protocol::SubagentMigration {
                                name: subagent.name,
                            })
                            .collect(),
                        commands: details
    ...
    async fn import_external_agent_session(
        ...
        if let Some(title) = title
            && let Some(name) = anecdoct_core::util::normalize_thread_name(&title)
        {
            imported_thread
                .thread
    ...
    fn prepare_validated_session_imports(
        ...
        prepare_validated_session_imports(&self.anecdoct_home, sessions)
    }

    fn record_imported_session(&self, source_path: &std::path::Path, imported_thread_id: ThreadId) {
        if let Err(err) =
            record_imported_session(&self.anecdoct_home, source_path, imported_thread_id)
        {
            tracing::warn!(
                error = %err,
