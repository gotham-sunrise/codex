...
use anecdoct_app_server_protocol::ConfigReadResponse;
use anecdoct_app_server_protocol::ConfigRequirements;
use anecdoct_app_server_protocol::ConfigRequirementsReadResponse;
use anecdoct_app_server_protocol::ConfigValueWriteParams;
use anecdoct_app_server_protocol::ConfigWriteErrorCode;
use anecdoct_app_server_protocol::ConfigWriteResponse;
use anecdoct_app_server_protocol::ConfiguredHookHandler;
use anecdoct_app_server_protocol::ConfiguredHookMatcherGroup;
use anecdoct_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use anecdoct_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use anecdoct_app_server_protocol::JSONRPCErrorError;
use anecdoct_app_server_protocol::ManagedHooksRequirements;
use anecdoct_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use anecdoct_app_server_protocol::NetworkDomainPermission;
use anecdoct_app_server_protocol::NetworkRequirements;
use anecdoct_app_server_protocol::NetworkUnixSocketPermission;
...
use anecdoct_config::HookEventsToml;
use anecdoct_config::HookHandlerConfig as CoreHookHandlerConfig;
use anecdoct_config::ManagedHooksRequirementsToml;
use anecdoct_config::MatcherGroup as CoreMatcherGroup;
use anecdoct_config::ResidencyRequirement as CoreResidencyRequirement;
use anecdoct_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use anecdoct_core::ThreadManager;
use anecdoct_features::canonical_feature_for_key;
use anecdoct_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    async fn refresh_apps_list_after_experimental_feature_enablement_set(&self) {
        ...
        if !config.features.apps_enabled_for_auth(
            auth.as_ref()
                .is_some_and(anecdoct_login::AnecdoctAuth::uses_anecdoct_backend),
        ) {
            return;
        }
    ...
    ) -> Result<anecdoct_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = anecdoct_core_plugins::toggles::collect_plugin_enabled_candidates(
    ...
    async fn emit_plugin_toggle_events(
        ...
        for (plugin_id, enabled) in pending_changes {
            ...
            let metadata = anecdoct_core_plugins::loader::installed_plugin_telemetry_metadata(
                self.config_manager.anecdoct_home(),
                &plugin_id,
            )
            .await;
            if enabled {
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(anecdoct_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(anecdoct_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
...
) -> anecdoct_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => anecdoct_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: anecdoct_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(anecdoct_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(anecdoct_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(anecdoct_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: anecdoct_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        anecdoct_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        anecdoct_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: anecdoct_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        anecdoct_config::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        anecdoct_config::NetworkUnixSocketPermissionToml::None => NetworkUnixSocketPermission::None,
    }
}
