...
impl ThreadGoalRequestProcessor {
    ...
    async fn thread_goal_set_inner(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => anecdoct_rollout::find_thread_path_by_id_str(
            ...
            if let Some(goal) = existing_goal.as_ref() {
                ...
                state_db
                    .replace_thread_goal(
                        thread_id,
                        objective,
                        status.unwrap_or(anecdoct_state::ThreadGoalStatus::Active),
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            anecdoct_rollout::find_thread_path_by_id_str(
                &self.config.anecdoct_home,
                &thread_id.to_string(),
                self.state_db.as_deref(),
            )
...
fn thread_goal_status_to_state(status: ThreadGoalStatus) -> anecdoct_state::ThreadGoalStatus {
    match status {
...
fn thread_goal_status_from_state(status: anecdoct_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(super) fn api_thread_goal_from_state(goal: anecdoct_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
