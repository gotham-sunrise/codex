...
use anecdoct_login::default_client::SetOriginatorError;
...
const NON_ORIGINATING_CLIENT_NAMES: &[&str] = &["anecdoct_app_server_daemon", "anecdoct-backend"];
...
impl InitializeRequestProcessor {
    ...
    pub(crate) async fn initialize(
        ...
        let anecdoct_home = self.config.anecdoct_home.clone();
        if session
            ...
            if let Err(error) = set_default_originator(originator.clone()) {
                match error {
                    ...
                    SetOriginatorError::AlreadyInitialized => {
                        ...
                        // TODO(owen): Once we remove support for ANECDOCT_INTERNAL_ORIGINATOR_OVERRIDE,
