...
) -> Vec<anecdoct_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            anecdoct_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    anecdoct_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| anecdoct_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[anecdoct_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[anecdoct_core::skills::SkillError],
) -> Vec<anecdoct_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| anecdoct_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
    ...
    async fn experimental_feature_list_response(
        ...
        let workspace_anecdoct_plugins_enabled = self
            ...
            .map(|spec| {
                ...
                ApiExperimentalFeature {
                    ...
                    enabled: config.features.enabled(spec.id)
                        && (workspace_anecdoct_plugins_enabled
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                anecdoct_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![anecdoct_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
                    ...
                    let effective_skill_roots = if workspace_anecdoct_plugins_enabled {
                    ...
                    let skills_input = anecdoct_core::skills::SkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(anecdoct_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![anecdoct_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let workspace_anecdoct_plugins_enabled = self
                ...
                .await;
            let plugins_enabled =
                config.features.enabled(Feature::Plugins) && workspace_anecdoct_plugins_enabled;
            ...
            let hooks = anecdoct_hooks::list_hooks(anecdoct_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.anecdoct_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {
