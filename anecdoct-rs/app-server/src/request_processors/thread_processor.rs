...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: anecdoct_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                anecdoct_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[ApiDynamicToolSpec]) -> Result<(), String> {
    ...
    for tool in tools {
        ...
        if let Err(err) = anecdoct_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    async fn instruction_sources_from_config(config: &Config) -> Vec<AbsolutePathBuf> {
        anecdoct_core::AgentsMdManager::new(config)
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<AnecdoctThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            anecdoct_home: self.config.anecdoct_home.to_path_buf(),
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<anecdoct_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<anecdoct_app_server_protocol::ThreadStartSource>,
        thread_source: Option<anecdoct_protocol::protocol::ThreadSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                anecdoct_core::config::set_project_trust_level(
                    &listener_task_context.anecdoct_home,
        ...
        let NewThread {
            ...
            .start_thread_with_options(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(anecdoct_app_server_protocol::ThreadStartSource::Startup)
                {
                    anecdoct_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    anecdoct_app_server_protocol::ThreadStartSource::Clear => {
            ...
            .map_err(|err| match err {
                AnecdoctErr::InvalidRequest(message) => invalid_request(message),
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<anecdoct_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<anecdoct_app_server_protocol::ApprovalsReviewer>,
        ...
        let mut overrides = ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(anecdoct_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(anecdoct_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            anecdoct_linux_sandbox_exe: self.arg0_paths.anecdoct_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = anecdoct_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.anecdoct_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.anecdoct_home.display()
                ))
            })?;
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                thread.thread_source = anecdoct_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = anecdoct_thread.config_snapshot().await;
                let sandbox = thread_response_sandbox_policy(
                    &config_snapshot.permission_profile,
                    config_snapshot.cwd.as_path(),
                );
                ...
                self.thread_goal_processor
                    .emit_resume_goal_snapshot_and_continue(thread_id, anecdoct_thread.as_ref())
                    .await;
            }
            Err(err) => {
    ...
    async fn thread_fork_inner(
        ...
        let NewThread {
            ...
            .await
            .map_err(|err| match err {
                AnecdoctErr::Io(_) | AnecdoctErr::Json(_) => {
...
fn core_thread_write_error(operation: &str, err: AnecdoctErr) -> JSONRPCErrorError {
    match err {
        AnecdoctErr::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        AnecdoctErr::InvalidRequest(message) => invalid_request(message),
        AnecdoctErr::UnsupportedOperation(message) => method_not_found(message),
...
) -> (Thread, Option<anecdoct_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<anecdoct_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(anecdoct_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match anecdoct_core::parse_turn_item(item) {
                Some(anecdoct_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            anecdoct_protocol::config_types::SandboxMode::WorkspaceWrite
                | anecdoct_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &anecdoct_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        anecdoct_protocol::models::PermissionProfile::Disabled
        | anecdoct_protocol::models::PermissionProfile::External { .. } => true,
        anecdoct_protocol::models::PermissionProfile::Managed { .. } => profile
