...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "ANECDOCT_APP_SERVER_LOGIN_ISSUER";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth.as_ref().map(AnecdoctAuth::api_auth_mode),
            plan_type: auth.as_ref().and_then(AnecdoctAuth::account_plan_type),
        }
    }
    ...
    async fn login_v2(
        ...
        match params {
            ...
            LoginAccountParams::Chatgpt {
                anecdoct_streamlined_login,
            } => {
                self.login_chatgpt_v2(request_id, anecdoct_streamlined_login)
                    .await;
            }
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(anecdoct_streamlined_login)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(anecdoct_streamlined_login)
            .await?;
    ...
    async fn login_chatgpt_device_code_response(
        ...
        let opts = self
            .login_chatgpt_common(/*anecdoct_streamlined_login*/ false)
            .await?;
    ...
    async fn logout_common(&self) -> std::result::Result<Option<AuthMode>, JSONRPCErrorError> {
        ...
        Ok(self
            ...
            .auth_cached()
            .as_ref()
            .map(AnecdoctAuth::api_auth_mode))
    }
    ...
    async fn get_auth_status_response(
        ...
        // If a custom provider is configured with `requires_lambogenius_auth == false`,
        // then no auth step is required; otherwise, default to requiring auth.
        let requires_lambogenius_auth = self.config.model_provider.requires_lambogenius_auth;

        let response = if !requires_lambogenius_auth {
            ...
            match auth {
                Some(auth) => {
                    ...
                    let (reported_auth_method, token_opt) =
                        if matches!(auth, AnecdoctAuth::AgentIdentity(_))
    ...
    async fn get_account_response(
        ...
        Ok(GetAccountResponse {
            account,
            requires_lambogenius_auth: account_state.requires_lambogenius_auth,
        })
    }
    ...
    > {
        ...
        let snapshots = client.get_rate_limits_many().await.map_err(|err| {
            internal_error(format!("failed to fetch anecdoct rate limits: {err}"))
        })?;
        if snapshots.is_empty() {
            return Err(internal_error(
                "failed to fetch anecdoct rate limits: no snapshots returned",
            ));
        }
        ...
        let rate_limits_by_limit_id: HashMap<String, CoreRateLimitSnapshot> = snapshots
            ...
            .map(|snapshot| {
                let limit_id = snapshot
                    .limit_id
                    .clone()
                    .unwrap_or_else(|| "anecdoct".to_string());
                (limit_id, snapshot)
            })
            .collect();
