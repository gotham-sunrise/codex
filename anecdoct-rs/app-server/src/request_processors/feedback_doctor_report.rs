...
//! the configured Anecdoct executable as a subprocess, accepts only valid JSON from
//! `anecdoct doctor --json`, derives a small set of Sentry tags, and otherwise
...
use anecdoct_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `anecdoct-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `anecdoct doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn Anecdoct, finish before the timeout, or parse JSON means the
