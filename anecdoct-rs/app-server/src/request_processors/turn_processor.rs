...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = anecdoct_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if has_any_overrides {
            thread
                .validate_turn_context_overrides(AnecdoctThreadTurnContextOverrides {
                    cwd: cwd.clone(),
        ...
        if turn_has_input {
            ...
            anecdoct_memories_write::start_memories_startup_task(
                ...
                let (message, data, error_type) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            anecdoct_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            anecdoct_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<AnecdoctThread>)>, JSONRPCErrorError> {
    ...
    async fn start_inline_review(
        &self,
        request_id: &ConnectionRequestId,
        parent_thread: Arc<AnecdoctThread>,
