...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match anecdoct_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: anecdoct_protocol::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> anecdoct_protocol::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        anecdoct_protocol::protocol::SessionSource::SubAgent(
...
) -> Option<anecdoct_app_server_protocol::ActivePermissionProfile> {
...
pub(super) fn thread_response_sandbox_policy(
    permission_profile: &anecdoct_protocol::models::PermissionProfile,
    cwd: &Path,
) -> anecdoct_app_server_protocol::SandboxPolicy {
    let file_system_policy = permission_profile.file_system_sandbox_policy();
    let sandbox_policy = anecdoct_sandboxing::compatibility_sandbox_policy_for_permission_profile(
        permission_profile,
        &file_system_policy,
