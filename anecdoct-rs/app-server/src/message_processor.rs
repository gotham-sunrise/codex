...
use anecdoct_app_server_protocol::AuthMode as LoginAuthMode;
use anecdoct_app_server_protocol::ChatgptAuthTokensRefreshParams;
...
use anecdoct_app_server_protocol::ExperimentalApi;
...
use anecdoct_app_server_protocol::experimental_required_message;
...
use anecdoct_login::auth::ExternalAuth;
use anecdoct_login::auth::ExternalAuthRefreshContext;
use anecdoct_login::auth::ExternalAuthRefreshReason;
use anecdoct_login::auth::ExternalAuthTokens;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: AnecdoctFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let anecdoct_request = serde_json::to_value(&request)
                    .map_err(|err| invalid_request(format!("Invalid request: {err}")))
                    .and_then(|request_json| {
                        serde_json::from_value::<ClientRequest>(request_json)
                            .map_err(|err| invalid_request(format!("Invalid request: {err}")))
                    });
                let result = match anecdoct_request {
                    Ok(anecdoct_request) => {
    ...
    async fn handle_client_request(
        ...
        anecdoct_request: ClientRequest,
        session: Arc<ConnectionSessionState>,
        ...
        if let ClientRequest::Initialize { request_id, params } = anecdoct_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = anecdoct_request.experimental_reason()
        ...
        let serialization_scope = anecdoct_request.serialization_scope();
