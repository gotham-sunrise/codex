...
use anecdoct_cloud_requirements::cloud_requirements_loader;
...
use anecdoct_utils_json_to_toml::json_to_toml;
...
/// Shared app-server entry point for loading effective Anecdoct configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn anecdoct_home(&self) -> &Path {
        self.anecdoct_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.anecdoct_home())
    }
    ...
    pub(crate) fn replace_cloud_requirements_loader(
        ...
        let loader =
            cloud_requirements_loader(auth_manager, chatgpt_base_url, self.anecdoct_home.clone());
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(anecdoct_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
            ...
            || self.loader_overrides.user_config_profile.is_some()
        {
            let user_config_path = self
                .loader_overrides
                .user_config_path(self.anecdoct_home())?;
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = anecdoct_core::config::ConfigBuilder::default()
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.anecdoct_self_exe = self.arg0_paths.anecdoct_self_exe.clone();
        config.anecdoct_linux_sandbox_exe = self.arg0_paths.anecdoct_linux_sandbox_exe.clone();
    ...
    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(anecdoct_home: PathBuf) -> Self {
        Self::new_for_tests(
            anecdoct_home,
            Vec::new(),
