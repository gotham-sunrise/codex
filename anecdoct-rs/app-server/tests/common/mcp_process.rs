...
use anecdoct_app_server_protocol::ClientNotification;
...
use anecdoct_app_server_protocol::ConfigReadParams;
use anecdoct_app_server_protocol::ConfigValueWriteParams;
use anecdoct_app_server_protocol::ExperimentalFeatureListParams;
use anecdoct_app_server_protocol::FeedbackUploadParams;
use anecdoct_app_server_protocol::FsCopyParams;
use anecdoct_app_server_protocol::FsCreateDirectoryParams;
use anecdoct_app_server_protocol::FsGetMetadataParams;
use anecdoct_app_server_protocol::FsReadDirectoryParams;
use anecdoct_app_server_protocol::FsReadFileParams;
use anecdoct_app_server_protocol::FsRemoveParams;
use anecdoct_app_server_protocol::FsUnwatchParams;
use anecdoct_app_server_protocol::FsWatchParams;
use anecdoct_app_server_protocol::FsWriteFileParams;
...
use anecdoct_app_server_protocol::JSONRPCError;
...
use anecdoct_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use anecdoct_app_server_protocol::ProcessKillParams;
use anecdoct_app_server_protocol::ProcessResizePtyParams;
use anecdoct_app_server_protocol::ProcessSpawnParams;
use anecdoct_app_server_protocol::ProcessWriteStdinParams;
...
use anecdoct_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use anecdoct_login::default_client::ANECDOCT_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
use anyhow::Context;
...
pub const DEFAULT_CLIENT_NAME: &str = "anecdoct-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "ANECDOCT_APP_SERVER_DISABLE_MANAGED_CONFIG";

impl McpProcess {
    pub async fn new(anecdoct_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(anecdoct_home, &[], &[DISABLE_PLUGIN_STARTUP_TASKS_ARG]).await
    }

    pub async fn new_without_managed_config(anecdoct_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(
            anecdoct_home,
    ...
    pub async fn new_without_managed_config_with_env(
        ...
        Self::new_with_env(anecdoct_home, &all_env_overrides).await
    }

    pub async fn new_with_plugin_startup_tasks(anecdoct_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(anecdoct_home, &[], &[]).await
    }
    ...
    pub async fn new_with_env_and_plugin_startup_tasks(
        ...
        Self::new_with_env_and_args(anecdoct_home, env_overrides, &[]).await
    }

    pub async fn new_with_args(anecdoct_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        let mut all_args = vec![DISABLE_PLUGIN_STARTUP_TASKS_ARG];
        all_args.extend_from_slice(args);
        Self::new_with_env_and_args(anecdoct_home, &[], &all_args).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = anecdoct_utils_cargo_bin::cargo_bin("anecdoct-app-server")
            .context("should find binary for anecdoct-app-server")?;
        let mut cmd = Command::new(program);

        cmd.stdin(Stdio::piped());
        cmd.stdout(Stdio::piped());
        cmd.stderr(Stdio::piped());
        cmd.current_dir(anecdoct_home);
        ...
        cmd.env(
            ...
            anecdoct_home.join("managed_config.toml"),
        );
        cmd.env_remove(ANECDOCT_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("anecdoct-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: anecdoct_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `anecdoct-app-server` child process.
        //
