...
fn create_config_toml_custom_provider(
    ...
    let requires_line = if requires_lambogenius_auth {
...
fn create_config_toml(anecdoct_home: &Path) -> std::io::Result<()> {
    let config_toml = anecdoct_home.join("config.toml");
    std::fs::write(
        config_toml,
        r#"
...
async fn get_auth_status_no_auth() -> Result<()> {
    ...
    create_config_toml(anecdoct_home.path())?;
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    ...
    assert_eq!(status.auth_token, None, "expected no token");
    assert_eq!(
        status.requires_lambogenius_auth,
        Some(false),
        "requires_lambogenius_auth should be false",
    );
    Ok(())
}
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    create_config_toml_forced_login(anecdoct_home.path(), "chatgpt")?;
