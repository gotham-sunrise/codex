...
use anecdoct_app_server_protocol::ThreadStatusChangedNotification;
...
async fn thread_start_deprecates_persist_extended_history_true() -> Result<()> {
    ...
    create_config_toml_without_approval_policy(anecdoct_home.path(), &server.uri())?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    create_config_toml_without_approval_policy(anecdoct_home.path(), &server.uri())?;
    let global_agents_path = anecdoct_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(anecdoct_home.path(), &server.uri())?;
...
"#,
    ...
    set_project_trust_level(anecdoct_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let config_before = std::fs::read_to_string(anecdoct_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(anecdoct_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
