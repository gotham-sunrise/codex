use anecdoct_app_server::in_process;
...
use anecdoct_core::ARCHIVED_SESSIONS_SUBDIR;
...
use anecdoct_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use anecdoct_thread_store::AppendThreadItemsParams;
...
async fn thread_turns_list_can_page_backward_and_forward() -> Result<()> {
    ...
    let rollout_path = rollout_path(anecdoct_home.path(), filename_ts, &conversation_id);
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let thread_id =
        anecdoct_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(anecdoct_home.path(), &store_id)?;
    ...
    let client = in_process::start(InProcessStartArgs {
        ...
        thread_config_loader: Arc::new(anecdoct_config::NoopThreadConfigLoader),
        ...
        initialize: InitializeParams {
            client_info: ClientInfo {
                name: "anecdoct-app-server-tests".to_string(),
                title: None,
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = anecdoct_protocol::ThreadId::from_string(&thread.id)?;
    store
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let thread_id =
        anecdoct_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(anecdoct_home.path(), &store_id)?;
    let store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };
...
async fn thread_read_can_return_archived_threads_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(anecdoct_home.path(), filename_ts, &conversation_id);
    let archived_dir = anecdoct_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
) -> anyhow::Result<anecdoct_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
fn turn_user_texts(turns: &[anecdoct_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[anecdoct_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
async fn seed_pathless_store_thread(
    ...
    thread_id: anecdoct_protocol::ThreadId,
) -> Result<()> {
    store
...
fn create_config_toml(anecdoct_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = anecdoct_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
