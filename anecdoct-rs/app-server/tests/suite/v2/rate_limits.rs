...
async fn get_account_rate_limits_requires_auth() -> Result<()> {
    ...
    assert_eq!(
        error.error.message,
        "anecdoct account authentication required to read rate limits"
    );
...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(anecdoct_home.path(), &server_url)?;
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "anecdoct_other",
                "metered_feature": "anecdoct_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
...
async fn send_add_credits_nudge_email_requires_auth() -> Result<()> {
    ...
    assert_eq!(
        error.error.message,
        "anecdoct account authentication required to notify workspace owner"
    );
...
async fn send_add_credits_nudge_email_posts_expected_body() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/anecdoct/accounts/send_add_credits_nudge_email"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
...
fn write_chatgpt_base_url(anecdoct_home: &Path, base_url: &str) -> std::io::Result<()> {
