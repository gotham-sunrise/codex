...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_anecdoct_home = AbsolutePathBuf::try_from(anecdoct_home.path().canonicalize()?)?;
    create_config_toml(anecdoct_home.path(), &server.uri(), "never")?;
    let mut mcp = McpProcess::new(anecdoct_home.path()).await?;

    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "anecdoct_vscode".to_string(),
    ...
    let InitializeResponse {
        user_agent,
        anecdoct_home: response_anecdoct_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("anecdoct_vscode/"));
    assert_eq!(response_anecdoct_home, expected_anecdoct_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "anecdoct_app_server_daemon".to_string(),
            title: Some("Anecdoct App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("anecdoct_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_anecdoct_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let anecdoct_home = TempDir::new()?;
    create_config_toml(anecdoct_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "anecdoct-backend".to_string(),
            title: Some("Anecdoct Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        ...
        &[(
            ...
            Some("anecdoct_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("anecdoct_originator_via_env_var/"));
    assert_eq!(response_anecdoct_home, expected_anecdoct_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        anecdoct_home.path(),
        &[("ANECDOCT_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let anecdoct_home = TempDir::new()?;
    let notify_file = anecdoct_home.path().join("notify.json");
    let notify_capture = cargo_bin("anecdoct-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(anecdoct_home, server_uri, approval_policy, "")
}
