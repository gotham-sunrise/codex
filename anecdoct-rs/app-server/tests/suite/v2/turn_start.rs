use anecdoct_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use anecdoct_app_server::INVALID_PARAMS_ERROR_CODE;
use anecdoct_app_server_protocol::ByteRange;
...
use anecdoct_app_server_protocol::FileChangePatchUpdatedNotification;
...
use anecdoct_app_server_protocol::TextElement;
...
use anecdoct_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use anecdoct_core::test_support::all_model_presets;
...
const TEST_ORIGINATOR: &str = "anecdoct_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two Anecdoct turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = anecdoct_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Anecdoct VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(anecdoct_home.path())?;
    let cache_path = anecdoct_home.path().join("models_cache.json");
    ...
    write_test_skill(anecdoct_home.path(), "alpha-skill")?;
    write_test_skill(anecdoct_home.path(), "beta-skill")?;
...
async fn turn_start_tracks_turn_event_analytics() -> Result<()> {
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "anecdoct_turn_event").await?;
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: anecdoct_home.path().to_path_buf().try_into()?,
            }]),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Anecdoct turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        anecdoct_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-anecdoct-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let anecdoct_home = tmp.path().to_path_buf();
    ...
    create_config_toml(
        anecdoct_home.as_path(),
        &server.uri(),
        "untrusted",
        &BTreeMap::default(),
    )?;

    let mut mcp = McpProcess::new(anecdoct_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(anecdoct_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(anecdoct_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(&anecdoct_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(
                anecdoct_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    create_config_toml(&anecdoct_home, &server.uri(), "never", &BTreeMap::default())?;
    std::fs::write(
        anecdoct_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![anecdoct_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    write_models_cache(&anecdoct_home)?;
    let cache_path = anecdoct_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        anecdoct_home.path().join("custom-role.toml"),
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(anecdoct_home.path().join("config.toml"))?;
...
fn write_test_skill(anecdoct_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = anecdoct_home.join("skills").join(name);
