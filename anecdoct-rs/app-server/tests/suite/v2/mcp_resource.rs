...
use anecdoct_app_server_protocol::McpResourceContent;
...
const TEST_RESOURCE_URI: &str = "test://anecdoct/resource";
const TEST_BLOB_RESOURCE_URI: &str = "test://anecdoct/resource.bin";
...
async fn start_resource_apps_mcp_server() -> Result<(String, JoinHandle<()>)> {
    ...
    let router = Router::new().nest_service("/api/anecdoct/apps", mcp_service);
