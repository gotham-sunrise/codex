...
use anecdoct_protocol::protocol::SessionSource as CoreSessionSource;
...
async fn init_mcp(anecdoct_home: &Path) -> Result<McpProcess> {
...
async fn list_threads_with_sort(
    ...
    let request_id = mcp
        .send_thread_list_request(anecdoct_app_server_protocol::ThreadListParams {
            cursor,
            limit,
            sort_key,
            sort_direction: None,
...
async fn thread_list_basic_empty() -> Result<()> {
    ...
    create_minimal_config(anecdoct_home.path())?;

    let mut mcp = init_mcp(anecdoct_home.path()).await?;
...
async fn thread_list_reports_system_error_idle_flag_after_failed_turn() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    create_runtime_config(anecdoct_home.path(), &server.uri())?;
    let mut mcp = init_mcp(anecdoct_home.path()).await?;
...
fn create_minimal_config(anecdoct_home: &std::path::Path) -> std::io::Result<()> {
...
fn create_runtime_config(anecdoct_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = anecdoct_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
async fn thread_list_respects_cwd_filters() -> Result<()> {
    ...
    let first_target_cwd = anecdoct_home.path().join("first-target-cwd");
    let second_target_cwd = anecdoct_home.path().join("second-target-cwd");
...
"#,
    ...
    state_db
        ...
        .await?;
    let rollout_config = anecdoct_rollout::RolloutConfig {
        anecdoct_home: anecdoct_home.path().to_path_buf(),
        sqlite_home: anecdoct_home.path().to_path_buf(),
        cwd: anecdoct_home.path().to_path_buf(),
        model_provider_id: "mock_provider".to_string(),
        generate_memories: false,
    };
    let repaired_page = anecdoct_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        anecdoct_core::ThreadSortKey::CreatedAt,
        anecdoct_core::SortDirection::Desc,
        &[],
...
"#,
    ...
    let stale_cwd = anecdoct_home.path().join("stale-cwd");
...
async fn thread_list_sort_updated_at_orders_by_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-01-01T10-00-00", &id_old).as_path(),
        "2025-01-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-01-01T11-00-00", &id_mid).as_path(),
        "2025-01-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-01-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_paginates_with_cursor() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-02-01T10-00-00", &id_a).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-02-01T11-00-00", &id_b).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-02-01T12-00-00", &id_c).as_path(),
...
async fn thread_list_backwards_cursor_can_seed_forward_delta_sync() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-02-01T10-00-00", &id_old).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-02-01T11-00-00", &id_watermark).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    ...
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-02-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_uses_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(anecdoct_home.path(), "2025-02-01T10-00-00", &thread_id).as_path(),
...
async fn thread_list_archived_filter() -> Result<()> {
    ...
    let archived_source = rollout_path(anecdoct_home.path(), "2025-03-01T09-00-00", &archived_id);
