...
use anecdoct_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut anecdoct_core::config::Config) {
...
pub(crate) async fn mount_analytics_capture(
    ...
    Mock::given(method("POST"))
        .and(path("/anecdoct/analytics-events/events"))
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        .iter()
        .find(|event| event["event_type"] == "anecdoct_thread_initialized")
        .ok_or_else(|| anyhow::anyhow!("anecdoct_thread_initialized event should be present"))
}
