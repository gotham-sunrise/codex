...
use anecdoct_app_server_protocol::PluginAuthPolicy;
use anecdoct_app_server_protocol::PluginInstallPolicy;
...
use anecdoct_core::config::set_project_trust_level;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "ANECDOCT_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
const ALTERNATE_MARKETPLACE_RELATIVE_PATH: &str = ".claude-plugin/marketplace.json";
const ALTERNATE_PLUGIN_MANIFEST_RELATIVE_PATH: &str = ".claude-plugin/plugin.json";

fn write_plugins_enabled_config(anecdoct_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
        anecdoct_home.join("config.toml"),
        r#"[features]
plugins = true
"#,
    )
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    let home = anecdoct_home.path().to_string_lossy().into_owned();
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.anecdoct-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.anecdoct-plugin/plugin.json"),
    ...
    assert_eq!(
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![PluginSummary {
                ...
                availability: anecdoct_app_server_protocol::PluginAvailability::Available,
                interface: None,
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_anecdoct_plugins_disabled() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_anecdoct_plugins_setting() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.anecdoct-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        anecdoct_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.anecdoct-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    auth_policy: PluginAuthPolicy::OnInstall,
                    availability: anecdoct_app_server_protocol::PluginAvailability::Available,
                    interface: Some(anecdoct_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    std::fs::create_dir_all(anecdoct_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(anecdoct_home.path())?;
    std::fs::write(
        anecdoct_home
            .path()
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&anecdoct_home, "anecdoct-curated", "enabled-plugin")?;
    write_installed_plugin(&anecdoct_home, "anecdoct-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "anecdoct-curated",
...
[plugins."enabled-plugin@anecdoct-curated"]
enabled = true

[plugins."disabled-plugin@anecdoct-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "anecdoct-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@anecdoct-curated");
    ...
    assert_eq!(
        ...
        "disabled-plugin@anecdoct-curated"
    );
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@anecdoct-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    std::fs::create_dir_all(anecdoct_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&anecdoct_home, "anecdoct-curated", "shared-plugin")?;
    std::fs::write(
        anecdoct_home
            .path()
            .join(".agents/plugins/marketplace.json"),
...
[plugins."shared-plugin@anecdoct-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".anecdoct"))?;
    std::fs::write(
        workspace_enabled.path().join(".anecdoct/config.toml"),
        r#"[plugins."shared-plugin@anecdoct-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@anecdoct-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    "developerName": "Lambogenius",
    "category": "Productivity",
    "capabilities": ["Interactive", "Write"],
    "websiteURL": "https://lambogenius.com/",
    "privacyPolicyURL": "https://lambogenius.com/policies/row-privacy-policy/",
    "termsOfServiceURL": "https://lambogenius.com/policies/row-terms-of-use/",
    "defaultPrompt": [
    ...
    assert_eq!(plugin.id, "demo-plugin@anecdoct-curated");
    ...
    assert_eq!(
        ...
        Some("https://lambogenius.com/")
    );
    assert_eq!(
        interface.privacy_policy_url.as_deref(),
        Some("https://lambogenius.com/policies/row-privacy-policy/")
    );
    assert_eq!(
        interface.terms_of_service_url.as_deref(),
        Some("https://lambogenius.com/policies/row-terms-of-use/")
    );
    assert_eq!(
...
async fn plugin_list_returns_installed_git_source_interface_from_cache() -> Result<()> {
  ...
  "plugins": [
    ...
    let cached_plugin_root = anecdoct_home
        .path()
        .join("plugins/cache/debug/toolkit/local");
    std::fs::create_dir_all(cached_plugin_root.join(".anecdoct-plugin"))?;
    std::fs::write(
...
async fn app_server_startup_remote_plugin_sync_runs_once() -> Result<()> {
    ...
    write_lambogenius_curated_marketplace(anecdoct_home.path(), &["linear"])?;
  ...
  {"id":"1","name":"linear","marketplace_name":"lambogenius-curated","version":"1.0.0","enabled":true}
    ...
    Mock::given(method("GET"))
        ...
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(
            ResponseTemplate::new(200).set_body_string(r#"["linear@lambogenius-curated"]"#),
        )
        .mount(&server)
        .await;

    let marker_path = anecdoct_home
        .path()
        .join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE);

    {
        let mut mcp = McpProcess::new_with_plugin_startup_tasks(anecdoct_home.path()).await?;
        timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
        ...
        let curated_marketplace = response
            ...
            .expect("expected lambogenius-curated marketplace entry");
        assert_eq!(
            curated_marketplace
                .plugins
                .into_iter()
                ...
                .collect::<Vec<_>>(),
            vec![("linear@lambogenius-curated".to_string(), true, true)]
        );
        wait_for_remote_plugin_request_count(&server, "/plugins/list", /*expected_count*/ 1)
            .await?;
    }
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let installed_path = anecdoct_home
        .path()
    ...
    wait_for_path_exists(&installed_path.join(".anecdoct-plugin/plugin.json")).await?;
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    write_installed_plugin_with_version(&anecdoct_home, "chatgpt-global", "linear", "1.0.0")?;
    write_installed_plugin_with_version(&anecdoct_home, "chatgpt-global", "stale", "1.0.0")?;
    ...
    let old_path = anecdoct_home
        .path()
        .join("plugins/cache/chatgpt-global/linear/1.0.0");
    let new_path = anecdoct_home
        .path()
        .join("plugins/cache/chatgpt-global/linear/1.2.3");
    let stale_path = anecdoct_home
        .path()
    ...
    wait_for_path_exists(&new_path.join(".anecdoct-plugin/plugin.json")).await?;
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        anecdoct_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
...
async fn plugin_list_marks_remote_plugin_disabled_by_admin() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        plugin.availability,
        anecdoct_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
    Ok(())
}
...
async fn plugin_list_remote_marketplace_replaces_local_marketplace_with_same_name() -> Result<()> {
    ...
    let local_plugin_root = anecdoct_home
        .path()
        .join(".agents/plugins/plugins/local-linear/.anecdoct-plugin");
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    write_lambogenius_curated_marketplace(anecdoct_home.path(), &["linear", "gmail"])?;
    ...
    assert_eq!(
        ...
        vec!["linear@lambogenius-curated".to_string()]
    );
    Ok(())
}
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(anecdoct_home, marketplace_name, plugin_name, "local")
}
...
fn write_lambogenius_curated_marketplace(
    anecdoct_home: &std::path::Path,
    plugin_names: &[&str],
) -> std::io::Result<()> {
    let curated_root = anecdoct_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.anecdoct-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(anecdoct_home.join(".tmp"))?;
    std::fs::write(
        anecdoct_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(anecdoct_home.join(".tmp"))?;
    std::fs::write(
        anecdoct_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
