...
use anecdoct_core::find_archived_thread_path_by_id_str;
use anecdoct_core::find_thread_path_by_id_str;
...
async fn thread_unarchive_moves_rollout_back_into_sessions_directory() -> Result<()> {
    ...
    let found_rollout_path =
        find_thread_path_by_id_str(anecdoct_home.path(), &thread.id, /*state_db_ctx*/ None)
            .await?
...
async fn thread_unarchive_preserves_pathless_store_metadata() -> Result<()> {
    ...
    create_config_toml_with_in_memory_thread_store(anecdoct_home.path(), &store_id)?;
