use anecdoct_app_server_protocol::GitInfo;
...
use anecdoct_protocol::protocol::GitInfo as RolloutGitInfo;
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_stored_thread() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    create_config_toml(anecdoct_home.path(), &server.uri())?;
    let _state_db = init_state_db(anecdoct_home.path()).await?;
...
async fn thread_metadata_update_repairs_loaded_thread_without_resetting_summary() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    create_config_toml(anecdoct_home.path(), &server.uri())?;
    let state_db = init_state_db(anecdoct_home.path()).await?;
    ...
    let rollout_path = rollout_path(anecdoct_home.path(), "2025-01-06T08-30-00", &thread_id);
    reconcile_rollout(
        Some(&state_db),
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_archived_thread() -> Result<()> {
    ...
    let archived_source = rollout_path(anecdoct_home.path(), "2025-01-06T08-30-00", &thread_id);
...
async fn init_state_db(anecdoct_home: &Path) -> Result<Arc<StateRuntime>> {
    let state_db = StateRuntime::init(anecdoct_home.to_path_buf(), "mock_provider".into()).await?;
    state_db
