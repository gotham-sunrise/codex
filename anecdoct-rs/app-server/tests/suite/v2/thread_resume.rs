...
use anecdoct_app_server_protocol::SessionSource;
...
use anecdoct_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use anecdoct_protocol::protocol::SessionSource as RolloutSessionSource;
...
use anecdoct_protocol::user_input::ByteRange;
use anecdoct_protocol::user_input::TextElement;
...
const ANECDOCT_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Anecdoct, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_rejects_unmaterialized_thread() -> Result<()> {
    ...
    create_config_toml(anecdoct_home.path(), &server.uri())?;

    let mut mcp = McpProcess::new(anecdoct_home.path()).await?;
...
async fn thread_goal_get_rejects_unmaterialized_thread() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_without_managed_config(anecdoct_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;

    let start_id = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("gpt-5.2-anecdoct".to_string()),
            ephemeral: Some(true),
            ..Default::default()
        })
        .await?;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(anecdoct_home.path(), &server.uri(), &server.uri())?;
    mount_analytics_capture(&server, anecdoct_home.path()).await?;
    ...
    assert_basic_thread_initialized_event(event, &thread.id, "gpt-5.3-anecdoct", "resumed", "user");
...
fn set_thread_source_on_fake_rollout(
    ...
    let path = rollout_path(anecdoct_home, filename_ts, thread_id);
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in [
        "anecdoct_chatgpt_android_remote",
        "anecdoct_chatgpt_ios_remote",
    ] {
...
) -> Result<anecdoct_app_server_protocol::Thread> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let anecdoct_home = TempDir::new()?;
    create_config_toml(anecdoct_home.path(), &server.uri())?;
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(anecdoct_home, filename_ts, conversation_id);
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    let state_db =
        StateRuntime::init(anecdoct_home.path().to_path_buf(), "mock_provider".into()).await?;
    ...
    state_db
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            anecdoct_state::ThreadGoalAccountingMode::ActiveOnly,
...
async fn thread_goal_clear_deletes_goal_and_notifies() -> Result<()> {
    ...
    .await??;
    let get: anecdoct_app_server_protocol::ThreadGoalGetResponse = to_response(get_resp)?;
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(anecdoct_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let anecdoct_home = TempDir::new()?;
    let config_toml = anecdoct_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
model = "gpt-5.3-anecdoct"
...
"#,
    ...
    let repo_path = anecdoct_home.path().join("repo");
    ...
    let rollout_path = rollout_path(anecdoct_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let anecdoct_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(anecdoct_home.path(), &server.uri())?;
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = McpProcess::new(anecdoct_home.path()).await?;
    ...
    let mut secondary = McpProcess::new(anecdoct_home.path()).await?;
...
async fn thread_resume_uses_path_over_thread_id_when_thread_is_running() -> Result<()> {
    ...
    let stale_path = rollout_path(anecdoct_home.path(), "2025-01-01T00-00-00", &thread_id);
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    let tmp = TempDir::new()?;
    let anecdoct_home = tmp.path().join("anecdoct_home");
    std::fs::create_dir(&anecdoct_home)?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&anecdoct_home, &server.uri())?;

    let mut primary = McpProcess::new(&anecdoct_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![anecdoct_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(anecdoct_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(anecdoct_home.path(), &server.uri())?;
    create_config_toml_with_required_broken_mcp(anecdoct_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(anecdoct_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = McpProcess::new(anecdoct_home).await?;
    ...
    let mut second_mcp = McpProcess::new(anecdoct_home).await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    let start_id = primary
        .send_thread_start_request(ThreadStartParams {
            model: Some("gpt-5.3-anecdoct".to_string()),
            ..Default::default()
        })
        .await?;
    let start_resp: JSONRPCResponse = timeout(
    ...
    assert!(
        instructions_text.contains(ANECDOCT_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn create_config_toml(anecdoct_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = anecdoct_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
...
command = "anecdoct-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
fn setup_rollout_fixture(anecdoct_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(anecdoct_home, server_uri)?;
