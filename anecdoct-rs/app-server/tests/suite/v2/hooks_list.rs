...
use anecdoct_app_server_protocol::ConfigEdit;
use anecdoct_app_server_protocol::HookEventName;
use anecdoct_app_server_protocol::HookHandlerType;
use anecdoct_app_server_protocol::HookMetadata;
use anecdoct_app_server_protocol::HookSource;
use anecdoct_app_server_protocol::HookTrustStatus;
...
fn command_hook_hash(
    ...
    let Ok(value) = anecdoct_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    anecdoct_config::version_for_toml(&value)
}

fn write_user_hook_config(anecdoct_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(anecdoct_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = anecdoct_home.join("plugins/cache/test/demo/local");
...
fn write_project_hook_config(dot_anecdoct_folder: &std::path::Path, command: &str) -> Result<()> {
    std::fs::create_dir_all(dot_anecdoct_folder)?;
    std::fs::write(
        dot_anecdoct_folder.join("config.toml"),
        format!(
            r#"[features]
hooks = true
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(anecdoct_home.path())?;
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(anecdoct_home.path(), "{ not-json")?;
...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".anecdoct"))?;
    std::fs::write(
        workspace.path().join(".anecdoct/config.toml"),
        r#"[features]
hooks = true
...
"#,
    ...
    set_project_trust_level(anecdoct_home.path(), workspace.path(), TrustLevel::Trusted)?;
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".anecdoct/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".anecdoct"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".anecdoct"), "echo worktree hook")?;
    set_project_trust_level(anecdoct_home.path(), &repo_root, TrustLevel::Trusted)?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".anecdoct/config.toml"))?;
    ...
    .await??;
    let _: anecdoct_app_server_protocol::ConfigWriteResponse = to_response(response)?;
...
async fn config_batch_write_updates_hook_trust_for_loaded_session() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    let hook_script_path = anecdoct_home.path().join("user_prompt_submit_hook.py");
    let hook_log_path = anecdoct_home
        .path()
...
"#,
    ...
    let hook_list_id = mcp
        .send_hooks_list_request(HooksListParams {
            cwds: vec![anecdoct_home.path().to_path_buf()],
        })
        .await?;
    let response: JSONRPCResponse = timeout(
        DEFAULT_TIMEOUT,
