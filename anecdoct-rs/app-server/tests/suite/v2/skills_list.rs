...
use anecdoct_app_server_protocol::SkillsChangedNotification;
...
use anecdoct_exec_server::ANECDOCT_EXEC_SERVER_URL_ENV_VAR;
use anyhow::Context;
use anyhow::Result;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = anecdoct_home.join("plugins/cache/chatgpt-global/linear/local");
    std::fs::create_dir_all(plugin_root.join(".anecdoct-plugin"))?;
    std::fs::write(
        plugin_root.join(".anecdoct-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(anecdoct_home.path())?)?;
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_anecdoct_plugins_disabled() -> Result<()>
{
    let anecdoct_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    write_skill(&anecdoct_home, "home-skill")?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace Anecdoct plugins are disabled"
    );
    Ok(())
}
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_skill(&anecdoct_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".anecdoct/skills/repo-skill");
    ...
    let mut mcp = McpProcess::new_with_env(
        anecdoct_home.path(),
        &[(ANECDOCT_EXEC_SERVER_URL_ENV_VAR, Some("none"))],
    )
    .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(anecdoct_home.path().join(&relative_cwd))?;
...
async fn skills_list_uses_cached_result_until_force_reload() -> Result<()> {
    ...
    let skill_dir = cwd.path().join(".anecdoct/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
...
async fn skills_changed_notification_is_emitted_after_skill_change() -> Result<()> {
    ...
    write_skill(&anecdoct_home, "demo")?;

    let mut mcp = McpProcess::new_with_env(
        anecdoct_home.path(),
        &[(ANECDOCT_EXEC_SERVER_URL_ENV_VAR, None)],
    )
    .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
