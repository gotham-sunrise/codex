...
async fn websocket_transport_routes_per_connection_handshake_and_responses() -> Result<()> {
    ...
    let (mut process, bind_addr) = spawn_websocket_server(anecdoct_home.path()).await?;
...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let anecdoct_home = TempDir::new()?;
    let token_file = anecdoct_home.path().join("app-server-token");
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(anecdoct_home.path(), "ws://0.0.0.0:0", &auth_args)
            .await?;
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let anecdoct_home = TempDir::new()?;
    let shared_secret_file = anecdoct_home.path().join("app-server-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "anecdoct-enroller".to_string(),
        "--ws-audience".to_string(),
        "anecdoct-app-server".to_string(),
        "--ws-max-clock-skew-seconds".to_string(),
        "1".to_string(),
    ];

    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(anecdoct_home.path(), "ws://127.0.0.1:0", &auth_args)
            .await?;
    let expired_token = signed_bearer_token(
        shared_secret.as_bytes(),
        json!({
            "exp": OffsetDateTime::now_utc().unix_timestamp() - 30,
            "iss": "anecdoct-enroller",
...
async fn websocket_transport_rejects_unauthenticated_non_loopback_startup() -> Result<()> {
    ...
    let output =
        run_websocket_server_to_completion_with_args(anecdoct_home.path(), "ws://0.0.0.0:0", &[])
            .await?;
    assert!(
        !output.status.success(),
...
pub(super) async fn spawn_websocket_server(anecdoct_home: &Path) -> Result<(Child, SocketAddr)> {
    spawn_websocket_server_with_args(anecdoct_home, "ws://127.0.0.1:0", &[]).await
}
...
pub(super) async fn spawn_websocket_server_with_args(
    ...
    cmd.arg("--listen")
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .env("ANECDOCT_HOME", anecdoct_home)
        .env("RUST_LOG", "warn");
    let mut process = cmd
        .kill_on_drop(true)
        .spawn()
