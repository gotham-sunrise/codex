...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "anecdoct@example.com"])?;
    run_git(root, &["config", "user.name", "Anecdoct Tests"])?;
...
fn disable_plugin_startup_tasks(anecdoct_home: &Path) -> Result<()> {
...
fn marketplace_install_root(anecdoct_home: &Path) -> std::path::PathBuf {
    anecdoct_home.join(INSTALLED_MARKETPLACES_DIR)
}
...
fn expected_installed_root(
    ...
    AbsolutePathBuf::try_from(
        marketplace_install_root(&anecdoct_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(anecdoct_home.path())?;

    let mut mcp = McpProcess::new(anecdoct_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;

    let debug_root = expected_installed_root(anecdoct_home.path(), "debug")?;
    let tools_root = expected_installed_root(anecdoct_home.path(), "tools")?;
