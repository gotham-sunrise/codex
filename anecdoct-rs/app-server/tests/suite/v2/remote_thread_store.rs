...
//! the temporary `anecdoct_home` must not contain rollout session files or sqlite
...
use anecdoct_app_server::in_process::InProcessServerEvent;
...
async fn thread_start_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    // Plugin startup warmups may create `.tmp` under anecdoct_home. Disable them
    // here so this regression stays focused on thread persistence artifacts.
    create_config_toml_with_thread_store(anecdoct_home.path(), &server.uri(), &store_id)?;
    ...
    assert_no_local_persistence_artifacts(anecdoct_home.path())?;

    Ok(())
}

fn assert_no_local_persistence_artifacts(anecdoct_home: &Path) -> Result<()> {
    ...
    // the isolated test anecdoct_home.
    assert!(
        !anecdoct_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !anecdoct_home.join("archived_sessions").exists(),
        "non-local thread persistence should not create archived rollout sessions"
    );
    assert!(
        !anecdoct_state::state_db_path(anecdoct_home).exists(),
        "non-local thread persistence should not create local thread sqlite"
    );

    let sqlite_artifacts = std::fs::read_dir(anecdoct_home)?
    ...
    let mut entries = anecdoct_home_entries(anecdoct_home)?;
    // Bazel test runs may initialize shell snapshot storage under anecdoct_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in anecdoct_home"
    );

    Ok(())
}

fn anecdoct_home_entries(anecdoct_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(anecdoct_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
