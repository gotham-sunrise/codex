...
const SERVER_MODEL: &str = "gpt-5.3-anecdoct";
const TRUSTED_ACCESS_FOR_CYBER_VERIFICATION: &str = "trusted_access_for_cyber";
const CYBER_POLICY_MESSAGE: &str =
    "This request has been flagged for potentially high-risk cyber activity.";

#[tokio::test]
async fn lambogenius_model_header_mismatch_emits_model_rerouted_notification_v2() -> Result<()> {
    ...
    let response = responses::sse_response(body).insert_header("Lambogenius-Model", SERVER_MODEL);
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: anecdoct_app_server_protocol::TurnError {
                message: CYBER_POLICY_MESSAGE.to_string(),
                anecdoct_error_info: Some(AnecdoctErrorInfo::CyberPolicy),
...
-> Result<()> {
    ...
    let body = responses::sse(vec![
        serde_json::json!({
            ...
            "response": {
                "id": "resp-1",
                "headers": {
                    "Lambogenius-Model": SERVER_MODEL
                }
            }
        }),
        responses::ev_assistant_message("msg-1", "Done"),
    ...
    let response =
        responses::sse_response(body).insert_header("Lambogenius-Model", REQUESTED_MODEL);
    let _response_mock = responses::mount_response_once(&server, response).await;
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.anecdoct_error_info == Some(AnecdoctErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
