...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(anecdoct_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".anecdoct-plugin/plugin.json"),
    )
    .await;
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_anecdoct_plugins_disabled() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("Anecdoct plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                anecdoct_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        McpProcess::new_with_args(anecdoct_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_tracks_analytics_event() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    write_analytics_config(anecdoct_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        anecdoct_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_analytics_config(anecdoct_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        anecdoct_home.join("config.toml"),
...
async fn mount_backend_analytics_events(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/backend-api/anecdoct/analytics-events/events"))
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST"
                    && request
                        .url
                        .path()
                        .ends_with("/anecdoct/analytics-events/events")
            }) {
...
fn configure_remote_plugin_test(
    ...
    write_remote_plugin_catalog_config(anecdoct_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        anecdoct_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
