use anecdoct_app_server_protocol::AppConfig;
use anecdoct_app_server_protocol::AppToolApproval;
use anecdoct_app_server_protocol::AppsConfig;
...
use anecdoct_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use anecdoct_app_server_protocol::ToolsV2;
use anecdoct_app_server_protocol::WriteStatus;
use anecdoct_core::config::set_project_trust_level;
use anecdoct_protocol::config_types::TrustLevel;
use anecdoct_protocol::config_types::WebSearchContextSize;
use anecdoct_protocol::config_types::WebSearchLocation;
...
fn write_config(anecdoct_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let anecdoct_home_path = anecdoct_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(anecdoct_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("anecdoct")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    write_config(&anecdoct_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_anecdoct_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let anecdoct_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path =
        AbsolutePathBuf::resolve_path_against_base("config.toml", anecdoct_home);
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let anecdoct_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(anecdoct_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    let config: toml::Value =
        toml::from_str(&std::fs::read_to_string(anecdoct_home.join("config.toml"))?)?;
    assert_eq!(
...
fn assert_layers_user_then_optional_system(
    layers: &[anecdoct_app_server_protocol::ConfigLayer],
