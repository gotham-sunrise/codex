...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&anecdoct_home, &workspace).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(anecdoct_protocol::lambogenius_models::ReasoningEffort::Medium),
            summary: Some(anecdoct_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(anecdoct_app_server_protocol::AskForApproval::UnlessTrusted),
            sandbox_policy: Some(
...
async fn create_zsh_test_mcp_process(anecdoct_home: &Path, zdotdir: &Path) -> Result<McpProcess> {
    let zdotdir = zdotdir.to_string_lossy().into_owned();
    McpProcess::new_with_env(anecdoct_home, &[("ZDOTDIR", Some(zdotdir.as_str()))]).await
}
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = anecdoct_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("anecdoct-rs/app-server/tests/suite/zsh");
