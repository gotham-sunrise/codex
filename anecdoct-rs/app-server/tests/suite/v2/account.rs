...
use anecdoct_app_server_protocol::ChatgptAuthTokensRefreshReason;
use anecdoct_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
const LOGIN_ISSUER_ENV_VAR: &str = "ANECDOCT_APP_SERVER_LOGIN_ISSUER";
...
struct CreateConfigTomlParams {
    ...
    requires_lambogenius_auth: Option<bool>,
    base_url: Option<String>,
    model_provider_id: Option<String>,
    extra_provider_config: Option<String>,
}

fn create_config_toml(anecdoct_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
    ...
    let requires_line = match params.requires_lambogenius_auth {
        Some(true) => "requires_lambogenius_auth = true\n".to_string(),
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(anecdoct_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(anecdoct_home.path().join("auth.json").exists());

    let mut mcp =
        McpProcess::new_with_env(anecdoct_home.path(), &[("LAMBOGENIUS_API_KEY", None)]).await?;
    ...
    assert!(
        !anecdoct_home.path().join("auth.json").exists(),
...
async fn set_auth_token_updates_account_and_notifies() -> Result<()> {
    ...
    create_config_toml(
        ...
        CreateConfigTomlParams {
            requires_lambogenius_auth: Some(true),
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(anecdoct_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<anecdoct_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(anecdoct_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![anecdoct_app_server_protocol::UserInput::Text {
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/anecdoct/device"));
...
async fn get_account_no_auth() -> Result<()> {
    ...
    assert_eq!(account.requires_lambogenius_auth, true);
    Ok(())
}
...
async fn get_account_when_auth_not_required() -> Result<()> {
    ...
    create_config_toml(
        anecdoct_home.path(),
        CreateConfigTomlParams {
            requires_lambogenius_auth: Some(false),
            ..Default::default()
        },
    )?;
