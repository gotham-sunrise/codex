...
use anecdoct_app_server_protocol::HookEventName;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                anecdoct_home.path().join("marketplace.json"),
            )?),
...
#[tokio::test]
async fn plugin_read_returns_canonical_lambogenius_curated_marketplace_name() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    ...
    std::fs::write(
        ...
        r#"{
  "name": "demo-plugin",
  "description": "Lambogenius curated plugin"
}"#,
    )?;
    std::fs::write(
        anecdoct_home.path().join("config.toml"),
...
[plugins."demo-plugin@lambogenius-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&anecdoct_home, "lambogenius-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "lambogenius-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(
        response.plugin.summary.id,
        "demo-plugin@lambogenius-curated"
    );
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(anecdoct_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&anecdoct_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "anecdoct-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(anecdoct_home.path().join(".tmp"))?;
    std::fs::write(
        anecdoct_home
            .path()
...
"#,
    ...
    std::fs::write(
        plugin_root.join("skills/thread-summarizer/agents/lambogenius.yaml"),
        r#"policy:
  products:
    - ANECDOCT
"#,
    ...
    std::fs::write(
        plugin_root.join("skills/chatgpt-only/agents/lambogenius.yaml"),
        r#"policy:
  products:
    - CHATGPT
"#,
...
[plugins."demo-plugin@anecdoct-curated"]
enabled = true

[hooks.state."demo-plugin@anecdoct-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&anecdoct_home, "anecdoct-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "anecdoct-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@anecdoct-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            anecdoct_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@anecdoct-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            anecdoct_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@anecdoct-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            anecdoct_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@anecdoct-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_returns_app_needs_auth() -> Result<()> {
    ...
    let anecdoct_home = TempDir::new()?;
    write_connectors_config(anecdoct_home.path(), &server_url)?;
    write_chatgpt_auth(
        anecdoct_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_plugins_enabled_config(anecdoct_home: &TempDir) -> Result<()> {
    std::fs::write(
        anecdoct_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/anecdoct/apps", mcp_service);
...
fn write_connectors_config(anecdoct_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
