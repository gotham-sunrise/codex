...
use anecdoct_app_server_protocol::RemoteControlDisableResponse;
use anecdoct_app_server_protocol::RemoteControlEnableResponse;
use anecdoct_app_server_protocol::RemoteControlStatusReadResponse;
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let _backend = BlockingRemoteControlBackend::start(anecdoct_home.path()).await?;
    let mut mcp = McpProcess::new(anecdoct_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_status_read_returns_connecting_status_after_enable() -> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(anecdoct_home.path()).await?;
    let mut mcp = McpProcess::new(anecdoct_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
impl BlockingRemoteControlBackend {
    async fn start(anecdoct_home: &std::path::Path) -> Result<Self> {
