...
-> Result<()> {
    ...
    let home_dir = anecdoct_home.path().display().to_string();
    let mut mcp =
        McpProcess::new_with_env(anecdoct_home.path(), &[("HOME", Some(home_dir.as_str()))])
            .await?;
...
{
    let anecdoct_home = TempDir::new()?;
    let marketplace_root = anecdoct_home.path().join("marketplace");
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(anecdoct_home.path().join(".claude"))?;
    ...
    std::fs::write(
        anecdoct_home.path().join(".claude").join("settings.json"),
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let project_root = anecdoct_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = anecdoct_home.path().join(".claude/projects/repo");
...
-> Result<()> {
    ...
    let project_config_dir = project_root.join(".anecdoct");
...
async fn external_agent_config_import_rejects_undetected_session_paths() -> Result<()> {
    ...
    let undetected_session_path = anecdoct_home.path().join("outside.jsonl");
    std::fs::create_dir_all(&project_root)?;
    std::fs::create_dir_all(&session_dir)?;
...
async fn external_agent_config_import_compacts_huge_session_before_first_follow_up() -> Result<()> {
    ...
    write_mock_responses_config_toml(
        ...
        /*requires_lambogenius_auth*/ None,
        "mock_provider",
