...
const MODEL_PROVIDER: &str = "lambogenius";
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_conversation_summary_by_relative_rollout_path_resolves_from_anecdoct_home()
-> Result<()> {
    let anecdoct_home = TempDir::new()?;
    let conversation_id = create_fake_rollout(
        anecdoct_home.path(),
    ...
    let relative_path = rollout_path
        .strip_prefix(anecdoct_home.path())?
        .to_path_buf();
