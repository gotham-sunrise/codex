...
`anecdoct-tools` is the shared support crate for building, adapting, planning, and
executing model-visible tool sets outside `anecdoct-core`.
...
compatibility-sensitive orchestration in `anecdoct-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `anecdoct-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `anecdoct-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `anecdoct-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `anecdoct-core`.
3. Leave compatibility-sensitive adapters in `anecdoct-core` while downstream
...
`anecdoct-core`, that is a sign to revisit the extraction boundary before adding
more here.
