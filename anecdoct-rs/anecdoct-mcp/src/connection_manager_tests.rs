use super::*;
use crate::anecdoct_apps::ANECDOCT_APPS_TOOLS_CACHE_SCHEMA_VERSION;
use crate::anecdoct_apps::AnecdoctAppsToolsCacheContext;
use crate::anecdoct_apps::load_startup_cached_anecdoct_apps_tools_snapshot;
use crate::anecdoct_apps::read_cached_anecdoct_apps_tools;
use crate::anecdoct_apps::write_cached_anecdoct_apps_tools;
use crate::declared_lambogenius_file_input_param_names;
...
fn create_anecdoct_apps_tools_cache_context(
    anecdoct_home: PathBuf,
    account_id: Option<&str>,
    chatgpt_user_id: Option<&str>,
) -> AnecdoctAppsToolsCacheContext {
    AnecdoctAppsToolsCacheContext {
        anecdoct_home,
        user_key: AnecdoctAppsToolsCacheKey {
...
#[test]
fn declared_lambogenius_file_fields_treat_names_literally() {
    let meta = serde_json::json!({
        "lambogenius/fileParams": ["file", "input_file", "attachments"]
    });
    let meta = meta.as_object().expect("meta object");

    assert_eq!(
        declared_lambogenius_file_input_param_names(Some(meta)),
        vec![
            "file".to_string(),
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(ANECDOCT_APPS_MCP_SERVER_NAME, "upload").tool;
    ...
    tool.meta = Some(Meta(
        serde_json::json!({
            "lambogenius/fileParams": ["file", "files"]
        })
        .as_object()
        .expect("object")
        .clone(),
...
#[test]
fn anecdoct_apps_tools_cache_is_overwritten_by_last_write() {
    let anecdoct_home = tempdir().expect("tempdir");
    let cache_context = create_anecdoct_apps_tools_cache_context(
        anecdoct_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(ANECDOCT_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(ANECDOCT_APPS_MCP_SERVER_NAME, "two")];

    write_cached_anecdoct_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 = read_cached_anecdoct_apps_tools(&cache_context)
        .expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_anecdoct_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 = read_cached_anecdoct_apps_tools(&cache_context)
        .expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn anecdoct_apps_tools_cache_is_scoped_per_user() {
    let anecdoct_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_anecdoct_apps_tools_cache_context(
        anecdoct_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_anecdoct_apps_tools_cache_context(
        anecdoct_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(ANECDOCT_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(ANECDOCT_APPS_MCP_SERVER_NAME, "two")];

    write_cached_anecdoct_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_anecdoct_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_anecdoct_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_anecdoct_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn anecdoct_apps_tools_cache_filters_disallowed_connectors() {
    let anecdoct_home = tempdir().expect("tempdir");
    let cache_context = create_anecdoct_apps_tools_cache_context(
        anecdoct_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached =
        read_cached_anecdoct_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn anecdoct_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let anecdoct_home = tempdir().expect("tempdir");
    let cache_context = create_anecdoct_apps_tools_cache_context(
        anecdoct_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": ANECDOCT_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(ANECDOCT_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_anecdoct_apps_tools(&cache_context).is_none());
}

#[test]
fn anecdoct_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let anecdoct_home = tempdir().expect("tempdir");
    let cache_context = create_anecdoct_apps_tools_cache_context(
        anecdoct_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_anecdoct_apps_tools_loads_from_disk_cache() {
    let anecdoct_home = tempdir().expect("tempdir");
    let cache_context = create_anecdoct_apps_tools_cache_context(
        anecdoct_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_anecdoct_apps_tools(&cache_context, &cached_tools);

    let startup_snapshot = load_startup_cached_anecdoct_apps_tools_snapshot(
        ANECDOCT_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
    );
    ...
    assert_eq!(startup_tools[0].server_name, ANECDOCT_APPS_MCP_SERVER_NAME);
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__anecdoct_apps__", "calendar_create_event")
        })
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, ANECDOCT_APPS_MCP_SERVER_NAME);
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = ANECDOCT_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `anecdoct mcp login {server_name}`."
    );
