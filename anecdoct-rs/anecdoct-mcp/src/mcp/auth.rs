...
use anecdoct_rmcp_client::OAuthProviderError;
use anecdoct_rmcp_client::determine_streamable_http_auth_status;
use anecdoct_rmcp_client::discover_streamable_http_oauth;
use anyhow::Result;
...
use super::ANECDOCT_APPS_MCP_SERVER_NAME;
    ...
    I: IntoIterator<Item = (&'a String, &'a EffectiveMcpServer)>,
{
    let futures = servers.into_iter().map(|(name, server)| {
        ...
        let has_runtime_auth = name == ANECDOCT_APPS_MCP_SERVER_NAME
