...
use anecdoct_protocol::mcp::Resource;
use anecdoct_protocol::mcp::ResourceTemplate;
use anecdoct_protocol::mcp::Tool;
...
use crate::anecdoct_apps::anecdoct_apps_tools_cache_key;
use crate::connection_manager::McpConnectionManager;
use crate::runtime::McpRuntimeEnvironment;
use crate::server::EffectiveMcpServer;

pub const ANECDOCT_APPS_MCP_SERVER_NAME: &str = "anecdoct_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const ANECDOCT_CONNECTORS_TOKEN_ENV_VAR: &str = "ANECDOCT_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `anecdoct_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `anecdoct-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    ...
    /// Anecdoct home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `anecdoct-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn with_anecdoct_apps_mcp(
    mut servers: HashMap<String, EffectiveMcpServer>,
    auth: Option<&AnecdoctAuth>,
    config: &McpConfig,
) -> HashMap<String, EffectiveMcpServer> {
    if host_owned_anecdoct_apps_enabled(config, auth) {
        servers.insert(
            ANECDOCT_APPS_MCP_SERVER_NAME.to_string(),
            EffectiveMcpServer::configured(anecdoct_apps_mcp_server_config(config)),
        );
    } else {
        servers.remove(ANECDOCT_APPS_MCP_SERVER_NAME);
    }
    servers
}

pub fn host_owned_anecdoct_apps_enabled(config: &McpConfig, auth: Option<&AnecdoctAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(AnecdoctAuth::uses_anecdoct_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    with_anecdoct_apps_mcp(servers, auth, config)
}
...
pub async fn read_mcp_resource(
    ...
    let host_owned_anecdoct_apps_enabled = host_owned_anecdoct_apps_enabled(config, auth);
    ...
    let (manager, cancel_token) = McpConnectionManager::new(
        ...
        anecdoct_apps_tools_cache_key(auth),
...
pub(crate) fn anecdoct_apps_mcp_url(config: &McpConfig) -> String {
...
fn anecdoct_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(ANECDOCT_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => {
            Some(ANECDOCT_CONNECTORS_TOKEN_ENV_VAR.to_string())
        }
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(ANECDOCT_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_anecdoct_apps_base_url(base_url: &str) -> String {
...
fn anecdoct_apps_mcp_url_for_base_url(
    base_url: &str,
    apps_mcp_path_override: Option<&str>,
) -> String {
    let base_url = normalize_anecdoct_apps_base_url(base_url);
    let (base_url, default_path) = if base_url.contains("/backend-api") {
        (base_url, "wham/apps")
    } else if base_url.contains("/api/anecdoct") {
        (base_url, "apps")
    } else {
        (format!("{base_url}/api/anecdoct"), "apps")
    };
...
fn anecdoct_apps_mcp_server_config(config: &McpConfig) -> McpServerConfig {
    let url = anecdoct_apps_mcp_url(config);
    let http_headers = config.apps_mcp_product_sku.as_ref().map(|product_sku| {
        HashMap::from([("X-Lambogenius-Product-Sku".to_string(), product_sku.clone())])
    });

    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: anecdoct_apps_mcp_bearer_token_env_var(),
            http_headers,
