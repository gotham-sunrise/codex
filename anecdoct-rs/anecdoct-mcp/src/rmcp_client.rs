...
use crate::anecdoct_apps::CachedAnecdoctAppsToolsLoad;
use crate::anecdoct_apps::filter_disallowed_anecdoct_apps_tools;
use crate::anecdoct_apps::load_cached_anecdoct_apps_tools;
use crate::anecdoct_apps::load_startup_cached_anecdoct_apps_tools_snapshot;
use crate::anecdoct_apps::normalize_anecdoct_apps_callable_name;
use crate::anecdoct_apps::normalize_anecdoct_apps_callable_namespace;
use crate::anecdoct_apps::normalize_anecdoct_apps_tool_title;
use crate::anecdoct_apps::write_cached_anecdoct_apps_tools_if_needed;
...
use anecdoct_config::McpServerTransportConfig;
use anecdoct_config::types::OAuthCredentialsStoreMode;
use anecdoct_exec_server::HttpClient;
use anecdoct_exec_server::ReqwestHttpClient;
use anecdoct_protocol::protocol::Event;
use anecdoct_rmcp_client::ExecutorStdioServerLauncher;
use anecdoct_rmcp_client::LocalStdioServerLauncher;
use anecdoct_rmcp_client::RmcpClient;
use anecdoct_rmcp_client::StdioServerLauncher;
use anyhow::Result;
use anyhow::anyhow;
...
/// MCP server capability indicating that Anecdoct should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "anecdoct/sandbox-state-meta";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "anecdoct.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "anecdoct.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) anecdoct_apps_tools_cache_context: Option<AnecdoctAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.anecdoct_apps_tools_cache_context.as_ref()
            && let CachedAnecdoctAppsToolsLoad::Hit(tools) =
                load_cached_anecdoct_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.anecdoct_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        anecdoct_apps_tools_cache_context: Option<AnecdoctAppsToolsCacheContext>,
        ...
        let startup_snapshot = load_startup_cached_anecdoct_apps_tools_snapshot(
            &server_name,
            anecdoct_apps_tools_cache_context.as_ref(),
        )
            ...
            let outcome = match async {
                ...
                start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        anecdoct_apps_tools_cache_context,
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_anecdoct_apps_callable_name(
                server_name,
                &tool_def.name,
            ...
            let callable_namespace =
                normalize_anecdoct_apps_callable_namespace(server_name, connector_name.as_deref());
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title = normalize_anecdoct_apps_tool_title(
                    server_name,
                    connector_name.as_deref(),
                    title,
                );
    ...
    if server_name == ANECDOCT_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_anecdoct_apps_tools(tools));
    }
    Ok(tools)
}
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "anecdoct-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Anecdoct".into()),
            description: None,
            icons: None,
            website_url: None,
        },
    ...
    write_cached_anecdoct_apps_tools_if_needed(
        &server_name,
        anecdoct_apps_tools_cache_context.as_ref(),
        &tools,
    );
...
mod tests {
    ...
    fn custom_mcp_connector_metadata_is_stripped() {
        ...
        assert!(meta.0.contains_key("lambogenius/fileParams"));
        assert_eq!(
    ...
    #[test]
    fn anecdoct_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();
