//! Aggregates MCP server connections for Anecdoct.
//!
...
use crate::anecdoct_apps::AnecdoctAppsToolsCacheKey;
...
use anecdoct_protocol::protocol::McpStartupCompleteEvent;
use anecdoct_protocol::protocol::McpStartupFailure;
use anecdoct_protocol::protocol::McpStartupStatus;
use anecdoct_protocol::protocol::McpStartupUpdateEvent;
...
pub struct McpConnectionManager {
    ...
    host_owned_anecdoct_apps_enabled: bool,
...
impl McpConnectionManager {
    ...
    pub fn new_uninitialized_with_permission_profile(
        ...
        Self {
            ...
            host_owned_anecdoct_apps_enabled: false,
    ...
    pub fn is_host_owned_anecdoct_apps_server(&self, server_name: &str) -> bool {
        self.host_owned_anecdoct_apps_enabled && server_name == ANECDOCT_APPS_MCP_SERVER_NAME
    }
    ...
    pub async fn new(
        ...
        anecdoct_apps_tools_cache_key: AnecdoctAppsToolsCacheKey,
        host_owned_anecdoct_apps_enabled: bool,
        ...
        let anecdoct_apps_auth_provider = auth
            .filter(|auth| auth.uses_anecdoct_backend())
            .map(anecdoct_model_provider::auth_provider_from_auth);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            .await;
            let anecdoct_apps_tools_cache_context = if server_name == ANECDOCT_APPS_MCP_SERVER_NAME
            {
                Some(AnecdoctAppsToolsCacheContext {
                    anecdoct_home: anecdoct_home.clone(),
                    user_key: anecdoct_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
            ...
            let runtime_auth_provider =
                if server_name == ANECDOCT_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    anecdoct_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    /// Force-refresh anecdoct apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_anecdoct_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            .clients
            .get(ANECDOCT_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{ANECDOCT_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{ANECDOCT_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_anecdoct_apps_tools_if_needed(
            ANECDOCT_APPS_MCP_SERVER_NAME,
            managed_client.anecdoct_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
