...
//! Session orchestration stays in `anecdoct-core`.

use anecdoct_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_ANECDOCT_APPS_META_KEY: &str = "_anecdoct_apps";
...
pub struct AnecdoctAppsConnectorAuthFailure {
...
pub struct AnecdoctAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct AnecdoctAppsAuthElicitationPlan {
    pub auth_failure: AnecdoctAppsConnectorAuthFailure,
    pub elicitation: AnecdoctAppsAuthElicitation,
}

#[derive(Serialize)]
struct AnecdoctAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<AnecdoctAppsConnectorAuthFailure> {
    ...
    Some(AnecdoctAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<AnecdoctAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(AnecdoctAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &AnecdoctAppsConnectorAuthFailure,
) -> AnecdoctAppsAuthElicitation {
    AnecdoctAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_ANECDOCT_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: AnecdoctAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("anecdoct_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &AnecdoctAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Anecdoct.",
            auth_failure.connector_name
        ),
        _ => format!(
