# anecdoct-network-proxy

`anecdoct-network-proxy` is Anecdoct's local network policy enforcement proxy. It runs:
...
`anecdoct-network-proxy` reads from Anecdoct's merged `config.toml` (via `anecdoct-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $ANECDOCT_HOME/proxy/ (ca.pem + ca.key).
...
# Use exact hosts or scoped wildcards like `*.lambogenius.com` or `**.lambogenius.com`.
...
```bash
cargo run -p anecdoct-network-proxy --
```
...
`anecdoct-network-proxy` can be embedded as a library with a thin API:

```rust
use anecdoct_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `anecdoct-network-proxy` is embedded in managed Anecdoct runtime, policy decisions emit structured
OTEL-compatible events with `target=anecdoct_otel.network_proxy`.

Event name:

- `anecdoct.network_proxy.policy_decision`
...
This section documents the protections implemented by `anecdoct-network-proxy`, and the boundaries of
