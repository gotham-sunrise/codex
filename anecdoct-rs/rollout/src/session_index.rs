...
pub async fn append_thread_name(
    ...
    append_session_index_entry(anecdoct_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(anecdoct_home);
...
fn session_index_path(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(SESSION_INDEX_FILE)
}
