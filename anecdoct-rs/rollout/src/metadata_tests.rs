...
use anecdoct_state::BackfillStatus;
...
async fn extract_metadata_from_rollout_uses_session_meta() {
    ...
    let session_meta = SessionMeta {
        ...
        agent_nickname: None,
        agent_role: None,
        model_provider: Some("lambogenius".to_string()),
    ...
    let outcome = extract_metadata_from_rollout(&path, "lambogenius")
        .await
        .expect("extract");

    let builder = builder_from_session_meta(&session_meta_line, path.as_path()).expect("builder");
    let mut expected = builder.build("lambogenius");
    apply_rollout_item(&mut expected, &rollout_line.item, "lambogenius");
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let anecdoct_home = dir.path().to_path_buf();
    ...
    let runtime =
        anecdoct_state::StateRuntime::init(anecdoct_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
    let first_watermark =
        backfill_watermark_for_path(anecdoct_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), anecdoct_home.as_path(), "test-provider").await;
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let anecdoct_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = anecdoct_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = anecdoct_home.join("sessions");
