...
pub use anecdoct_state::LogEntry;
...
pub type StateDbHandle = Arc<anecdoct_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        anecdoct_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .map_err(|err| {
                anyhow::anyhow!(
    ...
    .await;
    anecdoct_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    runtime: &anecdoct_state::StateRuntime,
    anecdoct_home: &Path,
    ...
    loop {
        ...
        if backfill_state.status == anecdoct_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, anecdoct_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = anecdoct_state::state_db_path(config.sqlite_home());
    ...
    let runtime = match anecdoct_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    metrics: anecdoct_otel::MetricsClient,
    originator: &str,
) -> anecdoct_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == anecdoct_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<anecdoct_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&anecdoct_state::StateRuntime>,
    anecdoct_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.anecdoct_home() != anecdoct_home {
        warn!(
            "state db anecdoct_home mismatch: expected {}, got {}",
            ctx.anecdoct_home().display(),
            anecdoct_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => anecdoct_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => anecdoct_state::SortKey::UpdatedAt,
            },
...
pub async fn list_threads_db(
...
) -> Option<anecdoct_state::ThreadsPage> {
    let ctx = context?;
    if ctx.anecdoct_home() != anecdoct_home {
        warn!(
            "state db anecdoct_home mismatch: expected {}, got {}",
    ...
    match ctx
        .list_threads(
            page_size,
            anecdoct_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => anecdoct_state::SortDirection::Asc,
                    SortDirection::Desc => anecdoct_state::SortDirection::Desc,
                },
                search_term,
            },
        )
