...
async fn insert_state_db_thread(
    ...
    let runtime = anecdoct_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
...
async fn test_model_provider_filter_selects_only_matching_sessions() -> Result<()> {
    ...
    let lambogenius_id = Uuid::from_u128(1);
    ...
    let lambogenius_id_str = lambogenius_id.to_string();
    let none_id_str = none_id.to_string();
    let lambogenius_filter = provider_vec(&["lambogenius"]);
    let lambogenius_sessions = get_threads(
        home,
        /*page_size*/ 10,
        /*cursor*/ None,
        ThreadSortKey::CreatedAt,
        NO_SOURCE_FILTER,
        Some(lambogenius_filter.as_slice()),
        /*cwd_filters*/ None,
        "lambogenius",
    )
    .await?;
    assert_eq!(lambogenius_sessions.items.len(), 2);
    let lambogenius_ids: Vec<_> = lambogenius_sessions
        .items
        .iter()
        .filter_map(|item| item.thread_id.as_ref().map(ToString::to_string))
        .collect();
    assert!(lambogenius_ids.contains(&lambogenius_id_str));
    assert!(lambogenius_ids.contains(&none_id_str));
