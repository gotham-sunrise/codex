//! Persist Anecdoct session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use anecdoct_protocol::protocol::GitInfo as ProtocolGitInfo;
...
///
/// ```ignore
/// $ jq -C . ~/.anecdoct/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.anecdoct/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Anecdoct home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let anecdoct_home = config.anecdoct_home();
        ...
        if state_db_ctx.is_none() {
            ...
            anecdoct_state::record_fallback(
                "list_threads",
                "db_unavailable",
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(anecdoct_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = anecdoct_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(anecdoct_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(anecdoct_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.anecdoct/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.anecdoct_home().to_path_buf();
...
impl From<anecdoct_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: anecdoct_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
...
fn thread_item_from_state_metadata(item: anecdoct_state::ThreadMetadata) -> ThreadItem {
    ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &anecdoct_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
