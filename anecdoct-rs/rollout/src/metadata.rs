...
use anecdoct_state::BackfillState;
use anecdoct_state::BackfillStats;
use anecdoct_state::BackfillStatus;
use anecdoct_state::DB_ERROR_METRIC;
use anecdoct_state::DB_METRIC_BACKFILL;
use anecdoct_state::DB_METRIC_BACKFILL_DURATION_MS;
use anecdoct_state::ExtractionOutcome;
use anecdoct_state::ThreadMetadataBuilder;
use anecdoct_state::apply_rollout_item;
use chrono::DateTime;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = anecdoct_otel::global();
    ...
    let sessions_root = anecdoct_home.join(SESSIONS_SUBDIR);
    let archived_root = anecdoct_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(anecdoct_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(anecdoct_home: &Path, path: &Path) -> String {
    path.strip_prefix(anecdoct_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
