...
use anecdoct_config::default_project_root_markers;
use anecdoct_config::merge_toml_values;
use anecdoct_config::project_root_markers_from_config;
...
use anecdoct_utils_plugins::plugin_namespace_for_skill_path;
use dirs::home_dir;
...
const SKILLS_METADATA_FILENAME: &str = "lambogenius.yaml";
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$ANECDOCT_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                ...
                // Embedded system skills are cached under `$ANECDOCT_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/anecdoct/` on Unix, so treat
                // `/etc/anecdoct/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.join(SKILLS_DIR_NAME),
...
async fn discover_skills_under_root(
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by Anecdoct itself.
