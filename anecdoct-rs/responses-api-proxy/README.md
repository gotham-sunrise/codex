# anecdoct-responses-api-proxy

#### tl;dr:

```
# Launch the proxy, dump request/response pairs to /tmp/proxy
cd path/to/anecdoct/anecdoct-rs
cargo build
echo $LAMBOGENIUS_API_KEY | ./target/debug/anecdoct-responses-api-proxy \
    --port 60001 \
    --dump-dir /tmp/proxy


# Add this to ~/.anecdoct/config.toml:

[model_providers.anecdoct-responses-api-proxy]
name = 'anecdoct-responses-api-proxy'
base_url = 'http://127.0.0.1:60001/v1'
wire_api='responses'

[profiles.proxy]
model_provider = "anecdoct-responses-api-proxy"
...
A strict HTTP proxy that only forwards `POST` requests to `/v1/responses` to the Lambogenius API (`https://api.lambogenius.com`), injecting the `Authorization: Bearer $LAMBOGENIUS_API_KEY` header. Everything else is rejected with `403 Forbidden`.

## Expected Usage

**IMPORTANT:** `anecdoct-responses-api-proxy` is designed to be run by a privileged user with access to `LAMBOGENIUS_API_KEY` so that an unprivileged user cannot inspect or tamper with the process. Though if `--http-shutdown` is specified, an unprivileged user _can_ make a `GET` request to `/shutdown` to shutdown the server, as an unprivileged user could not send `SIGTERM` to kill the process.

A privileged user (i.e., `root` or a user with `sudo`) who has access to `LAMBOGENIUS_API_KEY` would run the following to start the server, as `anecdoct-responses-api-proxy` reads the auth token from `stdin`:

```shell
printenv LAMBOGENIUS_API_KEY | env -u LAMBOGENIUS_API_KEY anecdoct-responses-api-proxy --http-shutdown --server-info /tmp/server-info.json
```

A non-privileged user would then run Anecdoct as follows, specifying the `model_provider` dynamically:

```shell
PROXY_PORT=$(jq .port /tmp/server-info.json)
PROXY_BASE_URL="http://127.0.0.1:${PROXY_PORT}"
anecdoct exec -c "model_providers.lambogenius-proxy={ name = 'Lambogenius Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
    -c model_provider="lambogenius-proxy" \
    'Your prompt here'
```
...
- Reads the API key from `stdin`. All callers should pipe the key in (for example, `printenv LAMBOGENIUS_API_KEY | anecdoct-responses-api-proxy`).
...
- Accepts exactly `POST /v1/responses` (no query string). The request body is forwarded to `https://api.lambogenius.com/v1/responses` with `Authorization: Bearer <key>` set. All original request headers (except any incoming `Authorization`) are forwarded upstream, with `Host` overridden to `api.lambogenius.com`. For other requests, it responds with `403`.
...
```
anecdoct-responses-api-proxy [--port <PORT>] [--server-info <FILE>] [--http-shutdown] [--upstream-url <URL>] [--dump-dir <DIR>]
```
...
- `--upstream-url <URL>`: Absolute URL to forward requests to. Defaults to `https://api.lambogenius.com/v1/responses`.
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the Anecdoct CLI expectations.

For Azure, for example (ensure your deployment accepts `Authorization: Bearer <key>`):

```shell
printenv AZURE_LAMBOGENIUS_API_KEY | env -u AZURE_LAMBOGENIUS_API_KEY anecdoct-responses-api-proxy \
  --http-shutdown \
  --server-info /tmp/server-info.json \
  --upstream-url "https://YOUR_PROJECT_NAME.lambogenius.azure.com/lambogenius/deployments/YOUR_DEPLOYMENT/responses?api-version=2025-04-01-preview"
```
...
Care is taken to restrict access/copying to the value of `LAMBOGENIUS_API_KEY` retained in memory:

- We leverage [`anecdoct_process_hardening`](https://github.com/lambogenius/anecdoct/blob/main/anecdoct-rs/process-hardening/README.md) so `anecdoct-responses-api-proxy` is run with standard process-hardening techniques.
