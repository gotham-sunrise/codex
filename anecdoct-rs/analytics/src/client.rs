...
use anecdoct_login::default_client::create_client;
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::AnecdoctCompactionEvent) {
...
async fn send_track_events(
    ...
    let url = format!("{base_url}/anecdoct/analytics-events/events");
...
async fn send_track_events_request(auth: &AnecdoctAuth, url: &str, events: Vec<TrackEventRequest>) {
    if events.is_empty() {
        return;
    }
    ...
    let response = create_client()
        ...
        .headers(anecdoct_model_provider::auth_provider_from_auth(auth).to_auth_headers())
