...
use anecdoct_login::AuthCredentialsStoreMode;
use anecdoct_login::AuthManager;
use anecdoct_login::ExternalAuth;
use anecdoct_login::ExternalAuthRefreshContext;
use anecdoct_login::ExternalAuthTokens;
use anecdoct_login::TokenData;
...
fn lambogenius_manager_for_tests(
    anecdoct_home: std::path::PathBuf,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
) -> OpenAiModelsManager {
    lambogenius_manager_for_tests_with_auth(
        anecdoct_home,
        endpoint_client,
...
fn lambogenius_manager_for_tests_with_auth(
    anecdoct_home: std::path::PathBuf,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
    auth_manager: Option<Arc<AuthManager>>,
) -> OpenAiModelsManager {
    OpenAiModelsManager::new(anecdoct_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(anecdoct_home: &Path) -> AnecdoctAuth {
    let auth_dot_json = anecdoct_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        lambogenius_api_key: None,
        tokens: Some(TokenData {
            id_token: anecdoct_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(anecdoct_home).expect("anecdoct home should be created");
    std::fs::write(
    ...
    AnecdoctAuth::from_auth_storage(
        anecdoct_home,
...
async fn get_model_info_tracks_fallback_usage() {
    let anecdoct_home = tempdir().expect("temp dir");
    let config = ModelsManagerConfig::default();
    let manager = lambogenius_manager_for_tests(
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "lambogenius-anecdoct/gpt-image".to_string();
...
async fn refresh_available_models_sorts_by_priority() {
    ...
    let manager =
        lambogenius_manager_for_tests(anecdoct_home.path().to_path_buf(), endpoint.clone());
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        lambogenius_manager_for_tests(anecdoct_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        lambogenius_manager_for_tests(anecdoct_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = lambogenius_manager_for_tests(anecdoct_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_keeps_merging_for_api_auth() {
    ...
    let manager = lambogenius_manager_for_tests_with_auth(
        anecdoct_home.path().to_path_buf(),
        endpoint.clone(),
        Some(AuthManager::from_auth_for_testing(
...
async fn refresh_available_models_skips_network_when_external_api_key_overrides_chatgpt_auth() {
    ...
    let anecdoct_home = tempdir().expect("temp dir");
    let auth_manager =
        AuthManager::from_auth_for_testing(AnecdoctAuth::create_dummy_chatgpt_auth_for_testing());
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(anecdoct_home.path()).await;
    let manager = lambogenius_manager_for_tests_with_auth(
        anecdoct_home.path().to_path_buf(),
        endpoint.clone(),
