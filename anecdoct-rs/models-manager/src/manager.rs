...
/// Remote endpoint used by the Lambogenius-compatible model manager.
///
...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether the currently resolved auth can use Anecdoct backend-only models.
...
pub trait ModelsManager: fmt::Debug + Send + Sync {
    ...
    fn build_available_models(&self, mut remote_models: Vec<ModelInfo>) -> Vec<ModelPreset> {
        ...
        presets = ModelPreset::filter_by_auth(presets, uses_anecdoct_backend);
...
/// Lambogenius-compatible model manager backed by bundled models, cache, and `/models`.
#[derive(Debug)]
...
impl OpenAiModelsManager {
    /// Construct an Lambogenius-compatible remote model manager.
    pub fn new(
        anecdoct_home: PathBuf,
        endpoint_client: Arc<dyn ModelsEndpointClient>,
        auth_manager: Option<Arc<AuthManager>>,
    ) -> Self {
        let cache_path = anecdoct_home.join(MODEL_CACHE_FILE);
...
impl OpenAiModelsManager {
    ...
    async fn should_refresh_models(&self) -> bool {
        self.endpoint_client.uses_anecdoct_backend().await
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            anecdoct_otel::start_global_timer("anecdoct.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::client_version_to_whole();
...
pub(crate) fn construct_model_info_from_candidates(
    ...
    // retry for namespaced slugs like `custom/gpt-5.3-anecdoct`.
