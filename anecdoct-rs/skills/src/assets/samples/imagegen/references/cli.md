...
Real API calls require **network access** + `LAMBOGENIUS_API_KEY`. `--dry-run` does not.

## Quick start (works from any repo)
Set a stable path to the skill CLI (default `ANECDOCT_HOME` is `~/.anecdoct`):

```
export ANECDOCT_HOME="${ANECDOCT_HOME:-$HOME/.anecdoct}"
export IMAGE_GEN="$ANECDOCT_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
Dry-run (no API call; no network required; does not require the `lambogenius` package):
...
Generate (requires `LAMBOGENIUS_API_KEY` + network):
...
- Network/sandbox notes for fallback CLI mode: `references/anecdoct-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$ANECDOCT_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
