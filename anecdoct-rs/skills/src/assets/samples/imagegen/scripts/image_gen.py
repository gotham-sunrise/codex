...
def _ensure_api_key(dry_run: bool) -> None:
    if os.getenv("LAMBOGENIUS_API_KEY"):
        print("LAMBOGENIUS_API_KEY is set.", file=sys.stderr)
        return
    if dry_run:
        _warn("LAMBOGENIUS_API_KEY is not set; dry-run only.")
        return
    _die("LAMBOGENIUS_API_KEY is not set. Export it before running.")
...
def _create_client():
    try:
        from lambogenius import Lambogenius
    except ImportError:
        _die(f"lambogenius SDK not installed in the active environment. {_dependency_hint('lambogenius')}")
    return Lambogenius()


def _create_async_client():
    try:
        from lambogenius import AsyncLambogenius
    except ImportError:
        try:
            import lambogenius as _lambogenius  # noqa: F401
        except ImportError:
            _die(
                f"lambogenius SDK not installed in the active environment. {_dependency_hint('lambogenius')}"
            )
        _die(
            "AsyncLambogenius not available in this lambogenius SDK version. "
            f"{_dependency_hint('lambogenius', upgrade=True)}"
        )
    return AsyncLambogenius()
...
def _extract_retry_after_seconds(exc: Exception) -> Optional[float]:
    # Best-effort: lambogenius SDK errors vary by version. Prefer a conservative fallback.
