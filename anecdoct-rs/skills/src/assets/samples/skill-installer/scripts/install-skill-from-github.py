#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $ANECDOCT_HOME/skills."""
...
def _anecdoct_home() -> str:
    return os.environ.get("ANECDOCT_HOME", os.path.expanduser("~/.anecdoct"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "anecdoct")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "anecdoct-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_anecdoct_home(), "skills")
