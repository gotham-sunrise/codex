---
name: plugin-creator
description: Create and scaffold plugin directories for Anecdoct with a required `.anecdoct-plugin/plugin.json`, optional plugin folders/files, and baseline placeholders you can edit before publishing or testing. Use when Anecdoct needs to create a new personal plugin, add optional plugin structure, or generate or update personal or repo-root `.agents/plugins/marketplace.json` entries for plugin ordering and availability metadata.
---
...
2. Open `<plugin-path>/.anecdoct-plugin/plugin.json` and replace `[TODO: ...]` placeholders.

3. Generate or update the personal marketplace entry when the plugin should appear in Anecdoct UI ordering:
...
- Always creates `/<parent-plugin-directory>/<plugin-name>/.anecdoct-plugin/plugin.json`.
...
- Treat plugin order in `plugins[]` as render order in Anecdoct. Append new entries unless a user explicitly asks to reorder the list.
...
- Do not remove required structure; keep `.anecdoct-plugin/plugin.json` present.
...
- When the workflow created or updated a marketplace-backed plugin, end the final user-facing
  response with a short Anecdoct app handoff. Say `To view this in the Anecdoct app:` and write
  `View <normalized plugin name>` and `Share <normalized plugin name>` as Markdown links, not raw
  URLs or code spans.
- The View deeplink uses `anecdoct://plugins/<normalized plugin name>?marketplacePath=<absolute marketplace.json path>`.
...
- Do not add `pluginName` or `hostId` query parameters to these deeplinks. Anecdoct derives both after
