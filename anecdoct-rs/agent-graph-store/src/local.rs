...
impl std::fmt::Debug for LocalAgentGraphStore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LocalAgentGraphStore")
            .field("anecdoct_home", &self.state_db.anecdoct_home())
...
fn to_state_status(
    status: ThreadSpawnEdgeStatus,
) -> anecdoct_state::DirectionalThreadSpawnEdgeStatus {
    match status {
        ThreadSpawnEdgeStatus::Open => anecdoct_state::DirectionalThreadSpawnEdgeStatus::Open,
        ThreadSpawnEdgeStatus::Closed => anecdoct_state::DirectionalThreadSpawnEdgeStatus::Closed,
    }
}
...
mod tests {
    ...
    async fn state_runtime() -> TestRuntime {
        let anecdoct_home = TempDir::new().expect("tempdir should be created");
