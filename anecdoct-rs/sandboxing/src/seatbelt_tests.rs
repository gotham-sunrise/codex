...
use anecdoct_network_proxy::ConfigReloader;
use anecdoct_network_proxy::ConfigState;
...
use anecdoct_network_proxy::NetworkProxyConstraints;
use anecdoct_network_proxy::NetworkProxyState;
use anecdoct_network_proxy::build_config_state;
...
use anecdoct_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/anecdoct-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.anecdoct".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/anecdoct-readable");
    let unreadable = absolute_path("/tmp/anecdoct-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/anecdoct-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/anecdoct-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/anecdoct-proxy-use";
    let explicit_socket = "/tmp/anecdoct-browser-use";
    ...
    let network_proxy = NetworkProxy::builder()
        ...
        .managed_by_anecdoct(/*managed_by_anecdoct*/ false)
        .build()
        .await?;
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_anecdoct_subpaths() {
    ...
    // write to .anecdoct/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.anecdoct carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_anecdoct_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .anecdoct/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_anecdoct_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .anecdoct is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_anecdoct_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_anecdoct = repo_root.join(".anecdoct");
    let config_toml = dot_anecdoct.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_anecdoct.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_anecdoct = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_anecdoct_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_anecdoct),
        "missing {expected_dot_anecdoct}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .anecdoct/config.toml to
    ...
    dot_anecdoct_canonical: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".anecdoct")).expect("create .anecdoct");
    fs::write(
        vulnerable_root.join(".anecdoct").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .anecdoct/config.toml");
    ...
    let dot_anecdoct_canonical = vulnerable_root_canonical.join(".anecdoct");
