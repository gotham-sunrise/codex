...
- The legacy rule matcher lives in `anecdoct-execpolicy-legacy`.
...
- From the Anecdoct CLI, run `anecdoct execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
anecdoct execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
anecdoct execpolicy check \
...
```bash
cargo run -p anecdoct-execpolicy -- check --rules path/to/policy.rules git status
```
