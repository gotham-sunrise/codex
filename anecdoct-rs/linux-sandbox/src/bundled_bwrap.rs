...
fn candidates_for_exe(exe: &Path) -> Vec<PathBuf> {
    ...
    candidates.push(exe_dir.join("anecdoct-resources").join("bwrap"));
    if let Some(package_target_dir) = exe_dir.parent() {
        candidates.push(package_target_dir.join("anecdoct-resources").join("bwrap"));
    }
...
fn expected_sha256() -> Option<[u8; 32]> {
    ...
    *EXPECTED.get_or_init(|| {
        let raw_digest = option_env!("ANECDOCT_BWRAP_SHA256")?;
        let digest = parse_sha256_hex(raw_digest)
            .unwrap_or_else(|err| panic!("invalid ANECDOCT_BWRAP_SHA256 value: {err}"));
...
mod tests {
    ...
    fn finds_standalone_bundled_bwrap_next_to_exe_resources() {
        ...
        let exe = temp_dir.path().join("anecdoct");
        let expected_bwrap = temp_dir.path().join("anecdoct-resources").join("bwrap");
    ...
    fn finds_npm_bundled_bwrap_next_to_target_vendor_dir() {
        ...
        let exe = target_dir.join("anecdoct").join("anecdoct");
        let expected_bwrap = target_dir.join("anecdoct-resources").join("bwrap");
        write_executable(&exe);
        write_executable(&expected_bwrap);
