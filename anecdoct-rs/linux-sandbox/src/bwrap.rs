...
//! - sensitive subpaths such as `.git`, `.agents`, and `.anecdoct` remain
...
use anecdoct_protocol::permissions::is_protected_metadata_name;
...
use anecdoct_protocol::protocol::FileSystemSpecialPath;
use anecdoct_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.anecdoct`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".anecdoct")
                {
                    return None;
                }
    ...
    for (search_root, globs) in patterns_by_search_root {
        for path in ripgrep_files(search_root.as_path(), &globs, max_depth)? {
            ...
            if expanded_paths.len() > MAX_UNREADABLE_GLOB_MATCHES {
                return Err(AnecdoctErr::Fatal(format!(
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_anecdoct = temp_dir.path().join("real-anecdoct");
        let logical_anecdoct = logical_home.join(".anecdoct");
        let real_memories = real_anecdoct.join("memories");
        let logical_memories = logical_anecdoct.join("memories");
        ...
        std::os::unix::fs::symlink(&real_anecdoct, &logical_anecdoct)
            .expect("create symlinked anecdoct home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".anecdoct"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_anecdoct = path_to_string(&temp_dir.path().join(".anecdoct"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_anecdoct));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_anecdoct)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.anecdoct"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.anecdoct"),
            ]
        );
        assert_eq!(
            args.args,
