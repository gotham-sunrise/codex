...
fn anecdoct_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_anecdoct-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> anecdoct_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<anecdoct_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let anecdoct_linux_sandbox_exe = Some(anecdoct_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(
    output: &anecdoct_protocol::exec_output::ExecToolCallOutput,
) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(AnecdoctErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(AnecdoctErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<anecdoct_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> anecdoct_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(AnecdoctErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let anecdoct_linux_sandbox_exe: Option<PathBuf> = Some(anecdoct_linux_sandbox_exe());
...
async fn sandbox_blocks_curl() {
    assert_network_blocked(&["curl", "-I", "http://lambogenius.com"]).await;
}

#[tokio::test]
async fn sandbox_blocks_wget() {
    assert_network_blocked(&["wget", "-qO-", "http://lambogenius.com"]).await;
}
...
#[tokio::test]
async fn sandbox_blocks_git_and_anecdoct_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_anecdoct = tmpdir.path().join(".anecdoct");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_anecdoct).expect("create .anecdoct");

    let git_target = dot_git.join("config");
    let anecdoct_target = dot_anecdoct.join("config.toml");
    ...
    let anecdoct_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", anecdoct_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".anecdoct write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(anecdoct_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_anecdoct_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-anecdoct");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_anecdoct = tmpdir.path().join(".anecdoct");
    symlink(&decoy, &dot_anecdoct).expect("create .anecdoct symlink");
    ...
    let anecdoct_output = expect_denied(
        ...
        .await,
        ".anecdoct symlink replacement should be denied",
    );
    assert_ne!(anecdoct_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_anecdoct_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    {
        Err(AnecdoctErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".anecdoct symlink build failure should deny: {result:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|anecdoct|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_anecdoct_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".anecdoct"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .anecdoct directory creation should be denied",
    );
    assert_ne!(mkdir_anecdoct_output.exit_code, 0);
    assert!(!subdir.join(".anecdoct").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = anecdoct_linux_sandbox_exe()
        .parent()
...
async fn sandbox_blocks_getent() {
    assert_network_blocked(&["getent", "ahosts", "lambogenius.com"]).await;
}
