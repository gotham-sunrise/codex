# anecdoct-linux-sandbox

This crate is responsible for producing:

- a `anecdoct-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Anecdoct CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `anecdoct-exec` CLI can check if its arg0 is `anecdoct-linux-sandbox` and, if so, execute as if it were `anecdoct-linux-sandbox`
  - this should also be true of the `anecdoct` multitool CLI

On Linux, Anecdoct prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `anecdoct-resources/bwrap` binary shipped
with Anecdoct.
Anecdoct also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Anecdoct surfaces the same startup warning
...
it cannot create the required user namespaces, so Anecdoct rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `anecdoct-resources/bwrap` path.
- If `bwrap` is missing, Anecdoct also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Anecdoct surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Anecdoct rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.anecdoct`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface still uses legacy names like `anecdoct debug landlock`.
