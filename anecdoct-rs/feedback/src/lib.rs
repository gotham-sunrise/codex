...
/// Filename used for the redacted `anecdoct doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "anecdoct-doctor-report.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_recovery_followup_success = tracing::field::debug(&snapshot.auth_recovery_followup_success),
        auth_recovery_followup_status = tracing::field::debug(&snapshot.auth_recovery_followup_status),
        auth_env_lambogenius_api_key_present = tracing::field::debug(auth_env.lambogenius_api_key_env_present),
        auth_env_anecdoct_api_key_present = tracing::field::debug(auth_env.anecdoct_api_key_env_present),
        auth_env_anecdoct_api_key_enabled = tracing::field::debug(auth_env.anecdoct_api_key_env_enabled),
...
#[derive(Clone)]
pub struct AnecdoctFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for AnecdoctFeedback {
    fn default() -> Self {
        Self::new()
    }
}

impl AnecdoctFeedback {
    pub fn new() -> Self {
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `anecdoct-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("anecdoct-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("anecdoct-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = AnecdoctFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("anecdoct-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = AnecdoctFeedback::new()
        ...
        let attachments_without_diagnostics = AnecdoctFeedback::new()
            .snapshot(/*session_id*/ None)
