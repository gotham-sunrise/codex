...
use anecdoct_app_server_protocol::DynamicToolSpec;
...
use anecdoct_app_server_protocol::JSONRPCMessage;
use anecdoct_app_server_protocol::JSONRPCNotification;
use anecdoct_app_server_protocol::JSONRPCRequest;
use anecdoct_app_server_protocol::JSONRPCResponse;
...
use anecdoct_app_server_protocol::SandboxPolicy;
...
use anecdoct_otel::OtelProvider;
...
const OTEL_SERVICE_NAME: &str = "anecdoct-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $ANECDOCT_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Anecdoct app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Anecdoct", version, about = "Bootstrap Anecdoct app-server", long_about = None)]
struct Cli {
    /// Path to the `anecdoct` CLI binary. When set, requests use stdio by
    /// spawning `anecdoct app-server` as a child process.
    #[arg(long, env = "ANECDOCT_BIN", global = true)]
    anecdoct_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--anecdoct-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "ANECDOCT_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `anecdoct` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `anecdoct app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/anecdoct-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `anecdoct app-server --listen`.
    ...
    /// Send a user message through the Anecdoct app-server.
    SendMessage {
        /// User message to send to Anecdoct.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the Anecdoct app-server.
    GetAccountRateLimits,
    /// List the available models from the Anecdoct app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Anecdoct app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "ANECDOCT_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let anecdoct_bin = anecdoct_bin.unwrap_or_else(|| PathBuf::from("anecdoct"));
            serve(&anecdoct_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(anecdoct_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(anecdoct_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(anecdoct_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(anecdoct_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if anecdoct_bin.is_some() && url.is_some() {
        bail!("--anecdoct-bin and --url are mutually exclusive");
    }
    if let Some(anecdoct_bin) = anecdoct_bin {
        return Ok(Endpoint::SpawnAnecdoct(anecdoct_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if anecdoct_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --anecdoct-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(anecdoct_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(anecdoct_bin);
        if let Some(anecdoct_bin_parent) = anecdoct_bin.parent() {
            let mut path = OsString::from(anecdoct_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", anecdoct_bin.display()))?;
...
fn serve(anecdoct_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/anecdoct-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,anecdoct_=trace {}",
        shell_quote(&anecdoct_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started anecdoct app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnAnecdoct(anecdoct_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut AnecdoctClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = AnecdoctClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = AnecdoctClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (anecdoct_bin, url) {
        (Some(_), Some(_)) => bail!("--anecdoct-bin and --url are mutually exclusive"),
        (Some(anecdoct_bin), None) => {
            let server = BackgroundAppServer::spawn(&anecdoct_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve anecdoct-app-server-test-client binary path")?;
...
impl AnecdoctClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnAnecdoct(anecdoct_bin) => {
                Self::spawn_stdio(anecdoct_bin, config_overrides)
            }
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(anecdoct_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let anecdoct_bin_display = anecdoct_bin.display();
        let mut cmd = Command::new(anecdoct_bin);
        if let Some(anecdoct_bin_parent) = anecdoct_bin.parent() {
            let mut path = OsString::from(anecdoct_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut anecdoct_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{anecdoct_bin_display}` app-server"))?;

        let stdin = anecdoct_app_server
            .stdin
            .take()
            .context("anecdoct app-server stdin unavailable")?;
        let stdout = anecdoct_app_server
            .stdout
            .take()
            .context("anecdoct app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: anecdoct_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `anecdoct-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "anecdoct-toy-app-server".to_string(),
                    title: Some("Anecdoct Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: anecdoct_app_server_protocol::LoginAccountParams::Chatgpt {
                anecdoct_streamlined_login: false,
            },
        };
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: anecdoct_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to anecdoct app-server")?;
                    return Ok(());
                }
                bail!("anecdoct app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from anecdoct app-server")?;
                if bytes == 0 {
                    bail!("anecdoct app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = anecdoct_core::otel_init::build_provider(
            &config,
...
impl Drop for AnecdoctClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[anecdoct app-server exited: {status}]");
            return;
        }
