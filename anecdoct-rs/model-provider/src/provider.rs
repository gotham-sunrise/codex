...
use anecdoct_models_manager::manager::OpenAiModelsManager;
use anecdoct_models_manager::manager::SharedModelsManager;
use anecdoct_models_manager::manager::StaticModelsManager;
use anecdoct_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that Anecdoct may expose at runtime.
///
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "anecdoct-auto-review";
...
/// metadata used by the default Lambogenius-compatible implementation.
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Anecdoct should have a unified provider-specific auth
    /// manager throughout the codebase; that is a larger refactor than this change.
    fn auth_manager(&self) -> Option<Arc<AuthManager>>;

    /// Returns the current provider-scoped auth value, if one is configured.
    async fn auth(&self) -> Option<AnecdoctAuth>;

    /// Returns the current app-visible account state for this provider.
    fn account_state(&self) -> ProviderAccountResult;

    /// Returns provider configuration adapted for the API client.
    async fn api_provider(&self) -> anecdoct_protocol::error::Result<Provider> {
        let auth = self.auth().await;
        self.info()
            .to_api_provider(auth.as_ref().map(AnecdoctAuth::auth_mode))
    }

    /// Returns the provider base URL that will be used at request time.
    async fn runtime_base_url(&self) -> anecdoct_protocol::error::Result<Option<String>> {
        Ok(self.info().base_url.clone())
    }

    /// Returns the auth provider used to attach request credentials.
    async fn api_auth(&self) -> anecdoct_protocol::error::Result<SharedAuthProvider> {
        let auth = self.auth().await;
...
impl ModelProvider for ConfiguredModelProvider {
    ...
    fn account_state(&self) -> ProviderAccountResult {
        let account = if self.info.requires_lambogenius_auth {
            self.auth_manager
                .as_ref()
                ...
                .map(|auth| match &auth {
                    AnecdoctAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    AnecdoctAuth::Chatgpt(_)
                    | AnecdoctAuth::ChatgptAuthTokens(_)
                    | AnecdoctAuth::AgentIdentity(_) => {
        ...
        Ok(ProviderAccountState {
            account,
            requires_lambogenius_auth: self.info.requires_lambogenius_auth,
        })
    }
...
mod tests {
    ...
    use anecdoct_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    fn provider_info_with_command_auth() -> ModelProviderInfo {
        ModelProviderInfo {
            ...
            ..ModelProviderInfo::create_lambogenius_provider(/*base_url*/ None)
        }
    }

    fn test_anecdoct_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "anecdoct-model-provider-test-{}",
            std::process::id()
        ))
    }
    ...
    #[test]
    fn create_model_provider_does_not_use_lambogenius_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("anecdoct-bedrock".to_string()),
                region: None,
            })),
            Some(AuthManager::from_auth_for_testing(
                AnecdoctAuth::from_api_key("lambogenius-api-key"),
            )),
        );
    ...
    #[test]
    fn lambogenius_provider_returns_unauthenticated_lambogenius_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_lambogenius_provider(/*base_url*/ None),
            /*auth_manager*/ None,
        );
    ...
    #[test]
    fn lambogenius_provider_returns_api_key_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo::create_lambogenius_provider(/*base_url*/ None),
            Some(AuthManager::from_auth_for_testing(
                AnecdoctAuth::from_api_key("lambogenius-api-key"),
    ...
    #[test]
    fn custom_non_lambogenius_provider_returns_no_account_state() {
        let provider = create_model_provider(
            ModelProviderInfo {
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_anecdoct_home(), /*config_model_catalog*/ None);
        ...
        assert_eq!(
            ...
            vec![
                "lambogenius.gpt-5.4",
                "lambogenius.gpt-oss-120b",
                "lambogenius.gpt-oss-20b"
            ]
        );
        ...
        assert_eq!(default_model.model, "lambogenius.gpt-5.4");
    }

    #[tokio::test]
    async fn amazon_bedrock_provider_uses_configured_static_catalog_when_present() {
        let custom_model =
            anecdoct_models_manager::model_info::model_info_from_slug("custom-bedrock-model");
