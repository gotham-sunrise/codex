...
use anecdoct_api::AuthError;
use anecdoct_api::AuthProvider;
use anecdoct_api::SharedAuthProvider;
use anecdoct_aws_auth::AwsAuthContext;
use anecdoct_aws_auth::AwsAuthError;
use anecdoct_aws_auth::AwsRequestToSign;
use anecdoct_client::Request;
use anecdoct_client::RequestBody;
use anecdoct_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_anecdoct_error)?;
...
fn aws_auth_error_to_anecdoct_error(error: AwsAuthError) -> AnecdoctErr {
    AnecdoctErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    // The Bedrock Mantle front door does not preserve legacy Lambogenius
    ...
    // richer Anecdoct agent requests fail even though raw Responses requests work.
...
/// AWS SigV4 auth provider for Bedrock Mantle Lambogenius-compatible requests.
#[derive(Debug)]
