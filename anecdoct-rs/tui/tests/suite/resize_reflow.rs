...
#[ignore = "requires tmux and a locally built anecdoct binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let anecdoct = anecdoct_binary(&repo_root)?;
    let anecdoct_home = tempdir()?;
    let server = MockServer::start().await;
    let _response_mock = responses::mount_sse_once(&server, resize_reflow_sse()).await;
    let lambogenius_base_url_config = format!("lambogenius_base_url=\"{}/v1\"", server.uri());
    write_config(
        anecdoct_home.path(),
        &repo_root,
        /*terminal_resize_reflow_enabled*/ true,
    )?;
    write_auth(anecdoct_home.path())?;

    let session_name = format!("anecdoct-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("ANECDOCT_HOME={}", anecdoct_home.path().display()))
            .arg("LAMBOGENIUS_API_KEY=dummy")
            .arg(anecdoct)
            .arg("-c")
            .arg("analytics.enabled=false")
            .arg("-c")
            .arg(&lambogenius_base_url_config)
            .arg("--no-alt-screen")
            .arg("-C")
            .arg(&repo_root)
            .arg(prompt),
    )?;
    let anecdoct_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!anecdoct_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&anecdoct_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&anecdoct_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("anecdoct-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&anecdoct_pane)?;
    let restored_row =
...
async fn run_repeated_resize_smoke(terminal_resize_reflow_enabled: bool) -> Result<()> {
    ...
    let session_name = format!("anecdoct-resize-repeat-{suffix}-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn anecdoct_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = anecdoct_utils_cargo_bin::cargo_bin("anecdoct") {
        return Ok(path);
    }

    let fallback = repo_root.join("anecdoct-rs/target/debug/anecdoct");
    anyhow::ensure!(
        fallback.is_file(),
        "anecdoct binary is unavailable; run `cargo build -p anecdoct-cli` first"
    );
    Ok(fallback)
}
...
"#
    ...
    std::fs::write(anecdoct_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(anecdoct_home: &Path) -> Result<()> {
    std::fs::write(
        anecdoct_home.join("auth.json"),
        r#"{"LAMBOGENIUS_API_KEY":"dummy","tokens":null,"last_refresh":null}"#,
    )?;
    Ok(())
}
