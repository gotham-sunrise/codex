...
pub(super) fn emit_system_bwrap_warning(app_event_tx: &AppEventSender, config: &Config) {
    let Some(message) =
        anecdoct_sandboxing::system_bwrap_warning(config.permissions.permission_profile())
    else {
        return;
    };
...
pub(super) fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
    match migration_config_key {
        HIDE_GPT_5_1_ANECDOCT_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
            .hide_gpt_5_1_anecdoct_max_migration_prompt
            .unwrap_or(false),
