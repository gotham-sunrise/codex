...
impl App {
    ...
    pub(super) async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        if windows_sandbox_changed {
            #[cfg(target_os = "windows")]
            {
                ...
                self.app_event_tx
                    .send(AppEvent::AnecdoctOp(AppCommand::override_turn_context(
                        /*cwd*/ None,
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("anecdoct-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
...
mod tests {
    ...
    -> Result<()> {
        ...
        let anecdoct_home = tempdir()?;
        app.config.anecdoct_home = anecdoct_home.path().to_path_buf().abs();
        std::fs::write(anecdoct_home.path().join("config.toml"), "[broken")?;
