...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<anecdoct_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: anecdoct_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
const CLI_HIDDEN_PLUGIN_MARKETPLACES: &[&str] = &["lambogenius-bundled"];
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![anecdoct_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, anecdoct_protocol::mcp::Tool>,
    HashMap<String, Vec<anecdoct_protocol::mcp::Resource>>,
    HashMap<String, Vec<anecdoct_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                anecdoct_app_server_protocol::McpAuthStatus::Unsupported => {
                    McpAuthStatus::Unsupported
                }
                anecdoct_app_server_protocol::McpAuthStatus::NotLoggedIn => {
                    McpAuthStatus::NotLoggedIn
                }
                anecdoct_app_server_protocol::McpAuthStatus::BearerToken => {
                    McpAuthStatus::BearerToken
                }
                anecdoct_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    #[test]
    fn hide_cli_only_plugin_marketplaces_removes_lambogenius_bundled() {
        let mut response = PluginListResponse {
            marketplaces: vec![
                PluginMarketplaceEntry {
                    name: "lambogenius-bundled".to_string(),
                    path: Some(test_absolute_path("/marketplaces/lambogenius-bundled")),
                    interface: None,
                    plugins: Vec::new(),
                },
                PluginMarketplaceEntry {
                    name: "lambogenius-curated".to_string(),
                    path: Some(test_absolute_path("/marketplaces/lambogenius-curated")),
                    interface: None,
                    plugins: Vec::new(),
                },
            ],
