...
use anecdoct_app_server_protocol::AdditionalFileSystemPermissions;
use anecdoct_app_server_protocol::AdditionalNetworkPermissions;
use anecdoct_app_server_protocol::AdditionalPermissionProfile;
use anecdoct_app_server_protocol::AgentMessageDeltaNotification;
...
use anecdoct_app_server_protocol::FileUpdateChange;
...
use anecdoct_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use anecdoct_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use anecdoct_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use anecdoct_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use anecdoct_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use anecdoct_app_server_protocol::PatchChangeKind;
use anecdoct_app_server_protocol::PermissionsRequestApprovalParams;
use anecdoct_app_server_protocol::RequestId as AppServerRequestId;
...
use anecdoct_app_server_protocol::TokenUsageBreakdown;
...
use anecdoct_app_server_protocol::UserInput as AppServerUserInput;
...
use anecdoct_protocol::config_types::CollaborationModeMask;
...
use anecdoct_protocol::models::NetworkPermissions;
...
async fn handle_mcp_inventory_result_clears_committed_loading_cell() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: anecdoct_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        feedback: anecdoct_feedback::AnecdoctFeedback::new(),
        is_first_run: false,
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::AnecdoctOp(Op::UserTurn { items, .. }) => {
                submitted_items = Some(items);
            }
            _ => {}
        }
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: anecdoct_app_server_protocol::CollabAgentTool::Wait,
                status: anecdoct_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(
            anecdoct_app_server_protocol::ItemCompletedNotification {
                thread_id: ThreadId::new().to_string(),
                turn_id: "turn-1".to_string(),
                completed_at_ms: 0,
                item: ThreadItem::CollabAgentToolCall {
                    id: "send-1".to_string(),
                    tool: anecdoct_app_server_protocol::CollabAgentTool::SendInput,
                    status: anecdoct_app_server_protocol::CollabAgentToolCallStatus::Failed,
                    sender_thread_id: ThreadId::new().to_string(),
                    receiver_thread_ids: vec![receiver_thread_id.to_string()],
                    ...
                    agents_states: HashMap::from([(
                        ...
                        anecdoct_app_server_protocol::CollabAgentState {
                            status: anecdoct_app_server_protocol::CollabAgentStatus::NotFound,
                            message: None,
                        },
                    )]),
                },
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.anecdoct_home = anecdoct_home.path().to_path_buf().abs();
    ...
    let config = std::fs::read_to_string(anecdoct_home.path().join("config.toml"))?;
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let anecdoct_home = tempdir()?;
        app.config.anecdoct_home = anecdoct_home.path().to_path_buf().abs();
        app.config.sqlite_home = anecdoct_home.path().to_path_buf();
        ...
        let state_db = anecdoct_state::StateRuntime::init(
            anecdoct_home.path().to_path_buf(),
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = anecdoct_home.path().join("memories");
...
-> Result<()> {
    ...
    let anecdoct_home = tempdir()?;
    app.config.anecdoct_home = anecdoct_home.path().to_path_buf().abs();
    let config_toml_path = anecdoct_home.path().join("config.toml").abs();
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        available_decisions,
        vec![
            anecdoct_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            anecdoct_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            anecdoct_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            anecdoct_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: anecdoct_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: anecdoct_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: anecdoct_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                created_at: 1,
                updated_at: 2,
                status: anecdoct_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: anecdoct_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            anecdoct_app_server_protocol::ServerRequestResolvedNotification {
...
async fn side_thread_ignores_global_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        anecdoct_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            anecdoct_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        anecdoct_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        anecdoct_error_info: Some(AppServerAnecdoctErrorInfo::ActiveTurnNotSteerable {
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    anecdoct_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: anecdoct_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                anecdoct_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
