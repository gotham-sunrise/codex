...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<anecdoct_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<anecdoct_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(anecdoct_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`anecdoct-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `anecdoct mcp add`.
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://developers.lambogenius.com/anecdoct/mcp\u{7}MCP docs\u{1b}]8;;\u{7}"
                .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, anecdoct_protocol::mcp::Tool>,
