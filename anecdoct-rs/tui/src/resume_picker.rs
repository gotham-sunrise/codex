...
use anecdoct_utils_path as path_utils;
use chrono::DateTime;
use chrono::Utc;
            ...
            ThreadItem::UserMessage { content, .. } => Some(TranscriptPreviewLine {
                ...
                text: content
                    ...
                    .filter_map(|input| match input {
                        anecdoct_app_server_protocol::UserInput::Text { text, .. } => {
                            Some(text.as_str())
                        }
                        _ => None,
                    })
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.anecdoct_home)
...
mod tests {
    ...
    fn local_picker_thread_list_params_include_cwd_filter() {
        ...
        let params = thread_list_params(
            ...
            cwd_filter.as_deref(),
            ProviderFilter::MatchDefault(String::from("lambogenius")),
    ...
    fn row_search_matches_metadata_fields() {
        ...
        let row = Row {
            ...
            created_at: None,
            updated_at: None,
            cwd: Some(PathBuf::from("/tmp/anecdoct-session-picker")),
    ...
    fn expanded_session_details_include_metadata() {
        ...
        let row = Row {
            ...
            cwd: Some(PathBuf::from("/Users/felipe.coury/code/anecdoct")),
            git_branch: Some(String::from("anecdoct/raw-scrollback-mode")),
        };
        ...
        assert!(rendered.contains("Branch:      anecdoct/raw-scrollback-mode"));
    ...
    fn footer_prioritizes_active_sort_timestamp() {
        ...
        assert_metadata_order(&updated[0], "⌁ tmp/anecdoct", " main");
        assert_metadata_order(&created[0], "⌁ tmp/anecdoct", " main");
    }
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains("⌁ /tmp/anecdoct"));
        assert!(rendered.contains(" no branch"));
        assert_metadata_order(&footer[0], "⌁ /tmp/anecdoct", " no branch");
    }
    ...
    fn footer_branch_expands_when_line_has_room() {
        ...
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some(branch),
            Some("~/code/anecdoct.etraut-animations-false-improvements/anecdoct-rs"),
            /*show_cwd*/ true,
            /*width*/ 140,
        );
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        let cwd =
            "~/code/anecdoct.owner-extremely-long-worktree-name-that-needs-truncating/anecdoct-rs";
        ...
        assert!(footer.contains("⌁ ~/code/anecdoct."));
    ...
    fn footer_omits_cwd_when_hidden() {
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some("owner/branch"),
            Some("~/code/anecdoct.owner-worktree/anecdoct-rs"),
            /*show_cwd*/ false,
            /*width*/ 80,
        );
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let anecdoct_home_file = tmp.path().join("anecdoct-home-file");
        std::fs::write(&anecdoct_home_file, "not a directory").expect("write anecdoct home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
        ...
        state.view_persistence = Some(SessionPickerViewPersistence {
            anecdoct_home: anecdoct_home_file,
            active_profile: None,
        });
    ...
    fn dense_snapshot_row() -> Row {
        Row {
            ...
            cwd: Some(PathBuf::from(
                "/Users/felipe.coury/code/anecdoct.fcoury-session-picker/anecdoct-rs",
            )),
            git_branch: Some(String::from("fcoury/session-picker")),
        }
    }
    ...
    fn expanded_session_snapshot() {
        ...
        let row = Row {
            ...
            created_at: parse_timestamp_str("2026-04-28T16:30:00Z"),
            updated_at: parse_timestamp_str("2026-04-28T17:45:00Z"),
            cwd: Some(PathBuf::from("/tmp/anecdoct")),
            git_branch: Some(String::from("fcoury/session-picker")),
        };
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
    ...
    fn app_server_row_keeps_pathless_threads() {
        ...
        let thread = Thread {
            ...
            ephemeral: false,
            model_provider: String::from("lambogenius"),
            created_at: 1,
            updated_at: 2,
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![anecdoct_app_server_protocol::Turn {
