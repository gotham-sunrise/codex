...
use anecdoct_ansi_escape::ansi_escape_line;
use anecdoct_app_server_client::AppServerRequestHandle;
use anecdoct_app_server_client::RemoteAppServerEndpoint;
...
use anecdoct_app_server_protocol::SkillErrorInfo;
...
use anecdoct_models_manager::model_presets::HIDE_GPT_5_1_ANECDOCT_MAX_MIGRATION_PROMPT_CONFIG;
use anecdoct_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use anecdoct_protocol::permissions::FileSystemSandboxKind;
...
use anecdoct_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        anecdoct_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&anecdoct_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&anecdoct_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[anecdoct_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&anecdoct_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<anecdoct_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn session_summary(
    ...
    let resume_command = anecdoct_utils_cli::resume_command(/*thread_name*/ None, thread_id);
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.anecdoct_error_info,
        Some(AppServerAnecdoctErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    ...
    pub async fn run(
        ...
        let requires_lambogenius_auth = bootstrap.requires_lambogenius_auth;
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            anecdoct_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("anecdoct.status_line", /*inc*/ 1, &[]);
        }
...
See the Anecdoct keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.anecdoct_home.clone();
        ...
        if requires_lambogenius_auth && has_chatgpt_account {
