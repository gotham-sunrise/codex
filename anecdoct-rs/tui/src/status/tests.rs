...
async fn test_config(temp_home: &TempDir) -> Config {
    let mut config = ConfigBuilder::default()
        .anecdoct_home(temp_home.path().to_path_buf())
...
async fn status_snapshot_includes_reasoning_details() {
    ...
    config.model = Some("gpt-5.1-anecdoct-max".to_string());
    config.model_provider_id = "lambogenius".to_string();
...
async fn status_model_provider_uses_bedrock_runtime_base_url() {
    ...
    config.model_provider.base_url =
        Some("https://bedrock-mantle.us-east-1.api.aws/lambogenius/v1".to_string());
    let usage = TokenUsage::default();
    let captured_at = chrono::Local
        .with_ymd_and_hms(2024, 1, 2, 3, 4, 5)
        .single()
        .expect("timestamp");
    let model_slug = crate::legacy_core::test_support::get_model_offline(config.model.as_deref());
    let runtime_base_url = "https://bedrock-mantle.eu-west-1.api.aws/lambogenius/v1";
...
async fn status_snapshot_includes_credits_and_limits() {
    let temp_home = TempDir::new().expect("temp home");
    let mut config = test_config(&temp_home).await;
    config.model = Some("gpt-5.1-anecdoct".to_string());
    set_workspace_cwd(&mut config, test_path_buf("/workspace/tests").abs());
