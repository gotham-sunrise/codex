...
// The standalone `anecdoct-tui` binary prints a short help message before the
...
use anecdoct_app_server_client::RemoteAppServerClient;
use anecdoct_app_server_client::RemoteAppServerConnectArgs;
pub use anecdoct_app_server_client::RemoteAppServerEndpoint;
use anecdoct_app_server_protocol::Account as AppServerAccount;
use anecdoct_app_server_protocol::AskForApproval;
use anecdoct_app_server_protocol::AuthMode as AppServerAuthMode;
...
use anecdoct_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use anecdoct_rollout::StateDbHandle;
use anecdoct_rollout::state_db;
use anecdoct_state::log_db;
...
use anecdoct_utils_home_dir::find_anecdoct_home;
...
pub(crate) use anecdoct_app_server_client::legacy_core;
...
async fn start_embedded_app_server(
    ...
    feedback: anecdoct_feedback::AnecdoctFeedback,
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            std::io::Error::other(LocalStateDbStartupError::new(
                anecdoct_state::state_db_path(config.sqlite_home.as_path()),
                err.to_string(),
            ))
        }),
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let anecdoct_home = find_anecdoct_home().wrap_err("failed to resolve ANECDOCT_HOME")?;
            anecdoct_app_server_client::app_server_control_socket_path(&anecdoct_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(anecdoct_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path =
        anecdoct_app_server_client::app_server_control_socket_path(anecdoct_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_anecdoct_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        anecdoct_feedback::AnecdoctFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = anecdoct_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let anecdoct_home = match find_anecdoct_home() {
        Ok(anecdoct_home) => anecdoct_home.to_path_buf(),
        Err(err) => {
    ...
    let remote_endpoint = match explicit_remote_endpoint {
        ...
        None => maybe_probe_default_daemon_socket(&anecdoct_home)
            .await
    ...
    let environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            EnvironmentManager::from_anecdoct_home(anecdoct_home.clone(), local_runtime_paths).await
        } else {
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&anecdoct_home, profile_v2);
    ...
    let config_toml = match load_config_as_toml_with_cli_and_load_options(
        ...
        anecdoct_config::ConfigLoadOptions {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&anecdoct_home).await?;
    ...
    // Anecdoct CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("anecdoct-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for anecdoct crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("anecdoct_core=info,anecdoct_tui=info,anecdoct_rmcp_client=info")
        })
    };
    ...
    let feedback = anecdoct_feedback::AnecdoctFeedback::new();
...
async fn run_ratatui_app(
    ...
    let login_status = if initial_config.model_provider.requires_lambogenius_auth {
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !remote_mode {
            cloud_requirements = cloud_requirements_loader_for_storage(
                initial_config.anecdoct_home.to_path_buf(),
                /*enable_anecdoct_api_key_env*/ false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `anecdoct {action}` without an ID to choose from existing sessions."
            )),
        })
    };
...
async fn get_login_status(
    ...
    if !config.model_provider.requires_lambogenius_auth {
...
fn should_show_login_screen(login_status: LoginStatus, config: &Config) -> bool {
    // Only show the login screen for providers that actually require Lambogenius auth
    // (Lambogenius or equivalents). For OSS/other providers, skip login entirely.
    if !config.model_provider.requires_lambogenius_auth {
        return false;
    }
...
mod tests {
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: anecdoct_app_server_client::app_server_control_socket_path(
                    &anecdoct_home
                )?,
            }
        );
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://anecdoct.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("anecdoct.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        ...
        let socket_path = temp_dir.path().join("anecdoct.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_probe_default_daemon_socket(anecdoct_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let anecdoct_home = TempDir::new()?;
        let socket_path =
            anecdoct_app_server_client::app_server_control_socket_path(anecdoct_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(anecdoct_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        fn write_session_rollout(
            ...
            let rollout_path = anecdoct_home
                .join("sessions")
                .join(year)
                .join(month)
                .join(day)
            ...
            let session_meta = anecdoct_protocol::protocol::SessionMeta {
                id: thread_id,
                timestamp: meta_rfc3339.to_string(),
                cwd: cwd.to_path_buf(),
                originator: "anecdoct".to_string(),
                cli_version: "0.0.0".to_string(),
                source: anecdoct_protocol::protocol::SessionSource::Cli,
                model_provider: Some(model_provider.to_string()),
                ..Default::default()
            };
            let session_meta =
                serde_json::to_value(anecdoct_protocol::protocol::SessionMetaLine {
                    meta: session_meta,
                    git: None,
                })?;
            let lines = [
        ...
        let mut app_server =
            AppServerSession::new(anecdoct_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = anecdoct_state::StateRuntime::init(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            anecdoct_state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let anecdoct_home = temp_dir.path().to_path_buf();
...
"#
        ...
        let trusted_config = ConfigBuilder::default()
            ...
            .anecdoct_home(anecdoct_home.clone())
