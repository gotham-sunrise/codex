...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerAnecdoctErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerAnecdoctErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerAnecdoctErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerAnecdoctErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerAnecdoctErrorInfo) -> bool {
    matches!(info, AppServerAnecdoctErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_anecdoct_limit = limit_id.eq_ignore_ascii_case("anecdoct");
            if is_anecdoct_limit
                && let Some(rate_limit_reached_type) = snapshot.rate_limit_reached_type
            {
                self.anecdoct_rate_limit_reached_type = Some(rate_limit_reached_type);
            }
            let warnings = if is_anecdoct_limit {
            ...
            let high_usage = is_anecdoct_limit
                && (snapshot
                    .secondary
                    .as_ref()
            ...
            self.anecdoct_rate_limit_reached_type = None;
        }
    ...
    pub(super) fn should_prefetch_rate_limits(&self) -> bool {
        self.config.model_provider.requires_lambogenius_auth && self.has_chatgpt_account
    }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::AnecdoctOp(AppCommand::override_turn_context(
                /*cwd*/ None,
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using anecdoct. Request increase?",
            ),
        };
