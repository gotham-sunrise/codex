...
impl ChatWidget {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match anecdoct_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.anecdoct_home.as_path(),
            cwd.as_path(),
            &env_map,
            &policy,
            Some(self.config.anecdoct_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        if !crate::legacy_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED {
            ...
            header.push(*Box::new(
                Paragraph::new(vec![
                    ...
                    line!["Learn more: https://developers.lambogenius.com/anecdoct/windows"],
                ])
        ...
        self.session_telemetry.counter(
            "anecdoct.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["Set up the Anecdoct agent sandbox to protect your files and control network access. Learn more <https://developers.lambogenius.com/anecdoct/windows>"],
            ])
            .wrap(Wrap { trim: false }),
        ));
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    accept_otel.counter(
                        "anecdoct.windows_sandbox.elevated_prompt_accept",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "anecdoct.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "anecdoct.windows_sandbox.elevated_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(&mut self, preset: ApprovalPreset) {
        ...
        lines.push(line![""]);
        lines.push(line![
            "You can still use Anecdoct in a non-admin sandbox. It carries greater risk if prompt injected."
        ]);
        lines.push(line![
            "Learn more <https://developers.lambogenius.com/anecdoct/windows>"
        ]);
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "anecdoct.windows_sandbox.fallback_retry_elevated",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
            ...
            SelectionItem {
                name: "Use Anecdoct with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "anecdoct.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "anecdoct.windows_sandbox.fallback_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
