...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        anecdoct_error_info: &AppServerAnecdoctErrorInfo,
    ) -> bool {
        matches!(
            anecdoct_error_info,
            AppServerAnecdoctErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Anecdoct is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        let rate_limit_reached_type = self.anecdoct_rate_limit_reached_type.map(|kind| {
        ...
        self.anecdoct_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        anecdoct_error_info: Option<AppServerAnecdoctErrorInfo>,
    ) {
        if anecdoct_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if anecdoct_error_info
            .as_ref()
            .is_some_and(is_app_server_cyber_policy_error)
        {
            self.on_cyber_policy_error();
        } else if let Some(info) = anecdoct_error_info
            .as_ref()
