...
const LAMBOGENIUS_CURATED_TAB_ID: &str = "marketplace:lambogenius-curated";
...
const APPS_HELP_ARTICLE_URL: &str =
    "https://help.lambogenius.com/en/articles/11487775-apps-in-chatgpt";
...
impl ChatWidget {
    ...
    fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from Anecdoct.".dim(),
        ));
    ...
    fn plugins_popup_params(
        ...
        let curated_installed = curated_entries
            ...
            .count();
        tabs.push(SelectionTab {
            id: LAMBOGENIUS_CURATED_TAB_ID.to_string(),
            label: "Lambogenius Curated".to_string(),
            header: plugins_header(
                "Lambogenius Curated marketplace.".to_string(),
                format!(
                    "Installed {curated_installed} of {curated_total} Lambogenius Curated plugins."
                ),
            ),
            items: self.plugin_selection_items(
                curated_entries,
                /*include_marketplace_names*/ false,
                "No Lambogenius Curated plugins available",
                "No Lambogenius Curated plugins available.",
            ),
        });
        ...
        let mut additional_marketplaces: Vec<&PluginMarketplaceEntry> = marketplaces
            .iter()
            .copied()
            .filter(|marketplace| marketplace.name != LAMBOGENIUS_CURATED_MARKETPLACE_NAME)
            .collect();
            ...
            let actions: Vec<SelectionAction> = if let Some(marketplace_path) = marketplace_path {
                vec![Box::new(move |tx| {
                    ...
                    tx.send(AppEvent::FetchPluginDetail {
                        cwd: cwd.clone(),
                        params: anecdoct_app_server_protocol::PluginReadParams {
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(anecdoct_app_server_protocol::HookEventName, usize)>::new();
