...
async fn app_server_cyber_policy_error_renders_dedicated_notice() {
    ...
    handle_error(
        ...
        Some(AnecdoctErrorInfo::CyberPolicy),
    );
...
async fn status_line_git_summary_items_render_values() {
    ...
    chat.status_line_git_summary = Some(StatusLineGitSummary {
        pull_request: Some(crate::branch_summary::StatusLinePullRequest {
            number: 20_252,
            url: "https://github.com/lambogenius/anecdoct/pull/20252".to_string(),
        }),
...
async fn completed_plan_table_tail_skips_provisional_history_insert() {
    ...
    controller.push("| --- | --- |\n");
    controller.push("| Verify | Anecdoct |\n");
    assert!(
        controller.has_live_tail(),
...
async fn configured_pet_load_is_deferred_until_after_construction() {
    ...
    crate::pets::write_test_pack(&cfg.anecdoct_home);
...
async fn prefetch_rate_limits_is_gated_on_chatgpt_auth_provider() {
    ...
    chat.config.model_provider.requires_lambogenius_auth = false;
    assert!(!chat.should_prefetch_rate_limits());
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    chat.on_rate_limit_snapshot(Some(RateLimitSnapshot {
        ...
        limit_name: Some("anecdoct".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 20,
            window_duration_mins: Some(300),
            resets_at: Some(100),
    ...
    let anecdoct = chat
        .rate_limit_snapshots_by_limit_id
        .get("anecdoct")
        .expect("anecdoct snapshot should exist");
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("anecdoct_other")
        .expect("anecdoct_other snapshot should exist");

    assert_eq!(
        anecdoct.primary.as_ref().map(|w| w.used_percent),
        Some(20.0)
    );
    assert_eq!(
        anecdoct
...
#[tokio::test]
async fn rate_limit_switch_prompt_skips_non_anecdoct_limit() {
...
async fn workspace_owner_limit_states_render_state_specific_messages() {
    let cases = [
        (
            RateLimitReachedType::WorkspaceOwnerCreditsDepleted,
            RateLimitErrorKind::Generic,
            "You're out of credits. Your workspace is out of credits. Add credits to continue using Anecdoct.",
        ),
        (
            RateLimitReachedType::WorkspaceOwnerUsageLimitReached,
            RateLimitErrorKind::UsageLimit,
            "Usage limit reached. You've reached your usage limit. Increase your limits to continue using anecdoct.",
        ),
    ];
...
async fn fast_status_indicator_is_hidden_for_models_without_fast_support() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.3-anecdoct")).await;
    set_fast_mode_test_catalog(&mut chat);
    assert!(!get_available_model(&chat, "gpt-5.3-anecdoct").supports_fast_mode());
    chat.set_service_tier(Some(ServiceTier::Fast.request_value().to_string()));
    set_chatgpt_auth(&mut chat);
    set_fast_mode_test_catalog(&mut chat);
    assert!(!get_available_model(&chat, "gpt-5.3-anecdoct").supports_fast_mode());
...
async fn ambient_pet_stays_hidden_until_a_pet_is_selected() {
    ...
    crate::pets::write_test_pack(&chat.config.anecdoct_home);
    chat.set_tui_pet(Some("anecdoct".to_string()));
...
async fn interrupted_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            anecdoct_app_server_protocol::HookEventName::PreToolUse,
...
async fn status_line_model_with_reasoning_includes_fast_for_fast_capable_models() {
    ...
    chat.set_model("gpt-5.3-anecdoct");
    chat.refresh_status_line();

    assert_eq!(
        status_line_text(&chat),
        Some(format!(
            "gpt-5.3-anecdoct xhigh · Context 0% used · {test_cwd}"
        ))
    );
}
...
async fn terminal_title_model_updates_on_model_change_without_manual_refresh() {
    ...
    assert_eq!(
        ...
        Some("gpt-5.3-anecdoct".to_string())
    );
}
...
async fn status_line_model_with_reasoning_updates_on_mode_switch_without_manual_refresh() {
    ...
    assert_eq!(
        status_line_text(&chat),
        Some("gpt-5.3-anecdoct high".to_string())
    );
    ...
    assert_eq!(
        status_line_text(&chat),
        Some("gpt-5.3-anecdoct medium".to_string())
    );
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            anecdoct_app_server_protocol::ThreadNameUpdatedNotification {
...
async fn status_line_goal_active_token_budget_footer_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            anecdoct_app_server_protocol::ThreadGoalUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: None,
                goal: test_thread_goal(
                    anecdoct_app_server_protocol::ThreadGoalStatus::Active,
...
async fn status_line_goal_complete_elapsed_footer_snapshot() {
    ...
    let mut goal = test_thread_goal(
        anecdoct_app_server_protocol::ThreadGoalStatus::Complete,
        /*token_budget*/ None,
        /*tokens_used*/ 40_000,
    );
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        anecdoct_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn test_thread_goal(
    status: anecdoct_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> anecdoct_app_server_protocol::ThreadGoal {
    anecdoct_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::HookStarted(AppServerHookStartedNotification {
            ...
            run: AppServerHookRunSummary {
                ...
                source: anecdoct_app_server_protocol::HookSource::User,
                display_order: 0,
...
async fn post_tool_use_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        anecdoct_app_server_protocol::HookEventName::PostToolUse,
...
async fn completed_hook_with_no_entries_stays_out_of_history() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "post-tool-use:0:/tmp/hooks.json",
            anecdoct_app_server_protocol::HookEventName::PostToolUse,
            anecdoct_app_server_protocol::HookRunStatus::Completed,
            Vec::new(),
        ),
    );
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "pre-tool-use:0:/tmp/hooks.json",
            anecdoct_app_server_protocol::HookEventName::PreToolUse,
            anecdoct_app_server_protocol::HookRunStatus::Blocked,
            vec![anecdoct_app_server_protocol::HookOutputEntry {
                kind: anecdoct_app_server_protocol::HookOutputEntryKind::Feedback,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            anecdoct_app_server_protocol::HookEventName::PostToolUse,
            anecdoct_app_server_protocol::HookRunStatus::Failed,
            vec![anecdoct_app_server_protocol::HookOutputEntry {
                kind: anecdoct_app_server_protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            hook_id,
            anecdoct_app_server_protocol::HookEventName::Stop,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            hook_id,
            anecdoct_app_server_protocol::HookEventName::Stop,
            anecdoct_app_server_protocol::HookRunStatus::Stopped,
            vec![anecdoct_app_server_protocol::HookOutputEntry {
                kind: anecdoct_app_server_protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            anecdoct_app_server_protocol::HookEventName::SessionStart,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            anecdoct_app_server_protocol::HookRunStatus::Completed,
            vec![anecdoct_app_server_protocol::HookOutputEntry {
                kind: anecdoct_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_started_run(
    id: &str,
    event_name: anecdoct_app_server_protocol::HookEventName,
    status_message: Option<&str>,
) -> anecdoct_app_server_protocol::HookRunSummary {
    hook_run_summary(
        id,
        event_name,
        anecdoct_app_server_protocol::HookRunStatus::Running,
        status_message,
        Vec::new(),
    )
}
...
fn hook_completed_run(
    id: &str,
    event_name: anecdoct_app_server_protocol::HookEventName,
    status: anecdoct_app_server_protocol::HookRunStatus,
    entries: Vec<anecdoct_app_server_protocol::HookOutputEntry>,
) -> anecdoct_app_server_protocol::HookRunSummary {
    hook_run_summary(
...
fn hook_run_summary(
    ...
    anecdoct_app_server_protocol::HookRunSummary {
        id: id.to_string(),
        event_name,
        handler_type: anecdoct_app_server_protocol::HookHandlerType::Command,
        execution_mode: anecdoct_app_server_protocol::HookExecutionMode::Sync,
        scope: anecdoct_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: anecdoct_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        started_at: 1,
        completed_at: (status != anecdoct_app_server_protocol::HookRunStatus::Running).then_some(2),
        duration_ms: (status != anecdoct_app_server_protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
...
async fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    handle_exec_begin(
        ...
        AppServerThreadItem::CommandExecution {
            id: "c1".into(),
            command: anecdoct_shell_command::parse_command::shlex_join(&command),
            cwd: cwd.clone(),
            process_id: None,
