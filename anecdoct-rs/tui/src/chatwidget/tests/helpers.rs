...
pub(super) async fn test_config() -> Config {
    ...
    let anecdoct_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config = Config::load_default_with_cli_overrides_for_anecdoct_home(
        anecdoct_home.clone(),
        Vec::new(),
    )
    .await
    .expect("config");
    config.anecdoct_home = anecdoct_home.abs();
    config.sqlite_home = anecdoct_home.clone();
    config.log_dir = anecdoct_home.join("log");
...
pub(super) async fn make_chatwidget_manual(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::AnecdoctOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask Anecdoct to do anything".to_string();
...
fn token_usage_breakdown(usage: TokenUsage) -> anecdoct_app_server_protocol::TokenUsageBreakdown {
    anecdoct_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    anecdoct_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: anecdoct_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    anecdoct_error_info: Option<AnecdoctErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = anecdoct_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            anecdoct_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("anecdoct-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_curated_marketplace(
    ...
    PluginMarketplaceEntry {
        name: LAMBOGENIUS_CURATED_MARKETPLACE_NAME.to_string(),
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(anecdoct_app_server_protocol::HookEventName, usize)],
    apps: &[(&str, bool)],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                anecdoct_app_server_protocol::HookOutputEntry {
                    kind: anecdoct_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: anecdoct_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        anecdoct_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        anecdoct_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        anecdoct_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        anecdoct_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        anecdoct_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        anecdoct_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        anecdoct_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        anecdoct_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
