...
use anecdoct_app_server_protocol::HookErrorInfo;
    ...
    chat.on_plugin_detail_loaded(
        ...
        Ok(PluginReadResponse {
            plugin: plugins_test_detail(
                ...
                &[
                    (anecdoct_app_server_protocol::HookEventName::PreToolUse, 1),
                    (anecdoct_app_server_protocol::HookEventName::Stop, 2),
                ],
...
#[tokio::test]
async fn plugins_popup_lambogenius_curated_tab_omits_marketplace_in_rows() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("Lambogenius Curated marketplace."),
        "expected Lambogenius Curated tab header, got:\n{popup}"
    );
    assert!(
        popup.contains("Calendar") && !popup.contains("Repo Plugin"),
        "expected Lambogenius Curated tab to show only official marketplace plugins, got:\n{popup}"
    );
    assert!(
...
async fn apps_popup_keeps_existing_full_snapshot_while_partial_refresh_loads() {
    ...
    chat.on_connectors_loaded(
        Ok(ConnectorsSnapshot {
            connectors: vec![
                ...
                AppInfo {
                    id: "connector_lambogenius_hidden".to_string(),
                    name: "Hidden Lambogenius".to_string(),
                    ...
                    app_metadata: None,
                    labels: None,
                    install_url: Some("https://example.test/hidden-lambogenius".to_string()),
                    is_accessible: true,
                    is_enabled: true,
                    plugin_display_names: Vec::new(),
                },
    ...
    assert!(
        !popup.contains("Hidden Lambogenius"),
...
async fn realtime_audio_picker_emits_persist_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.3-anecdoct")).await;
    chat.open_realtime_audio_device_selection_with_names(
...
async fn server_overloaded_error_does_not_switch_models() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(Some("gpt-5.3-anecdoct")).await;
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(AnecdoctErrorInfo::ServerOverloaded),
    );
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &anecdoct_feedback::FeedbackDiagnostics::new(vec![anecdoct_feedback::FeedbackDiagnostic {
