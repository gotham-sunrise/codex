...
use anecdoct_app_server_protocol::SkillsListEntry;
use anecdoct_app_server_protocol::SkillsListResponse;
use anecdoct_core_skills::model::SkillDependencies;
use anecdoct_core_skills::model::SkillInterface;
use anecdoct_core_skills::model::SkillMetadata;
use anecdoct_core_skills::model::SkillToolDependency;
...
use anecdoct_utils_plugins::mention_syntax::TOOL_MENTION_SIGIL;
...
pub(crate) fn find_app_mentions(
    ...
    for app in apps.iter().filter(|app| app.is_enabled) {
        let slug = anecdoct_connectors::metadata::connector_mention_slug(app);
