...
impl ChatWidget {
    ...
    fn model_menu_warning_line(&self) -> Option<Line<'static>> {
        let base_url = self.custom_lambogenius_base_url()?;
        let warning = format!(
            "Warning: Lambogenius base URL is overridden to {base_url}. Selecting models may not be supported or work properly."
        );
        Some(Line::from(warning.red()))
    }

    fn custom_lambogenius_base_url(&self) -> Option<String> {
        if !self.config.model_provider.is_lambogenius() {
            return None;
        }
        ...
        if normalized == DEFAULT_LAMBOGENIUS_BASE_URL {
            return None;
        }
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("anecdoct-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "anecdoct-auto-fast" => 0,
            "anecdoct-auto-balanced" => 1,
            "anecdoct-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running anecdoct -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("gpt-5.1-anecdoct")
            || preset.model.starts_with("gpt-5.1-anecdoct-max")
