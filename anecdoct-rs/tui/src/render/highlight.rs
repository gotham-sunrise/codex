...
//! | `ANECDOCT_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static ANECDOCT_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and anecdoct home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `anecdoct_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), anecdoct_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let anecdoct_home_set_ok = ANECDOCT_HOME.set(anecdoct_home.clone()).is_ok();
    if THEME.get().is_some() {
        set_syntax_theme(resolve_theme_with_override(
            name.as_deref(),
            anecdoct_home.as_deref(),
        ));
    }
    if !override_set_ok || !anecdoct_home_set_ok {
...
pub(crate) fn validate_theme_name(
    name: Option<&str>,
    anecdoct_home: Option<&Path>,
) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = anecdoct_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$ANECDOCT_HOME/themes/{name}.tmTheme"));
    ...
    if let Some(home) = anecdoct_home {
...
fn custom_theme_path(name: &str, anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{anecdoct_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, anecdoct_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, anecdoct_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, anecdoct_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {ANECDOCT_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = anecdoct_home
...
fn build_default_theme() -> Theme {
    ...
    let anecdoct_home = ANECDOCT_HOME
        .get()
        .and_then(|anecdoct_home| anecdoct_home.as_deref());
    resolve_theme_with_override(name, anecdoct_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = ANECDOCT_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, anecdoct_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{ANECDOCT_HOME}/themes/`.
...
/// found in `{anecdoct_home}/themes/`.
pub(crate) fn list_available_themes(anecdoct_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*anecdoct_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*anecdoct_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*anecdoct_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*anecdoct_home*/ None).is_none());
    }
