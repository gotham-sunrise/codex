...
use anecdoct_app_server_client::AppServerClient;
use anecdoct_app_server_client::AppServerEvent;
use anecdoct_app_server_client::AppServerRequestHandle;
use anecdoct_app_server_client::TypedRequestError;
...
use anecdoct_app_server_protocol::ConfigBatchWriteParams;
use anecdoct_app_server_protocol::ConfigWriteResponse;
use anecdoct_app_server_protocol::ExternalAgentConfigDetectParams;
use anecdoct_app_server_protocol::ExternalAgentConfigDetectResponse;
use anecdoct_app_server_protocol::ExternalAgentConfigImportParams;
use anecdoct_app_server_protocol::ExternalAgentConfigImportResponse;
use anecdoct_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use anecdoct_app_server_protocol::Model as ApiModel;
...
use anecdoct_app_server_protocol::ThreadMemoryMode;
...
use anecdoct_app_server_protocol::ThreadRealtimeAudioChunk;
...
use anecdoct_protocol::approvals::GuardianAssessmentEvent;
use anecdoct_protocol::lambogenius_models::ModelAvailabilityNux;
use anecdoct_protocol::lambogenius_models::ModelPreset;
use anecdoct_protocol::lambogenius_models::ModelServiceTier;
use anecdoct_protocol::lambogenius_models::ModelUpgrade;
use anecdoct_protocol::lambogenius_models::ReasoningEffortPreset;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<anecdoct_protocol::account::PlanType>,
    /// Whether the configured model provider needs Lambogenius-style auth. Combined
    /// with `has_chatgpt_account` to decide if a startup rate-limit prefetch
    /// should be fired.
    pub(crate) requires_lambogenius_auth: bool,
...
impl AppServerSession {
    ...
    pub(crate) async fn bootstrap(&mut self, config: &Config) -> Result<AppServerBootstrap> {
        ...
        let (
            ...
            Some(Account::Chatgpt { email, plan_type }) => {
                let feedback_audience = if email.ends_with("@lambogenius.com") {
        ...
        Ok(AppServerBootstrap {
            ...
            status_account_display,
            plan_type,
            requires_lambogenius_auth: account.requires_lambogenius_auth,
            default_model,
            feedback_audience,
            has_chatgpt_account,
            available_models,
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: anecdoct_protocol::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<anecdoct_protocol::lambogenius_models::ReasoningEffort>,
        summary: Option<anecdoct_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<anecdoct_protocol::config_types::CollaborationMode>,
        personality: Option<anecdoct_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<anecdoct_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<anecdoct_app_server_protocol::ApprovalsReviewer> {
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &std::path::Path,
) -> Option<anecdoct_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(anecdoct_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(anecdoct_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(anecdoct_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(anecdoct_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn turn_permissions_overrides(
    ...
    Option<anecdoct_app_server_protocol::SandboxPolicy>,
...
fn display_permission_profile_from_thread_response(
    sandbox: &anecdoct_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<anecdoct_protocol::lambogenius_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let history_config =
        anecdoct_message_history::HistoryConfig::new(config.anecdoct_home.clone(), &config.history);
    let (log_id, entry_count) = anecdoct_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> Config {
        ConfigBuilder::default()
            .anecdoct_home(temp_dir.path().to_path_buf())
            .build()
            .await
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(anecdoct_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: anecdoct_app_server_protocol::Thread {
                ...
                source: anecdoct_app_server_protocol::SessionSource::Cli,
                thread_source: None,
                agent_nickname: None,
                agent_role: None,
                git_info: None,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        anecdoct_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            content: vec![anecdoct_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        anecdoct_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: anecdoct_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: anecdoct_app_server_protocol::ApprovalsReviewer::User,
        ...
        let embedded_config = ConfigBuilder::default()
            .anecdoct_home(temp_dir.path().join("embedded-anecdoct-home"))
            .harness_overrides(ConfigOverrides {
                default_permissions: Some(BUILT_IN_PERMISSION_PROFILE_WORKSPACE.to_string()),
                ..ConfigOverrides::default()
            })
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &anecdoct_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        let history_config = anecdoct_message_history::HistoryConfig::new(
            config.anecdoct_home.clone(),
            &config.history,
        );

        anecdoct_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        anecdoct_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            /*rollout_path*/ None,
            "gpt-5.4".to_string(),
            "lambogenius".to_string(),
            /*service_tier*/ None,
            AskForApproval::Never,
            anecdoct_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(anecdoct_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(anecdoct_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
