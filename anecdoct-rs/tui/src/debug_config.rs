...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project {
            ...
            format!(
                "project ({}/config.toml)",
                dot_anecdoct_folder.as_path().display()
            )
        }
...
mod tests {
    ...
    use anecdoct_config::FilesystemConstraints;
    use anecdoct_config::HookEventsToml;
    use anecdoct_config::HookHandlerConfig;
    use anecdoct_config::ManagedHooksRequirementsToml;
    use anecdoct_config::MatcherGroup;
    ...
    use anecdoct_config::NetworkUnixSocketPermissionToml;
    use anecdoct_config::NetworkUnixSocketPermissionsToml;
    use anecdoct_config::RequirementSource;
    use anecdoct_config::ResidencyRequirement;
    use anecdoct_config::SandboxModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\anecdoct\\config.toml")
        } else {
            absolute_path("/etc/anecdoct/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.anecdoct")
        } else {
            absolute_path("/repo/.anecdoct")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_anecdoct_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\Lambogenius\\Anecdoct\\requirements.toml")
        } else {
            absolute_path("/etc/anecdoct/requirements.toml")
        };
                ...
                BTreeMap::from([(
                    ...
                    McpServerRequirement {
                        identity: McpServerIdentity::Command {
                            command: "anecdoct-mcp".to_string(),
                        },
                    },
                )]),
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.anecdoct\\config.toml")
        } else {
            absolute_path("/home/alice/.anecdoct/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let rendered = render_to_text(&render_debug_config_lines(&stack));
        assert!(rendered.contains(
            "experimental_network: unix_sockets={/tmp/blocked.sock=none, /tmp/anecdoct.sock=allow} (source: cloud requirements)"
        ));
    }
