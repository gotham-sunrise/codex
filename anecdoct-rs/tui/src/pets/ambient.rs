//! Ambient terminal rendering for the Anecdoct companion.
//!
...
impl AmbientPet {
    ...
    /// ANECDOCT_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_anecdoct_home(
            ...
            /*anecdoct_home*/ Some(anecdoct_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = anecdoct_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_anecdoct_app_vocabulary() {
