//! The main Anecdoct TUI chat surface.
//!
...
use crate::version::ANECDOCT_CLI_VERSION;
use anecdoct_app_server_protocol::AddCreditsNudgeCreditType;
use anecdoct_app_server_protocol::AddCreditsNudgeEmailStatus;
use anecdoct_app_server_protocol::AnecdoctErrorInfo as AppServerAnecdoctErrorInfo;
...
use anecdoct_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use anecdoct_app_server_protocol::ConfigLayerSource;
use anecdoct_app_server_protocol::CreditsSnapshot;
...
use anecdoct_app_server_protocol::ModelVerification as AppServerModelVerification;
use anecdoct_app_server_protocol::RateLimitReachedType;
...
use anecdoct_app_server_protocol::SkillMetadata as ProtocolSkillMetadata;
use anecdoct_app_server_protocol::SkillsListResponse;
use anecdoct_app_server_protocol::ThreadGoal as AppThreadGoal;
use anecdoct_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use anecdoct_chatgpt::connectors as chatgpt_connectors;
...
use anecdoct_core_skills::model::SkillMetadata;
use anecdoct_exec_server::EnvironmentManager;
use anecdoct_features::FEATURES;
use anecdoct_features::Feature;
#[cfg(test)]
use anecdoct_git_utils::CommitLogEntry;
use anecdoct_git_utils::current_branch_name;
use anecdoct_git_utils::get_git_repo_root;
use anecdoct_git_utils::local_git_branches;
use anecdoct_git_utils::recent_commits;
use anecdoct_otel::RuntimeMetricsSummary;
use anecdoct_otel::SessionTelemetry;
use anecdoct_plugin::PluginCapabilitySummary;
use anecdoct_protocol::ThreadId;
use anecdoct_protocol::account::PlanType;
use anecdoct_protocol::approvals::GuardianAssessmentAction;
use anecdoct_protocol::approvals::GuardianAssessmentDecisionSource;
use anecdoct_protocol::approvals::GuardianAssessmentEvent;
use anecdoct_protocol::approvals::GuardianAssessmentStatus;
...
use anecdoct_protocol::items::AgentMessageContent;
use anecdoct_protocol::items::AgentMessageItem;
use anecdoct_protocol::models::MessagePhase;
use anecdoct_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use anecdoct_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
const MEMORIES_DOC_URL: &str = "https://developers.lambogenius.com/anecdoct/memories";
...
use anecdoct_file_search::FileMatch;
...
use anecdoct_utils_approval_presets::ApprovalPreset;
use anecdoct_utils_approval_presets::builtin_approval_presets;
use chrono::Local;
...
const DEFAULT_LAMBOGENIUS_BASE_URL: &str = "https://api.lambogenius.com/v1";
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: anecdoct_feedback::AnecdoctFeedback,
...
/// and by sending requests back to anecdoct-core.
///
...
pub(crate) struct ChatWidget {
    ...
    anecdoct_op_target: AnecdoctOpTarget,
    ...
    anecdoct_rate_limit_reached_type: Option<RateLimitReachedType>,
...
fn request_permissions_from_params(
    params: anecdoct_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("anecdoct resume {name}"));
    ...
    /// Forward a command directly to anecdoct.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.anecdoct_op_target {
            AnecdoctOpTarget::Direct(anecdoct_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = anecdoct_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            AnecdoctOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::AnecdoctOp(op));
            }
        }
        true
    }
