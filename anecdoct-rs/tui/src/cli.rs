use anecdoct_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `anecdoct resume` subcommand.
    // These are not exposed as user flags on the base `anecdoct` command.
    #[clap(skip)]
    ...
    /// top-level `anecdoct resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `anecdoct fork` subcommand.
    // These are not exposed as user flags on the base `anecdoct` command.
    #[clap(skip)]
    ...
    /// top-level `anecdoct fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
