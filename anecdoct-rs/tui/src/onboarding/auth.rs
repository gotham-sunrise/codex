...
fn onboarding_request_id() -> anecdoct_app_server_protocol::RequestId {
    anecdoct_app_server_protocol::RequestId::String(Uuid::new_v4().to_string())
}
...
pub(super) async fn cancel_login_attempt(
    ...
    let _ = request_handle
        .request_typed::<anecdoct_app_server_protocol::CancelLoginAccountResponse>(
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Sign in with ChatGPT to use Anecdoct as part of your paid plan".into(),
            ]),
            Line::from(vec![
                "  ".into(),
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Decide how much autonomy you want to grant Anecdoct".into(),
            Line::from(vec![
                "  For more details see the ".into(),
                "\u{1b}]8;;https://developers.lambogenius.com/anecdoct/security\u{7}Anecdoct docs\u{1b}]8;;\u{7}".underlined(),
            ])
            .dim(),
            "".into(),
            "  Anecdoct can make mistakes".into(),
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Anecdoct will use usage-based billing with your API key.".into(),
        ];
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "Use your own Lambogenius API key for usage-based billing".bold(),
            ]),
            "".into(),
        ...
        if state.prepopulated_from_env {
            intro_lines.push("  Detected LAMBOGENIUS_API_KEY environment variable.".into());
            intro_lines.push(
    ...
    fn start_api_key_entry(&mut self) {
        ...
        self.set_error(/*message*/ None);
        let prefill_from_env = read_lambogenius_api_key_from_env();
        let mut guard = self.sign_in_state.write().unwrap();
        match &mut *guard {
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let config = ConfigBuilder::default()
            .anecdoct_home(anecdoct_home_path.clone())
            .build()
            .await
            .unwrap();
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            environment_manager: Arc::new(
                anecdoct_app_server_client::EnvironmentManager::default_for_tests(),
            ),
