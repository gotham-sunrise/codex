...
pub(crate) fn load_local_chatgpt_auth(
    ...
    let auth = load_auth_dot_json(anecdoct_home, auth_credentials_store_mode)
        .map_err(|err| format!("failed to load local auth: {err}"))?
        .ok_or_else(|| "no local auth available".to_string())?;
    if matches!(auth.auth_mode, Some(AuthMode::ApiKey)) || auth.lambogenius_api_key.is_some() {
...
mod tests {
    ...
    fn write_chatgpt_auth(anecdoct_home: &Path, plan_type: &str) {
        ...
        let auth = AuthDotJson {
            ...
            tokens: Some(TokenData {
                id_token: anecdoct_login::token_data::parse_chatgpt_jwt_claims(&id_token)
        ...
        save_auth(anecdoct_home, &auth, AuthCredentialsStoreMode::File)
    ...
    fn loads_local_chatgpt_auth_from_managed_auth() {
        ...
        write_chatgpt_auth(anecdoct_home.path(), "business");
    ...
    fn preserves_usage_based_plan_type_wire_name() {
        let anecdoct_home = TempDir::new().expect("tempdir");
        write_chatgpt_auth(anecdoct_home.path(), "self_serve_business_usage_based");
