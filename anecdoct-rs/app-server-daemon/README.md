# anecdoct-app-server-daemon

> `anecdoct-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`anecdoct-app-server-daemon` backs the machine-readable `anecdoct app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Anecdoct instances launched over SSH, including fresh developer
...
```sh
anecdoct app-server daemon start
anecdoct app-server daemon restart
anecdoct app-server daemon enable-remote-control
anecdoct app-server daemon disable-remote-control
anecdoct app-server daemon stop
anecdoct app-server daemon version
anecdoct app-server daemon bootstrap --remote-control
```
...
$HOME/.anecdoct/packages/standalone/current/anecdoct app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `ANECDOCT_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Anecdoct is installed through `install.sh` and always launches
the standalone managed binary under `ANECDOCT_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `ANECDOCT_HOME/packages/standalone/current/anecdoct` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `ANECDOCT_HOME/packages/standalone/current/anecdoct` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `anecdoct remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `ANECDOCT_HOME`, so a concurrent
...
The daemon stores its local state under `ANECDOCT_HOME/app-server-daemon/`:
