...
impl PidBackend {
    ...
    pub(crate) fn new_update_loop(anecdoct_bin: PathBuf, pid_file: PathBuf) -> Self {
        let lock_file = pid_file.with_extension("pid.lock");
        Self {
            anecdoct_bin,
            pid_file,
    ...
    pub(crate) async fn start(&self) -> Result<Option<u32>> {
        ...
        let mut command = Command::new(&self.anecdoct_bin);
        command
