...
#[cfg(unix)]
use crate::managed_install::resolved_managed_anecdoct_bin;
...
async fn update_once(
    ...
    let managed_anecdoct_bin = resolved_managed_anecdoct_bin(&daemon.managed_anecdoct_bin).await?;
    let managed_identity = executable_identity(&managed_anecdoct_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_anecdoct_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_anecdoct_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_anecdoct_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Anecdoct binary {}",
            managed_anecdoct_bin.display()
        )
    })
}
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://chatgpt.com/anecdoct/install.sh")
        .await
        .context("failed to fetch standalone Anecdoct updater")?
        .error_for_status()
        .context("standalone Anecdoct updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone Anecdoct updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Anecdoct updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Anecdoct updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Anecdoct updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Anecdoct updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Anecdoct updater exited with status {status}")
    }
}
