...
use super::parse_anecdoct_version;

#[test]
fn parses_anecdoct_cli_version_output() {
    assert_eq!(
        parse_anecdoct_version("anecdoct 1.2.3\n").expect("version"),
        "1.2.3"
    );
}

#[test]
fn rejects_malformed_anecdoct_cli_version_output() {
    assert!(parse_anecdoct_version("anecdoct\n").is_err());
}
