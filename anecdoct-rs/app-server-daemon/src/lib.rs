...
use anecdoct_app_server_transport::app_server_control_socket_path;
...
use managed_install::managed_anecdoct_bin;
#[cfg(unix)]
use managed_install::managed_anecdoct_version;
use serde::Serialize;
...
pub struct BootstrapOutput {
    ...
    pub remote_control_enabled: bool,
    pub managed_anecdoct_path: PathBuf,
    pub socket_path: PathBuf,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "anecdoct app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
struct Daemon {
    ...
    managed_anecdoct_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let anecdoct_home = find_anecdoct_home().context("failed to resolve ANECDOCT_HOME")?;
        let socket_path = app_server_control_socket_path(anecdoct_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = anecdoct_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_anecdoct_bin: managed_anecdoct_bin(anecdoct_home.as_path()),
        })
    }
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        self.ensure_managed_anecdoct_bin()?;
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by anecdoct app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_anecdoct_version(managed_anecdoct_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_anecdoct_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_anecdoct_bin)?;
        }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        Ok(BootstrapOutput {
            ...
            managed_anecdoct_path: self.managed_anecdoct_bin.clone(),
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_anecdoct_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_anecdoct_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_anecdoct_bin(&self) -> Result<()> {
        if self.managed_anecdoct_bin.is_file() {
            return Ok(());
        }

        let managed_anecdoct_path = self.managed_anecdoct_bin.display();
        Err(anyhow!(
            "managed standalone Anecdoct install not found at {managed_anecdoct_path}\n\n\
             This command requires the standalone install managed by the Anecdoct installer, because \
             the daemon starts and updates app-server from that fixed path.\n\n\
             Install it with:\n  curl -fsSL https://chatgpt.com/anecdoct/install.sh | sh\n\n\
             Then rerun the command you just tried."
        ))
    }

    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_anecdoct_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            anecdoct_bin: managed_anecdoct_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            socket_path: "anecdoct.sock".into(),
        ...
        let bootstrap_output = BootstrapOutput {
            ...
            auto_update_enabled: true,
            remote_control_enabled: true,
            managed_anecdoct_path: "anecdoct".into(),
            socket_path: "anecdoct.sock".into(),
