...
use crate::protocol::AnecdoctErrorInfo;
use crate::protocol::ErrorEvent;
use crate::protocol::RateLimitSnapshot;
use crate::protocol::TruncationPolicy;
use anecdoct_async_utils::CancelErr;
use anecdoct_utils_string::truncate_middle_chars;
use anecdoct_utils_string::truncate_middle_with_token_budget;
use chrono::DateTime;
use chrono::Datelike;
use chrono::Local;
use chrono::Utc;
...
pub type Result<T> = std::result::Result<T, AnecdoctErr>;
...
pub enum AnecdoctErr {
    ...
    #[error(
        "Anecdoct ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `AnecdoctError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use Anecdoct with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("anecdoct-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for AnecdoctErr {
    fn from(_: CancelErr) -> Self {
        AnecdoctErr::TurnAborted
    }
}
...
impl AnecdoctErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            AnecdoctErr::TurnAborted
            | AnecdoctErr::Interrupted
            | AnecdoctErr::EnvVar(_)
            | AnecdoctErr::Fatal(_)
            | AnecdoctErr::UsageNotIncluded
            | AnecdoctErr::QuotaExceeded
            | AnecdoctErr::InvalidImageRequest()
            | AnecdoctErr::InvalidRequest(_)
            | AnecdoctErr::RefreshTokenFailed(_)
            | AnecdoctErr::UnsupportedOperation(_)
            | AnecdoctErr::Sandbox(_)
            | AnecdoctErr::LandlockSandboxExecutableNotProvided
            | AnecdoctErr::RetryLimit(_)
            | AnecdoctErr::ContextWindowExceeded
            | AnecdoctErr::ThreadNotFound(_)
            | AnecdoctErr::AgentLimitReached { .. }
            | AnecdoctErr::Spawn
            | AnecdoctErr::SessionConfiguredNotFirstEvent
            | AnecdoctErr::UsageLimitReached(_)
            | AnecdoctErr::ServerOverloaded
            | AnecdoctErr::CyberPolicy { .. } => false,
            AnecdoctErr::Stream(..)
            | AnecdoctErr::Timeout
            | AnecdoctErr::UnexpectedStatus(_)
            | AnecdoctErr::ResponseStreamFailed(_)
            | AnecdoctErr::ConnectionFailed(_)
            | AnecdoctErr::InternalServerError
            | AnecdoctErr::InternalAgentDied
            | AnecdoctErr::Io(_)
            | AnecdoctErr::Json(_)
            | AnecdoctErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            AnecdoctErr::LandlockRuleset(_) | AnecdoctErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<AnecdoctErr>()` checks continue to compile
    ...
    pub fn to_anecdoct_protocol_error(&self) -> AnecdoctErrorInfo {
        match self {
            AnecdoctErr::ContextWindowExceeded => AnecdoctErrorInfo::ContextWindowExceeded,
            AnecdoctErr::UsageLimitReached(_)
            | AnecdoctErr::QuotaExceeded
            | AnecdoctErr::UsageNotIncluded => AnecdoctErrorInfo::UsageLimitExceeded,
            AnecdoctErr::ServerOverloaded => AnecdoctErrorInfo::ServerOverloaded,
            AnecdoctErr::CyberPolicy { .. } => AnecdoctErrorInfo::CyberPolicy,
            AnecdoctErr::RetryLimit(_) => AnecdoctErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            AnecdoctErr::ConnectionFailed(_) => AnecdoctErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            AnecdoctErr::ResponseStreamFailed(_) => {
                AnecdoctErrorInfo::ResponseStreamConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            AnecdoctErr::RefreshTokenFailed(_) => AnecdoctErrorInfo::Unauthorized,
            AnecdoctErr::SessionConfiguredNotFirstEvent
            | AnecdoctErr::InternalServerError
            | AnecdoctErr::InternalAgentDied => AnecdoctErrorInfo::InternalServerError,
            AnecdoctErr::UnsupportedOperation(_)
            | AnecdoctErr::ThreadNotFound(_)
            | AnecdoctErr::AgentLimitReached { .. } => AnecdoctErrorInfo::BadRequest,
            AnecdoctErr::Sandbox(_) => AnecdoctErrorInfo::SandboxError,
            _ => AnecdoctErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            anecdoct_error_info: Some(self.to_anecdoct_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            AnecdoctErr::RetryLimit(err) => Some(err.status),
            AnecdoctErr::UnexpectedStatus(err) => Some(err.status),
            AnecdoctErr::ConnectionFailed(err) => err.source.status(),
            AnecdoctErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("anecdoct")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/anecdoct/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Anecdoct (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/anecdoct/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &AnecdoctErr) -> String {
    let message = match e {
        AnecdoctErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        AnecdoctErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
