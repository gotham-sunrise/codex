...
use anecdoct_utils_image::PromptImageMode;
use anecdoct_utils_image::load_for_prompt_bytes;
...
use anecdoct_utils_image::ImageProcessingError;
...
pub enum SandboxEnforcement {
    /// Anecdoct owns sandbox construction for this profile.
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where Anecdoct owns sandbox construction.
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Anecdoct could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Anecdoct cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
/// The payload we send back to Lambogenius when reporting a tool call result.
///
...
fn convert_mcp_content_to_items(
    ...
    const ANECDOCT_IMAGE_DETAIL_META_KEY: &str = "anecdoct/imageDetail";
            ...
            Ok(McpContent::Image {
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: meta
                        ...
                        .and_then(|meta| meta.get(ANECDOCT_IMAGE_DETAIL_META_KEY))
...
// (Moved event mapping logic into anecdoct-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__anecdoct_apps__gmail_get_recent_emails",
            "namespace": "mcp__anecdoct_apps__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__anecdoct_apps__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__anecdoct_apps__gmail".to_string()),
        ...
        for i in 0..200 {
            exec_policy
                .add_prefix_rule(
                    ...
                    anecdoct_execpolicy::Decision::Allow,
                )
                .expect("add rule");
        }
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__anecdoct_apps__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Anecdoct cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
