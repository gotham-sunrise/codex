//! Defines the protocol for a Anecdoct session between a client and an agent.
//!
...
use crate::lambogenius_models::ReasoningEffort as ReasoningEffortConfig;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Anecdoct:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::anecdoct_thread::AnecdoctThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// Request Anecdoct to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down anecdoct instance.
    Shutdown,
...
/// running the command proposed by Anecdoct.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.anecdoct`, `.git`, notably `.git/hooks`) under
...
/// Anecdoct errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum AnecdoctErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub anecdoct_error_info: Option<AnecdoctErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.anecdoct_error_info
            .as_ref()
            .is_none_or(AnecdoctErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Anecdoct),
...
impl From<crate::lambogenius_models::TruncationPolicyConfig> for TruncationPolicy {
    fn from(config: crate::lambogenius_models::TruncationPolicyConfig) -> Self {
        match config.mode {
            crate::lambogenius_models::TruncationMode::Bytes => Self::Bytes(config.limit as usize),
            crate::lambogenius_models::TruncationMode::Tokens => {
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(anecdoct_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                anecdoct_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_anecdoct() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("anecdoct".to_string()).restriction_product(),
            Some(Product::Anecdoct)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Anecdoct])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Anecdoct]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd =
            anecdoct_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".anecdoct")).expect("create .anecdoct");
        let canonical_cwd =
            anecdoct_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_anecdoct =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".anecdoct"))
                .expect("canonical .anecdoct");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_anecdoct.as_path())
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_anecdoct.to_path_buf(),
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            anecdoct_error_info: Some(AnecdoctErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
    ...
    fn serialize_event() -> Result<()> {
        ...
        let expected = json!({
            ...
            "msg": {
                ...
                "model": "anecdoct-mini-latest",
                "model_provider_id": "lambogenius",
                "approval_policy": "never",
