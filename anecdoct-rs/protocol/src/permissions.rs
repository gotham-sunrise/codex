...
use anecdoct_utils_absolute_path::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_ANECDOCT_PATH_NAME: &str = ".anecdoct";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_ANECDOCT_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_ANECDOCT_PATH_NAME)
}
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Anecdoct degrades to warn-and-ignore
    /// instead of failing to load. Anecdoct 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "anecdoct-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_anecdoct*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_anecdoct*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_anecdoct*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_anecdoct = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_anecdoct)
                    .into_iter()
            ...
            // Example: if `<root>/.anecdoct -> <root>/decoy`, bwrap must still see
            // `<root>/.anecdoct`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.anecdoct` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_anecdoct: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .anecdoct read-only to the agent by
    ...
    let top_level_anecdoct = writable_root.join(PROTECTED_METADATA_ANECDOCT_PATH_NAME);
    if protect_missing_dot_anecdoct || top_level_anecdoct.as_path().is_dir() {
        subpaths.push(top_level_anecdoct);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "ANECDOCT_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_anecdoct() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_dot_anecdoct = expected_root.join(".anecdoct");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_dot_anecdoct)
        );
    }
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".anecdoct".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_anecdoct_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_anecdoct = expected_root.join(".anecdoct");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_anecdoct.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".anecdoct".to_string()),
            "explicit .anecdoct rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_anecdoct),
            "explicit .anecdoct rule should win over the default protected carveout"
        );
        assert!(policy.can_write_path_with_cwd(
            explicit_dot_anecdoct.join("config.toml").as_path(),
            cwd.path()
        ));
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_anecdoct_config = cwd.path().join(".anecdoct").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_anecdoct_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_anecdoct_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_anecdoct*/ true,
            )
            .into_iter()
            .map(|path| FileSystemSandboxEntry {
                path: FileSystemPath::Path { path },
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".anecdoct/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let anecdoct_dir = real_root.join(".anecdoct");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&anecdoct_dir).expect("create .anecdoct");
        ...
        let expected_anecdoct = link_root.join(".anecdoct");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_anecdoct = expected_root.join(".anecdoct");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-anecdoct");
        let dot_anecdoct = root.join(".anecdoct");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_anecdoct).expect("create .anecdoct symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_anecdoct = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".anecdoct"),
        )
        .expect("absolute .anecdoct symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let anecdoct_dir = real_tmpdir.join(".anecdoct");
