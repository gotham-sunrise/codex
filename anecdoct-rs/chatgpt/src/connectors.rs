...
use anecdoct_connectors::DirectoryListResponse;
use anecdoct_connectors::filter::filter_disallowed_connectors;
use anecdoct_connectors::merge::merge_connectors;
use anecdoct_connectors::merge::merge_plugin_connectors;
use anecdoct_core::config::Config;
pub use anecdoct_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use anecdoct_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use anecdoct_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use anecdoct_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use anecdoct_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
pub use anecdoct_core::connectors::with_app_enabled_state;
...
async fn connector_auth(config: &Config) -> anyhow::Result<AnecdoctAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_anecdoct_api_key_env*/ false).await;
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require Anecdoct backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(config: &Config) -> Option<Vec<AppInfo>> {
    ...
    let connectors = anecdoct_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = anecdoct_connectors::list_all_connectors_with_options(
        cache_context,
...
async fn plugin_apps_for_config(config: &Config) -> Vec<AppConnectorId> {
    ...
    PluginsManager::new(config.anecdoct_home.to_path_buf())
...
mod tests {
    use super::*;
    use anecdoct_connectors::metadata::connector_install_url;
    use anecdoct_plugin::AppConnectorId;
