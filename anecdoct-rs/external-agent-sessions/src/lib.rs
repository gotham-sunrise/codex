...
pub fn prepare_pending_session_imports(
    ...
    for session in requested_sessions {
        let has_been_imported =
            match has_current_session_been_imported(anecdoct_home, &session.path) {
...
pub fn prepare_validated_session_imports(
    ...
    requested_sessions
        .into_iter()
        .filter_map(|session| pending_session_import(anecdoct_home, session))
        .collect()
}
...
fn pending_session_import(
    ...
    let has_been_imported = match has_current_session_been_imported(anecdoct_home, &session.path) {
        Ok(has_been_imported) => has_been_imported,
        Err(_) => return None,
    };
    if has_been_imported {
...
mod tests {
    ...
    fn rejects_session_that_was_not_detected() {
        ...
        let anecdoct_home = root.path().join("anecdoct-home");
    ...
    fn skips_session_that_was_already_imported() {
        ...
        let source_path = root.path().join("session.jsonl");
        std::fs::write(&source_path, "{}\n").expect("session");
        record_imported_session(&anecdoct_home, &source_path, ThreadId::new())
