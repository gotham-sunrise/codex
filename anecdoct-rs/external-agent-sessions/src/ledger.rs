...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(anecdoct_home)?.contains_current_source(source_path)
}
...
pub fn record_imported_session(
    ...
    let mut ledger = load_import_ledger(anecdoct_home)?;
    ...
    save_import_ledger(anecdoct_home, &ledger)
}
...
pub(super) fn load_import_ledger(
    ...
    let path = import_ledger_path(anecdoct_home);
...
fn import_ledger_path(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(SESSION_IMPORT_LEDGER_FILE)
}
