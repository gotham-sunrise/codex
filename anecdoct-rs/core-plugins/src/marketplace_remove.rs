...
use anecdoct_config::RemoveMarketplaceConfigOutcome;
use anecdoct_config::remove_user_marketplace_config;
...
pub async fn remove_marketplace(
    ...
    tokio::task::spawn_blocking(move || remove_marketplace_sync(anecdoct_home.as_path(), request))
        .await
        .map_err(|err| {
...
fn remove_marketplace_sync(
    ...
    let destination = marketplace_install_root(anecdoct_home).join(&marketplace_name);
    let config_outcome =
        remove_user_marketplace_config(anecdoct_home, &marketplace_name).map_err(|err| {
...
mod tests {
    ...
    fn remove_marketplace_sync_removes_config_and_installed_root() {
        ...
        .unwrap();
        let installed_root = marketplace_install_root(anecdoct_home.path()).join("debug");
    ...
    fn remove_marketplace_sync_keeps_installed_root_when_config_removal_fails() {
        let anecdoct_home = TempDir::new().unwrap();
        fs::write(
            anecdoct_home.path().join(anecdoct_config::CONFIG_TOML_FILE),
            "[marketplaces.debug\n",
        )
        .unwrap();
        let installed_root = marketplace_install_root(anecdoct_home.path()).join("debug");
