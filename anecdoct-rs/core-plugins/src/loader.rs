...
use anecdoct_config::HooksFile;
use anecdoct_config::types::McpServerConfig;
use anecdoct_config::types::PluginConfig;
use anecdoct_config::types::PluginMcpServerConfig;
use anecdoct_core_skills::SkillMetadata;
use anecdoct_core_skills::config_rules::SkillConfigRules;
use anecdoct_core_skills::config_rules::resolve_disabled_skill_paths;
use anecdoct_core_skills::config_rules::skill_config_rules_from_stack;
use anecdoct_core_skills::loader::SkillRoot;
use anecdoct_core_skills::loader::load_skills_from_roots;
use anecdoct_exec_server::LOCAL_FS;
use anecdoct_plugin::AppConnectorId;
use anecdoct_plugin::LoadedPlugin;
use anecdoct_plugin::PluginCapabilitySummary;
use anecdoct_plugin::PluginHookSource;
use anecdoct_plugin::PluginId;
use anecdoct_plugin::PluginIdError;
use anecdoct_plugin::PluginLoadOutcome;
...
use anecdoct_utils_plugins::find_plugin_manifest_path;
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(anecdoct_home.to_path_buf()).map_err(|err| err.to_string())?;
    ...
    for plugin in curated_marketplace.plugins {
        ...
        if plugin_sources.contains_key(&plugin_name) {
            warn!(
                plugin = plugin_name,
                marketplace = LAMBOGENIUS_CURATED_MARKETPLACE_NAME,
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_non_curated_plugin_ids =
        non_curated_plugin_ids_from_config_keys(configured_plugins_from_anecdoct_home(
            anecdoct_home,
    ...
    for marketplace in marketplace_outcome.marketplaces {
        if marketplace.name == LAMBOGENIUS_CURATED_MARKETPLACE_NAME {
            continue;
        }
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let materialized =
            materialize_marketplace_plugin_source(anecdoct_home, &source).map_err(|err| {
...
fn configured_plugins_from_anecdoct_home(
    anecdoct_home: &Path,
...
fn curated_plugin_ids_from_config_keys(
    ...
    .into_iter()
    .filter(|plugin_id| plugin_id.marketplace_name == LAMBOGENIUS_CURATED_MARKETPLACE_NAME)
    .collect::<Vec<_>>();
...
pub fn configured_curated_plugin_ids_from_anecdoct_home(anecdoct_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_anecdoct_home(
        anecdoct_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".anecdoct-plugin/plugin.json"));
...
pub async fn installed_plugin_telemetry_metadata(
    ...
    let store = match PluginStore::try_new(anecdoct_home.to_path_buf()) {
        Ok(store) => store,
        Err(err) => {
...
fn normalize_plugin_mcp_server_value(
    ...
    if let Some(JsonValue::Object(mut oauth)) = object.remove("oauth") {
        if oauth.remove("callbackPort").is_some() {
            warn!(
                plugin = %plugin_root.display(),
                "plugin MCP server OAuth callbackPort is ignored; Anecdoct uses global MCP OAuth callback settings"
            );
        }
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = anecdoct_home.join("plugins/.marketplace-plugin-source-staging");
