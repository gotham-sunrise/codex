...
use crate::test_support::write_lambogenius_curated_marketplace;
use anecdoct_app_server_protocol::ConfigLayerSource;
use anecdoct_config::AppToolApproval;
...
use anecdoct_config::McpServerConfig;
use anecdoct_config::McpServerOAuthConfig;
use anecdoct_config::McpServerToolConfig;
...
use anecdoct_protocol::protocol::Product;
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".anecdoct-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "anecdoct-test@example.com"]);
    run_git(repo, &["config", "user.name", "Anecdoct Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(config_toml: &str, anecdoct_home: &Path) -> PluginLoadOutcome {
    write_file(&anecdoct_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(anecdoct_home, anecdoct_home).await;
    PluginsManager::new(anecdoct_home.to_path_buf())
        .plugins_for_config(&config)
        .await
}

async fn load_config(anecdoct_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(anecdoct_home, cwd).await
}
...
"#;
    ...
    let outcome = load_plugins_from_config(config_toml, anecdoct_home.path()).await;
...
"#,
    ...
    let config = load_config(anecdoct_home.path(), anecdoct_home.path()).await;
    let manager = PluginsManager::new(anecdoct_home.path().to_path_buf());
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let anecdoct_home = TempDir::new().unwrap();
    let plugin_a_root = anecdoct_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = anecdoct_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".anecdoct-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".anecdoct-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(anecdoct_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![anecdoct_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![anecdoct_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(anecdoct_home.path(), anecdoct_home.path()).await;
    let outcome = PluginsManager::new(anecdoct_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugins_for_config_reloads_when_plugin_hooks_enablement_changes() {
  ...
  "hooks": {
    ...
    let config_without_plugin_hooks = load_config(anecdoct_home.path(), anecdoct_home.path()).await;
    ...
    let config_with_plugin_hooks = load_config(anecdoct_home.path(), anecdoct_home.path()).await;
...
#[tokio::test]
async fn install_lambogenius_curated_plugin_uses_short_sha_cache_version() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_lambogenius_curated_marketplace(&curated_root, &["slack"]);
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/lambogenius-curated/slack/{TEST_CURATED_PLUGIN_CACHE_VERSION}"
    ));
    assert_eq!(
        result,
        PluginInstallOutcome {
            plugin_id: PluginId::new(
                "slack".to_string(),
                LAMBOGENIUS_CURATED_MARKETPLACE_NAME.to_string()
            )
            .unwrap(),
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(
        installed_path
            .join(".anecdoct-plugin/plugin.json")
            .is_file()
    );
}
...
"#,
    ...
    write_file(
        &repo_root.join(".anecdoct/config.toml"),
        r#"[[skills.config]]
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".anecdoct-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
...
async fn sync_plugins_from_remote_reconciles_cache_and_config() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_lambogenius_curated_marketplace(&curated_root, &["linear", "gmail", "calendar"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    write_plugin(
        &tmp.path().join("plugins/cache/lambogenius-curated"),
        "linear/local",
        "linear",
    );
    write_plugin(
...
[plugins."linear@lambogenius-curated"]
enabled = false

[plugins."gmail@lambogenius-curated"]
enabled = false

[plugins."calendar@lambogenius-curated"]
enabled = true
"#,
    );
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"[
  {"id":"1","name":"linear","marketplace_name":"lambogenius-curated","version":"1.0.0","enabled":true},
  {"id":"2","name":"gmail","marketplace_name":"lambogenius-curated","version":"1.0.0","enabled":false}
]"#,
        ))
        .mount(&server)
        .await;
    ...
    let result = manager
        .sync_plugins_from_remote(
            &config,
            Some(&AnecdoctAuth::create_dummy_chatgpt_auth_for_testing()),
            /*additive_only*/ false,
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            ...
            enabled_plugin_ids: vec!["linear@lambogenius-curated".to_string()],
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: vec![
                "gmail@lambogenius-curated".to_string(),
                "calendar@lambogenius-curated".to_string(),
            ],
        }
    );
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/lambogenius-curated/linear/local")
            .is_dir()
    );
    assert!(
        !tmp.path()
            .join("plugins/cache/lambogenius-curated/gmail")
            .exists()
    );
    assert!(
        !tmp.path()
            .join("plugins/cache/lambogenius-curated/calendar")
            .exists()
    );

    let config = fs::read_to_string(tmp.path().join(CONFIG_TOML_FILE)).unwrap();
    assert!(config.contains(r#"[plugins."linear@lambogenius-curated"]"#));
    assert!(config.contains("enabled = true"));
    assert!(!config.contains(r#"[plugins."gmail@lambogenius-curated"]"#));
    assert!(!config.contains(r#"[plugins."calendar@lambogenius-curated"]"#));
    ...
    let curated_marketplace = manager
        ...
        .marketplaces
        .into_iter()
        .find(|marketplace| marketplace.name == LAMBOGENIUS_CURATED_MARKETPLACE_NAME)
        .unwrap();
    assert_eq!(
        curated_marketplace
            .plugins
            ...
            .collect::<Vec<_>>(),
        vec![
            ("linear@lambogenius-curated".to_string(), true, true),
            ("gmail@lambogenius-curated".to_string(), false, false),
            ("calendar@lambogenius-curated".to_string(), false, false),
        ]
    );
}
...
"#,
  ...
  {"id":"2","name":"gmail","marketplace_name":"lambogenius-curated","version":"1.0.0","enabled":false}
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/lambogenius-curated/gmail/local")
            .is_dir()
    );
    assert!(
        tmp.path()
            .join("plugins/cache/lambogenius-curated/calendar/local")
            .is_dir()
    );
...
async fn sync_plugins_from_remote_ignores_unknown_remote_plugins() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_lambogenius_curated_marketplace(&curated_root, &["linear"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_string(
                r#"[
  {"id":"1","name":"plugin-one","marketplace_name":"lambogenius-curated","version":"1.0.0","enabled":true}
]"#,
            ))
            .mount(&server)
            .await;
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            ...
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: vec!["linear@lambogenius-curated".to_string()],
        }
    );
    let config = fs::read_to_string(tmp.path().join(CONFIG_TOML_FILE)).unwrap();
    assert!(!config.contains(r#"[plugins."linear@lambogenius-curated"]"#));
    assert!(
        !tmp.path()
            .join("plugins/cache/lambogenius-curated/linear")
            .exists()
    );
}
...
async fn sync_plugins_from_remote_keeps_existing_plugins_when_install_fails() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_lambogenius_curated_marketplace(&curated_root, &["linear", "gmail"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"[
  {"id":"1","name":"gmail","marketplace_name":"lambogenius-curated","version":"1.0.0","enabled":true}
]"#,
        ))
        .mount(&server)
        .await;
...
"#,
  ...
  {"id":"1","name":"gmail","marketplace_name":"lambogenius-curated","version":"1.0.0","enabled":true}
    ...
    assert_eq!(
        result,
        RemotePluginSyncResult {
            installed_plugin_ids: vec!["gmail@lambogenius-curated".to_string()],
            enabled_plugin_ids: vec!["gmail@lambogenius-curated".to_string()],
            disabled_plugin_ids: Vec::new(),
            uninstalled_plugin_ids: Vec::new(),
        }
    );
    assert_eq!(
        fs::read_to_string(tmp.path().join(format!(
            "plugins/cache/lambogenius-curated/gmail/{TEST_CURATED_PLUGIN_CACHE_VERSION}/marker.txt"
        )))
        .unwrap(),
        "first"
    );
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_anecdoct() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "anecdoct"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["anecdoct-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["anecdoct-plugin".to_string()]);
}
...
fn refresh_curated_plugin_cache_replaces_existing_local_version_with_short_sha_version() {
    ...
    assert!(
        !tmp.path()
            .join("plugins/cache/lambogenius-curated/slack/local")
            .exists()
    );
    assert!(
        tmp.path()
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_anecdoct_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
[plugins."slack@lambogenius-curated"]
enabled = true
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_anecdoct_home(tmp.path())
            .into_iter()
            .map(|plugin_id| plugin_id.as_key())
            .collect::<Vec<_>>(),
        vec!["slack@lambogenius-curated".to_string()]
    );
...
fn refresh_curated_plugin_cache_migrates_full_sha_cache_version_to_short_version() {
    ...
    assert!(
        !tmp.path()
            .join(format!(
                "plugins/cache/lambogenius-curated/slack/{TEST_CURATED_PLUGIN_SHA}"
            ))
            .exists()
    );
    assert!(
...
async fn load_plugins_ignores_project_config_files() {
    let anecdoct_home = TempDir::new().unwrap();
    let project_root = anecdoct_home.path().join("project");
    let plugin_root = anecdoct_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".anecdoct/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_anecdoct_folder: AbsolutePathBuf::try_from(project_root.join(".anecdoct"))
                    .unwrap(),
            },
    ...
    let outcome = load_plugins_from_layer_stack(
        ...
        &PluginStore::new(anecdoct_home.path().to_path_buf()),
        Some(Product::Anecdoct),
