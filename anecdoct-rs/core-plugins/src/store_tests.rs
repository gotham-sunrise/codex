...
#[test]
fn try_new_rejects_relative_anecdoct_home() {
    let err = PluginStore::try_new(PathBuf::from("relative"))
        .expect_err("relative anecdoct home should fail");
...
fn install_with_version_uses_requested_cache_version() {
    ...
    let installed_path = tmp.path().join(format!(
        "plugins/cache/lambogenius-curated/sample-plugin/{plugin_version}"
    ));
    assert_eq!(
        result,
        PluginInstallResult {
