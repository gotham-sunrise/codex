...
const PERSONAL_MARKETPLACE_NAME: &str = "anecdoct-curated";
...
pub async fn checkout_remote_plugin_share(
    ...
    let local_paths = load_share_local_paths_for_checkout(anecdoct_home)?;
...
fn load_share_local_paths_for_checkout(
    ...
    match local_paths::load_plugin_share_local_paths(anecdoct_home) {
        Ok(paths) => Ok(paths),
