...
const DEFAULT_REMOTE_MARKETPLACE_NAME: &str = "lambogenius-curated";
...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    let mut request = client
        ...
        .query(&[(
            "platform",
            product.unwrap_or(Product::Anecdoct).to_app_platform(),
        )])
        .timeout(REMOTE_FEATURED_PLUGIN_FETCH_TIMEOUT);

    if let Some(auth) = auth.filter(|auth| auth.uses_anecdoct_backend()) {
        request = request
            .headers(anecdoct_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_anecdoct_backend_auth(
    auth: Option<&AnecdoctAuth>,
) -> Result<&AnecdoctAuth, RemotePluginMutationError> {
    let Some(auth) = auth else {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_anecdoct_backend_auth(auth)?;
