...
use crate::LAMBOGENIUS_CURATED_MARKETPLACE_NAME;
use crate::installed_marketplaces::installed_marketplace_roots_from_layer_stack;
use crate::loader::configured_curated_plugin_ids_from_anecdoct_home;
...
use crate::startup_sync::sync_lambogenius_plugins_repo;
...
use anecdoct_config::PluginConfigEdit;
use anecdoct_config::apply_user_plugin_config_edits;
use anecdoct_config::clear_user_plugin;
use anecdoct_config::set_user_plugin_enabled;
use anecdoct_config::types::PluginConfig;
use anecdoct_config::version_for_toml;
use anecdoct_core_skills::SkillMetadata;
use anecdoct_hooks::plugin_hook_declarations;
...
use anecdoct_plugin::PluginIdError;
use anecdoct_plugin::prompt_safe_plugin_description;
...
use anecdoct_utils_plugins::PluginSkillRoot;
...
impl PluginsManager {
    pub fn new(anecdoct_home: PathBuf) -> Self {
        Self::new_with_restriction_product(anecdoct_home, Some(Product::Anecdoct))
    }
    ...
    pub fn new_with_restriction_product(
        ...
        // Product restrictions are enforced at marketplace admission time for a given ANECDOCT_HOME:
        ...
        // contents of that ANECDOCT_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single ANECDOCT_HOME is only used by one product.
        Self {
            anecdoct_home: anecdoct_home.clone(),
            store: PluginStore::new(anecdoct_home),
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if resolved.plugin_id.marketplace_name == LAMBOGENIUS_CURATED_MARKETPLACE_NAME {
                let curated_plugin_version = read_curated_plugins_sha(self.anecdoct_home.as_path())
                    .ok_or_else(|| {
        ...
        let anecdoct_home = self.anecdoct_home.clone();
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(anecdoct_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        let plugin_telemetry = if self.store.active_plugin_root(&plugin_id).is_some() {
            Some(
                installed_plugin_telemetry_metadata(self.anecdoct_home.as_path(), &plugin_id).await,
            )
        } else {
            None
        };
        ...
        clear_user_plugin(&self.anecdoct_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let curated_marketplace_root = curated_plugins_repo_path(self.anecdoct_home.as_path());
        ...
        let config_result = if config_edits.is_empty() {
            ...
            apply_user_plugin_config_edits(&self.anecdoct_home, config_edits).await
        };
        self.clear_cache();
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path =
            if matches!(plugin.source, MarketplacePluginSource::Git { .. }) && plugin.installed {
                ...
                let materialized = tokio::task::spawn_blocking(move || {
                    materialize_marketplace_plugin_source(anecdoct_home.as_path(), &source)
                })
                .await
                .map_err(|err| {
                    MarketplaceError::InvalidPlugin(format!(
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest.paths,
            self.restriction_product,
            &anecdoct_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
        )
        .await;
    ...
    fn start_curated_repo_sync(self: &Arc<Self>) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(
                move || match sync_lambogenius_plugins_repo(anecdoct_home.as_path()) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_anecdoct_home(
    ...
    fn run_non_curated_plugin_cache_refresh_loop(self: Arc<Self>) {
        loop {
            ...
            let refresh_result = match request.mode {
                NonCuratedCacheRefreshMode::IfVersionChanged => {
                    refresh_non_curated_plugin_cache(self.anecdoct_home.as_path(), &request.roots)
                }
    ...
    fn marketplace_roots(
        ...
        let curated_repo_root = curated_plugins_repo_path(self.anecdoct_home.as_path());
