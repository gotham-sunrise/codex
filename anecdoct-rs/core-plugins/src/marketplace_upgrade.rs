...
use anecdoct_config::types::MarketplaceConfig;
use anecdoct_config::types::MarketplaceSourceType;
...
pub fn upgrade_configured_git_marketplaces(
    ...
    for marketplace in marketplaces {
        match upgrade_configured_git_marketplace(anecdoct_home, &install_root, &marketplace) {
...
fn marketplace_install_root(anecdoct_home: &Path) -> PathBuf {
...
fn upgrade_configured_git_marketplace(
    ...
    activate_marketplace_root(&destination, staged_dir, || {
        ensure_configured_git_marketplace_unchanged(anecdoct_home, marketplace)?;
        record_user_marketplace(anecdoct_home, &marketplace.name, &update).map_err(|err| {
            format!(
...
fn ensure_configured_git_marketplace_unchanged(
    ...
    let current = read_configured_git_marketplace(anecdoct_home, &expected.name)?;
    match current {
