...
use anecdoct_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use anecdoct_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
const LAMBOGENIUS_PLUGINS_OWNER: &str = "lambogenius";
const LAMBOGENIUS_PLUGINS_REPO: &str = "plugins";
...
// Keep this comfortably above a normal sync attempt so we do not race another Anecdoct process.
...
pub fn curated_plugins_repo_path(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn read_curated_plugins_sha(anecdoct_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(anecdoct_home).as_path())
}

fn curated_plugins_sha_path(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_lambogenius_plugins_repo(anecdoct_home: &Path) -> Result<String, String> {
...
fn sync_lambogenius_plugins_repo_with_transport_overrides(
    anecdoct_home: &Path,
    git_binary: &str,
    api_base_url: &str,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    match sync_lambogenius_plugins_repo_via_git(anecdoct_home, git_binary) {
        Ok(remote_sha) => {
        ...
        Err(err) => {
            ...
            match sync_lambogenius_plugins_repo_via_http(anecdoct_home, api_base_url) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(anecdoct_home) {
                        ...
                        let result = sync_lambogenius_plugins_repo_via_backup_archive(
                            anecdoct_home,
                            backup_archive_api_url,
                        );
...
fn sync_lambogenius_plugins_repo_via_git(
    anecdoct_home: &Path,
    git_binary: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(anecdoct_home);
    let sha_path = anecdoct_home.join(CURATED_PLUGINS_SHA_FILE);
    ...
    let clone_output = run_git_command_with_timeout(
        Command::new(git_binary)
            ...
            .arg("--depth")
            .arg("1")
            .arg("https://github.com/lambogenius/plugins.git")
...
fn sync_lambogenius_plugins_repo_via_http(
    anecdoct_home: &Path,
    api_base_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(anecdoct_home);
...
fn sync_lambogenius_plugins_repo_via_backup_archive(
    anecdoct_home: &Path,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(anecdoct_home);
    let sha_path = curated_plugins_sha_path(anecdoct_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(anecdoct_home: &Path) -> bool {
    curated_plugins_repo_path(anecdoct_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && anecdoct_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
...
async fn fetch_curated_repo_remote_sha(api_base_url: &str) -> Result<String, String> {
    ...
    let repo_url =
        format!("{api_base_url}/repos/{LAMBOGENIUS_PLUGINS_OWNER}/{LAMBOGENIUS_PLUGINS_REPO}");
