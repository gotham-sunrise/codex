...
pub(crate) async fn load_plugins_config(anecdoct_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let anecdoct_home =
        AbsolutePathBuf::try_from(anecdoct_home).expect("anecdoct home should be absolute");
