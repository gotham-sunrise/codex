//! Cloud-hosted config requirements for Anecdoct.
//!
...
//! requirements cannot be loaded for those accounts, Anecdoct fails configuration loading rather than
...
use anecdoct_core::util::backoff;
...
const CLOUD_REQUIREMENTS_FETCH_ATTEMPT_METRIC: &str = "anecdoct.cloud_requirements.fetch_attempt";
const CLOUD_REQUIREMENTS_FETCH_FINAL_METRIC: &str = "anecdoct.cloud_requirements.fetch_final";
const CLOUD_REQUIREMENTS_LOAD_METRIC: &str = "anecdoct.cloud_requirements.load";
...
const CLOUD_REQUIREMENTS_CACHE_WRITE_HMAC_KEY: &[u8] =
    b"anecdoct-cloud-requirements-cache-v3-064f8542-75b4-494c-a294-97d3ce597271";
...
fn auth_identity(auth: &AnecdoctAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_requirements_eligible_auth(auth: &AnecdoctAuth) -> bool {
    let Some(plan_type) = auth.account_plan_type() else {
        return false;
    };
    auth.uses_anecdoct_backend()
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: anecdoct_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
    ...
    async fn fetch_with_timeout(
        ...
        let _timer =
            anecdoct_otel::start_global_timer("anecdoct.cloud_requirements.fetch.duration_ms", &[]);
...
pub async fn cloud_requirements_loader_for_storage(
    ...
    .await;
    cloud_requirements_loader(auth_manager, chatgpt_base_url, anecdoct_home)
}
...
fn emit_metric(metric_name: &str, tags: Vec<(&str, String)>) {
    if let Some(metrics) = anecdoct_otel::global() {
        let tag_refs = tags
            .iter()
...
mod tests {
    ...
    use anecdoct_config::types::AuthCredentialsStoreMode;
    use anecdoct_login::auth::AgentIdentityAuth;
    use anecdoct_login::auth::AgentIdentityAuthRecord;
    ...
    fn write_auth_json(anecdoct_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
        std::fs::write(
            anecdoct_home.join("auth.json"),
    ...
    async fn auth_manager_with_api_key() -> Arc<AuthManager> {
        ...
        let auth_json = json!({
            "LAMBOGENIUS_API_KEY": "sk-test-key",
            "tokens": null,
            "last_refresh": null,
        });
    ...
    fn chatgpt_auth_json_with_mode(
        ...
        let payload = json!({
            ...
            "https://api.lambogenius.com/auth": auth_payload,
        });
    ...
    async fn cloud_requirements_eligible_auth_allows_agent_identity_business_plan() {
        ...
        let original_authapi_base_url =
            std::env::var_os("ANECDOCT_AGENT_IDENTITY_AUTHAPI_BASE_URL");
        unsafe {
            std::env::set_var(
                "ANECDOCT_AGENT_IDENTITY_AUTHAPI_BASE_URL",
                &authapi_base_url,
            );
        }
        let _authapi_guard = EnvVarGuard {
            key: "ANECDOCT_AGENT_IDENTITY_AUTHAPI_BASE_URL",
            original: original_authapi_base_url,
        };
        let auth = AgentIdentityAuth::load(record)
            .await
            .map(AnecdoctAuth::AgentIdentity)
...
"#,
        ...
        assert_eq!(
            result,
            Some(ConfigRequirementsToml {
                apps: Some(anecdoct_config::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        "connector_5f3c8c41a1e54ad7a76272c89e2554fa".to_string(),
                        anecdoct_config::AppRequirementToml {
                            enabled: Some(false),
                            tools: None,
                        },
                    )]),
            ...
            Some(ConfigRequirementsToml {
                apps: Some(anecdoct_config::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        ...
                        anecdoct_config::AppRequirementToml {
                            enabled: None,
                            tools: Some(anecdoct_config::AppToolsRequirementsToml {
                                tools: BTreeMap::from([(
                                    "calendar/list_events".to_string(),
                                    anecdoct_config::AppToolRequirementToml {
...
"#,
                ...
                plugins: Some(BTreeMap::from([(
                    ...
                    anecdoct_config::PluginRequirementsToml {
                        mcp_servers: Some(BTreeMap::from([(
                            "sample".to_string(),
                            anecdoct_config::McpServerRequirement {
                                identity: anecdoct_config::McpServerIdentity::Command {
    ...
    async fn fetch_cloud_requirements_recovers_after_unauthorized_reload_updates_cache_identity() {
        ...
        let path = anecdoct_home.path().join(CLOUD_REQUIREMENTS_CACHE_FILENAME);
