...
use anecdoct_config::HookStateToml;
...
use anecdoct_protocol::protocol::HookTrustStatus;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project {
            ...
            .hooks_config_folder()
            .unwrap_or_else(|| dot_anecdoct_folder.clone())
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: anecdoct_protocol::protocol::HookEventName,
...
mod tests {
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.anecdoct/config.toml").abs();
        let dot_anecdoct_folder = test_path_buf("/tmp/worktree/.anecdoct").abs();
        ...
        assert_eq!(
            super::hook_metadata_for_config_layer_source(&ConfigLayerSource::Mdm {
                domain: "com.lambogenius.anecdoct".to_string(),
