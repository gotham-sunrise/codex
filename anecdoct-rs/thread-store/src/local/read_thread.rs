...
fn resolve_requested_rollout_path(
    ...
    let path = if rollout_path.is_relative() {
        store.config.anecdoct_home.join(rollout_path)
    } else {
        rollout_path
    };
...
async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        ...
        && (include_archived
            || !rollout_path_is_archived(store.config.anecdoct_home.as_path(), &path))
    {
        return Ok(Some(path));
    }
...
async fn read_thread_from_rollout_path(
    ...
    let archived = rollout_path_is_archived(store.config.anecdoct_home.as_path(), path.as_path());
    ...
    if let Ok(Some(title)) =
        find_thread_name_by_id(store.config.anecdoct_home.as_path(), &thread.thread_id).await
    {
...
async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<anecdoct_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let name = match distinct_thread_metadata_title(&metadata) {
        ...
        None => find_thread_name_by_id(store.config.anecdoct_home.as_path(), &metadata.id)
            .await
            .ok()
            .flatten()
...
mod tests {
    ...
    async fn read_thread_returns_rollout_path_summary() {
        ...
        let relative_path = active_path
            ...
            .expect("path should be under anecdoct home")
            .to_path_buf();
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        anecdoct_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
