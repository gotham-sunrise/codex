...
use anecdoct_rollout::ThreadItem;
...
pub(super) fn rollout_path_is_archived(anecdoct_home: &Path, path: &Path) -> bool {
    path.starts_with(anecdoct_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
