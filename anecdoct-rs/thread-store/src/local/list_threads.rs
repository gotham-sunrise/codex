...
use anecdoct_rollout::RolloutConfig;
use anecdoct_rollout::RolloutRecorder;
use anecdoct_rollout::find_thread_names_by_ids;
use anecdoct_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => anecdoct_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => anecdoct_rollout::ThreadSortKey::UpdatedAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => anecdoct_rollout::SortDirection::Asc,
        SortDirection::Desc => anecdoct_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        anecdoct_home: store.config.anecdoct_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.anecdoct_home.clone(),
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.anecdoct_home.as_path(), &thread_ids).await
    {
...
async fn list_rollout_threads(
    state_db: Option<anecdoct_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&anecdoct_rollout::Cursor>,
    sort_key: anecdoct_rollout::ThreadSortKey,
    sort_direction: anecdoct_rollout::SortDirection,
) -> ThreadStoreResult<anecdoct_rollout::ThreadsPage> {
