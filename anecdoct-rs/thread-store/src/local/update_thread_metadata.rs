...
use anecdoct_rollout::ARCHIVED_SESSIONS_SUBDIR;
use anecdoct_rollout::append_rollout_item_to_path;
use anecdoct_rollout::append_thread_name;
use anecdoct_rollout::find_archived_thread_path_by_id_str;
use anecdoct_rollout::find_thread_path_by_id_str;
use anecdoct_rollout::read_session_meta_line;
...
pub(super) async fn update_thread_metadata(
    ...
    anecdoct_rollout::state_db::reconcile_rollout(
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    anecdoct_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(anecdoct_git_utils::GitSha::new),
        branch: branch.clone(),
...
async fn apply_thread_name(
    ...
    append_thread_name(store.config.anecdoct_home.as_path(), thread_id, &name)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
async fn resolve_rollout_path(
    ...
    let active_path = find_thread_path_by_id_str(
        store.config.anecdoct_home.as_path(),
        &thread_id.to_string(),
        state_db_ctx.as_deref(),
    )
    .await
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &Path) -> bool {
    path.starts_with(store.config.anecdoct_home.join(ARCHIVED_SESSIONS_SUBDIR))
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = anecdoct_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn update_thread_metadata_sets_memory_mode_on_active_rollout() {
        ...
        let runtime = anecdoct_state::StateRuntime::init(
    ...
    async fn update_thread_metadata_sets_git_info() {
                ...
                patch: ThreadMetadataPatch {
                    git_info: Some(GitInfoPatch {
                        ...
                        origin_url: Some(Some(
                            "https://github.com/lambogenius/anecdoct".to_string(),
                        )),
                    }),
                    ..Default::default()
                },
        ...
        assert_eq!(
            ...
            Some("https://github.com/lambogenius/anecdoct")
        );
    }
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd =
            anecdoct_utils_path::normalize_for_path_comparison(workspace.as_path())
