...
//!
//! If the keyring is not available or fails, we fall back to ANECDOCT_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Anecdoct MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_anecdoct_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    struct TempAnecdoctHome {
        _guard: MutexGuard<'static, ()>,
        _dir: tempfile::TempDir,
    }

    impl TempAnecdoctHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create ANECDOCT_HOME temp dir");
            unsafe {
                std::env::set_var("ANECDOCT_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempAnecdoctHome {
        fn drop(&mut self) {
            unsafe {
