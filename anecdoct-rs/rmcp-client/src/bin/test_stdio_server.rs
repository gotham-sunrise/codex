...
const MEMO_URI: &str = "memo://anecdoct/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "anecdoct/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Anecdoct TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Anecdoct TUI):
    /// - Build this binary: `cargo build -p anecdoct-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `anecdoct mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Anecdoct TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://anecdoct/{slug}".to_string(),
            name: "anecdoct-memo".to_string(),
            title: Some("Anecdoct Memo".to_string()),
            description: Some(
                "Template for memo://anecdoct/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Anecdoct TUI handling of MCP image outputs.
///
    ...
    fn image_scenario_result(args: ImageScenarioArgs) -> Result<CallToolResult, McpError> {
        ...
        match args.scenario {
            ...
            ImageScenario::ImageOnlyOriginalDetail => {
                ...
                meta.insert(
                    "anecdoct/imageDetail".to_string(),
