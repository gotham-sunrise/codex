...
//!
//! The history is stored at `~/.anecdoct/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.anecdoct`.
...
impl HistoryConfig {
    pub fn new(anecdoct_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            anecdoct_home: anecdoct_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.anecdoct_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.anecdoct/history.jsonl` and ensure the parent directory exists.
