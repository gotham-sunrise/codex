...
use anecdoct_api::ResponsesClient;
use anecdoct_api::ResponsesOptions;
use anecdoct_client::HttpTransport;
use anecdoct_client::Request;
use anecdoct_client::RequestBody;
use anecdoct_client::Response;
...
fn provider(name: &str) -> Provider {
    Provider {
        ...
        query_params: None,
        headers: HeaderMap::new(),
        retry: anecdoct_api::RetryConfig {
            max_attempts: 1,
...
async fn responses_client_uses_responses_path() -> Result<()> {
    ...
    let client = ResponsesClient::new(transport, provider("lambogenius"), Arc::new(NoAuth));
...
async fn streaming_client_adds_auth_headers() -> Result<()> {
    ...
    let client = ResponsesClient::new(transport, provider("lambogenius"), auth);
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    let mut provider = provider("lambogenius");
