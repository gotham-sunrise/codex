...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, AnecdoctErr::ServerOverloaded));
}
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let AnecdoctErr::CyberPolicy { message } = err else {
        panic!("expected AnecdoctErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let AnecdoctErr::InvalidRequest(message) = err else {
        panic!("expected AnecdoctErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("anecdoct_other"),
    );
    headers.insert(
        "x-anecdoct-other-limit-name",
        http::HeaderValue::from_static("anecdoct_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let AnecdoctErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected AnecdoctErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let AnecdoctErr::UnexpectedStatus(err) = err else {
        panic!("expected AnecdoctErr::UnexpectedStatus, got {err:?}");
    };
