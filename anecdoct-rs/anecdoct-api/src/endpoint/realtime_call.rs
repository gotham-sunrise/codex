...
use anecdoct_client::RequestTelemetry;
use bytes::Bytes;
use http::HeaderMap;
...
const MULTIPART_BOUNDARY: &str = "anecdoct-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str =
    "multipart/form-data; boundary=anecdoct-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "anecdoct_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    async fn sends_sdp_offer_as_raw_body() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://api.lambogenius.com/v1"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(request.url, "https://api.lambogenius.com/v1/realtime/calls");
        assert_eq!(
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://chatgpt.com/backend-api/anecdoct"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/anecdoct/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
