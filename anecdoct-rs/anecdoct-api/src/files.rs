...
pub const LAMBOGENIUS_FILE_URI_PREFIX: &str = "sediment://";
pub const LAMBOGENIUS_FILE_UPLOAD_LIMIT_BYTES: u64 = 512 * 1024 * 1024;

const LAMBOGENIUS_FILE_REQUEST_TIMEOUT: Duration = Duration::from_secs(60);
const LAMBOGENIUS_FILE_FINALIZE_TIMEOUT: Duration = Duration::from_secs(30);
const LAMBOGENIUS_FILE_FINALIZE_RETRY_DELAY: Duration = Duration::from_millis(250);
const LAMBOGENIUS_FILE_USE_CASE: &str = "anecdoct";
...
pub enum OpenAiFileError {
    ...
    #[error("failed to send Lambogenius file request to {url}: {source}")]
    Request {
        url: String,
        #[source]
        source: reqwest::Error,
    },
    #[error("Lambogenius file request to {url} failed with status {status}: {body}")]
    UnexpectedStatus {
        url: String,
        status: StatusCode,
        body: String,
    },
    #[error("failed to parse Lambogenius file response from {url}: {source}")]
    Decode {
        url: String,
        #[source]
        source: serde_json::Error,
    },
    #[error("Lambogenius file upload for `{file_id}` is not ready yet")]
    UploadNotReady { file_id: String },
    #[error("Lambogenius file upload for `{file_id}` failed: {message}")]
...
pub fn lambogenius_file_uri(file_id: &str) -> String {
    format!("{LAMBOGENIUS_FILE_URI_PREFIX}{file_id}")
}
...
pub async fn upload_local_file(
    ...
    if metadata.len() > LAMBOGENIUS_FILE_UPLOAD_LIMIT_BYTES {
        return Err(OpenAiFileError::FileTooLarge {
            path: path.to_path_buf(),
            size_bytes: metadata.len(),
            limit_bytes: LAMBOGENIUS_FILE_UPLOAD_LIMIT_BYTES,
        });
    }
    ...
    let create_response = authorized_request(auth, reqwest::Method::POST, &create_url)
        .json(&serde_json::json!({
            ...
            "use_case": LAMBOGENIUS_FILE_USE_CASE,
        }))
        .send()
        .await
    ...
    let upload_response = build_reqwest_client()
        ...
        .timeout(LAMBOGENIUS_FILE_REQUEST_TIMEOUT)
        ...
        match finalize_payload.status.as_str() {
            "success" => {
                return Ok(UploadedOpenAiFile {
                    ...
                    uri: lambogenius_file_uri(&create_payload.file_id),
            ...
            "retry" => {
                if finalize_started_at.elapsed() >= LAMBOGENIUS_FILE_FINALIZE_TIMEOUT {
                    return Err(OpenAiFileError::UploadNotReady {
                        file_id: create_payload.file_id,
                    });
                }
                tokio::time::sleep(LAMBOGENIUS_FILE_FINALIZE_RETRY_DELAY).await;
            }
            _ => {
...
fn build_reqwest_client() -> reqwest::Client {
    build_reqwest_client_with_custom_ca(reqwest::Client::builder()).unwrap_or_else(|error| {
        tracing::warn!(error = %error, "failed to build Lambogenius file upload client");
        reqwest::Client::new()
    })
}
