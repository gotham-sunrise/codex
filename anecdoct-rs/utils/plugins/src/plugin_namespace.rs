...
const DISCOVERABLE_PLUGIN_MANIFEST_PATHS: &[&str] =
    &[".anecdoct-plugin/plugin.json", ".claude-plugin/plugin.json"];
...
/// plugin manifest (same `name` rules as full manifest loading in anecdoct-core).
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".anecdoct-plugin")).expect("mkdir manifest");
        fs::write(
