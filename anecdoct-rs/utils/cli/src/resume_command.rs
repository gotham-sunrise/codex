//! Shared formatting for user-facing `anecdoct resume` command hints.
...
pub fn resume_command(thread_name: Option<&str>, thread_id: Option<ThreadId>) -> Option<String> {
    ...
    resume_target.map(|target| {
        ...
        if needs_double_dash {
            format!("anecdoct resume -- {escaped}")
        } else {
            format!("anecdoct resume {escaped}")
        }
    })
...
mod tests {
    ...
    fn prefers_name_over_id() {
        ...
        assert_eq!(command, Some("anecdoct resume my-thread".to_string()));
    }
    ...
    fn formats_thread_id_when_name_is_missing() {
        ...
        assert_eq!(
            command,
            Some("anecdoct resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    fn quotes_thread_names_when_needed() {
        ...
        assert_eq!(
            command,
            Some("anecdoct resume -- -starts-with-dash".to_string())
        );

        let command = resume_command(Some("two words"), /*thread_id*/ None);
        assert_eq!(command, Some("anecdoct resume 'two words'".to_string()));

        let command = resume_command(Some("quote'case"), /*thread_id*/ None);
        assert_eq!(command, Some("anecdoct resume \"quote'case\"".to_string()));
    }
}
