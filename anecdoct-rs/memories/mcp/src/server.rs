...
impl<B: MemoriesBackend> ServerHandler for MemoriesMcpServer<B> {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            instructions: Some(
                "Use these tools to list, read, and search Anecdoct memory files.".to_string(),
            ),
...
{
    let backend = LocalMemoriesBackend::from_anecdoct_home(anecdoct_home);
...
pub async fn run_stdio_server(anecdoct_home: &AbsolutePathBuf) -> anyhow::Result<()> {
    run_server(anecdoct_home, (tokio::io::stdin(), tokio::io::stdout())).await
}
...
fn list_tool() -> Tool {
    let mut tool = Tool::new(
        ...
        Cow::Borrowed(
            "List immediate files and directories under a path in the Anecdoct memories store.",
        ),
...
fn read_tool() -> Tool {
    let mut tool = Tool::new(
        ...
        Cow::Borrowed(
            "Read a Anecdoct memory file by relative path, optionally starting at a 1-indexed line offset and limiting the number of lines returned.",
        ),
...
fn search_tool() -> Tool {
    let mut tool = Tool::new(
        ...
        Cow::Borrowed(
            "Search Anecdoct memory files for substring matches, optionally normalizing separators or requiring all query substrings on the same line or within a line window.",
        ),
