...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let anecdoct_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = anecdoct_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&anecdoct_home)
        .await
        .unwrap();
