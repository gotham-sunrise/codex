...
The folder `{{ memory_root }}/` is a git repository managed by Anecdoct. Read
`{{ phase2_workspace_diff_file }}` in this same folder first. It contains the git-style diff from
the previous successful Phase 2 baseline to the current worktree. It is generated by Anecdoct for
...
- Retrieval bias: preserve distinctive nouns and verbatim strings that a future grep/search
  ...
  `api.lambogenius.org/v1/files`, `Lambogenius Internal Slack`, etc.).
...
- Typical workflows and tools (how they like to work, how they use Anecdoct/agents, preferred formats)
