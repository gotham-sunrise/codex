...
use anecdoct_state::Stage1Output;
...
pub async fn run(context: Arc<MemoryStartupContext>, config: Arc<Config>) {
    ...
    let root = memory_root(&config.anecdoct_home);
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            anecdoct_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            anecdoct_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
                return Err("skipped_retry_unavailable");
            }
            anecdoct_state::Phase2JobClaimOutcome::SkippedCooldown => {
                return Err("skipped_cooldown");
            }
            anecdoct_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[anecdoct_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
...
mod agent {
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<anecdoct_state::Stage1Output>,
        memory_root: anecdoct_utils_absolute_path::AbsolutePathBuf,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<anecdoct_otel::Timer>,
    ) {
        let Some(db) = context.state_db() else {
            return;
        };
    ...
    async fn loop_agent(
        ...
        token: String,
        thread_id: ThreadId,
        thread: &anecdoct_core::AnecdoctThread,
    ) -> AgentStatus {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[anecdoct_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
