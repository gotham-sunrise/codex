...
use anecdoct_git_utils::diff_since_latest_init;
use anecdoct_git_utils::reset_git_repository;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_anecdoct(&server, home.clone()).await?;
    ...
    shutdown_test_anecdoct(&test).await?;
    Ok(())
}
...
async fn memories_startup_phase1_uses_live_thread_service_tier() -> anyhow::Result<()> {
    let server = start_mock_server().await;
    let home = Arc::new(TempDir::new()?);
    let test = build_test_anecdoct(&server, home).await?;
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<anecdoct_state::StateRuntime>> {
    let db = anecdoct_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into())
        .await?;
    db.mark_backfill_complete(/*last_watermark*/ None).await?;
    Ok(db)
}

async fn trigger_memories_startup(test: &TestAnecdoct) {
    let config_snapshot = test.anecdoct.config_snapshot().await;
...
async fn seed_stage1_output(
    db: &anecdoct_state::StateRuntime,
    anecdoct_home: &Path,
    ...
    let mut metadata_builder = anecdoct_state::ThreadMetadataBuilder::new(
        thread_id,
        anecdoct_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = anecdoct_home.join(format!("workspace-{rollout_slug}"));
...
) -> anyhow::Result<anecdoct_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.anecdoct.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        anecdoct_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_anecdoct(test: &TestAnecdoct) -> anyhow::Result<()> {
