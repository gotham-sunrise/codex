...
Runtime orchestration for Phase 1 and Phase 2 still lives in `anecdoct-core` under
`anecdoct-rs/core/src/memories/`.

## Crates

- `anecdoct-rs/memories/read` (`anecdoct-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `anecdoct-rs/memories/mcp` (`anecdoct-memories-mcp`) owns the read-only memory
  filesystem MCP server implementation.
- `anecdoct-rs/memories/write` (`anecdoct-memories-write`) owns the write path:
...
- In `anecdoct`, edit those undated template files in place.
- The dated snapshot-copy workflow is used in the separate `lambogenius/project/agent_memory/write` harness repo, not here.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.anecdoct/memories/.git` by `anecdoct-git-utils`
