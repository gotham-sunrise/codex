...
use anecdoct_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use anecdoct_app_server_client::EnvironmentManager;
use anecdoct_app_server_client::ExecServerRuntimePaths;
use anecdoct_app_server_client::InProcessAppServerClient;
use anecdoct_app_server_client::InProcessClientStartArgs;
use anecdoct_app_server_client::InProcessServerEvent;
...
use anecdoct_app_server_protocol::Thread as AppServerThread;
use anecdoct_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use anecdoct_cloud_requirements::cloud_requirements_loader_for_storage;
use anecdoct_config::ConfigLoadError;
use anecdoct_config::ConfigLoadOptions;
use anecdoct_config::LoaderOverrides;
use anecdoct_config::format_config_error_with_source;
use anecdoct_core::StateDbHandle;
use anecdoct_core::check_execpolicy_for_warnings;
...
use anecdoct_core::config::load_config_as_toml_with_cli_and_load_options;
use anecdoct_core::config::resolve_oss_provider;
use anecdoct_core::config::resolve_profile_v2_config_path;
use anecdoct_core::find_thread_meta_by_name_str;
use anecdoct_core::format_exec_policy_error_with_source;
use anecdoct_core::path_utils;
use anecdoct_feedback::AnecdoctFeedback;
use anecdoct_git_utils::get_git_repo_root;
use anecdoct_login::AuthConfig;
use anecdoct_login::default_client::set_default_client_residency_requirement;
use anecdoct_login::default_client::set_default_originator;
use anecdoct_login::enforce_login_restrictions;
use anecdoct_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use anecdoct_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use anecdoct_otel::set_parent_from_context;
use anecdoct_otel::traceparent_context_from_env;
...
use anecdoct_protocol::protocol::ReviewTarget;
use anecdoct_protocol::protocol::RolloutItem;
use anecdoct_protocol::protocol::RolloutLine;
...
use anecdoct_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use anecdoct_utils_cli::SharedCliOptions;
use anecdoct_utils_oss::ensure_oss_provider_ready;
use anecdoct_utils_oss::get_default_model_for_oss_provider;
pub use cli::Cli;
pub use cli::Command;
pub use cli::ReviewArgs;
use event_processor_with_human_output::EventProcessorWithHumanOutput;
pub use event_processor_with_jsonl_output::AnecdoctStatus;
...
enum StdinPromptBehavior {
    ...
    /// `anecdoct exec` behavior for `anecdoct exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `anecdoct exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("anecdoct_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set anecdoct exec originator override {err:?}"
        );
    }
    ...
    #[allow(clippy::print_stderr)]
    let anecdoct_home = match find_anecdoct_home() {
        Ok(anecdoct_home) => anecdoct_home,
        Err(err) => {
            eprintln!("Error finding anecdoct home: {err}");
            std::process::exit(1);
        }
    };
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&anecdoct_home, profile_v2));
    ...
    let cloud_requirements = cloud_requirements_loader_for_storage(
        ...
        /*enable_anecdoct_api_key_env*/ false,
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        anecdoct_core::otel_init::build_provider(
            &config,
    ...
    anecdoct_core::otel_init::record_process_start(otel.as_ref(), "anecdoct_exec");
    anecdoct_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "anecdoct_exec");
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.anecdoct_self_exe.clone(),
        arg0_paths.anecdoct_linux_sandbox_exe.clone(),
    )?;
    let state_db = anecdoct_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        EnvironmentManager::from_env(local_runtime_paths).await?
    } else {
        EnvironmentManager::from_anecdoct_home(config.anecdoct_home.clone(), local_runtime_paths)
            .await?
    };
    let in_process_start_args = InProcessClientStartArgs {
        ...
        feedback: AnecdoctFeedback::new(),
        log_db: None,
        state_db: state_db.clone(),
        environment_manager: std::sync::Arc::new(environment_manager),
        config_warnings,
        session_source: SessionSource::Exec,
        enable_anecdoct_api_key_env: true,
        client_name: "anecdoct_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = anecdoct_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Anecdoct
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            anecdoct_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Anecdoct initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        anecdoct_app_server_protocol::TurnStatus::Failed
                            | anecdoct_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        AnecdoctStatus::Running => {}
                        AnecdoctStatus::InitiateShutdown => {
...
fn session_configured_from_thread_response(
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<anecdoct_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.anecdoct_home, session_id, Some(state_db.as_ref()))
                .await?
