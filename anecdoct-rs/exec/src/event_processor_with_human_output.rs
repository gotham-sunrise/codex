...
use anecdoct_protocol::num_format::format_with_separators;
use anecdoct_protocol::protocol::SessionConfiguredEvent;
use anecdoct_utils_sandbox_summary::summarize_permission_profile;
use owo_colors::OwoColorize;
use owo_colors::Style;

use crate::event_processor::AnecdoctStatus;
    ...
    fn render_item_completed(&mut self, item: ThreadItem) {
        match item {
            ThreadItem::AgentMessage { text, .. } => {
                eprintln!(
                    "{}\n{}",
                    "anecdoct".style(self.italic).style(self.magenta),
                    text
                );
...
impl EventProcessor for EventProcessorWithHumanOutput {
    fn print_config_summary(
        ...
        eprintln!("Lambogenius Anecdoct v{VERSION}\n--------");
    ...
    fn process_server_notification(&mut self, notification: ServerNotification) -> AnecdoctStatus {
        match notification {
            ...
            ServerNotification::TurnPlanUpdated(notification) => {
                ...
                for step in notification.plan {
                    match step.status {
                        ...
                        anecdoct_app_server_protocol::TurnPlanStepStatus::InProgress => {
                            eprintln!("  {} {}", "→".style(self.cyan), step.step);
                        }
                        anecdoct_app_server_protocol::TurnPlanStepStatus::Pending => {
                            eprintln!(
                                "  {} {}",
                                "•".style(self.dimmed),
            ...
            ServerNotification::TurnStarted(_) => AnecdoctStatus::Running,
