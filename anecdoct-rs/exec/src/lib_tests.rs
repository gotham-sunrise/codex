...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("anecdoct-exec-tests");
...
fn exec_default_stderr_filter_suppresses_otel_self_diagnostics() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::error!(target: "anecdoct_exec_test", "real exec error");
    });
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = anecdoct_protocol::protocol::W3cTraceContext {
...
async fn resume_lookup_model_providers_filters_only_last_lookup() {
    let anecdoct_home = tempdir().expect("create temp anecdoct home");
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: anecdoct_app_server_protocol::SessionSource::Exec,
        thread_source: None,
        agent_nickname: None,
        agent_role: None,
        git_info: None,
        name: None,
        turns: vec![
            anecdoct_app_server_protocol::Turn {
                id: "turn-1".to_string(),
...
async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
    ...
    assert_eq!(
        ...
        Some(anecdoct_app_server_protocol::ApprovalsReviewer::User)
    );
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(anecdoct_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        ...
        approval_policy: anecdoct_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: anecdoct_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: anecdoct_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
