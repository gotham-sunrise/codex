...
#[command(
    version,
    override_usage = "anecdoct exec [OPTIONS] [PROMPT]\n       anecdoct exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Anecdoct outside a Git repository.
    ...
    /// Do not load `$ANECDOCT_HOME/config.toml`; auth still uses `ANECDOCT_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "anecdoct resume --last <prompt>" treats the positional as a prompt, not a session id.
