...
/// While we may add an `apply-patch` subcommand to the `anecdoct` CLI multitool
/// at some point, we must ensure that the smaller `anecdoct-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(anecdoct_utils_cargo_bin::cargo_bin("anecdoct-exec")?)
        .arg(ANECDOCT_CORE_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
