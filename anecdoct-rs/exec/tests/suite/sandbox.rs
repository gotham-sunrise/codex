#![cfg(unix)]
use anecdoct_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use anecdoct_core::exec::build_exec_request;
...
async fn spawn_command_under_sandbox(
    ...
    use anecdoct_core::spawn_command_under_linux_sandbox;

    let anecdoct_linux_sandbox_exe = core_test_support::find_anecdoct_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_anecdoct_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .anecdoct && echo 'sandbox_mode = \"danger-full-access\"' > .anecdoct/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .anecdoct");

    let status = child
        .wait()
        .await
        .expect("should wait for .anecdoct command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .anecdoct creation: {status:?}"
    );
    let dot_anecdoct_metadata = tokio::fs::symlink_metadata(&dot_anecdoct).await;
    if let Ok(metadata) = dot_anecdoct_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_anecdoct.display()
        );
    } else if let Err(err) = &dot_anecdoct_metadata {
        assert_eq!(
            err.kind(),
