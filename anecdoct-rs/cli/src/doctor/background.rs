...
pub(super) fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let state_dir = config.anecdoct_home.join(STATE_DIR_NAME);
    ...
    let socket_path =
        match anecdoct_app_server::app_server_control_socket_path(&config.anecdoct_home) {
    ...
    if status.check_status() == CheckStatus::Warning {
        check = check.remediation("Run anecdoct app-server daemon version for more details.");
    }
    check
}
