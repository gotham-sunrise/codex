...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run anecdoct doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.anecdoct_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.anecdoct_version);
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("LAMBOGENIUS_API_KEY: present")
            .remediation("Run `anecdoct login`."),
            DoctorCheck::new(
                "updates.status",
                "updates",
                CheckStatus::Ok,
    ...
    fn render_human_report_includes_details_by_default_without_color() {
        ...
        let expected = format!(
            "\
Anecdoct Doctor v0.0.0

Notes
   ⚠ terminal     narrow terminal
   ✗ auth         token expired - Run `anecdoct login`.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `anecdoct login`.
-------------------------------------------------------------
...
",
    ...
    fn render_human_report_includes_redacted_details() {
        ...
        assert!(rendered.contains("      LAMBOGENIUS_API_KEY           present"));
    }
        ...
        let report = DoctorReport {
            ...
            checks: vec![
                DoctorCheck::new("config.load", "config", CheckStatus::Ok, "config loaded")
                    ...
                    .detail("model provider: lambogenius")
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/anecdoct/target/debug/anecdoct · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/anecdoct/target/debug/anecdoct"));
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: LAMBOGENIUS_API_KEY, ANECDOCT_API_KEY"),
            "auth env vars present: LAMBOGENIUS_API_KEY, ANECDOCT_API_KEY"
        );
    }
