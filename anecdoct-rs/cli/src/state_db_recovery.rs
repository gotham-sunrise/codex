...
use anecdoct_tui::LocalStateDbStartupError;
...
pub(crate) fn confirm_repair(startup_error: &LocalStateDbStartupError) -> std::io::Result<bool> {
    eprintln!("Anecdoct couldn't start because its local database appears to be damaged.");
    eprintln!("Anecdoct can try a safe repair by backing up those files and rebuilding them.");
    print_technical_details(startup_error);
    crate::confirm("Repair Anecdoct local data now? [y/N]: ")
}
...
pub(crate) async fn repair_files(
    ...
    let repair_suffix = format!("anecdoct-repair-{timestamp}");
    ...
    let logs_db_path = anecdoct_state::logs_db_path(sqlite_home);
    ...
    if backups.is_empty() {
        return Err(std::io::Error::other(
            "no repairable Anecdoct local data files were found",
        ));
    }
...
pub(crate) fn print_repair_backups(backups: &[PathBuf]) {
    eprintln!("Backed up Anecdoct local data before repair:");
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Anecdoct couldn't start because its local database appears to be damaged.");
    eprintln!("Run `anecdoct doctor` to check your setup and get next-step guidance.");
...
pub(crate) fn print_locked_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Anecdoct couldn't start because another Anecdoct process is using its local data.");
    eprintln!("Quit any other copies of Anecdoct that may still be running, then try again.");
    print_technical_details(startup_error);
}
...
mod tests {
    ...
    async fn repair_backs_up_owned_database_files() -> std::io::Result<()> {
        ...
        let state_path = anecdoct_state::state_db_path(temp_dir.path());
        let logs_path = anecdoct_state::logs_db_path(temp_dir.path());
    ...
    async fn repair_replaces_blocking_sqlite_home_file() -> std::io::Result<()> {
        ...
        let startup_error = LocalStateDbStartupError::new(
            anecdoct_state::state_db_path(sqlite_home.as_path()),
