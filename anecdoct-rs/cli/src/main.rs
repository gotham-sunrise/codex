use anecdoct_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use anecdoct_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use anecdoct_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use anecdoct_arg0::Arg0DispatchPaths;
use anecdoct_arg0::arg0_dispatch_or_else;
use anecdoct_chatgpt::apply_command::ApplyCommand;
use anecdoct_chatgpt::apply_command::run_apply_command;
use anecdoct_cli::LandlockCommand;
use anecdoct_cli::SeatbeltCommand;
use anecdoct_cli::WindowsCommand;
use anecdoct_cli::read_access_token_from_stdin;
use anecdoct_cli::read_api_key_from_stdin;
use anecdoct_cli::run_login_status;
use anecdoct_cli::run_login_with_access_token;
use anecdoct_cli::run_login_with_api_key;
use anecdoct_cli::run_login_with_chatgpt;
use anecdoct_cli::run_login_with_device_code;
use anecdoct_cli::run_logout;
use anecdoct_cloud_tasks::Cli as CloudTasksCli;
use anecdoct_exec::Cli as ExecCli;
use anecdoct_exec::Command as ExecCommand;
use anecdoct_exec::ReviewArgs;
use anecdoct_execpolicy::ExecPolicyCheckCommand;
use anecdoct_responses_api_proxy::Args as ResponsesApiProxyArgs;
use anecdoct_rollout_trace::REDUCED_STATE_FILE_NAME;
use anecdoct_rollout_trace::replay_bundle;
use anecdoct_state::StateRuntime;
use anecdoct_state::state_db_path;
use anecdoct_tui::AppExitInfo;
use anecdoct_tui::Cli as TuiCli;
use anecdoct_tui::ExitReason;
use anecdoct_tui::UpdateAction;
use anecdoct_utils_absolute_path::AbsolutePathBuf;
use anecdoct_utils_cli::CliConfigOverrides;
use anecdoct_utils_cli::ProfileV2Name;
...
use anecdoct_core::build_models_manager;
use anecdoct_core::config::ConfigBuilder;
use anecdoct_core::config::ConfigOverrides;
use anecdoct_core::config::edit::ConfigEditsBuilder;
use anecdoct_core::config::find_anecdoct_home;
use anecdoct_core::config::resolve_profile_v2_config_path;
use anecdoct_features::FEATURES;
use anecdoct_features::Stage;
use anecdoct_features::is_known_feature_key;
use anecdoct_login::AnecdoctAuth;
use anecdoct_login::AuthManager;
use anecdoct_login::read_anecdoct_access_token_from_env;
use anecdoct_memories_write::clear_memory_roots_contents;
use anecdoct_models_manager::bundled_models_response;
use anecdoct_models_manager::manager::RefreshStrategy;
use anecdoct_protocol::protocol::AskForApproval;
use anecdoct_protocol::user_input::UserInput;
use anecdoct_terminal_detection::TerminalName;
...
#[clap(
    ...
    // `anecdoct-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `anecdoct` command name that users run.
    bin_name = "anecdoct",
    override_usage = "anecdoct [OPTIONS] [PROMPT]\n       anecdoct [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run Anecdoct non-interactively.
    ...
    /// Manage external MCP servers for Anecdoct.
    Mcp(McpCli),

    /// Manage Anecdoct plugins.
    Plugin(PluginCli),

    /// Start Anecdoct as an MCP server (stdio).
    ...
    /// Launch the Anecdoct desktop app (opens the app installer if missing).
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    ...
    /// Update Anecdoct to the latest version.
    Update,

    /// Diagnose local Anecdoct installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Anecdoct-provided sandbox.
    Sandbox(SandboxArgs),
    ...
    /// Apply the latest diff produced by Anecdoct agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Anecdoct Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Anecdoct.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv LAMBOGENIUS_API_KEY | anecdoct login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv ANECDOCT_ACCESS_TOKEN | anecdoct login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = anecdoct_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: anecdoct_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://developers.lambogenius.com/anecdoct/config-advanced/#metrics for more details.
    #[arg(long = "analytics-default-enabled")]
    analytics_default_enabled: bool,

    #[command(flatten)]
    auth: anecdoct_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    /// Use Agent Identity auth from ANECDOCT_ACCESS_TOKEN for remote registration.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Anecdoct tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Anecdoct via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Anecdoct.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`anecdoct update` is not available in debug builds. Install a release build of Anecdoct to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = anecdoct_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Anecdoct installation method. Please update manually: https://developers.lambogenius.com/anecdoct/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let anecdoct_bin = std::env::current_exe()?;
            anecdoct_app_server_test_client::send_message_v2(
                &anecdoct_bin,
                &[],
                cmd.user_message,
                &None,
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            anecdoct_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["anecdoct", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
        ...
        Some(Subcommand::AppServer(app_server_cli)) => {
            ...
            match subcommand {
                None => {
                    ...
                    let runtime_options = anecdoct_app_server::AppServerRuntimeOptions {
                        remote_control_enabled: remote_control,
                        ..Default::default()
                    };
                    anecdoct_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        ...
                        analytics_default_enabled,
                        transport,
                        anecdoct_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            anecdoct_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        anecdoct_app_server_daemon::run_pid_update_loop().await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            let anecdoct_home = find_anecdoct_home()?;
                            anecdoct_app_server::app_server_control_socket_path(&anecdoct_home)?
                        }
                    };
                    anecdoct_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = anecdoct_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    anecdoct_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    anecdoct_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    anecdoct_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        Some(Subcommand::RemoteControl(remote_control_cli)) => {
                ...
                .unwrap_or(RemoteControlSubcommand::Start)
            {
                RemoteControlSubcommand::Start => {
                    let output =
                        anecdoct_app_server_daemon::ensure_remote_control_started().await?;
                    println!("{}", serde_json::to_string(&output)?);
                }
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv LAMBOGENIUS_API_KEY | anecdoct login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            anecdoct_cloud_tasks::run_main(
                cloud_cli,
                arg0_paths.anecdoct_linux_sandbox_exe.clone(),
            )
            .await?;
        }
        Some(Subcommand::Sandbox(sandbox_args)) => match sandbox_args.cmd {
            SandboxCommand::Macos(mut seatbelt_cli) => {
                ...
                anecdoct_cli::run_command_under_seatbelt(
                    seatbelt_cli,
                    arg0_paths.anecdoct_linux_sandbox_exe.clone(),
                )
                .await?;
            ...
            SandboxCommand::Linux(mut landlock_cli) => {
                ...
                anecdoct_cli::run_command_under_landlock(
                    landlock_cli,
                    arg0_paths.anecdoct_linux_sandbox_exe.clone(),
                )
                .await?;
            ...
            SandboxCommand::Windows(mut windows_cli) => {
                ...
                anecdoct_cli::run_command_under_windows(
                    windows_cli,
                    arg0_paths.anecdoct_linux_sandbox_exe.clone(),
                )
                .await?;
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || anecdoct_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile-v2 only applies to runtime commands: `anecdoct`, `anecdoct exec`, `anecdoct review`, `anecdoct resume`, `anecdoct fork`, and `anecdoct debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let anecdoct_self_exe = arg0_paths
        .anecdoct_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Anecdoct executable path is not configured"))?;
    let runtime_paths = anecdoct_exec_server::ExecServerRuntimePaths::new(
        anecdoct_self_exe,
        arg0_paths.anecdoct_linux_sandbox_exe.clone(),
    )?;
    if let Some(base_url) = cmd.remote {
        ...
        let mut remote_config =
            anecdoct_exec_server::RemoteExecutorConfig::new(base_url, executor_id, auth_provider)?;
        if let Some(name) = cmd.name {
            remote_config.name = name;
        }
        anecdoct_exec_server::run_remote_executor(remote_config, runtime_paths).await?;
        return Ok(());
    }
    let listen_url = cmd
        .listen
        .as_deref()
        .unwrap_or(anecdoct_exec_server::DEFAULT_LISTEN_URL);
    anecdoct_exec_server::run_main(listen_url, runtime_paths)
        .await
...
async fn load_exec_server_remote_auth_provider(
    ...
    config_profile: Option<String>,
    use_agent_identity_auth: bool,
) -> anyhow::Result<anecdoct_api::SharedAuthProvider> {
    let config = load_exec_server_remote_config(root_config_overrides, config_profile).await?;
    if use_agent_identity_auth {
        let agent_identity_jwt = read_anecdoct_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!(
                "ANECDOCT_ACCESS_TOKEN is required when --use-agent-identity-auth is set"
            )
        })?;
        let auth = AnecdoctAuth::from_agent_identity_jwt(
            &agent_identity_jwt,
            Some(&config.chatgpt_base_url),
        )
        .await?;
        return Ok(anecdoct_model_provider::auth_provider_from_auth(&auth));
    }

    let auth = load_exec_server_remote_auth(
        &config,
        "remote exec-server registration requires ChatGPT authentication; run `anecdoct login` first",
    )
    .await?;
    ...
    Ok(anecdoct_model_provider::auth_provider_from_auth(&auth))
}

async fn load_exec_server_remote_config(
    root_config_overrides: &CliConfigOverrides,
    config_profile: Option<String>,
) -> anyhow::Result<anecdoct_core::config::Config> {
...
async fn load_exec_server_remote_auth(
    config: &anecdoct_core::config::Config,
    missing_auth_error: &'static str,
) -> anyhow::Result<anecdoct_login::AnecdoctAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_anecdoct_api_key_env*/ true).await;
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: &str) -> anyhow::Result<()> {
    ...
    let anecdoct_home = find_anecdoct_home()?;
    ConfigEditsBuilder::new(&anecdoct_home)
        .with_profile(interactive.config_profile.as_deref())
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&anecdoct_home, interactive, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let anecdoct_home = find_anecdoct_home()?;
            Ok(LoaderOverrides {
                user_config_path: Some(resolve_profile_v2_config_path(&anecdoct_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(
    ...
    let config_path = anecdoct_home.join(anecdoct_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(anecdoct_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        anecdoct_self_exe: arg0_paths.anecdoct_self_exe,
        anecdoct_linux_sandbox_exe: arg0_paths.anecdoct_linux_sandbox_exe,
    ...
    let prompt_input = anecdoct_core::build_prompt_input(config, input, /*state_db*/ None).await?;
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.anecdoct_home).await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `anecdoct {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `anecdoct {subcommand}`"
        );
    }
    Ok(())
}
...
/// `anecdoct --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `anecdoct {subcommand}`");
    }
    Ok(())
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = anecdoct_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn print_app_server_remote_control_output(
    ...
    let output = anecdoct_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = anecdoct_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Anecdoct's interactive TUI may not work in this terminal."
        );
    ...
    let mut remote_endpoint = remote
        .as_deref()
        .map(anecdoct_tui::resolve_remote_addr)
        .transpose()
    ...
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !anecdoct_tui::remote_addr_supports_auth_token(endpoint) {
            return Ok(AppExitInfo::fatal(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let anecdoct_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Ok(AppExitInfo::fatal(
    ...
    let start_tui = || {
        anecdoct_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            anecdoct_config::LoaderOverrides::default(),
        ...
        match local_state_db::repair_files(startup_error).await {
            ...
            Err(repair_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to repair Anecdoct local data automatically: {repair_err}"
                )));
            }
        }
...
/// Build the final `TuiCli` for a `anecdoct resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `anecdoct` without additional flags.
...
/// Build the final `TuiCli` for a `anecdoct fork` invocation.
...
/// Merge flags provided to `anecdoct resume`/`anecdoct fork` so they take precedence over any
...
mod tests {
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(
            profile_v2_for_args(&["anecdoct", "--profile-v2", "work", "features", "list"]).is_err()
        );
    }
    ...
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["anecdoct", "--profile-v2", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["anecdoct", "--profile-v2", "work", "debug", "prompt-input"])
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["anecdoct", "--profile-v2", "nested/work", "resume"])
                .is_err()
        );
    }
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["anecdoct", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(anecdoct_exec::Command::Resume(args)) = exec.command else {
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let anecdoct_home = find_anecdoct_home().expect("anecdoct home");
        anecdoct_app_server::app_server_control_socket_path(&anecdoct_home)
    ...
    fn debug_models_parses_bundled_flag() {
        let cli = MultitoolCli::try_parse_from(["anecdoct", "debug", "models", "--bundled"])
            .expect("parse");
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["anecdoct", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: anecdoct plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: anecdoct plugin marketplace add"),
            ("list", "Usage: anecdoct plugin marketplace list"),
            ("upgrade", "Usage: anecdoct plugin marketplace upgrade"),
            ("remove", "Usage: anecdoct plugin marketplace remove"),
        ] {
            let help = help_from_args(&["anecdoct", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["anecdoct", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["anecdoct", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["anecdoct", "update"]).expect("parse");
    ...
    fn sandbox_macos_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["anecdoct", "sandbox", "macos", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["anecdoct", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["anecdoct", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["anecdoct", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["anecdoct", "marketplace", "remove", "debug"]);
        assert!(remove_result.is_err());
    }

    #[test]
    fn full_auto_no_longer_parses_at_top_level() {
        let result = MultitoolCli::try_parse_from(["anecdoct", "--full-auto"]);
    ...
    fn exec_full_auto_reports_migration_path() {
        let cli = MultitoolCli::try_parse_from(["anecdoct", "exec", "--full-auto", "summarize"])
    ...
    fn sandbox_full_auto_no_longer_parses() {
        let result =
            MultitoolCli::try_parse_from(["anecdoct", "sandbox", "linux", "--full-auto", "--"]);
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run anecdoct resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["anecdoct", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["anecdoct", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["anecdoct", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["anecdoct", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["anecdoct", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["anecdoct", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(anecdoct_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(anecdoct_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["anecdoct", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["anecdoct", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["anecdoct", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["anecdoct", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["anecdoct", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["anecdoct", "app-server"].as_ref());
        assert!(!app_server.analytics_default_enabled);
        assert!(!app_server.remote_control);
        assert_eq!(
            app_server.listen,
            anecdoct_app_server::AppServerTransport::Stdio
        );
    }
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server = app_server_from_args(
            ["anecdoct", "app-server", "--analytics-default-enabled"].as_ref(),
        );
        assert!(app_server.analytics_default_enabled);
    }
    ...
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["anecdoct", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli = MultitoolCli::try_parse_from(["anecdoct", "mcp-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["anecdoct", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `anecdoct mcp`"
        );

        let cli = MultitoolCli::try_parse_from(["anecdoct", "--strict-config", "remote-control"])
            .expect("parse");
        let err = reject_root_strict_config_for_subcommand(
            cli.interactive.strict_config,
            &cli.subcommand,
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `anecdoct remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["anecdoct", "app-server", "--strict-config", "proxy"].as_ref());
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `anecdoct app-server proxy`"
        );
    }
    ...
    fn reject_remote_flag_for_remote_control() {
        let cli =
            MultitoolCli::try_parse_from(["anecdoct", "--remote", "unix://", "remote-control"])
                .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["anecdoct", "--remote", "unix://anecdoct.sock"])
            .expect("parse");
        assert_eq!(cli.remote.remote.as_deref(), Some("unix://anecdoct.sock"));
    }
    ...
    fn remote_auth_token_env_flag_parses_for_interactive_root() {
        ...
        assert_eq!(
            cli.remote.remote_auth_token_env.as_deref(),
            Some("ANECDOCT_REMOTE_AUTH_TOKEN")
        );
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://anecdoct.sock"));
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("ANECDOCT_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("ANECDOCT_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["anecdoct", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            anecdoct_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["anecdoct", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            anecdoct_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["anecdoct", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
            anecdoct_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        ...
        assert_eq!(
            app_server.listen,
            anecdoct_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/anecdoct.sock")
    ...
    fn app_server_listen_off_parses() {
        let app_server =
            app_server_from_args(["anecdoct", "app-server", "--listen", "off"].as_ref());
        assert_eq!(
            app_server.listen,
            anecdoct_app_server::AppServerTransport::Off
        );
    }
    ...
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["anecdoct", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["anecdoct", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        assert!(matches!(
            app_server_from_args(["anecdoct", "app-server", "daemon", "start"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Start
            }))
        ));
        assert!(matches!(
            app_server_from_args(["anecdoct", "app-server", "daemon", "restart"].as_ref())
                .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["anecdoct", "app-server", "daemon", "enable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["anecdoct", "app-server", "daemon", "disable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["anecdoct", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Stop
            }))
        ));
        assert!(matches!(
            app_server_from_args(["anecdoct", "app-server", "daemon", "version"].as_ref())
                .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server = app_server_from_args(
            ["anecdoct", "app-server", "proxy", "--sock", "anecdoct.sock"].as_ref(),
        );
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("anecdoct.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(anecdoct_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/anecdoct-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(anecdoct_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/anecdoct-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["anecdoct", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["anecdoct", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    fn strict_config_feature_toggle_error(args: &[&str]) -> anyhow::Error {
        let cli_args = std::iter::once("anecdoct")
