...
#[cfg(target_os = "macos")]
use anecdoct_core::spawn::CODEX_SANDBOX_ENV_VAR;
...
#[cfg(target_os = "macos")]
use anecdoct_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use anecdoct_sandboxing::seatbelt::create_seatbelt_command_args;
...
async fn run_command_under_sandbox(
    ...
    let network = network_proxy
        .as_ref()
        .map(anecdoct_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Landlock => {
            ...
            let anecdoct_linux_sandbox_exe = config
                .anecdoct_linux_sandbox_exe
                .expect("anecdoct-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use anecdoct_windows_sandbox::spawn_windows_sandbox_session_elevated;
    use anecdoct_windows_sandbox::spawn_windows_sandbox_session_legacy;
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_anecdoct_home(
        cli_overrides,
        anecdoct_linux_sandbox_exe,
        options,
        /*anecdoct_home*/ None,
...
async fn load_debug_sandbox_config_with_anecdoct_home(
    ...
    // For legacy configs, `anecdoct sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            anecdoct_linux_sandbox_exe: anecdoct_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        anecdoct_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config(
    ...
    if let Some(anecdoct_home) = anecdoct_home {
        builder = builder
            .anecdoct_home(anecdoct_home.clone())
            .fallback_cwd(Some(anecdoct_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    fn write_permissions_profile_config(
        ...
        std::fs::write(anecdoct_home.path().join("config.toml"), config)?;
        Ok(())
    }
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_anecdoct_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_anecdoct_home(
            ...
            Some(anecdoct_home.path().to_path_buf()),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = anecdoct_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
