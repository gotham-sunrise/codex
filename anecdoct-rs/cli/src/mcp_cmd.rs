...
use anecdoct_core::config::load_global_mcp_servers;
use anecdoct_core_plugins::PluginsManager;
use anecdoct_mcp::McpOAuthLoginSupport;
use anecdoct_mcp::ResolvedMcpOAuthScopes;
...
use anecdoct_rmcp_client::delete_oauth_tokens;
use anecdoct_rmcp_client::perform_oauth_login;
use anecdoct_utils_cli::CliConfigOverrides;
use anecdoct_utils_cli::format_env_display;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use anyhow::bail;
...
/// - `add`    — add a server launcher entry to `~/.anecdoct/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "anecdoct mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: anecdoct_config::types::OAuthCredentialsStoreMode,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&anecdoct_home)
        .await
        .with_context(|| {
            format!(
    ...
    ConfigEditsBuilder::new(&anecdoct_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", anecdoct_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `anecdoct mcp login {name}` to login."
        ),
    }
...
async fn run_remove(config_overrides: &CliConfigOverrides, remove_args: RemoveArgs) -> Result<()> {
    ...
    if removed {
        ConfigEditsBuilder::new(&anecdoct_home)
            ...
            .await
            .with_context(|| {
                format!("failed to write MCP servers to {}", anecdoct_home.display())
            })?;
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `anecdoct mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: anecdoct mcp remove {}", get_args.name);
