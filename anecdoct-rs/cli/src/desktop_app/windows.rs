...
const ANECDOCT_WINDOWS_INSTALLER_URL: &str =
    "https://get.microsoft.com/installer/download/9PLM9XGG6VKS?cid=website_cta_psi";
const ANECDOCT_MICROSOFT_STORE_WEB_URL: &str = "https://apps.microsoft.com/detail/9plm9xgg6vks";

pub async fn run_windows_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_id) = find_anecdoct_app_id().await? {
        eprintln!("Opening Anecdoct Desktop...");
        open_installed_anecdoct_app(&app_id).await?;
        eprintln!(
            "In Anecdoct Desktop, open workspace {workspace}.",
            workspace = display_workspace_path(&workspace)
        );
        return Ok(());
    }

    eprintln!("Anecdoct Desktop not found; opening Windows installer...");
    let download_url = download_url_override
        .as_deref()
        .unwrap_or(ANECDOCT_WINDOWS_INSTALLER_URL);
    if open_url(download_url).await.is_err() && download_url_override.is_none() {
        open_url(ANECDOCT_MICROSOFT_STORE_WEB_URL).await?;
    }
    eprintln!(
        "After installing Anecdoct Desktop, open workspace {workspace}.",
        workspace = display_workspace_path(&workspace)
    );
    Ok(())
}

async fn find_anecdoct_app_id() -> anyhow::Result<Option<String>> {
    let output = Command::new("powershell.exe")
        .arg("-NoProfile")
        .arg("-Command")
        .arg("Get-StartApps -Name 'Anecdoct' | Select-Object -First 1 -ExpandProperty AppID")
        .output()
        .await
...
async fn open_installed_anecdoct_app(app_id: &str) -> anyhow::Result<()> {
...
mod tests {
    ...
    fn display_workspace_path_removes_windows_extended_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\C:\Users\fcoury\code\anecdoct")),
            r"C:\Users\fcoury\code\anecdoct"
        );
    }
    ...
    fn display_workspace_path_preserves_unc_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\UNC\server\share\anecdoct")),
            r"\\server\share\anecdoct"
        );
    }
