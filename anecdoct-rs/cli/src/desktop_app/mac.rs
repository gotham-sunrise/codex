...
const ANECDOCT_DMG_URL_ARM64: &str =
    "https://persistent.oaistatic.com/anecdoct-app-prod/Anecdoct.dmg";
const ANECDOCT_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/anecdoct-app-prod/Anecdoct-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_anecdoct_app_path() {
        eprintln!(
            "Opening Anecdoct Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_anecdoct_app(&app_path, &workspace).await?;
        return Ok(());
    }
    eprintln!("Anecdoct Desktop not found; downloading installer...");
    let download_url = download_url_override.unwrap_or_else(|| {
        let default_url = if is_apple_silicon_mac() {
            ANECDOCT_DMG_URL_ARM64
        } else {
            ANECDOCT_DMG_URL_X64
        };
        default_url.to_string()
    });
    let installed_app = download_and_install_anecdoct_to_user_applications(&download_url)
        .await
        .context("failed to download/install Anecdoct Desktop")?;
    eprintln!(
        "Launching Anecdoct Desktop from {installed_app}...",
        installed_app = installed_app.display()
    );
    open_anecdoct_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_anecdoct_app_path() -> Option<PathBuf> {
    candidate_anecdoct_app_paths()
        .into_iter()
        .find(|candidate| candidate.is_dir())
}

fn candidate_anecdoct_app_paths() -> Vec<PathBuf> {
    let mut paths = vec![PathBuf::from("/Applications/Anecdoct.app")];
...
async fn open_anecdoct_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
...
async fn download_and_install_anecdoct_to_user_applications(
    dmg_url: &str,
) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("anecdoct-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Anecdoct.dmg");
    download_dmg(dmg_url, &dmg_path).await?;

    eprintln!("Mounting Anecdoct Desktop installer...");
    ...
    let result = async {
        let app_in_volume = find_anecdoct_app_in_mount(&mount_point)
            .context("failed to locate Anecdoct.app in mounted dmg")?;
        install_anecdoct_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_anecdoct_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        eprintln!(
            "Installing Anecdoct Desktop into {applications_dir}...",
        ...
        let dest_app = applications_dir.join("Anecdoct.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Anecdoct.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Anecdoct.app to any applications directory");
}
...
fn find_anecdoct_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Anecdoct.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tAnecdoct\t/Volumes/Anecdoct\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Anecdoct")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tAnecdoct Installer\t/Volumes/Anecdoct Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Anecdoct Installer")
        );
    }
}
