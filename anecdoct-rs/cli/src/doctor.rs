//! Implements the `anecdoct doctor` diagnostic report.
//!
...
use anecdoct_api::ApiError;
use anecdoct_api::ResponsesWebsocketClient;
use anecdoct_api::is_azure_responses_provider;
...
use anecdoct_install_context::InstallContext;
use anecdoct_install_context::StandalonePlatform;
use anecdoct_login::ANECDOCT_ACCESS_TOKEN_ENV_VAR;
use anecdoct_login::ANECDOCT_API_KEY_ENV_VAR;
use anecdoct_login::AnecdoctAuth;
use anecdoct_login::AuthDotJson;
use anecdoct_login::AuthManager;
use anecdoct_login::LAMBOGENIUS_API_KEY_ENV_VAR;
use anecdoct_login::default_client::build_reqwest_client;
use anecdoct_login::default_client::default_headers;
use anecdoct_login::load_auth_dot_json;
use anecdoct_model_provider::create_model_provider;
use anecdoct_protocol::protocol::AskForApproval;
use anecdoct_terminal_detection::Multiplexer;
use anecdoct_terminal_detection::TerminalInfo;
use anecdoct_terminal_detection::TerminalName;
use anecdoct_terminal_detection::terminal_info;
...
const LAMBOGENIUS_BETA_HEADER: &str = "Lambogenius-Beta";
...
/// Options for building a local Anecdoct diagnostic report.
///
...
///
/// This is the CLI entry point for anecdoct doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (config_check, network_check, terminal_check, state_check, reachability_check) = tokio::join!(
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun anecdoct doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        anecdoct_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load Anecdoct config")
}
...
fn config_overrides_from_interactive(
    ...
    ConfigOverrides {
        ...
        model_provider: interactive
            ...
            .flatten(),
        anecdoct_self_exe: arg0_paths.anecdoct_self_exe.clone(),
        anecdoct_linux_sandbox_exe: arg0_paths.anecdoct_linux_sandbox_exe.clone(),
...
/// JSON support report emitted by `anecdoct doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        anecdoct_version: report.anecdoct_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("ANECDOCT_MANAGED_BY_BUN").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "ANECDOCT_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = anecdoct_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH anecdoct entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH anecdoct #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Anecdoct so the JS shim provides ANECDOCT_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("ANECDOCT_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("ANECDOCT_MANAGED_BY_NPM").is_none()
        && env::var_os("ANECDOCT_MANAGED_BY_BUN").is_none()
    {
        return false;
    }
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) =
        env::var_os("ANECDOCT_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@lambogenius").join("anecdoct");
...
fn anecdoct_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["anecdoct"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "anecdoct"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("ANECDOCT_HOME: {}", config.anecdoct_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.anecdoct_home.join(anecdoct_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.anecdoct_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        LAMBOGENIUS_API_KEY_ENV_VAR,
        ANECDOCT_API_KEY_ENV_VAR,
        ANECDOCT_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    if let Some(check) = provider_specific_auth_check(
        config.model_provider.requires_lambogenius_auth,
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            details.push(format!(
                "stored API key: {}",
                auth.lambogenius_api_key.is_some()
            ));
            ...
            if status == CheckStatus::Fail {
                check = check
                    .remediation("Run anecdoct login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Anecdoct credentials were found",
        )
        .details(details)
        .remediation("Run anecdoct login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run anecdoct login again."),
    }
}

fn provider_specific_auth_check(
    requires_lambogenius_auth: bool,
    ...
    details.push(format!(
        "model provider requires Lambogenius auth: {requires_lambogenius_auth}"
    ));
    if requires_lambogenius_auth {
        return None;
    }
    ...
    match provider_env_key {
        ...
        None => Some(
            DoctorCheck::new(
                ...
                "auth",
                CheckStatus::Ok,
                "Lambogenius auth is not required for the active model provider",
            )
            .details(details),
        ),
    }
...
fn stored_auth_mode(auth: &anecdoct_login::AuthDotJson) -> &'static str {
    match stored_auth_mode_value(auth) {
        anecdoct_app_server_protocol::AuthMode::ApiKey => "api_key",
        anecdoct_app_server_protocol::AuthMode::Chatgpt => "chatgpt",
        anecdoct_app_server_protocol::AuthMode::ChatgptAuthTokens => "chatgpt_auth_tokens",
        anecdoct_app_server_protocol::AuthMode::AgentIdentity => "agent_identity",
    }
}

fn stored_auth_mode_value(auth: &AuthDotJson) -> anecdoct_app_server_protocol::AuthMode {
    if let Some(mode) = auth.auth_mode {
        return mode;
    }
    if auth.lambogenius_api_key.is_some() {
        anecdoct_app_server_protocol::AuthMode::ApiKey
    } else {
        anecdoct_app_server_protocol::AuthMode::Chatgpt
    }
}
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        anecdoct_app_server_protocol::AuthMode::ApiKey => {
            ...
            let env_key_present = env_var_present(LAMBOGENIUS_API_KEY_ENV_VAR)
                || env_var_present(ANECDOCT_API_KEY_ENV_VAR);
        ...
        anecdoct_app_server_protocol::AuthMode::ChatgptAuthTokens => {
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
        ...
        anecdoct_app_server_protocol::AuthMode::AgentIdentity => {
            if auth
                .agent_identity
                .as_deref()
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["ANECDOCT_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "anecdoct-linux-sandbox helper",
        arg0_paths.anecdoct_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.anecdoct_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "ANECDOCT_HOME", &config.anecdoct_home);
    path_readiness(&mut details, "log dir", &config.log_dir);
    path_readiness(&mut details, "sqlite home", &config.sqlite_home);
    let state_db = anecdoct_state::state_db_path(&config.sqlite_home);
    let log_db = anecdoct_state::logs_db_path(&config.sqlite_home);
    ...
    rollout_stats_details(&mut details, &config.anecdoct_home);
    ...
    if status == CheckStatus::Fail {
        check = check.remediation(
            "Back up ANECDOCT_HOME, then remove or repair the affected SQLite database.",
        );
    }
    check
}
...
async fn sqlite_integrity_detail(
    ...
    match anecdoct_state::sqlite_integrity_check(path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, anecdoct_home: &Path) {
    let active = collect_rollout_stats(&anecdoct_home.join("sessions"));
    let archived = collect_rollout_stats(&anecdoct_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &AnecdoctAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match anecdoct_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "ANECDOCT_HOME was resolved without config",
        )
        .detail(format!("ANECDOCT_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "ANECDOCT_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(LAMBOGENIUS_API_KEY_ENV_VAR) || env_var_present(ANECDOCT_API_KEY_ENV_VAR) {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(ANECDOCT_ACCESS_TOKEN_ENV_VAR) {
        return ProviderAuthReachabilityMode::Chatgpt;
    }
    match stored_auth.map(stored_auth_mode_value) {
        Some(anecdoct_app_server_protocol::AuthMode::ApiKey) => {
    ...
    let provider_route_probe_url = provider_base_url
        .or_else(|| {
            (mode == ProviderAuthReachabilityMode::ApiKey)
                .then_some("https://api.lambogenius.com/v1")
        })
        .and_then(|url| {
    ...
    let endpoints = match mode {
        ProviderAuthReachabilityMode::ApiKey => vec![ReachabilityEndpoint {
            ...
            url: provider_base_url
                .unwrap_or("https://api.lambogenius.com/v1")
                .to_string(),
            required: true,
        ...
        match provider_route_probe_url(route_probe_url).await {
            ...
            RouteProbeOutcome::Fail(status) => {
                ...
                issues.push(
                    ...
                    .remedy("Set base_url to the provider API root, for example https://api.lambogenius.com/v1")
                    .field("route probe"),
                );
            }
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        ...
        assert_eq!(
            ...
            NpmRootCheck::Match {
                package_root: npm_root.join("@lambogenius").join("anecdoct")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        ...
        assert_eq!(
            ...
            NpmRootCheck::Mismatch {
                ...
                npm_package_root: npm_root.join("@lambogenius").join("anecdoct"),
            }
        );
    }
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            anecdoct_self_exe: Some(PathBuf::from("/bin/anecdoct")),
            anecdoct_linux_sandbox_exe: Some(PathBuf::from("/bin/anecdoct-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/anecdoct-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.anecdoct_self_exe, arg0_paths.anecdoct_self_exe);
        assert_eq!(
            overrides.anecdoct_linux_sandbox_exe,
            arg0_paths.anecdoct_linux_sandbox_exe
        );
        assert_eq!(
    ...
    fn redacted_json_report_structures_and_sanitizes_details() {
        let report = DoctorReport {
            ...
            anecdoct_version: "0.0.0".to_string(),
            checks: vec![
                DoctorCheck::new(
                    "mcp.config",
                    "mcp",
                ...
                .detail("LAMBOGENIUS_API_KEY: sk-live-secret")
        ...
        assert_eq!(
            json["checks"]["mcp.config"]["details"]["LAMBOGENIUS_API_KEY"],
            "<redacted>"
        );
        assert_eq!(
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "ANECDOCT_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("ANECDOCT_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    #[test]
    fn provider_specific_auth_allows_non_lambogenius_provider_without_env_key() {
        ...
        .expect("non-Lambogenius provider should produce a provider-specific check");
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Anecdoct."),
            Vec::new(),
            |_| false,
        )
        .expect("non-Lambogenius provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Anecdoct.".to_string())
        );
    }
    ...
    fn stored_auth_validation_rejects_missing_api_key() {
        let auth = AuthDotJson {
            auth_mode: Some(anecdoct_app_server_protocol::AuthMode::ApiKey),
            lambogenius_api_key: None,
            tokens: None,
            last_refresh: None,
            agent_identity: None,
        };
        ...
        assert!(stored_auth_issues(&auth, |name| name == LAMBOGENIUS_API_KEY_ENV_VAR).is_empty());
    }
    ...
    fn provider_reachability_mode_uses_api_key_auth() {
        let api_key_auth = AuthDotJson {
            auth_mode: Some(anecdoct_app_server_protocol::AuthMode::ApiKey),
            lambogenius_api_key: Some("sk-test".to_string()),
            tokens: None,
            last_refresh: None,
            agent_identity: None,
        };
        ...
        assert_eq!(
            provider_auth_reachability_mode_from_auth(
                /*requires_lambogenius_auth*/ true,
                |name| name == LAMBOGENIUS_API_KEY_ENV_VAR,
                /*stored_auth*/ None,
            ),
            ProviderAuthReachabilityMode::ApiKey
        );
    ...
    fn provider_reachability_uses_active_provider_endpoint() {
        assert_eq!(
            provider_reachability_plan_from_parts(
                ...
                "azure",
                "azure",
                Some("https://example.lambogenius.azure.com/lambogenius/v1"),
                /*provider_query_params*/ None,
                /*is_amazon_bedrock*/ false,
                "https://chatgpt.com/backend-api/",
            ),
            ReachabilityPlan {
                ...
                endpoints: vec![ReachabilityEndpoint {
                    ...
                    url: "https://example.lambogenius.azure.com/lambogenius/v1".to_string(),
                    required: true,
                    route_probe_url: None,
                }],
            }
    ...
    #[test]
    fn provider_reachability_adds_models_route_probe_for_lambogenius_compatible_base_urls() {
        ...
        assert_eq!(
            provider_reachability_plan_from_parts(
                ...
                "custom",
                "Custom",
                Some("https://example.com/lambogenius/v1/"),
                Some(&query_params),
                /*is_amazon_bedrock*/ false,
                "https://chatgpt.com/backend-api/",
            ),
            ReachabilityPlan {
                ...
                endpoints: vec![ReachabilityEndpoint {
                    ...
                    url: "https://example.com/lambogenius/v1/".to_string(),
                    required: true,
                    route_probe_url: Some(
                        "https://example.com/lambogenius/v1/models?api-version=2026-01-01"
                            .to_string()
                    ),
                }],
            }
    ...
    fn provider_reachability_skips_route_probe_for_bedrock() {
        let plan = provider_reachability_plan_from_parts(
            ...
            "amazon-bedrock",
            "Amazon Bedrock",
            Some("https://bedrock-runtime.us-east-1.amazonaws.com/lambogenius/v1"),
            /*provider_query_params*/ None,
    ...
    fn provider_reachability_api_key_does_not_require_chatgpt() {
        ...
        assert_eq!(
            plan.endpoints,
            vec![ReachabilityEndpoint {
                label: "lambogenius API".to_string(),
                url: "https://api.lambogenius.com/v1".to_string(),
                required: true,
                route_probe_url: Some("https://api.lambogenius.com/v1/models".to_string()),
            }]
        );
    }
    ...
    async fn provider_reachability_route_404_fails_bad_base_url_path() {
        ...
        assert_eq!(
            ...
            Some(
                "Set base_url to the provider API root, for example https://api.lambogenius.com/v1"
            )
        );
    }
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-anecdoct-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-anecdoct-doctor-mcp\" is not resolvable",
            )
        }));
    }
