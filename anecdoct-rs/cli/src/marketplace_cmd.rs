...
#[derive(Debug, Parser)]
#[command(bin_name = "anecdoct plugin marketplace")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "anecdoct plugin marketplace add",
    after_help = "Examples:\n  anecdoct plugin marketplace add ./path/to/marketplace\n  anecdoct plugin marketplace add owner/repo --ref main\n  anecdoct plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "anecdoct plugin marketplace upgrade",
    after_help = "Examples:\n  anecdoct plugin marketplace upgrade\n  anecdoct plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "anecdoct plugin marketplace remove",
    after_help = "Example:\n  anecdoct plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>) -> Result<()> {
    ...
    let default_install_root = marketplace_install_root(config.anecdoct_home.as_path());
...
async fn run_upgrade(
    ...
    let config = Config::load_with_cli_overrides(overrides)
        ...
        .context("failed to load configuration")?;
    let anecdoct_home = find_anecdoct_home().context("failed to resolve ANECDOCT_HOME")?;
    let manager = PluginsManager::new(anecdoct_home.to_path_buf());
