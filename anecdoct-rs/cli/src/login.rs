...
//! interactive-session layers. Direct `anecdoct login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `anecdoct-login.log` artifact that
...
use anecdoct_login::ServerOptions;
use anecdoct_login::login_with_access_token;
use anecdoct_login::login_with_api_key;
use anecdoct_login::logout_with_revoke;
use anecdoct_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `anecdoct login` flows.
///
...
/// command produce a durable `anecdoct-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match anecdoct_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("anecdoct-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("anecdoct_cli=info,anecdoct_core=info,anecdoct_login=info")
    });
    ...
    // Direct `anecdoct login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `anecdoct login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv LAMBOGENIUS_API_KEY | anecdoct login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv ANECDOCT_ACCESS_TOKEN | anecdoct login --with-access-token`.",
...
/// `anecdoct login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match AnecdoctAuth::from_auth_storage(
        &config.anecdoct_home,
        config.cli_auth_credentials_store_mode,
        Some(&config.chatgpt_base_url),
    )
