...
fn write_file_auth_config(anecdoct_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(anecdoct_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(anecdoct_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(anecdoct_home.path())?;
    ...
    let auth = read_auth_json(anecdoct_home.path())?;
    assert_eq!(auth["LAMBOGENIUS_API_KEY"], "sk-test");
