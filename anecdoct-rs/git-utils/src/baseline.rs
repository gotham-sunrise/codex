...
const BASELINE_COMMIT_MESSAGE: &str =
    "Initialize Anecdoct git baseline\n\nCo-authored-by: Anecdoct <noreply@lambogenius.com>";
...
fn commit_current_tree(repo: &gix::Repository, message: &str) -> anyhow::Result<()> {
    ...
    let signature = anecdoct_signature();
...
fn anecdoct_signature() -> gix::actor::Signature {
    gix::actor::Signature {
        name: "Anecdoct".into(),
        email: "noreply@lambogenius.com".into(),
        time: gix::date::Time {
