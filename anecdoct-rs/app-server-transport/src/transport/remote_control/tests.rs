...
use anecdoct_app_server_protocol::RemoteControlConnectionStatus;
use anecdoct_app_server_protocol::RemoteControlStatusChangedNotification;
...
use anecdoct_core::test_support::auth_manager_from_auth_with_home;
use anecdoct_login::AnecdoctAuth;
use anecdoct_login::AuthDotJson;
use anecdoct_login::AuthManager;
use anecdoct_login::save_auth;
use anecdoct_login::token_data::TokenData;
use anecdoct_login::token_data::parse_chatgpt_jwt_claims;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(AnecdoctAuth::create_dummy_chatgpt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(anecdoct_home: &TempDir) -> Arc<AuthManager> {
...
async fn remote_control_state_runtime(anecdoct_home: &TempDir) -> Arc<StateRuntime> {
    StateRuntime::init(
        anecdoct_home.path().to_path_buf(),
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
    ...
    let anecdoct_home = TempDir::new().expect("temp dir should create");
    ...
    let (remote_task, remote_handle) = start_remote_control(
        ...
        Some(remote_control_state_runtime(&anecdoct_home).await),
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    anecdoct_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message =
        JSONRPCMessage::Request(anecdoct_app_server_protocol::JSONRPCRequest {
            id: anecdoct_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(anecdoct_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(anecdoct_app_server_protocol::JSONRPCRequest {
                    id: anecdoct_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-anecdoct-server-id"),
        Some(&"srv_e_test".to_string())
    );
    assert_eq!(
        handshake_request.headers.get("x-anecdoct-name"),
        Some(&base64::engine::general_purpose::STANDARD.encode(&expected_server_name))
    );
    assert_eq!(
        handshake_request.headers.get("x-anecdoct-protocol-version"),
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(anecdoct_app_server_protocol::JSONRPCRequest {
                id: anecdoct_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
...
async fn remote_control_http_mode_reuses_persisted_enrollment_before_reenrolling() {
    ...
    let remote_control_url = remote_control_url_for_listener(&listener);
    let anecdoct_home = TempDir::new().expect("temp dir should create");
    let state_db = remote_control_state_runtime(&anecdoct_home).await;
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        ...
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&anecdoct_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        /*initial_enabled*/ true,
...
async fn remote_control_http_mode_clears_stale_persisted_enrollment_after_404() {
    ...
    assert_eq!(
        websocket_request.headers.get("x-anecdoct-server-id"),
