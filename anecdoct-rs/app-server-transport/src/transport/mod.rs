...
pub fn app_server_control_socket_path(anecdoct_home: &Path) -> std::io::Result<AbsolutePathBuf> {
            ...
            let socket_path = if raw_socket_path.is_empty() {
                let anecdoct_home = find_anecdoct_home().map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        ...
                        message: format!("failed to resolve ANECDOCT_HOME: {err}"),
                    }
                })?;
                app_server_control_socket_path(&anecdoct_home).map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        listen_url: listen_url.to_string(),
