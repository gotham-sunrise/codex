...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "anecdoct-cli"), ("env", "prod")])?;
    ...
    let counter = find_metric(&resource_metrics, "anecdoct.turns").expect("counter metric missing");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "anecdoct-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "anecdoct.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "anecdoct.tool_latency").and_then(
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "anecdoct.alpha").expect("anecdoct.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "anecdoct.beta").expect("anecdoct.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("anecdoct.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("anecdoct.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
