...
`anecdoct-otel` is the OpenTelemetry integration crate for Anecdoct. It provides:

- Provider wiring for log/trace/metric exporters (`anecdoct_otel::OtelProvider`
  and `anecdoct_otel::provider`).
- Session-scoped business event emission via `anecdoct_otel::SessionTelemetry`.
- Low-level metrics APIs via `anecdoct_otel::metrics`.
- Trace-context helpers via `anecdoct_otel::trace_context` and crate-root re-exports.
...
```rust
use anecdoct_otel::config::OtelExporter;
use anecdoct_otel::config::OtelHttpProtocol;
use anecdoct_otel::config::OtelSettings;
use anecdoct_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "anecdoct-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    anecdoct_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Anecdoct
...
Anecdoct-specific session events. Rich session/business events should go through
...
`anecdoct-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Anecdoct-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use anecdoct_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("anecdoct.session_started", 1, &[("source", "tui")])?;
metrics.histogram("anecdoct.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("anecdoct.turns", 1, &[("model", "gpt-5.1")])?;
