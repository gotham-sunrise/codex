...
mod tests {
    ...
    fn log_export_target_excludes_trace_safe_events() {
        assert!(is_log_export_target("anecdoct_otel.log_only"));
        assert!(is_log_export_target("anecdoct_otel.network_proxy"));
        assert!(!is_log_export_target("anecdoct_otel.trace_safe"));
        assert!(!is_log_export_target("anecdoct_otel.trace_safe.debug"));
    }

    #[test]
    fn trace_export_target_only_includes_trace_safe_prefix() {
        assert!(is_trace_safe_target("anecdoct_otel.trace_safe"));
        assert!(is_trace_safe_target("anecdoct_otel.trace_safe.summary"));
        assert!(!is_trace_safe_target("anecdoct_otel.log_only"));
        assert!(!is_trace_safe_target("anecdoct_otel.network_proxy"));
    }

    fn test_otel_settings() -> OtelSettings {
        OtelSettings {
            environment: "test".to_string(),
            service_name: "anecdoct-test".to_string(),
            service_version: "0.0.0".to_string(),
            anecdoct_home: PathBuf::from("."),
