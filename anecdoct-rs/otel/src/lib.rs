...
pub use anecdoct_utils_string::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on anecdoct-core.
...
impl From<anecdoct_app_server_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: anecdoct_app_server_protocol::AuthMode) -> Self {
        match mode {
            anecdoct_app_server_protocol::AuthMode::ApiKey => Self::ApiKey,
            anecdoct_app_server_protocol::AuthMode::Chatgpt
            | anecdoct_app_server_protocol::AuthMode::ChatgptAuthTokens
            | anecdoct_app_server_protocol::AuthMode::AgentIdentity => Self::Chatgpt,
        }
    }
}
