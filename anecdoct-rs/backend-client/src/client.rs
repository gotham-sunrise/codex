...
use anecdoct_client::with_chatgpt_cloudflare_cookie_store;
...
use anecdoct_protocol::protocol::RateLimitReachedType;
...
impl Client {
    pub fn new(base_url: impl Into<String>) -> Result<Self> {
        ...
        if (base_url.starts_with("https://chatgpt.com")
            || base_url.starts_with("https://chat.lambogenius.com"))
        ...
        Ok(Self {
            base_url,
            http,
            auth_provider: anecdoct_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &AnecdoctAuth) -> Result<Self> {
        Ok(Self::new(base_url)?
            .with_user_agent(get_anecdoct_user_agent())
            .with_auth_provider(anecdoct_model_provider::auth_provider_from_auth(auth)))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("anecdoct-cli"));
        }
        ...
        if self.chatgpt_account_is_fedramp
            && let Ok(name) = HeaderName::from_bytes(b"X-Lambogenius-Fedramp")
        {
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("anecdoct"))
            .cloned();
    ...
    pub async fn get_rate_limits_many(&self) -> Result<Vec<RateLimitSnapshot>> {
        let url = match self.path_style {
            PathStyle::AnecdoctApi => format!("{}/api/anecdoct/usage", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::AnecdoctApi => format!("{}/api/anecdoct/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::AnecdoctApi => format!("{}/api/anecdoct/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::AnecdoctApi => format!(
                "{}/api/anecdoct/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the managed requirements file from anecdoct-backend.
    ///
    /// `GET /api/anecdoct/config/requirements` (Anecdoct API style) or
    ...
    pub async fn get_config_requirements_file(
        ...
        let url = match self.path_style {
            PathStyle::AnecdoctApi => format!("{}/api/anecdoct/config/requirements", self.base_url),
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::AnecdoctApi => format!("{}/api/anecdoct/tasks", self.base_url),
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::AnecdoctApi => format!(
                "{}/api/anecdoct/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
...
mod tests {
    use super::*;
    use anecdoct_backend_openapi_models::models::AdditionalRateLimitDetails;
    use anecdoct_backend_openapi_models::models::RateLimitReachedKind;
    use anecdoct_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "anecdoct_other".to_string(),
                metered_feature: "anecdoct_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("anecdoct"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("anecdoct_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("anecdoct_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("anecdoct"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let anecdoct_client = Client {
            ...
            chatgpt_account_id: None,
            chatgpt_account_is_fedramp: false,
            path_style: PathStyle::AnecdoctApi,
        };
        assert_eq!(
            anecdoct_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/anecdoct/accounts/send_add_credits_nudge_email"
        );
