WARNING: this code is mainly generated by Anecdoct and should not be used in production

# anecdoct-debug-client

A tiny interactive client for `anecdoct app-server` (protocol v2 only). It prints
all JSON-RPC lines from the server and lets you send new turns as you type.

## Usage

Start the app-server client (it will spawn `anecdoct app-server` itself):

```
cargo run -p anecdoct-debug-client -- \
  --anecdoct-bin anecdoct \
...
```
cargo run -p anecdoct-debug-client -- --thread-id thr_123
```

### CLI flags

- `--anecdoct-bin <path>`: path to the `anecdoct` binary (default: `anecdoct`).
- `-c, --config key=value`: pass through `--config` overrides to `anecdoct`.
