...
use crate::url::is_lambogenius_compatible_base_url;
...
impl OllamaClient {
    ...
    pub(crate) async fn try_from_provider(provider: &ModelProviderInfo) -> io::Result<Self> {
        ...
        let uses_lambogenius_compat = is_lambogenius_compatible_base_url(base_url);
    ...
    async fn probe_server(&self) -> io::Result<()> {
        let url = if self.uses_lambogenius_compat {
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(anecdoct_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
    ...
    #[tokio::test]
    async fn test_probe_server_happy_path_lambogenius_compat_and_native() {
        if std::env::var(anecdoct_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
                "{} set; skipping test_probe_server_happy_path_lambogenius_compat_and_native",
                anecdoct_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
            );
            return;
        }
        ...
        // Lambogenius compatibility endpoint
        wiremock::Mock::given(wiremock::matchers::method("GET"))
        ...
        let ollama_client =
            OllamaClient::try_from_provider_with_base_url(&format!("{}/v1", server.uri()))
                .await
                .expect("probe Lambogenius compat");
        ollama_client
            .probe_server()
            .await
            .expect("probe Lambogenius compat");
    ...
    async fn test_try_from_oss_provider_ok_when_server_running() {
        ...
        // Lambogenius‑compat models endpoint responds OK.
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/v1/models"))
            .respond_with(wiremock::ResponseTemplate::new(200))
            .mount(&server)
