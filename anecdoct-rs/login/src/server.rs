...
use anecdoct_client::build_reqwest_client_with_custom_ca;
...
const DEFAULT_ISSUER: &str = "https://auth.lambogenius.com";
const DEFAULT_PORT: u16 = 1455;
// Keep in sync with the Anecdoct CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub anecdoct_streamlined_login: bool,
    ...
    match path.as_str() {
                ...
                .await
            {
                Ok(tokens) => {
                    ...
                    let success_url = compose_success_url(
                        ...
                        &tokens.id_token,
                        &tokens.access_token,
                        opts.anecdoct_streamlined_login,
                    );
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Anecdoct failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "anecdoct_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        (
            "anecdoct_cli_simplified_flow".to_string(),
            "true".to_string(),
        ),
...
pub(crate) async fn persist_tokens_async(
    ...
    let anecdoct_home = anecdoct_home.to_path_buf();
    let (previous_auth, auth) = tokio::task::spawn_blocking(move || {
        let previous_auth = match load_auth_dot_json(&anecdoct_home, auth_credentials_store_mode) {
            Ok(auth) => auth,
            Err(err) => {
        ...
        let auth = AuthDotJson {
            ...
            lambogenius_api_key: api_key,
            tokens: Some(tokens),
            last_refresh: Some(Utc::now()),
            agent_identity: None,
        };
        save_auth(&anecdoct_home, &auth, auth_credentials_store_mode)?;
...
fn compose_success_url(
    ...
    id_token: &str,
    access_token: &str,
    anecdoct_streamlined_login: bool,
) -> String {
    ...
    let platform_url = if issuer == DEFAULT_ISSUER {
        "https://platform.lambogenius.com"
    } else {
        "https://platform.api.lambogenius.org"
    };
    ...
    if anecdoct_streamlined_login {
        params.push(("anecdoct_streamlined_login", "true".to_string()));
    }
    let qs = params
        .drain(..)
        .map(|(k, v)| format!("{}={}", k, urlencoding::encode(&v)))
...
fn jwt_auth_claims(jwt: &str) -> serde_json::Map<String, serde_json::Value> {
    ...
    match base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(payload_b64) {
        Ok(bytes) => match serde_json::from_slice::<serde_json::Value>(&bytes) {
            Ok(mut v) => {
                if let Some(obj) = v
                    .get_mut("https://api.lambogenius.com/auth")
                    .and_then(|x| x.as_object_mut())
                {
                    return obj.clone();
                }
                eprintln!("JWT payload missing expected 'https://api.lambogenius.com/auth' object");
            }
            Err(e) => {
...
/// Returns true when the OAuth callback represents a missing Anecdoct entitlement.
fn is_missing_anecdoct_entitlement_error(
    error_code: &str,
    error_description: Option<&str>,
) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_anecdoct_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_anecdoct_entitlement_error(error_code, error_description) {
        return "Anecdoct is not enabled for your workspace. Contact your workspace administrator to request access to Anecdoct.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_anecdoct_entitlement_error(code, error_description) {
            (
                "You do not have access to Anecdoct".to_string(),
                "This account is not currently authorized to use Anecdoct in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Anecdoct.".to_string(),
                "Contact your workspace administrator to get access to Anecdoct, then return to Anecdoct and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Anecdoct to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
pub(crate) async fn obtain_api_key(
    ...
    let resp = client
        ...
        .body(format!(
            ...
            urlencoding::encode(client_id),
            urlencoding::encode("lambogenius-api-key"),
...
mod tests {
    ...
    use super::is_missing_anecdoct_entitlement_error;
    ...
    async fn persist_tokens_async_revokes_previous_auth_without_failing_login() -> anyhow::Result<()>
    {
        ...
        let auth = load_auth_dot_json(anecdoct_home.path(), AuthCredentialsStoreMode::File)?
    ...
    fn redact_sensitive_url_parts_preserves_safe_url_shape() {
        let mut url = url::Url::parse(
            "https://user:pass@auth.lambogenius.com/oauth/token?code=abc123&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback#frag",
        )
        .expect("valid url");
        ...
        assert_eq!(
            url.as_str(),
            "https://auth.lambogenius.com/oauth/token?code=%3Credacted%3E&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback"
        );
    }
    ...
    fn compose_success_url_omits_streamlined_success_by_default() {
        ...
        assert_eq!(
            url.query_pairs()
                .find(|(key, _)| key == "anecdoct_streamlined_login"),
            None
        );
    }
    ...
    fn compose_success_url_includes_streamlined_success_when_requested() {
        let url = url::Url::parse(&compose_success_url(
            ...
            "e30.eyJodHRwczovL2FwaS5vcGVuYWkuY29tL2F1dGgiOnt9fQ.sig",
            "e30.eyJodHRwczovL2FwaS5vcGVuYWkuY29tL2F1dGgiOnt9fQ.sig",
            /*anecdoct_streamlined_login*/ true,
        ))
        .expect("success url should parse");
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_anecdoct_entitlement");
        assert!(is_missing_anecdoct_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to Anecdoct"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_anecdoct_entitlement"));
    }
}
