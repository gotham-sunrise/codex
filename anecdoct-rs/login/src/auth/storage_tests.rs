...
use anecdoct_keyring_store::tests::MockKeyringStore;
use keyring::Error as KeyringError;

#[tokio::test]
async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
    let anecdoct_home = tempdir()?;
    let storage = FileAuthStorage::new(anecdoct_home.path().to_path_buf());
    let auth_dot_json = AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
        lambogenius_api_key: Some("test-key".to_string()),
        tokens: None,
...
async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
    ...
    let file = get_auth_file(anecdoct_home.path());
    storage
        .save(&auth_dot_json)
        .context("failed to save auth file")?;
...
async fn file_storage_loads_agent_identity_as_jwt() -> anyhow::Result<()> {
    ...
    let auth_file = get_auth_file(anecdoct_home.path());
    std::fs::write(
        &auth_file,
...
fn ephemeral_storage_save_load_delete_is_in_memory_only() -> anyhow::Result<()> {
    ...
    let auth_dot_json = AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
        lambogenius_api_key: Some("sk-ephemeral".to_string()),
        tokens: None,
        last_refresh: Some(Utc::now()),
        agent_identity: None,
    };
...
fn auth_with_prefix(prefix: &str) -> AuthDotJson {
    AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
        lambogenius_api_key: Some(format!("{prefix}-api-key")),
...
fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
    ...
    seed_keyring_with_auth(
        &mock_keyring,
        || compute_store_key(anecdoct_home.path()),
        &expected,
    )?;
...
fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
    let anecdoct_home = PathBuf::from("~/.anecdoct");

    let key = compute_store_key(anecdoct_home.as_path())?;
...
fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
    ...
    let key = compute_store_key(anecdoct_home.path())?;
...
fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
    let anecdoct_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = AutoAuthStorage::new(anecdoct_home.path().to_path_buf(), Arc::new(mock_keyring));
