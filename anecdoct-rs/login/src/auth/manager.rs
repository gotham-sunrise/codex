...
use anecdoct_agent_identity::decode_agent_identity_jwt;
use anecdoct_agent_identity::fetch_agent_identity_jwks;
use anecdoct_app_server_protocol::AuthMode;
use anecdoct_app_server_protocol::AuthMode as ApiAuthMode;
...
use anecdoct_client::AnecdoctHttpClient;
use anecdoct_config::types::AuthCredentialsStoreMode;
use anecdoct_protocol::account::PlanType as AccountPlanType;
use anecdoct_protocol::auth::PlanType as InternalPlanType;
use anecdoct_protocol::auth::RefreshTokenFailedError;
use anecdoct_protocol::auth::RefreshTokenFailedReason;
use serde_json::Value;
use thiserror::Error;

/// Authentication mechanism used by the current user.
#[derive(Debug, Clone)]
pub enum AnecdoctAuth {
    ApiKey(ApiKeyAuth),
    Chatgpt(ChatgptAuth),
...
impl PartialEq for AnecdoctAuth {
...
const REFRESH_TOKEN_URL: &str = "https://auth.lambogenius.com/oauth/token";
pub(super) const REVOKE_TOKEN_URL: &str = "https://auth.lambogenius.com/oauth/revoke";
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "ANECDOCT_REFRESH_TOKEN_URL_OVERRIDE";
pub const REVOKE_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "ANECDOCT_REVOKE_TOKEN_URL_OVERRIDE";
...
impl AnecdoctAuth {
    async fn from_auth_dot_json(
        ...
        if auth_mode == ApiAuthMode::ApiKey {
            let Some(api_key) = auth_dot_json.lambogenius_api_key.as_deref() else {
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(anecdoct_home.to_path_buf(), storage_mode);
    ...
    pub fn uses_anecdoct_backend(&self) -> bool {
        matches!(
            self,
    ...
    /// Returns `None` if Anecdoct backend auth does not expose an account id.
    ...
    /// Returns false if Anecdoct backend auth omits the FedRAMP claim.
    ...
    /// Returns `None` if Anecdoct backend auth does not expose an account email.
    ...
    /// Returns `None` if Anecdoct backend auth does not expose a ChatGPT user id.
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &AnecdoctHttpClient {
        &self.state.client
    }
}

pub const LAMBOGENIUS_API_KEY_ENV_VAR: &str = "LAMBOGENIUS_API_KEY";
pub const ANECDOCT_API_KEY_ENV_VAR: &str = "ANECDOCT_API_KEY";
pub const ANECDOCT_ACCESS_TOKEN_ENV_VAR: &str = "ANECDOCT_ACCESS_TOKEN";

pub fn read_lambogenius_api_key_from_env() -> Option<String> {
    env::var(LAMBOGENIUS_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

pub fn read_anecdoct_api_key_from_env() -> Option<String> {
    read_non_empty_env_var(ANECDOCT_API_KEY_ENV_VAR)
}

pub fn read_anecdoct_access_token_from_env() -> Option<String> {
    read_non_empty_env_var(ANECDOCT_ACCESS_TOKEN_ENV_VAR)
}
...
/// Delete the auth.json file inside `anecdoct_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    anecdoct_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(anecdoct_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    let auth_dot_json = AuthDotJson {
        ...
        lambogenius_api_key: Some(api_key.to_string()),
        tokens: None,
        last_refresh: None,
        agent_identity: None,
    };
    save_auth(anecdoct_home, &auth_dot_json, auth_credentials_store_mode)
}
...
pub async fn enforce_login_restrictions(config: &AuthConfig) -> std::io::Result<()> {
    ...
    if let Some(expected_account_ids) = config.forced_chatgpt_workspace_id.as_deref() {
        let chatgpt_account_id = match &auth {
            AnecdoctAuth::ApiKey(_) => return Ok(()),
            AnecdoctAuth::AgentIdentity(_) => auth.get_account_id(),
            AnecdoctAuth::Chatgpt(_) | AnecdoctAuth::ChatgptAuthTokens(_) => {
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(anecdoct_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(anecdoct_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(anecdoct_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(anecdoct_home, auth_credentials_store_mode)?;
    Ok(removed_ephemeral || removed_managed)
}

async fn load_auth(
    anecdoct_home: &Path,
    enable_anecdoct_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    chatgpt_base_url: Option<&str>,
) -> std::io::Result<Option<AnecdoctAuth>> {
    // API key via env var takes precedence over any other auth method.
    if enable_anecdoct_api_key_env && let Some(api_key) = read_anecdoct_api_key_from_env() {
        return Ok(Some(AnecdoctAuth::from_api_key(api_key.as_str())));
    }
    ...
    if let Some(auth_dot_json) = ephemeral_storage.load()? {
        let auth = AnecdoctAuth::from_auth_dot_json(
            anecdoct_home,
            auth_dot_json,
            AuthCredentialsStoreMode::Ephemeral,
            chatgpt_base_url,
    ...
    if let Some(agent_identity) = read_anecdoct_access_token_from_env() {
        return AnecdoctAuth::from_agent_identity_jwt(&agent_identity, chatgpt_base_url)
            .await
            .map(Some);
    }
...
impl AuthDotJson {
    ...
    fn resolved_mode(&self) -> ApiAuthMode {
        ...
        if self.lambogenius_api_key.is_some() {
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(AnecdoctAuth::api_auth_mode),
            )
            .field(
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(AnecdoctAuth::get_account_id);
        let mode = if manager.has_external_api_key_auth()
            || cached_auth
                .as_ref()
                .is_some_and(AnecdoctAuth::is_external_chatgpt_tokens)
        {
    ...
    pub fn has_next(&self) -> bool {
        ...
        if !self
            ...
            .auth_cached()
            .as_ref()
            .is_some_and(AnecdoctAuth::is_chatgpt_auth)
        {
            return false;
        }
...
/// hands out cloned `AnecdoctAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
/// `anecdoct_core::config::Config`, but this trait keeps `anecdoct-login` independent
/// from `anecdoct-core`.
pub trait AuthManagerConfig {
    /// Returns the Anecdoct home directory used for auth storage.
...
impl Debug for AuthManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AuthManager")
            .field("anecdoct_home", &self.anecdoct_home)
            .field("inner", &self.inner)
            .field(
                "enable_anecdoct_api_key_env",
                &self.enable_anecdoct_api_key_env,
            )
            .field(
                "auth_credentials_store_mode",
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific AnecdoctAuth, for testing only.
    pub fn from_auth_for_testing(auth: AnecdoctAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            anecdoct_home: PathBuf::from("non-existent"),
    ...
    /// Create an AuthManager with a specific AnecdoctAuth and anecdoct home, for testing only.
    ...
    pub fn auth_cached(&self) -> Option<AnecdoctAuth> {
        self.inner.read().ok().and_then(|c| c.auth.clone())
    }

    pub fn refresh_failure_for_auth(&self, auth: &AnecdoctAuth) -> Option<RefreshTokenFailedError> {
    ...
    pub async fn auth(&self) -> Option<AnecdoctAuth> {
    ...
    async fn reload_if_account_id_matches(
        ...
        let new_auth = self.load_auth_from_storage().await;
        let new_account_id = new_auth.as_ref().and_then(AnecdoctAuth::get_account_id);
    ...
    fn auths_equal_for_refresh(a: Option<&AnecdoctAuth>, b: Option<&AnecdoctAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => match (a.api_auth_mode(), b.api_auth_mode()) {
                ...
                (ApiAuthMode::AgentIdentity, ApiAuthMode::AgentIdentity) => match (a, b) {
                    (AnecdoctAuth::AgentIdentity(a), AnecdoctAuth::AgentIdentity(b)) => {
                        a.record() == b.record()
                    }
                    _ => false,
                },
    ...
    fn auths_equal(a: Option<&AnecdoctAuth>, b: Option<&AnecdoctAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    async fn load_auth_from_storage(&self) -> Option<AnecdoctAuth> {
        load_auth(
            &self.anecdoct_home,
            self.enable_anecdoct_api_key_env,
            self.auth_credentials_store_mode,
    ...
    fn set_cached_auth(&self, new_auth: Option<AnecdoctAuth>) -> bool {
        if let Ok(mut guard) = self.inner.write() {
    ...
    pub fn anecdoct_api_key_env_enabled(&self) -> bool {
        self.enable_anecdoct_api_key_env
    }
    ...
    async fn resolve_external_api_key_auth(&self) -> Option<AnecdoctAuth> {
        ...
        match external_auth.resolve().await {
            Ok(Some(tokens)) => Some(AnecdoctAuth::from_api_key(&tokens.access_token)),
            Ok(None) => None,
            Err(err) => {
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        if auth_before_reload
            .as_ref()
            .is_some_and(AnecdoctAuth::is_api_key_auth)
        {
            return Ok(());
        }
        let expected_account_id = auth_before_reload
            .as_ref()
            .and_then(AnecdoctAuth::get_account_id);
    ...
    async fn refresh_token_from_authority_impl(&self) -> Result<(), RefreshTokenError> {
        ...
        let result = match auth {
            AnecdoctAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            AnecdoctAuth::Chatgpt(chatgpt_auth) => {
            ...
            AnecdoctAuth::ApiKey(_) | AnecdoctAuth::AgentIdentity(_) => Ok(()),
        };
    ...
    pub async fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.anecdoct_home, self.auth_credentials_store_mode)?;
    ...
    pub async fn logout_with_revoke(&self) -> std::io::Result<bool> {
        ...
        let result = logout_all_stores(&self.anecdoct_home, self.auth_credentials_store_mode)?;
        // Always reload to clear any cached auth (even if file absent).
        self.reload().await;
        Ok(result)
    }
    ...
    pub fn get_api_auth_mode(&self) -> Option<ApiAuthMode> {
        ...
        self.auth_cached().as_ref().map(AnecdoctAuth::api_auth_mode)
    }
    ...
    pub fn auth_mode(&self) -> Option<AuthMode> {
        ...
        self.auth_cached().as_ref().map(AnecdoctAuth::auth_mode)
    }

    pub fn current_auth_uses_anecdoct_backend(&self) -> bool {
        matches!(
            self.auth_mode(),
    ...
    fn is_stale_for_proactive_refresh(auth: &AnecdoctAuth) -> bool {
        let chatgpt_auth = match auth {
            AnecdoctAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return false,
        };
