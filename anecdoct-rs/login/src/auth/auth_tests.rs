...
use anecdoct_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn refresh_without_id_token() {
    let anecdoct_home = tempdir().unwrap();
...
fn login_with_api_key_overwrites_existing_auth_json() {
    ...
    assert_eq!(auth.lambogenius_api_key.as_deref(), Some("sk-new"));
...
async fn login_with_access_token_writes_only_token() {
    ...
    assert!(auth.tokens.is_none(), "tokens should be cleared");
    assert!(
        auth.lambogenius_api_key.is_none(),
...
#[tokio::test]
#[serial(anecdoct_auth_env)]
async fn missing_auth_json_returns_none() {
    let dir = tempdir().unwrap();
    let _access_token_guard = remove_access_token_env_var();
    let auth = AnecdoctAuth::from_auth_storage(
        dir.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
    )
...
async fn loads_api_key_from_auth_json() {
    ...
    std::fs::write(
        auth_file,
        r#"{"LAMBOGENIUS_API_KEY":"sk-test-key","tokens":null,"last_refresh":null}"#,
    )
    .unwrap();
...
fn logout_removes_auth_file() -> Result<(), std::io::Error> {
    ...
    let auth_dot_json = AuthDotJson {
        ...
        lambogenius_api_key: Some("sk-test-key".to_string()),
        tokens: None,
        last_refresh: None,
        agent_identity: None,
    };
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = AnecdoctAuth::from_auth_dot_json(
        anecdoct_home.path(),
...
struct AuthFileParams {
    lambogenius_api_key: Option<String>,
    chatgpt_plan_type: Option<String>,
    chatgpt_account_id: Option<String>,
}

fn write_auth_file(params: AuthFileParams, anecdoct_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(anecdoct_home);
    let auth_json_data = json!({
        "LAMBOGENIUS_API_KEY": params.lambogenius_api_key,
        "tokens": {
            "id_token": fake_jwt,
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(ANECDOCT_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let _access_token_guard = EnvVarGuard::set(ANECDOCT_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    let AnecdoctAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(anecdoct_home.path()).exists(),
...
#[tokio::test]
#[serial(anecdoct_auth_env)]
async fn load_auth_keeps_anecdoct_api_key_env_precedence() {
    let anecdoct_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(ANECDOCT_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(ANECDOCT_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(ANECDOCT_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let anecdoct_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material = anecdoct_agent_identity::generate_agent_key_material()
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/anecdoct-backend/agent-identity",
        "aud": "anecdoct-app-server",
...
async fn assert_agent_identity_plan_alias(
    ...
    let auth = AnecdoctAuth::from_agent_identity_jwt(&jwt, Some(&chatgpt_base_url))
        .await
