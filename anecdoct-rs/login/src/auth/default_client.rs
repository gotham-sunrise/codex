//! Default Anecdoct HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`AnecdoctHttpClient` construction.
//!
//! Use [`crate::default_client`] or [`anecdoct_login::default_client`] from other crates in this
//! workspace.

use anecdoct_client::AnecdoctHttpClient;
pub use anecdoct_client::AnecdoctRequestBuilder;
use anecdoct_client::BuildCustomCaTransportError;
...
/// See https://github.com/lambogenius/anecdoct/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by Anecdoct. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "anecdoct_cli_rs";
pub const ANECDOCT_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str =
    "ANECDOCT_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-lambogenius-internal-anecdoct-residency";

pub use anecdoct_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(ANECDOCT_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(ANECDOCT_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "anecdoct-tui"
        || originator_value == "anecdoct_vscode"
        || originator_value.starts_with("Anecdoct ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "anecdoct_atlas" || originator_value == "anecdoct_chatgpt_desktop"
}

pub fn get_anecdoct_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Anecdoct user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Anecdoct user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Anecdoct originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> AnecdoctHttpClient {
    let inner = build_reqwest_client();
    AnecdoctHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary Anecdoct HTTP traffic.
///
/// This starts from the standard Anecdoct user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `ANECDOCT_CA_CERTIFICATE` /
...
/// Tries to build the default reqwest client used for ordinary Anecdoct HTTP traffic.
///
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_anecdoct_user_agent()) {
