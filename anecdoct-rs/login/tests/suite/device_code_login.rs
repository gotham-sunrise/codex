...
use anecdoct_login::auth::load_auth_dot_json;
...
fn server_opts(
    anecdoct_home: &tempfile::TempDir,
    issuer: String,
...
async fn device_code_login_integration_succeeds() -> anyhow::Result<()> {
    ...
    let opts = server_opts(&anecdoct_home, issuer, AuthCredentialsStoreMode::File);
    ...
    // assert_eq!(auth.lambogenius_api_key.as_deref(), Some("api-key-321"));
...
async fn device_code_login_rejects_workspace_mismatch() -> anyhow::Result<()> {
    ...
    let issuer = mock_server.uri();
    let mut opts = server_opts(&anecdoct_home, issuer, AuthCredentialsStoreMode::File);
...
-> anyhow::Result<()> {
    ...
    let auth = load_auth_dot_json(anecdoct_home.path(), AuthCredentialsStoreMode::File)
        .context("auth.json should load after login succeeds")?
        .context("auth.json written")?;
    assert!(auth.lambogenius_api_key.is_none());
    let tokens = auth.tokens.expect("tokens persisted");
    assert_eq!(tokens.access_token, "access-token-123");
    assert_eq!(tokens.refresh_token, "refresh-token-123");
    assert_eq!(tokens.id_token.raw_jwt, jwt);
