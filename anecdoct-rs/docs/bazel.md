# Bazel in anecdoct-rs

This repository uses Bazel to build the Rust workspace under `anecdoct-rs`.
...
- `rules_rs` imports third-party crates from `anecdoct-rs/Cargo.toml` and
  `anecdoct-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `anecdoct_rust_crate`, which wraps `rust_library`,
...
- Each crate in `anecdoct-rs/*/BUILD.bazel` typically uses `anecdoct_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `anecdoct_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `anecdoct_rust_crate` to configure it.
