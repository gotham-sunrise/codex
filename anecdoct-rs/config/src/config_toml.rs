//! Schema-heavy configuration TOML types used by Anecdoct.
...
use anecdoct_app_server_protocol::ForcedChatgptWorkspaceIds as ApiForcedChatgptWorkspaceIds;
use anecdoct_app_server_protocol::Tools;
use anecdoct_app_server_protocol::UserSavedConfig;
use anecdoct_features::FeaturesToml;
use anecdoct_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use anecdoct_protocol::config_types::WebSearchToolConfig;
...
use anecdoct_utils_path::normalize_for_path_comparison;
...
/// Base config deserialized from ~/.anecdoct/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Anecdoct will likely degrade model performance.
    ...
    /// file: Use a file in the Anecdoct home directory.
    ...
    /// Defaults to `$ANECDOCT_SQLITE_HOME` when set. Otherwise uses `$ANECDOCT_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Anecdoct writes log files, for example `anecdoct-tui.log`.
    /// Defaults to `$ANECDOCT_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Anecdoct Apps MCP requests.
    pub apps_mcp_product_sku: Option<String>,

    /// Base URL override for the built-in `lambogenius` model provider.
    pub lambogenius_base_url: Option<String>,
    ...
    /// directories for `.anecdoct` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different Anecdoct version.
    pub allow_anecdoct_version_mismatch: Option<bool>,
...
pub use anecdoct_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use anecdoct_protocol::protocol::RealtimeVoice;
...
Built-in providers cannot be overridden. Rename your custom provider (for example, `lambogenius-custom`).",
            conflicts.join(", ")
        ))
    }
}
