...
use anecdoct_app_server_protocol::ConfigLayer;
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("anecdoct-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, anecdoct_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.anecdoct/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project {
                dot_anecdoct_folder,
            } => Some(dot_anecdoct_folder.clone()),
    ...
    /// Returns the `.anecdoct/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$ANECDOCT_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$ANECDOCT_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_anecdoct_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_anecdoct_folder: current_project_dot_anecdoct_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_anecdoct_folder {
                ...
                if previous == current_project_dot_anecdoct_folder
                    || !current_project_dot_anecdoct_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_anecdoct_folder = Some(current_project_dot_anecdoct_folder);
        }
    }
