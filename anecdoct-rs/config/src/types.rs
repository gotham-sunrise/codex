//! Types used to define loaded and effective Anecdoct configuration values.
...
use anecdoct_protocol::config_types::EnvironmentVariablePattern;
pub use anecdoct_protocol::config_types::ModeKind;
pub use anecdoct_protocol::config_types::Personality;
pub use anecdoct_protocol::config_types::ServiceTier;
use anecdoct_protocol::config_types::ShellEnvironmentPolicy;
use anecdoct_protocol::config_types::ShellEnvironmentPolicyInherit;
...
/// Determine where Anecdoct should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in ANECDOCT_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in ANECDOCT_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Anecdoct should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// ANECDOCT_HOME/.credentials.json
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Anecdoct product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Anecdoct product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Anecdoct rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Anecdoct does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when Anecdoct cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$ANECDOCT_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against ANECDOCT_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Anecdoct's terminal-specific default. Set to `0` to
...
/// (primarily the Anecdoct IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Anecdoct-managed fast defaults.
    pub fast_default_opt_out: Option<bool>,
    /// Tracks whether the user opted out of the rate limit model switch reminder.
    pub hide_rate_limit_model_nudge: Option<bool>,
    /// Tracks whether the user has seen the model migration prompt
    pub hide_gpt5_1_migration_prompt: Option<bool>,
    /// Tracks whether the user has seen the gpt-5.1-anecdoct-max migration prompt
    #[serde(rename = "hide_gpt-5.1-anecdoct-max_migration_prompt")]
    pub hide_gpt_5_1_anecdoct_max_migration_prompt: Option<bool>,
...
pub struct PluginMcpServerConfig {
    /// When `false`, Anecdoct skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Anecdoct successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Anecdoct last successfully activated for this marketplace.
    #[serde(default)]
...
impl From<SandboxWorkspaceWrite> for anecdoct_app_server_protocol::SandboxSettings {
