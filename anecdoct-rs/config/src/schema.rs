...
use anecdoct_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == anecdoct_features::Feature::Artifact {
            continue;
        }
        if feature.id == anecdoct_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<anecdoct_features::FeatureToml<
                    anecdoct_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == anecdoct_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<anecdoct_features::FeatureToml<
                    anecdoct_features::AppsMcpPathOverrideConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == anecdoct_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<anecdoct_features::FeatureToml<
                    anecdoct_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
