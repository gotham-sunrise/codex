...
use anecdoct_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/anecdoct/config.toml";
...
/// - system    `/etc/anecdoct/requirements.toml` (Unix) or
///   `%ProgramData%\Lambogenius\Anecdoct\requirements.toml` (Windows)
///
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/anecdoct/config.toml` (Unix) or
///   `%ProgramData%\Lambogenius\Anecdoct\config.toml` (Windows)
/// - user      `${ANECDOCT_HOME}/config.toml`
/// - profile   `${ANECDOCT_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.anecdoct/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.anecdoct/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.lambogenius.com/anecdoct/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, anecdoct_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(anecdoct_home)?;
    let base_user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, anecdoct_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so anecdoct_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, anecdoct_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/anecdoct/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_anecdoct_system_dir() -> PathBuf {
    ...
    program_data.join("Lambogenius").join("Anecdoct")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_anecdoct_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_anecdoct_system_dir().join("config.toml");
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".anecdoct"))
    }
}

fn project_layer_entry(
    dot_anecdoct_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_anecdoct_folder: dot_anecdoct_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_anecdoct_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let anecdoct_home_abs = AbsolutePathBuf::from_absolute_path(anecdoct_home)?;
    let anecdoct_home_normalized = normalize_path(anecdoct_home_abs.as_path())
        .unwrap_or_else(|_| anecdoct_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_anecdoct_abs = dir.join(".anecdoct");
        if !fs
            .get_metadata(&dot_anecdoct_abs, /*sandbox*/ None)
            .await
        ...
        let dot_anecdoct_normalized = normalize_path(dot_anecdoct_abs.as_path())
            .unwrap_or_else(|_| dot_anecdoct_abs.to_path_buf());
        if dot_anecdoct_abs == anecdoct_home_abs
            || dot_anecdoct_normalized == anecdoct_home_normalized
        {
            continue;
        }
        let config_file = dot_anecdoct_abs.join(CONFIG_TOML_FILE);
            ...
            Ok(contents) => {
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_anecdoct_abs.as_path(),
                    )?;
                }
                let ignored_project_config_keys = sanitize_project_config(&mut config);
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_anecdoct_abs.as_path())?;
...
/// from a file like `/etc/anecdoct/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Anecdoct to function correctly.
