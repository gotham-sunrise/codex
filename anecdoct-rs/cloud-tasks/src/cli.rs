...
pub enum Command {
    /// Submit a new Anecdoct Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Anecdoct Cloud task.
    Status(StatusCommand),
    /// List Anecdoct Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Anecdoct Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Anecdoct Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Anecdoct Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `anecdoct cloud` to browse).
    ...
    /// Git branch to run in Anecdoct Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Anecdoct Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Anecdoct Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Anecdoct Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
