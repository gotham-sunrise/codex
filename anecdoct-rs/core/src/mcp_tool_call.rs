...
use anecdoct_app_server_protocol::ConfigLayerSource;
use anecdoct_app_server_protocol::McpElicitationObjectType;
...
use crate::mcp_lambogenius_file::rewrite_mcp_tool_arguments_for_lambogenius_files;
...
use anecdoct_analytics::AppInvocation;
use anecdoct_analytics::InvocationType;
use anecdoct_analytics::build_track_events_context;
use anecdoct_config::types::AppToolApproval;
use anecdoct_features::Feature;
use anecdoct_hooks::PermissionRequestDecision;
use anecdoct_mcp::ANECDOCT_APPS_MCP_SERVER_NAME;
use anecdoct_mcp::MCP_TOOL_ANECDOCT_APPS_META_KEY;
use anecdoct_mcp::McpPermissionPromptAutoApproveContext;
use anecdoct_mcp::SandboxState;
use anecdoct_mcp::auth_elicitation_completed_result;
use anecdoct_mcp::build_auth_elicitation_plan;
use anecdoct_mcp::declared_lambogenius_file_input_param_names;
use anecdoct_mcp::mcp_permission_prompt_is_auto_approved;
use anecdoct_otel::sanitize_metric_tag_value;
use anecdoct_protocol::items::McpToolCallError;
use anecdoct_protocol::items::McpToolCallItem;
use anecdoct_protocol::items::McpToolCallStatus;
use anecdoct_protocol::items::TurnItem;
use anecdoct_protocol::lambogenius_models::InputModality;
use anecdoct_protocol::mcp::CallToolResult;
use anecdoct_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use anecdoct_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use anecdoct_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use anecdoct_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use anecdoct_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use anecdoct_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use anecdoct_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use anecdoct_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use anecdoct_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use anecdoct_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use anecdoct_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use anecdoct_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use anecdoct_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use anecdoct_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
use anecdoct_protocol::protocol::AskForApproval;
use anecdoct_protocol::protocol::McpInvocation;
...
use anecdoct_protocol::request_user_input::RequestUserInputQuestion;
use anecdoct_protocol::request_user_input::RequestUserInputQuestionOption;
...
use anecdoct_utils_output_truncation::truncate_text;
...
const MCP_CALL_COUNT_METRIC: &str = "anecdoct.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "anecdoct.mcp.call.duration_ms";
const MCP_RESULT_TELEMETRY_META_KEY: &str = "anecdoct/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "anecdoct.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "anecdoct.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == ANECDOCT_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == ANECDOCT_APPS_MCP_SERVER_NAME {
    ...
    if server == ANECDOCT_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
async fn handle_approved_mcp_tool_call(
    ...
    let rewrite = rewrite_mcp_tool_arguments_for_lambogenius_files(
        sess,
        turn_context,
        arguments_value.clone(),
        metadata.and_then(|metadata| metadata.lambogenius_file_input_params.as_deref()),
    )
    .await;
    ...
    .await;
    maybe_track_anecdoct_app_used(sess, turn_context, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        anecdoct.mcp.target.id = Empty,
        anecdoct.mcp.server_user_flow.triggered = Empty,
    );
...
async fn execute_mcp_tool_call(
    ...
    Ok(maybe_request_anecdoct_apps_auth_elicitation(
        sess,
        turn_context,
        call_id,
        &invocation.server,
...
async fn maybe_request_anecdoct_apps_auth_elicitation(
    sess: &Session,
    turn_context: &TurnContext,
    call_id: &str,
    server: &str,
    ...
    if !sess
        ...
        .read()
        .await
        .is_host_owned_anecdoct_apps_server(server)
    {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        anecdoct_connectors::metadata::connector_install_url(
    ...
    let params = McpServerElicitationRequestParams {
        ...
        server_name: ANECDOCT_APPS_MCP_SERVER_NAME.to_string(),
    ...
    refresh_anecdoct_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

#[expect(
    clippy::await_holding_invalid_type,
    reason = "Anecdoct Apps cache refresh reads through the session-owned manager guard"
)]
async fn refresh_anecdoct_apps_after_connector_auth(sess: &Session, turn_context: &TurnContext) {
    let mcp_tools_result = {
        let manager = sess.services.mcp_connection_manager.read().await;
        manager.hard_refresh_anecdoct_apps_tools_cache().await
    };
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh Anecdoct Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        anecdoct_linux_sandbox_exe: turn_context.anecdoct_linux_sandbox_exe.clone(),
        #[allow(deprecated)]
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                anecdoct_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
async fn maybe_track_anecdoct_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != ANECDOCT_APPS_MCP_SERVER_NAME {
        return;
    }
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    anecdoct_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
    lambogenius_file_input_params: Option<Vec<String>>,
}

const MCP_TOOL_LAMBOGENIUS_OUTPUT_TEMPLATE_META_KEY: &str = "lambogenius/outputTemplate";
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, anecdoct_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_ANECDOCT_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == ANECDOCT_APPS_MCP_SERVER_NAME {
        let mut anecdoct_apps_meta = metadata
            .and_then(|metadata| metadata.anecdoct_apps_meta.clone())
            .unwrap_or_default();
        anecdoct_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_ANECDOCT_APPS_META_KEY.to_string(),
            serde_json::Value::Object(anecdoct_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__anecdoct_mcp_decline__";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == ANECDOCT_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == ANECDOCT_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        anecdoct_apps_meta: tool_info
            .tool
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_ANECDOCT_APPS_META_KEY))
            .and_then(serde_json::Value::as_object)
            .cloned(),
        // Disallow custom MCPs from uploading files via fileParams.
        lambogenius_file_input_params: lambogenius_file_input_params_for_server(
            server,
...
fn lambogenius_file_input_params_for_server(
    server: &str,
    meta: Option<&serde_json::Map<String, serde_json::Value>>,
) -> Option<Vec<String>> {
    (server == ANECDOCT_APPS_MCP_SERVER_NAME)
        .then_some(declared_lambogenius_file_input_param_names(meta))
...
fn get_mcp_app_resource_uri(
    ...
    meta.and_then(|meta| {
        meta.get("ui")
            ...
            .or_else(|| {
                meta.get(MCP_TOOL_LAMBOGENIUS_OUTPUT_TEMPLATE_META_KEY)
                    .and_then(serde_json::Value::as_str)
            })
            .map(str::to_string)
    })
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == ANECDOCT_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_anecdoct_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_anecdoct_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, anecdoct_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
    ...
    config
        ...
        .find_map(|layer| {
            ...
            let servers = layer
                ...
                .cloned()
                .and_then(|value| {
                    HashMap::<String, anecdoct_config::types::McpServerConfig>::deserialize(value)
                        .ok()
                })?;
