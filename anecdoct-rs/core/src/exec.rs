...
use anecdoct_protocol::error::SandboxErr;
...
use anecdoct_protocol::protocol::ExecCommandOutputDeltaEvent;
use anecdoct_protocol::protocol::ExecOutputStream;
use anecdoct_protocol::protocol::SandboxPolicy;
use anecdoct_sandboxing::SandboxCommand;
use anecdoct_sandboxing::SandboxManager;
use anecdoct_sandboxing::SandboxTransformRequest;
use anecdoct_sandboxing::SandboxType;
use anecdoct_sandboxing::SandboxablePreference;
use anecdoct_utils_absolute_path::AbsolutePathBuf;
use anecdoct_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use anecdoct_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: anecdoct_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: anecdoct_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            anecdoct_linux_sandbox_exe: anecdoct_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(AnecdoctErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        anecdoct_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    ...
    if let Some(metrics) = anecdoct_otel::global() {
        let _ = metrics.counter(
            "anecdoct.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_anecdoct_home;
    use anecdoct_windows_sandbox::run_windows_sandbox_capture_elevated;
    use anecdoct_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        AnecdoctErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let anecdoct_home = find_anecdoct_home().map_err(|err| {
        AnecdoctErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve anecdoct_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_elevated(
                anecdoct_windows_sandbox::ElevatedSandboxCaptureRequest {
                    policy_json_or_preset: policy_str.as_str(),
                    sandbox_policy_cwd: &sandbox_cwd,
                    anecdoct_home: anecdoct_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(AnecdoctErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, AnecdoctErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(AnecdoctErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(AnecdoctErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(AnecdoctErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
pub(crate) fn resolve_windows_restricted_token_filesystem_overrides(
    ...
    let additional_deny_read_paths = anecdoct_windows_sandbox::resolve_windows_deny_read_paths(
        file_system_sandbox_policy,
        sandbox_policy_cwd,
    )?;
...
pub(crate) fn resolve_windows_elevated_filesystem_overrides(
    ...
    let split_readable_roots: Vec<PathBuf> = file_system_sandbox_policy
        ...
        .into_iter()
        .map(anecdoct_utils_absolute_path::AbsolutePathBuf::into_path_buf)
        .map(&normalize_path)
        .collect();
...
fn has_reopened_writable_descendant(
    writable_roots: &[anecdoct_protocol::protocol::WritableRoot],
) -> bool {
