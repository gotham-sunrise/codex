...
use anecdoct_protocol::protocol::GuardianAssessmentAction;
...
use anecdoct_protocol::request_user_input::RequestUserInputEvent;
...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    let cancel = CancellationToken::new();
    let forward = tokio::spawn(forward_ops(Arc::clone(&anecdoct), rx_ops, cancel));

    let submission = Submission {
        id: "sub-1".to_string(),
        op: Op::Interrupt,
        trace: Some(anecdoct_protocol::protocol::W3cTraceContext {
            traceparent: Some(
...
#[tokio::test]
async fn run_anecdoct_thread_interactive_respects_pre_cancelled_spawn() {
    ...
    let result = timeout(
        ...
        run_anecdoct_thread_interactive(
    ...
    assert!(matches!(result, Err(AnecdoctErr::TurnAborted)));
}
