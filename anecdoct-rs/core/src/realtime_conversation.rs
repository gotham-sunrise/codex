...
use anecdoct_api::RealtimeAudioFrame;
use anecdoct_api::RealtimeEvent;
use anecdoct_api::RealtimeEventParser;
use anecdoct_api::RealtimeSessionConfig;
use anecdoct_api::RealtimeSessionMode;
use anecdoct_api::RealtimeWebsocketClient;
use anecdoct_api::RealtimeWebsocketEvents;
use anecdoct_api::RealtimeWebsocketWriter;
...
use anecdoct_login::read_lambogenius_api_key_from_env;
...
use anecdoct_protocol::protocol::RealtimeConversationClosedEvent;
use anecdoct_protocol::protocol::RealtimeConversationRealtimeEvent;
use anecdoct_protocol::protocol::RealtimeConversationSdpEvent;
use anecdoct_protocol::protocol::RealtimeConversationStartedEvent;
use anecdoct_protocol::protocol::RealtimeHandoffRequested;
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> AnecdoctResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> AnecdoctResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            session_config,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> AnecdoctResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(AnecdoctErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> AnecdoctResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(text)
            .await
            .map_err(|_| AnecdoctErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> AnecdoctResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn handoff_complete(&self) -> AnecdoctResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> AnecdoctResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> AnecdoctResult<PreparedRealtimeConversationStart> {
...
) -> AnecdoctResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> AnecdoctResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), AnecdoctErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(
    ...
    if let Some(api_key) = auth.and_then(AnecdoctAuth::api_key) {
    ...
    if provider.is_lambogenius()
        && let Some(api_key) = read_lambogenius_api_key_from_env()
    {
        return Ok(api_key);
    }

    Err(AnecdoctErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    api_key: Option<&str>,
) -> AnecdoctResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            AnecdoctErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    anecdoct_error_info: AnecdoctErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
