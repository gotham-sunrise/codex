...
use anecdoct_config::AppRequirementToml;
use anecdoct_config::AppToolRequirementToml;
use anecdoct_config::AppToolsRequirementsToml;
...
use anecdoct_config::types::AppsDefaultConfig;
use anecdoct_connectors::merge::plugin_connector_to_app_info;
use anecdoct_connectors::metadata::connector_install_url;
use anecdoct_connectors::metadata::sanitize_name;
...
fn anecdoct_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{ANECDOCT_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| ANECDOCT_APPS_MCP_SERVER_NAME.to_string());
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let anecdoct_home = tempdir().expect("tempdir should succeed");
    std::fs::write(anecdoct_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(anecdoct_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .anecdoct_home(anecdoct_home.path().to_path_buf())
        .fallback_cwd(Some(anecdoct_home.path().to_path_buf()))
        .build()
