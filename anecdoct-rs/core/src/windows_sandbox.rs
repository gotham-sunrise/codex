...
pub fn sandbox_setup_is_complete(anecdoct_home: &Path) -> bool {
    anecdoct_windows_sandbox::sandbox_setup_is_complete(anecdoct_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_anecdoct_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = anecdoct_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = anecdoct_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if anecdoct_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            anecdoct_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
        "anecdoct.windows_sandbox.elevated_setup_canceled"
    } else {
        "anecdoct.windows_sandbox.elevated_setup_failure"
    }
}
...
pub fn run_elevated_setup(
    ...
    anecdoct_windows_sandbox::run_elevated_setup(
        anecdoct_windows_sandbox::SandboxSetupRequest {
            policy,
            policy_cwd,
            command_cwd,
            env_map,
        ...
        anecdoct_windows_sandbox::SetupRootOverrides::default(),
    )
}
...
pub fn run_legacy_setup_preflight(
    ...
    anecdoct_windows_sandbox::run_windows_sandbox_legacy_preflight(
        policy,
        policy_cwd,
        anecdoct_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    anecdoct_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let anecdoct_home = request.anecdoct_home;
    let active_profile = request.active_profile;
    let setup_anecdoct_home = anecdoct_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_anecdoct_home.as_path()) {
                    run_elevated_setup(
                        &policy,
                        policy_cwd.as_path(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_anecdoct_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(anecdoct_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let _ = metrics.record_duration(
        "anecdoct.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "anecdoct.windows_sandbox.setup_success",
        /*inc*/ 1,
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "anecdoct.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("originator", originator_tag), ("mode", mode_tag)],
    );
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        ...
        let _ = metrics.counter(
            "anecdoct.windows_sandbox.legacy_setup_preflight_failed",
            /*inc*/ 1,
            &[("originator", originator_tag)],
        );
    }
