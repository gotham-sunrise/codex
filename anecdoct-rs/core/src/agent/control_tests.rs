...
use anecdoct_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    async fn new() -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(anecdoct_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<AnecdoctThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<AnecdoctThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<AnecdoctThread>, message: &str) {
    thread
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, AnecdoctErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn send_inter_agent_communication_without_turn_queues_message_without_triggering_turn() {
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            if thread.anecdoct.session.has_pending_input().await {
                break;
            }
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    parent_thread
        ...
        .await;
    let turn_context = parent_thread.anecdoct.session.new_default_turn().await;
    ...
    let history = child_thread.anecdoct.session.clone_history().await;
...
async fn spawn_agent_fork_last_n_turns_keeps_only_recent_turns() {
    ...
    let queued_turn_context = parent_thread.anecdoct.session.new_default_turn().await;
    parent_thread
        .anecdoct
        .session
        .record_conversation_items(
    ...
    let triggered_turn_context = parent_thread.anecdoct.session.new_default_turn().await;
    parent_thread
        .anecdoct
        .session
        .record_conversation_items(
    ...
    parent_thread
        ...
        .await;
    let spawn_turn_context = parent_thread.anecdoct.session.new_default_turn().await;
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let AnecdoctErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected AnecdoctErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let AnecdoctErr::AgentLimitReached { max_threads } = err else {
        panic!("expected AnecdoctErr::AgentLimitReached");
    };
...
async fn multi_agent_v2_completion_ignores_dead_direct_parent() {
    ...
    let tester_turn = tester_thread.anecdoct.session.new_default_turn().await;
    tester_thread
        .anecdoct
        .session
        .send_event(
