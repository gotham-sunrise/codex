...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> AnecdoctThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    async fn spawn_agent_internal(
        ...
        )) = notification_source.as_ref()
        {
            ...
            let thread_config = new_thread.thread.anecdoct.thread_config_snapshot().await;
    ...
    ) -> AnecdoctResult<crate::thread_manager::NewThread> {
        if options.fork_parent_spawn_call_id.is_none() {
            return Err(AnecdoctErr::Fatal(
    ...
    async fn resume_single_agent_from_rollout(
        ...
        let history = stored_thread
            .history
            .ok_or_else(|| AnecdoctErr::ThreadNotFound(thread_id))?
            .items;
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> AnecdoctResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: AnecdoctResult<String>,
    ) -> AnecdoctResult<String> {
        if matches!(result, Err(AnecdoctErr::InternalAgentDied)) {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> AnecdoctResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread.anecdoct.session.ensure_rollout_materialized().await;
            thread.anecdoct.session.flush_rollout().await?;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> AnecdoctResult<String> {
        let state = self.upgrade()?;
    ...
    async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> AnecdoctResult<String> {
        let descendant_ids = self.live_thread_spawn_descendants(agent_id).await?;
        let result = self.shutdown_live_agent(agent_id).await;
        for descendant_id in descendant_ids {
            match self.shutdown_live_agent(descendant_id).await {
                Ok(_)
                | Err(AnecdoctErr::ThreadNotFound(_))
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(AnecdoctErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> AnecdoctResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> AnecdoctResult<Vec<ListedAgent>> {
        let state = self.upgrade()?;
    ...
    ) -> AnecdoctResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> AnecdoctResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| AnecdoctErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_shell_snapshot_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        parent_thread.anecdoct.session.user_shell().shell_snapshot()
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        let parent_config = parent_thread.anecdoct.session.get_config().await;
        if !crate::exec_policy::child_uses_parent_exec_policy(&parent_config, child_config) {
            return None;
        }

        Some(Arc::clone(
            &parent_thread.anecdoct.session.services.exec_policy,
        ))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> AnecdoctResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> AnecdoctResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
