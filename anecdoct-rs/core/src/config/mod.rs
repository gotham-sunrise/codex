...
use anecdoct_config::ConfigLayerSource;
use anecdoct_config::ConfigLayerStack;
use anecdoct_config::ConfigLayerStackOrdering;
use anecdoct_config::ConfigRequirements;
use anecdoct_config::ConfigRequirementsToml;
use anecdoct_config::ConstrainedWithSource;
use anecdoct_config::FeatureRequirementsToml;
use anecdoct_config::McpServerIdentity;
use anecdoct_config::McpServerRequirement;
use anecdoct_config::PluginRequirementsToml;
use anecdoct_config::ProfileV2Name;
use anecdoct_config::ResidencyRequirement;
use anecdoct_config::SandboxModeRequirement;
use anecdoct_config::Sourced;
use anecdoct_config::ThreadConfigLoader;
use anecdoct_config::config_toml::ConfigLockfileToml;
use anecdoct_config::config_toml::ConfigToml;
use anecdoct_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
...
use anecdoct_config::config_toml::ThreadStoreToml;
use anecdoct_config::config_toml::validate_model_providers;
...
use anecdoct_config::sandbox_mode_requirement_for_permission_profile;
use anecdoct_config::types::ApprovalsReviewer;
use anecdoct_config::types::AuthCredentialsStoreMode;
use anecdoct_config::types::History;
use anecdoct_config::types::McpServerConfig;
use anecdoct_config::types::McpServerDisabledReason;
...
use anecdoct_config::types::OAuthCredentialsStoreMode;
...
use anecdoct_config::types::UriBasedFileOpener;
use anecdoct_config::types::WindowsSandboxModeToml;
use anecdoct_core_plugins::PluginsConfigInput;
use anecdoct_exec_server::ExecutorFileSystem;
use anecdoct_exec_server::LOCAL_FS;
use anecdoct_features::AppsMcpPathOverrideConfigToml;
use anecdoct_features::Feature;
use anecdoct_features::FeatureConfigSource;
use anecdoct_features::FeatureOverrides;
use anecdoct_features::FeatureToml;
use anecdoct_features::Features;
use anecdoct_features::FeaturesToml;
use anecdoct_features::MultiAgentV2ConfigToml;
use anecdoct_features::NetworkProxyConfigToml;
use anecdoct_git_utils::resolve_root_git_project_for_trust;
use anecdoct_login::AuthManagerConfig;
use anecdoct_mcp::McpConfig;
use anecdoct_memories_read::memory_root;
use anecdoct_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use anecdoct_model_provider_info::ModelProviderInfo;
use anecdoct_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use anecdoct_model_provider_info::built_in_model_providers;
use anecdoct_model_provider_info::merge_configured_model_providers;
use anecdoct_models_manager::ModelsManagerConfig;
use anecdoct_protocol::config_types::AltScreenMode;
...
use anecdoct_protocol::config_types::SandboxMode;
use anecdoct_protocol::config_types::ServiceTier;
use anecdoct_protocol::config_types::ShellEnvironmentPolicy;
use anecdoct_protocol::config_types::TrustLevel;
use anecdoct_protocol::config_types::Verbosity;
use anecdoct_protocol::config_types::WebSearchConfig;
use anecdoct_protocol::config_types::WebSearchMode;
use anecdoct_protocol::config_types::WindowsSandboxLevel;
use anecdoct_protocol::lambogenius_models::ModelsResponse;
...
use anecdoct_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_anecdoct_runtime;
...
#[cfg(test)]
mod schema;
pub use anecdoct_config::ConfigLoadOptions;
pub use anecdoct_config::Constrained;
pub use anecdoct_config::ConstraintError;
pub use anecdoct_config::ConstraintResult;
pub use anecdoct_config::LoaderOverrides;
pub use anecdoct_network_proxy::NetworkProxyAuditMetadata;
use anecdoct_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use anecdoct_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(anecdoct_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(anecdoct_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal Anecdoct-only writable roots are intentionally excluded.
...
// when Anecdoct is still responsible for the network policy. `Disabled` means no
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Anecdoct will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Anecdoct
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.anecdoct/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Anecdoct"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Anecdoct '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Anecdoct home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Anecdoct can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Anecdoct unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/lambogenius/anecdoct/blob/main/anecdoct-rs/rmcp-client/src/oauth.rs#L2
    /// file: ANECDOCT_HOME/.credentials.json
    ///       This file will be readable to Anecdoct and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Anecdoct will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Anecdoct state (defaults to `~/.anecdoct` but can be
    /// overridden by the `ANECDOCT_HOME` environment variable).
    pub anecdoct_home: AbsolutePathBuf,

    /// Directory where Anecdoct stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Anecdoct writes log files (defaults to `$ANECDOCT_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where Anecdoct writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores Anecdoct version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_anecdoct_version_mismatch: bool,
    ...
    /// Settings that govern if and what will be written to `~/.anecdoct/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Anecdoct executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub anecdoct_self_exe: Option<PathBuf>,

    /// Path to the `anecdoct-linux-sandbox` executable. This must be set if
    /// [`anecdoct_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `anecdoct-linux-sandbox`.
    pub anecdoct_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `anecdoct-execve-wrapper` executable used for shell
    ...
    /// Base URL for requests to ChatGPT (as opposed to the Lambogenius API).
    ...
    /// When `true`, checks for Anecdoct updates on startup and surfaces update prompts.
    /// Set to `false` only if your Anecdoct updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Anecdoct product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Anecdoct product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: anecdoct_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn anecdoct_home(&self) -> PathBuf {
        self.anecdoct_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn anecdoct_home(mut self, anecdoct_home: PathBuf) -> Self {
        self.anecdoct_home = Some(anecdoct_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let anecdoct_home = match anecdoct_home {
            Some(anecdoct_home) => AbsolutePathBuf::from_absolute_path(anecdoct_home)?,
            None => find_anecdoct_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&anecdoct_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = anecdoct_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    anecdoct_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(anecdoct_config::io_error_from_config_error(
        ...
        {
            let allow_anecdoct_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_anecdoct_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_anecdoct_version_mismatch = allow_anecdoct_version_mismatch;
...
impl Config {
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &anecdoct_core_plugins::PluginsManager,
    ) -> McpConfig {
        ...
        McpConfig {
            ...
            anecdoct_home: self.anecdoct_home.to_path_buf(),
            ...
            anecdoct_linux_sandbox_exe: self.anecdoct_linux_sandbox_exe.clone(),
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.anecdoct_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_anecdoct_home(
            anecdoct_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific Anecdoct home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_anecdoct_home(
        anecdoct_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = anecdoct_config::build_cli_overrides_layer(&cli_overrides);
        anecdoct_config::merge_toml_values(&mut merged, &cli_layer);
        let anecdoct_home = AbsolutePathBuf::from_absolute_path_checked(anecdoct_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &anecdoct_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `anecdoct exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `anecdoct_self_exe`, `anecdoct_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, anecdoct_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .anecdoct/ folders.
...
/// Patch `ANECDOCT_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(anecdoct_home: &Path, provider: &str) -> std::io::Result<()> {
    anecdoct_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &anecdoct_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            anecdoct_protocol::permissions::FileSystemSandboxEntry {
                path: anecdoct_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: anecdoct_protocol::permissions::FileSystemAccessMode::None,
            }
        } else {
            ...
            anecdoct_protocol::permissions::FileSystemSandboxEntry {
                path: anecdoct_protocol::permissions::FileSystemPath::Path { path },
                access: anecdoct_protocol::permissions::FileSystemAccessMode::None,
            }
        };
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let user_instructions = AgentsMdManager::load_global_instructions(Some(&anecdoct_home))
        ...
        let memories_root = memory_root(&anecdoct_home);
        ...
        let (
            ...
            {
                // Keep Anecdoct runtime write access while storing the runtime
        ...
        let lambogenius_base_url = cfg
            .lambogenius_base_url
            .clone()
            .filter(|value| !value.is_empty());

        let model_providers =
            merge_configured_model_providers(built_in_model_providers(lambogenius_base_url), cfg.model_providers)
                .map_err(|message| std::io::Error::new(std::io::ErrorKind::InvalidData, message))?;

        let model_provider_id = model_provider
            .or(config_profile.model_provider)
            .or(cfg.model_provider)
            .unwrap_or_else(|| "lambogenius".to_string());
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(anecdoct_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| anecdoct_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| anecdoct_home.to_path_buf());
        ...
        let helper_readable_roots = get_readable_roots_required_for_anecdoct_runtime(
            &anecdoct_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_anecdoct_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_anecdoct_version_mismatch)
...
pub fn find_anecdoct_home() -> std::io::Result<AbsolutePathBuf> {
    anecdoct_utils_home_dir::find_anecdoct_home()
}

/// Returns the path to the folder where Anecdoct logs are stored. Does not verify
