...
use anecdoct_config::ConfigLayerEntry;
use anecdoct_config::ProfileV2Name;
use anecdoct_config::RequirementSource;
use anecdoct_config::config_toml::AgentRoleToml;
use anecdoct_config::config_toml::AgentsToml;
use anecdoct_config::config_toml::AutoReviewToml;
use anecdoct_config::config_toml::ConfigToml;
use anecdoct_config::config_toml::ProjectConfig;
use anecdoct_config::config_toml::RealtimeAudioConfig;
use anecdoct_config::config_toml::RealtimeConfig;
use anecdoct_config::config_toml::RealtimeToml;
use anecdoct_config::config_toml::RealtimeTransport;
use anecdoct_config::config_toml::RealtimeWsMode;
use anecdoct_config::config_toml::RealtimeWsVersion;
use anecdoct_config::config_toml::ToolsToml;
use anecdoct_config::loader::project_trust_key;
use anecdoct_config::permissions_toml::FilesystemPermissionToml;
use anecdoct_config::permissions_toml::FilesystemPermissionsToml;
use anecdoct_config::permissions_toml::NetworkDomainPermissionToml;
use anecdoct_config::permissions_toml::NetworkDomainPermissionsToml;
use anecdoct_config::permissions_toml::NetworkToml;
use anecdoct_config::permissions_toml::PermissionProfileToml;
use anecdoct_config::permissions_toml::PermissionsToml;
use anecdoct_config::permissions_toml::WorkspaceRootsToml;
use anecdoct_config::profile_toml::ConfigProfile;
use anecdoct_config::types::AppToolApproval;
use anecdoct_config::types::ApprovalsReviewer;
use anecdoct_config::types::BundledSkillsConfig;
use anecdoct_config::types::FeedbackConfigToml;
use anecdoct_config::types::HistoryPersistence;
use anecdoct_config::types::McpServerEnvVar;
use anecdoct_config::types::McpServerOAuthConfig;
use anecdoct_config::types::McpServerToolConfig;
use anecdoct_config::types::McpServerTransportConfig;
use anecdoct_config::types::MemoriesConfig;
use anecdoct_config::types::MemoriesToml;
use anecdoct_config::types::ModelAvailabilityNuxConfig;
use anecdoct_config::types::Notice;
use anecdoct_config::types::NotificationCondition;
use anecdoct_config::types::NotificationMethod;
use anecdoct_config::types::Notifications;
use anecdoct_config::types::OtelConfig;
use anecdoct_config::types::OtelConfigToml;
use anecdoct_config::types::OtelExporterKind;
use anecdoct_config::types::SandboxWorkspaceWrite;
use anecdoct_config::types::SessionPickerViewMode;
use anecdoct_config::types::SkillsConfig;
use anecdoct_config::types::ToolSuggestDisabledTool;
use anecdoct_config::types::ToolSuggestDiscoverableType;
use anecdoct_config::types::Tui;
use anecdoct_config::types::TuiKeymap;
use anecdoct_config::types::TuiNotificationSettings;
use anecdoct_config::types::TuiPetAnchor;
use anecdoct_config::types::WindowsSandboxModeToml;
use anecdoct_config::types::WindowsToml;
use anecdoct_core_plugins::PluginsManager;
use anecdoct_exec_server::LOCAL_FS;
use anecdoct_features::Feature;
use anecdoct_features::FeaturesToml;
use anecdoct_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use anecdoct_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use anecdoct_model_provider_info::WireApi;
...
use anecdoct_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use anecdoct_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
...
use anecdoct_protocol::protocol::NetworkAccess;
...
async fn load_config_loads_global_agents_instructions() -> std::io::Result<()> {
    let anecdoct_home = tempdir()?;
    std::fs::write(
        anecdoct_home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "\n  global instructions  \n",
    )?;
...
async fn load_config_prefers_global_agents_override_instructions() -> std::io::Result<()> {
    ...
    let global_agents_override_path = anecdoct_home.path().join(LOCAL_AGENTS_MD_FILENAME);
...
requires_lambogenius_auth = true
                ...
                PermissionProfileToml {
                    workspace_roots: Some(WorkspaceRootsToml {
                        entries: BTreeMap::from([
                            ...
                            ("~/code/lambogenius".to_string(), true),
                        ]),
                    }),
...
async fn experimental_network_requirements_enable_proxy_without_feature() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .anecdoct_home(anecdoct_home.path().to_path_buf())
        .fallback_cwd(Some(anecdoct_home.path().to_path_buf()))
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(anecdoct_config::ConfigRequirementsToml {
                network: Some(anecdoct_config::NetworkRequirementsToml {
                    enabled: Some(true),
                    ..Default::default()
                }),
                ..Default::default()
...
async fn default_permissions_profile_populates_runtime_sandbox_policy() -> std::io::Result<()> {
    ...
    let memories_root = anecdoct_home.path().join("memories").abs();
    assert_eq!(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        anecdoct_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let anecdoct_home = temp_dir.path().join("anecdoct-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".anecdoct"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".anecdoct".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".anecdoct"), cwd.as_path()),
            "expected .anecdoct carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".anecdoct"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".anecdoct"] {
...
-> std::io::Result<()> {
    ...
    let memories_root = AbsolutePathBuf::from_absolute_path(std::fs::canonicalize(
        anecdoct_home.path().join("memories"),
    )?)?;
    assert!(
        config
            .permissions
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Anecdoct and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Anecdoct and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `workspace` does not define any recognized filesystem entries for this version of Anecdoct."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            anecdoct_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, anecdoct_home.path());
    let project_dot_anecdoct =
        AbsolutePathBuf::resolve_path_against_base("project/.anecdoct", anecdoct_home.path());
    ...
    let requirements_toml = anecdoct_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = anecdoct_config::ConfigRequirements {
    ...
    let refreshed_layer_stack = ConfigLayerStack::new(
        vec![
            ConfigLayerEntry::new(
                anecdoct_app_server_protocol::ConfigLayerSource::User {
            ...
            ConfigLayerEntry::new(
                anecdoct_app_server_protocol::ConfigLayerSource::Project {
                    dot_anecdoct_folder: project_dot_anecdoct.clone(),
                },
                toml::toml! {
            ...
            ConfigLayerEntry::new(
                anecdoct_app_server_protocol::ConfigLayerSource::LegacyManagedConfigTomlFromMdm,
                toml::toml! {
                    [mcp_servers.managed_overrides_session]
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(anecdoct_home.path().to_path_buf()),
            ..Default::default()
        },
        anecdoct_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                anecdoct_app_server_protocol::ConfigLayerSource::Project {
                    dot_anecdoct_folder: project_dot_anecdoct,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
            ...
            ConfigLayerEntry::new(
                anecdoct_app_server_protocol::ConfigLayerSource::SessionFlags,
                toml::toml! {
                    [mcp_servers.session_overrides_user]
...
-> anyhow::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let plugin_root = anecdoct_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".anecdoct-plugin"))?;
    std::fs::write(
...
"#,
    ...
    let requirements = anecdoct_config::ConfigRequirementsToml {
...
#[tokio::test]
async fn sqlite_home_defaults_to_anecdoct_home_for_workspace_write() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, anecdoct_home.path().to_path_buf());
...
async fn workspace_write_always_includes_memories_root_once() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let memories_root = anecdoct_home.path().join("memories");
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".anecdoct");
...
async fn cli_override_takes_precedence_over_profile_sandbox_mode() -> std::io::Result<()> {
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, anecdoct_home.abs()).await?;
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let managed_path = anecdoct_home.path().join("managed_config.toml");
    let config_path = anecdoct_home.path().join(CONFIG_TOML_FILE);
    ...
    let cwd = anecdoct_home.path().abs();
    let config_layer_stack = load_config_layers_state(
        ...
        &anecdoct_config::NoopThreadConfigLoader,
    )
    .await?;
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(anecdoct_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(anecdoct_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(anecdoct_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/anecdoct-mcp");
    let servers = BTreeMap::from([(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/anecdoct-mcp""#),
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/anecdoct-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-anecdoct-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-anecdoct-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-anecdoct-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(anecdoct_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(anecdoct_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let base_config = anecdoct_home.path().join(CONFIG_TOML_FILE);
    let selected_config = anecdoct_home.path().join("work.config.toml");
    tokio::fs::write(&base_config, r#"model_provider = "lambogenius""#).await?;
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(anecdoct_home.path(), &profile_name),
        anecdoct_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
struct PrecedenceTestFixture {
    cwd: TempDir,
    anecdoct_home: TempDir,
    cfg: ConfigToml,
    model_provider_map: HashMap<String, ModelProviderInfo>,
    lambogenius_provider: ModelProviderInfo,
    lambogenius_custom_provider: ModelProviderInfo,
}
...
impl PrecedenceTestFixture {
    ...
    fn anecdoct_home(&self) -> AbsolutePathBuf {
        self.anecdoct_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let workspace = anecdoct_home.path().join("workspace");
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        anecdoct_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let missing_path = anecdoct_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let role_config_path = anecdoct_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = anecdoct_config::ConfigLayerStack::new(
        vec![anecdoct_config::ConfigLayerEntry::new(
            anecdoct_app_server_protocol::ConfigLayerSource::User {
                file: anecdoct_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        anecdoct_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".anecdoct").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let researcher_path = anecdoct_home.path().join("agents").join("researcher.toml");
    let reviewer_path = anecdoct_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = anecdoct_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let catalog_path = anecdoct_home.path().join("catalog.json");
...
[model_providers.lambogenius-custom]
name = "Lambogenius custom"
base_url = "https://api.lambogenius.com/v1"
env_key = "LAMBOGENIUS_API_KEY"
wire_api = "responses"
...
[profiles.gpt3]
model = "gpt-3.5-turbo"
model_provider = "lambogenius-custom"
...
"#;
    ...
    let anecdoct_home_temp_dir = TempDir::new().unwrap();

    let lambogenius_custom_provider = ModelProviderInfo {
        name: "Lambogenius custom".to_string(),
        base_url: Some("https://api.lambogenius.com/v1".to_string()),
        env_key: Some("LAMBOGENIUS_API_KEY".to_string()),
        ...
        requires_lambogenius_auth: false,
        supports_websockets: false,
    };
    let model_provider_map = {
        let mut model_provider_map = built_in_model_providers(
            /* lambogenius_base_url */ /*lambogenius_base_url*/ None,
        );
        model_provider_map.insert(
            "lambogenius-custom".to_string(),
            lambogenius_custom_provider.clone(),
        );
        model_provider_map
    };

    let lambogenius_provider = model_provider_map
        .get("lambogenius")
        .expect("lambogenius provider should exist")
        .clone();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        anecdoct_home: anecdoct_home_temp_dir,
        cfg,
        model_provider_map,
        lambogenius_provider,
        lambogenius_custom_provider,
    })
}
...
async fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            service_tier: None,
            model_provider_id: "lambogenius".to_string(),
            model_provider: fixture.lambogenius_provider.clone(),
            ...
            anecdoct_home: fixture.anecdoct_home(),
            sqlite_home: fixture.anecdoct_home().to_path_buf(),
            log_dir: fixture.anecdoct_home().join("log").to_path_buf(),
            config_lock_export_dir: None,
            config_lock_allow_anecdoct_version_mismatch: false,
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: anecdoct_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let anecdoct_home = fixture.anecdoct_home();
...
async fn test_precedence_fixture_with_gpt3_profile() -> std::io::Result<()> {
    ...
    let expected_gpt3_profile_config = Config {
        ...
        model_auto_compact_token_limit: None,
        service_tier: None,
        model_provider_id: "lambogenius-custom".to_string(),
        model_provider: fixture.lambogenius_custom_provider.clone(),
        permissions: Permissions {
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = anecdoct_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![anecdoct_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = anecdoct_config::RequirementSource::Unknown;
    ...
    let requirements = anecdoct_config::ConfigRequirements {
        web_search_mode: anecdoct_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        anecdoct_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/anecdoct4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/anecdoct3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/anecdoct2");
...
[projects."/Users/mbolin/code/anecdoct4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/anecdoct3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let anecdoct_home = temp_dir.path();
    let config_path = anecdoct_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(anecdoct_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(anecdoct_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(anecdoct_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(anecdoct_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(anecdoct_config::ConfigRequirementsToml {
                allowed_sandbox_modes: Some(vec![
                    anecdoct_config::SandboxModeRequirement::ReadOnly,
                ]),
                ..Default::default()
            }))
        }))
...
"#,
    ...
    let requirements = anecdoct_config::ConfigRequirementsToml {
        allowed_approval_policies: None,
        allowed_approvals_reviewers: None,
        allowed_sandbox_modes: Some(vec![anecdoct_config::SandboxModeRequirement::ReadOnly]),
        remote_sandbox_config: None,
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let cwd = anecdoct_home.path().join("workspace");
    let outside_root = anecdoct_home.path().join("outside-write");
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(anecdoct_config::ConfigRequirementsToml {
                allowed_web_search_modes: Some(vec![
                    anecdoct_config::WebSearchModeRequirement::Cached,
                ]),
                ..Default::default()
            }))
        }))
...
async fn feature_requirements_normalize_effective_feature_values() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        ...
        .cloud_requirements(CloudRequirementsLoader::new(async {
            Ok(Some(anecdoct_config::ConfigRequirementsToml {
                feature_requirements: Some(anecdoct_config::FeatureRequirementsToml {
                    entries: BTreeMap::from([
...
export_dir = "locks"
allow_anecdoct_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_anecdoct_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let anecdoct_home = TempDir::new()?;
    let lock_path = anecdoct_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
anecdoct_version = "older-version"
...
discoverables = [
  ...
  { type = "plugin", id = "plugin_alpha@lambogenius-curated" },
  { type = "connector", id = "   " }
]
"#,
    )
        ...
        Some(ToolSuggestConfig {
            discoverables: vec![
                ...
                ToolSuggestDiscoverable {
                    ...
                    id: "plugin_alpha@lambogenius-curated".to_string(),
                },
                ToolSuggestDiscoverable {
                    kind: ToolSuggestDiscoverableType::Connector,
                    id: "   ".to_string(),
