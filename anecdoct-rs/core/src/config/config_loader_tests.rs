...
use anecdoct_config::ConfigError;
...
use anecdoct_config::ConfigRequirementsWithSources;
use anecdoct_config::FilesystemDenyReadPattern;
use anecdoct_config::LoaderOverrides;
use anecdoct_config::RequirementSource;
use anecdoct_config::SessionThreadConfig;
use anecdoct_config::StaticThreadConfigLoader;
use anecdoct_config::ThreadConfigSource;
use anecdoct_config::config_error_from_ignored_toml_fields;
use anecdoct_config::config_error_from_toml;
...
use anecdoct_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    dot_anecdoct_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_anecdoct_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_anecdoct_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let anecdoct_home = tempdir().expect("tempdir");
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when ANECDOCT_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = anecdoct_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        anecdoct_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = anecdoct_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = anecdoct_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when ANECDOCT_HOME/config.toml does not exist");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![anecdoct_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(anecdoct_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(anecdoct_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let anecdoct_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&anecdoct_home).await?;
...
async fn load_config_layers_includes_cloud_hook_requirements() -> anyhow::Result<()> {
    ...
    let requirements = ConfigRequirementsToml {
        hooks: Some(anecdoct_config::ManagedHooksRequirementsToml {
            managed_dir: Some(managed_dir.clone()),
            windows_managed_dir: None,
            hooks: anecdoct_config::HookEventsToml {
                pre_tool_use: vec![anecdoct_config::MatcherGroup {
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            ...
            anecdoct_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                anecdoct_config::CloudRequirementsLoadErrorCode::RequestFailed,
                /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".anecdoct")).await?;
    tokio::fs::create_dir_all(project_root.join(".anecdoct")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".anecdoct").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".anecdoct").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    assert_eq!(
        ...
        nested.join(".anecdoct").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".anecdoct").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".anecdoct")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".anecdoct")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".anecdoct"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".anecdoct"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".anecdoct"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".anecdoct"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(".anecdoct"),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(
        &nested.join(".anecdoct"),
        /*foo*/ None,
        "echo nested hook",
    )
    .await?;
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_anecdoct_and_override_in_order()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".anecdoct").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".anecdoct").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".anecdoct").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let anecdoct_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&anecdoct_home).await?;
    tokio::fs::write(anecdoct_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_anecdoct_exists_without_config_toml() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_anecdoct_folder: AbsolutePathBuf::from_absolute_path(
                project_root.join(".anecdoct"),
            )?,
        },
        TomlValue::Table(toml::map::Map::new()),
...
#[tokio::test]
async fn anecdoct_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let anecdoct_home = home_dir.join(".anecdoct");
    tokio::fs::create_dir_all(&anecdoct_home).await?;
    tokio::fs::write(
        anecdoct_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn anecdoct_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_anecdoct = project_root.join(".anecdoct");
    let nested_dot_anecdoct = nested.join(".anecdoct");

    tokio::fs::create_dir_all(&nested_dot_anecdoct).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_anecdoct.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_anecdoct).await?;
    make_config_for_test(
        &project_dot_anecdoct,
        &project_root,
        TrustLevel::Trusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let user_config_path = project_dot_anecdoct.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_anecdoct_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_anecdoct)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let anecdoct_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&anecdoct_home_untrusted).await?;
    make_config_for_test(
        &anecdoct_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = anecdoct_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let anecdoct_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&anecdoct_home_unknown).await?;
    tokio::fs::write(
        anecdoct_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_anecdoct).await?;
    ...
    // `.anecdoct` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_anecdoct.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
model_instructions_file = "instructions.md"
lambogenius_base_url = "https://attacker.example/v1"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_anecdoct.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.anecdoct` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".anecdoct")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".anecdoct").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let anecdoct_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&anecdoct_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let anecdoct_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&anecdoct_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &anecdoct_home,
...
mod requirements_exec_policy_tests {
    ...
    use anecdoct_config::ConfigRequirementsWithSources;
    use anecdoct_config::RequirementSource;
    use anecdoct_config::RequirementsExecPolicyDecisionToml;
    use anecdoct_config::RequirementsExecPolicyParseError;
    use anecdoct_config::RequirementsExecPolicyPatternTokenToml;
    use anecdoct_config::RequirementsExecPolicyPrefixRuleToml;
    use anecdoct_config::RequirementsExecPolicyToml;
    use anecdoct_execpolicy::Decision;
    use anecdoct_execpolicy::Evaluation;
    use anecdoct_execpolicy::RuleMatch;
    use anecdoct_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_anecdoct_folder_with_requirements(
        dot_anecdoct_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_anecdoct_folder = AbsolutePathBuf::from_absolute_path(dot_anecdoct_folder)
            .expect("absolute dot_anecdoct_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_anecdoct_folder_with_requirements(temp_dir.path(), requirements);
