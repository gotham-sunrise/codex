...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(anecdoct_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = anecdoct_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let anecdoct_home = tmp.path();
    let link_a = anecdoct_home.join("a.toml");
    let link_b = anecdoct_home.join("b.toml");
    let config_path = anecdoct_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(anecdoct_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
#[test]
fn blocking_set_hide_gpt_5_1_anecdoct_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let anecdoct_home = tmp.path();
    std::fs::write(
        anecdoct_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        ...
        /*profile*/ None,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-anecdoct-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-anecdoct-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
client_id = \"eci-prd-pub-anecdoct-123\"
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !anecdoct_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    contents = std::fs::read_to_string(anecdoct_home.join(CONFIG_TOML_FILE)).expect("read config");
