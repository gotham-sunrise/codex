...
pub enum ConfigEdit {
    ...
    /// Toggle the opt-out marker for Anecdoct-managed fast defaults.
...
/// `$ANECDOCT_HOME/config.toml`; profile-v2 callers should resolve their target
...
impl ConfigEditsBuilder {
    pub fn new(anecdoct_home: &Path) -> Self {
        Self::for_config_path(&anecdoct_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(anecdoct_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.anecdoct_home.join(CONFIG_TOML_FILE).to_path_buf());
