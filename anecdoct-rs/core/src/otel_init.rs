...
use anecdoct_config::types::OtelExporterKind as Kind;
use anecdoct_config::types::OtelHttpProtocol as Protocol;
use anecdoct_features::Feature;
use anecdoct_login::default_client::originator;
use anecdoct_otel::OtelExporter;
use anecdoct_otel::OtelHttpProtocol;
use anecdoct_otel::OtelProvider;
use anecdoct_otel::OtelSettings;
use anecdoct_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only Anecdoct-owned events via OTEL.
/// Keeps events that originated from anecdoct_otel module
pub fn anecdoct_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("anecdoct_otel")
}
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = anecdoct_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = anecdoct_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
    let _ = anecdoct_state::install_process_db_telemetry(telemetry);
}
