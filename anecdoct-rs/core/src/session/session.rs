...
pub(crate) struct SessionConfiguration {
    /// Provider identifier ("lambogenius", "openrouter", ...).
    ...
    /// Directory containing all Anecdoct state for this session.
    pub(super) anecdoct_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    pub(crate) fn anecdoct_home(&self) -> &AbsolutePathBuf {
        &self.anecdoct_home
    }
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        extensions: Arc<anecdoct_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let trace_agent_path = session_configuration
                .session_source
                .get_agent_path()
                .unwrap_or_else(anecdoct_protocol::AgentPath::root);
            let trace_task_name =
            ...
            let config_path = config.anecdoct_home.join(CONFIG_TOML_FILE);
            ...
            let auth_mode = auth.map(AnecdoctAuth::auth_mode).map(TelemetryAuthMode::from);
            let account_id = auth.and_then(AnecdoctAuth::get_account_id);
            let account_email = auth.and_then(AnecdoctAuth::get_account_email);
            let originator = originator().value;
            let terminal_type = user_agent();
            let session_model = session_configuration.collaboration_mode.model().to_string();
            let auth_env_telemetry = collect_auth_env_telemetry(
                &session_configuration.provider,
                auth_manager.anecdoct_api_key_env_enabled(),
            );
            ...
            let session_extension_data =
                anecdoct_extension_api::ExtensionData::new(session_id.to_string());
            let thread_extension_data =
                anecdoct_extension_api::ExtensionData::new(thread_id.to_string());
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(anecdoct_extension_api::ThreadStartInput {
            ...
            let host_owned_anecdoct_apps_enabled = config
                .features
                .apps_enabled_for_auth(auth.as_ref().is_some_and(|auth| auth.uses_anecdoct_backend()));
            ...
            .map_err(|err| {
                AnecdoctErr::InvalidRequest(err.to_string().replace(
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => anecdoct_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    anecdoct_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => anecdoct_hooks::SessionStartSource::Clear,
            };
