...
use anecdoct_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use anecdoct_sandboxing::policy_transforms::effective_network_sandbox_policy;
use std::sync::atomic::AtomicBool;
use std::sync::atomic::Ordering;

pub(super) fn image_generation_tool_auth_allowed(auth_manager: Option<&AuthManager>) -> bool {
    auth_manager.is_some_and(AuthManager::current_auth_uses_anecdoct_backend)
}
...
pub struct TurnContext {
    ...
    pub(crate) anecdoct_self_exe: Option<PathBuf>,
    pub(crate) anecdoct_linux_sandbox_exe: Option<PathBuf>,
    pub(crate) truncation_policy: TruncationPolicy,
    pub(crate) dynamic_tools: Vec<DynamicToolSpec>,
    pub(crate) turn_metadata_state: Arc<TurnMetadataState>,
    pub(crate) extension_data: Arc<anecdoct_extension_api::ExtensionData>,
...
impl TurnContext {
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        let uses_anecdoct_backend = self
            .auth_manager
            .as_deref()
            .is_some_and(AuthManager::current_auth_uses_anecdoct_backend);
        self.features.apps_enabled_for_auth(uses_anecdoct_backend)
    }
    ...
    pub(crate) async fn with_model(
        ...
        Self {
            ...
            anecdoct_self_exe: self.anecdoct_self_exe.clone(),
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        let extension_data = Arc::new(anecdoct_extension_api::ExtensionData::new(sub_id.clone()));
        TurnContext {
            sub_id,
            ...
            anecdoct_self_exe: per_turn_config.anecdoct_self_exe.clone(),
            anecdoct_linux_sandbox_exe: per_turn_config.anecdoct_linux_sandbox_exe.clone(),
    ...
    pub(crate) async fn new_turn_with_sub_id(
    ...
    ) -> AnecdoctResult<Arc<TurnContext>> {
        let notify_config_contributors = !self.services.extensions.config_contributors().is_empty();
        let update_result: AnecdoctResult<_> = {
            ...
            match state.session_configuration.clone().apply(&updates) {
                Ok(next) => {
                    ...
                    let anecdoct_home = next.anecdoct_home.clone();
                ...
                Err(err) => Err(AnecdoctErr::InvalidRequest(err.to_string())),
            }
        };
        ...
        let (
            ...
            Err(err) => {
                ...
                .await;
                return Err(AnecdoctErr::InvalidRequest(message));
            }
        };
    ...
    ) -> AnecdoctResult<ResolvedTurnEnvironments> {
