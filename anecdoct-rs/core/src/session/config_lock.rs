...
pub(crate) async fn validate_config_lock_if_configured(
    ...
    let options = ConfigLockReplayOptions {
        allow_anecdoct_version_mismatch: config.config_lock_allow_anecdoct_version_mismatch,
    };
...
/// Anecdoct actually ran with, not only the user-authored TOML inputs.
...
mod tests {
    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        for spec in anecdoct_features::FEATURES {
            assert_eq!(
    ...
    #[tokio::test]
    async fn lock_validation_rejects_anecdoct_version_mismatch_by_default() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.anecdoct_version = "older-version".to_string();
        ...
        let message = error.to_string();
        assert!(
            message.contains("config lock Anecdoct version mismatch"),
            "{message}"
        );
        assert!(
            message.contains("debug.config_lockfile.allow_anecdoct_version_mismatch=true"),
            "{message}"
        );
    }

    #[tokio::test]
    async fn lock_validation_can_ignore_anecdoct_version_mismatch() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.anecdoct_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");
