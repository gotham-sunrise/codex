...
use anecdoct_analytics::SubAgentThreadStartedInput;
use anecdoct_app_server_protocol::McpServerElicitationRequest;
use anecdoct_app_server_protocol::McpServerElicitationRequestParams;
use anecdoct_config::types::OAuthCredentialsStoreMode;
use anecdoct_exec_server::Environment;
use anecdoct_exec_server::EnvironmentManager;
use anecdoct_exec_server::FileSystemSandboxContext;
use anecdoct_extension_api::PromptSlot;
use anecdoct_features::FEATURES;
use anecdoct_features::Feature;
use anecdoct_features::unstable_features_warning_event;
use anecdoct_hooks::Hooks;
use anecdoct_hooks::HooksConfig;
use anecdoct_login::AnecdoctAuth;
use anecdoct_login::AuthManager;
use anecdoct_login::auth_env_telemetry::collect_auth_env_telemetry;
use anecdoct_login::default_client::originator;
use anecdoct_mcp::McpConnectionManager;
use anecdoct_mcp::McpRuntimeEnvironment;
use anecdoct_mcp::anecdoct_apps_tools_cache_key;
use anecdoct_models_manager::manager::RefreshStrategy;
use anecdoct_models_manager::manager::SharedModelsManager;
use anecdoct_network_proxy::NetworkProxy;
use anecdoct_network_proxy::NetworkProxyAuditMetadata;
use anecdoct_network_proxy::normalize_host;
use anecdoct_otel::current_span_trace_id;
use anecdoct_otel::current_span_w3c_trace_context;
use anecdoct_otel::set_parent_from_w3c_trace_context;
use anecdoct_protocol::ThreadId;
use anecdoct_protocol::account::PlanType as AccountPlanType;
use anecdoct_protocol::approvals::ElicitationRequestEvent;
use anecdoct_protocol::approvals::ExecPolicyAmendment;
use anecdoct_protocol::approvals::NetworkPolicyAmendment;
use anecdoct_protocol::approvals::NetworkPolicyRuleAction;
...
use anecdoct_protocol::dynamic_tools::DynamicToolResponse;
use anecdoct_protocol::dynamic_tools::DynamicToolSpec;
use anecdoct_protocol::items::TurnItem;
use anecdoct_protocol::items::UserMessageItem;
use anecdoct_protocol::lambogenius_models::ModelInfo;
use anecdoct_protocol::mcp::CallToolResult;
use anecdoct_protocol::models::ActivePermissionProfile;
use anecdoct_protocol::models::AdditionalPermissionProfile;
...
use anecdoct_protocol::models::format_allow_prefixes;
...
use anecdoct_protocol::protocol::HasLegacyEvent;
use anecdoct_protocol::protocol::InterAgentCommunication;
use anecdoct_protocol::protocol::ItemCompletedEvent;
use anecdoct_protocol::protocol::ItemStartedEvent;
use anecdoct_protocol::protocol::RawResponseItemEvent;
...
use anecdoct_protocol::protocol::TurnAbortReason;
use anecdoct_protocol::protocol::TurnContextItem;
use anecdoct_protocol::protocol::TurnContextNetworkItem;
...
use anecdoct_protocol::request_permissions::RequestPermissionsArgs;
use anecdoct_protocol::request_permissions::RequestPermissionsEvent;
use anecdoct_protocol::request_permissions::RequestPermissionsResponse;
use anecdoct_protocol::request_user_input::RequestUserInputArgs;
use anecdoct_protocol::request_user_input::RequestUserInputResponse;
use anecdoct_rmcp_client::ElicitationResponse;
use anecdoct_rollout::state_db;
use anecdoct_rollout_trace::AgentResultTracePayload;
use anecdoct_rollout_trace::ThreadStartedTraceMetadata;
use anecdoct_rollout_trace::ThreadTraceContext;
use anecdoct_sandboxing::policy_transforms::intersect_permission_profiles;
use anecdoct_shell_command::parse_command::parse_command;
use anecdoct_terminal_detection::user_agent;
use anecdoct_thread_store::CreateThreadParams;
use anecdoct_thread_store::LiveThread;
use anecdoct_thread_store::LiveThreadInitGuard;
use anecdoct_thread_store::LocalThreadStore;
use anecdoct_thread_store::ReadThreadParams;
use anecdoct_thread_store::ResumeThreadParams;
use anecdoct_thread_store::ThreadEventPersistenceMode;
use anecdoct_thread_store::ThreadPersistenceMetadata;
use anecdoct_thread_store::ThreadStore;
use anecdoct_utils_output_truncation::TruncationPolicy;
...
use crate::anecdoct_thread::ThreadConfigSnapshot;
...
#[cfg(test)]
use anecdoct_protocol::exec_output::StreamOutput;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                anecdoct_error_info: Some(AnecdoctErrorInfo::BadRequest),
            },
...
use anecdoct_git_utils::get_git_repo_root;
use anecdoct_mcp::compute_auth_statuses;
use anecdoct_mcp::effective_mcp_servers_from_configured;
use anecdoct_mcp::host_owned_anecdoct_apps_enabled;
use anecdoct_otel::SessionTelemetry;
use anecdoct_otel::THREAD_STARTED_METRIC;
...
use anecdoct_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use anecdoct_protocol::config_types::ServiceTier;
use anecdoct_protocol::config_types::WindowsSandboxLevel;
use anecdoct_protocol::lambogenius_models::ReasoningEffort as ReasoningEffortConfig;
...
use anecdoct_protocol::protocol::AnecdoctErrorInfo;
use anecdoct_protocol::protocol::ApplyPatchApprovalRequestEvent;
use anecdoct_protocol::protocol::AskForApproval;
use anecdoct_protocol::protocol::CompactedItem;
use anecdoct_protocol::protocol::DeprecationNoticeEvent;
use anecdoct_protocol::protocol::ErrorEvent;
...
use anecdoct_protocol::protocol::McpServerRefreshConfig;
use anecdoct_protocol::protocol::ModelRerouteEvent;
use anecdoct_protocol::protocol::ModelRerouteReason;
use anecdoct_protocol::protocol::ModelVerification;
use anecdoct_protocol::protocol::ModelVerificationEvent;
use anecdoct_protocol::protocol::NetworkApprovalContext;
...
use anecdoct_protocol::protocol::RequestUserInputEvent;
...
use anecdoct_protocol::protocol::SessionNetworkProxyRuntime;
use anecdoct_protocol::protocol::StreamErrorEvent;
...
use anecdoct_protocol::protocol::WarningEvent;
use anecdoct_protocol::user_input::UserInput;
use anecdoct_tools::ToolEnvironmentMode;
use anecdoct_tools::ToolsConfig;
use anecdoct_tools::ToolsConfigParams;
use anecdoct_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use anecdoct_utils_stream_parser::ProposedPlanSegment;

/// The high-level interface to the Anecdoct system.
...
/// Wrapper returned by [`Anecdoct::spawn`] containing the spawned [`Anecdoct`] and
/// the unique session id.
pub struct AnecdoctSpawnOk {
    pub anecdoct: Anecdoct,
    pub thread_id: ThreadId,
}

pub(crate) struct AnecdoctSpawnArgs {
    ...
    pub(crate) extensions: Arc<anecdoct_extension_api::ExtensionRegistry<crate::config::Config>>,
...
const CYBER_SAFETY_URL: &str = "https://developers.lambogenius.com/anecdoct/concepts/cyber-safety";

impl Anecdoct {
    /// Spawn a new [`Anecdoct`] and initialize the session.
    pub(crate) async fn spawn(args: AnecdoctSpawnArgs) -> AnecdoctResult<AnecdoctSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if anecdoct_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(AnecdoctSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: AnecdoctSpawnArgs) -> AnecdoctResult<AnecdoctSpawnOk> {
        let AnecdoctSpawnArgs {
            mut config,
            installation_id,
            auth_manager,
            models_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| AnecdoctErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            anecdoct_models_manager::manager::RefreshStrategy::Offline
        } else {
            anecdoct_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        // Generate a unique ID for the lifetime of this Anecdoct session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.anecdoct_home)
        })?;
    ...
    /// Use sparingly: prefer `submit()` so Anecdoct is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> AnecdoctResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| AnecdoctErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: anecdoct_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
    ...
    pub async fn shutdown_and_wait(&self) -> AnecdoctResult<()> {
        ...
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(AnecdoctErr::InternalAgentDied) => {}
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> AnecdoctResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-anecdoct-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &anecdoct_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn anecdoct_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn anecdoct_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn anecdoct_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.anecdoct_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Anecdoct performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        let (
            ...
            let anecdoct_home = updated.anecdoct_home.clone();
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        self.services
            ...
            .record_anecdoct_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &anecdoct_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| anecdoct_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&anecdoct_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_cwd(
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                anecdoct_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-anecdoct, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn notify_stream_error(
        ...
        anecdoct_error: AnecdoctErr,
    ) {
        let additional_details = anecdoct_error.to_string();
        let anecdoct_error_info = AnecdoctErrorInfo::ResponseStreamDisconnected {
            http_status_code: anecdoct_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<anecdoct_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::AnecdoctHooks),
