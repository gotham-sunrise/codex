...
use anecdoct_config::ConfigLayerStackOrdering;
use anecdoct_config::LoaderOverrides;
use anecdoct_config::NetworkConstraints;
use anecdoct_config::NetworkDomainPermissionToml;
use anecdoct_config::NetworkDomainPermissionsToml;
use anecdoct_config::RequirementSource;
use anecdoct_config::Sourced;
...
use anecdoct_model_provider_info::ModelProviderInfo;
use anecdoct_models_manager::bundled_models_response;
use anecdoct_models_manager::model_info;
use anecdoct_models_manager::test_support::construct_model_info_offline_for_tests;
use anecdoct_models_manager::test_support::get_model_offline_for_tests;
use anecdoct_protocol::AgentPath;
use anecdoct_protocol::SessionId;
use anecdoct_protocol::ThreadId;
use anecdoct_protocol::account::PlanType as AccountPlanType;
use anecdoct_protocol::config_types::ServiceTier;
...
use anecdoct_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use anecdoct_protocol::models::FileSystemPermissions;
use anecdoct_protocol::models::FunctionCallOutputBody;
use anecdoct_protocol::models::FunctionCallOutputPayload;
use anecdoct_protocol::models::PermissionProfile;
use anecdoct_protocol::models::SandboxEnforcement;
...
use anecdoct_protocol::protocol::NonSteerableTurnKind;
use anecdoct_protocol::protocol::SandboxPolicy;
use anecdoct_protocol::protocol::TurnEnvironmentSelection;
use anecdoct_protocol::request_permissions::PermissionGrantScope;
use anecdoct_protocol::request_permissions::RequestPermissionProfile;
use tracing::Span;
...
use anecdoct_app_server_protocol::McpElicitationSchema;
use anecdoct_config::config_toml::ConfigToml;
use anecdoct_config::config_toml::ProjectConfig;
use anecdoct_execpolicy::Decision;
use anecdoct_execpolicy::NetworkRuleProtocol;
use anecdoct_execpolicy::Policy;
use anecdoct_network_proxy::NetworkProxyConfig;
use anecdoct_otel::MetricsClient;
use anecdoct_otel::MetricsConfig;
use anecdoct_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use anecdoct_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use anecdoct_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use anecdoct_otel::THREAD_SKILLS_TRUNCATED_METRIC;
use anecdoct_otel::TelemetryAuthMode;
...
use anecdoct_protocol::models::BaseInstructions;
...
use anecdoct_protocol::protocol::CompactedItem;
use anecdoct_protocol::protocol::ConversationAudioParams;
use anecdoct_protocol::protocol::CreditsSnapshot;
use anecdoct_protocol::protocol::GranularApprovalConfig;
use anecdoct_protocol::protocol::InitialHistory;
use anecdoct_protocol::protocol::InterAgentCommunication;
use anecdoct_protocol::protocol::NetworkApprovalProtocol;
use anecdoct_protocol::protocol::RateLimitSnapshot;
use anecdoct_protocol::protocol::RateLimitWindow;
use anecdoct_protocol::protocol::RealtimeAudioFrame;
use anecdoct_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use anecdoct_protocol::protocol::RealtimeVoice;
...
use anecdoct_protocol::protocol::SkillScope;
use anecdoct_protocol::protocol::Submission;
use anecdoct_protocol::protocol::ThreadGoalStatus;
use anecdoct_protocol::protocol::ThreadRolledBackEvent;
use anecdoct_protocol::protocol::TokenCountEvent;
use anecdoct_protocol::protocol::TokenUsage;
use anecdoct_protocol::protocol::TokenUsageInfo;
use anecdoct_protocol::protocol::TurnAbortedEvent;
use anecdoct_protocol::protocol::TurnCompleteEvent;
use anecdoct_protocol::protocol::TurnStartedEvent;
use anecdoct_protocol::protocol::UserMessageEvent;
use anecdoct_protocol::protocol::W3cTraceContext;
use anecdoct_protocol::request_user_input::RequestUserInputAnswer;
use anecdoct_protocol::request_user_input::RequestUserInputResponse;
use anecdoct_rmcp_client::ElicitationAction;
...
use anecdoct_protocol::mcp::CallToolResult as McpCallToolResult;
...
fn test_session_telemetry_without_metadata() -> SessionTelemetry {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "anecdoct-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
    ...
    let response = session
        .request_mcp_server_elicitation(
            ...
            McpServerElicitationRequestParams {
                ...
                server_name: "anecdoct_apps".to_string(),
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        ModelProviderInfo::create_lambogenius_provider(/* base_url */ /*base_url*/ None),
        anecdoct_protocol::protocol::SessionSource::Exec,
...
fn write_project_hooks(dot_anecdoct: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_anecdoct)?;
    std::fs::write(
        dot_anecdoct.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        anecdoct_home.join(anecdoct_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<anecdoct_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&anecdoct_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            source: anecdoct_hooks::SessionStartSource::Startup,
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn anecdoct_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { anecdoct_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> anecdoct_sandboxing::SandboxablePreference {
            anecdoct_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(anecdoct_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: anecdoct_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let anecdoct_home = session.anecdoct_home().await;
    std::fs::create_dir_all(&anecdoct_home).expect("create anecdoct home");
    let config_toml_path = anecdoct_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = anecdoct_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let anecdoct_home = session.anecdoct_home().await;
    std::fs::create_dir_all(&anecdoct_home).expect("create anecdoct home");
    let base_config_path = anecdoct_home.join(CONFIG_TOML_FILE);
    let profile_config_path = anecdoct_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(anecdoct_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            .features
            .enable(Feature::AnecdoctHooks)
            .expect("enable Anecdoct hooks");
    })
    .await?;
    let anecdoct_home = session.anecdoct_home().await;
    std::fs::create_dir_all(&anecdoct_home)?;
    let config_toml_path = anecdoct_home.join(CONFIG_TOML_FILE);
    let user_config: anecdoct_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = anecdoct_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = anecdoct_hooks::list_hooks(anecdoct_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..anecdoct_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        anecdoct_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: anecdoct_config::TomlValue =
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: anecdoct_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: anecdoct_config::MatcherGroup {
                matcher: None,
                hooks: vec![anecdoct_config::HookHandlerConfig::Command {
        ...
        let identity = anecdoct_config::TomlValue::try_from(identity)?;
        anecdoct_config::version_for_toml(&identity)
    };
...
disabled_tools = [
  ...
  { type = "plugin", id = "slack@lambogenius-curated" },
]
"#,
    )
    .expect("write user config");
...
fn filter_connectors_for_input_skips_plugin_mentions() {
    ...
    let input = vec![user_message(
        "use [@figma](plugin://figma@lambogenius-curated)",
    )];
    let explicitly_enabled_connectors = HashSet::new();
    let selected = filter_connectors_for_input(
        &connectors,
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl anecdoct_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage(
            &self,
            session_store: &anecdoct_extension_api::ExtensionData,
            thread_store: &anecdoct_extension_api::ExtensionData,
            turn_store: &anecdoct_extension_api::ExtensionData,
            token_usage: &TokenUsageInfo,
        ) {
            self.records
                .lock()
    ...
    let mut builder =
        anecdoct_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl anecdoct_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: anecdoct_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_anecdoct().with_config(move |config| {
        config
            .permissions
...
async fn session_permission_profile_rebinds_runtime_workspace_roots() -> anyhow::Result<()> {
    let anecdoct_home = tempfile::TempDir::new()?;
    ...
    let config = ConfigBuilder::default()
        .anecdoct_home(anecdoct_home.path().to_path_buf())
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_anecdoct().with_config(|config| {
        config.permissions.approval_policy =
            anecdoct_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        anecdoct_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "anecdoct_core__anecdoct_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            anecdoct_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            anecdoct_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            anecdoct_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.anecdoct_error_info,
        Some(AnecdoctErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    let anecdoct_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(anecdoct_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(anecdoct_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("anecdoct_other".to_string()),
        limit_name: Some("anecdoct_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(anecdoct_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("anecdoct".to_string()),
            limit_name: None,
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.anecdoct_error_info == Some(AnecdoctErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(anecdoct_home: &Path) -> Config {
...
fn turn_environments_for_tests(
    environment: &Arc<anecdoct_exec_server::Environment>,
    cwd: &anecdoct_utils_absolute_path::AbsolutePathBuf,
) -> crate::environment_selection::ResolvedTurnEnvironments {
    crate::environment_selection::ResolvedTurnEnvironments {
        turn_environments: vec![TurnEnvironment {
            environment_id: anecdoct_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = anecdoct_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        anecdoct_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let anecdoct_home = parent_config.anecdoct_home.clone();
    let skill_dir = anecdoct_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&anecdoct_exec_server::LOCAL_FS));
    ...
    let role_path = anecdoct_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    let anecdoct_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(anecdoct_home.path()).await;
    config
        .features
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(AnecdoctAuth::from_api_key("Test API Key"));
    let models_manager = models_manager_with_provider(
        config.anecdoct_home.to_path_buf(),
        auth_manager.clone(),
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.anecdoct_home.to_path_buf()));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(anecdoct_extension_api::ExtensionRegistryBuilder::new().build()),
        AgentControl::default(),
        Arc::new(anecdoct_exec_server::EnvironmentManager::default_for_tests()),
        /*analytics_events_client*/ None,
        Arc::new(anecdoct_thread_store::LocalThreadStore::new(
            anecdoct_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        anecdoct_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        anecdoct_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: anecdoct_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(anecdoct_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: anecdoct_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: anecdoct_extension_api::ExtensionData::new(thread_id.to_string()),
        agent_control,
        network_proxy: None,
        network_approval: Arc::clone(&network_approval),
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(anecdoct_thread_store::LocalThreadStore::new(
            anecdoct_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
        ...
        environment_manager: Arc::new(anecdoct_exec_server::EnvironmentManager::default_for_tests()),
    };
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    ...
    ConfigBuilder::default()
        .anecdoct_home(config.anecdoct_home.to_path_buf())
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(anecdoct_thread_store::LocalThreadStore::new(
            anecdoct_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                anecdoct_state::StateRuntime::init(
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            anecdoct_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(anecdoct_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = anecdoct_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(anecdoct_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    &turn_context,
                    call_id,
                    anecdoct_protocol::request_permissions::RequestPermissionsArgs {
...
async fn submit_with_id_captures_current_span_trace_context() {
    ...
    let _trace_test_context = install_test_tracing("anecdoct-core-tests");
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserTurn {
            ...
            approvals_reviewer: Some(anecdoct_config::types::ApprovalsReviewer::AutoReview),
    ...
    assert_eq!(
        ...
        anecdoct_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn unknown_turn_environment_returns_error() {
    ...
    assert!(matches!(err, AnecdoctErr::InvalidRequest(_)));
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        anecdoct_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context =
        anecdoct_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(anecdoct_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn anecdoct_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        anecdoct_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_emits_thread_stop_lifecycle() {
    ...
    impl anecdoct_extension_api::ThreadLifecycleContributor<crate::config::Config>
        for ThreadStopRecorder
    {
        fn on_thread_stop(&self, input: anecdoct_extension_api::ThreadStopInput<'_>) {
            assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl anecdoct_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort(&self, input: anecdoct_extension_api::TurnAbortInput<'_>) {
            assert_eq!(
                self.expected_thread_id.to_string(),
                input.thread_store.level_id()
            );
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let waiter_1 = {
        let anecdoct = Arc::clone(&anecdoct);
        tokio::spawn(async move { anecdoct.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let anecdoct = Arc::clone(&anecdoct);
        tokio::spawn(async move { anecdoct.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_anecdoct)
        .await;
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(anecdoct_home).await;
...
pub(crate) async fn make_session_and_context_with_dynamic_tools_and_rx(
    ...
    make_session_and_context_with_auth_and_config_and_rx(
        AnecdoctAuth::from_api_key("Test API Key"),
        dynamic_tools,
        |_config| {},
    )
    .await
...
async fn make_goal_session_and_context_with_rx() -> (
    ...
    (session, turn_context, rx, anecdoct_home)
}
...
async fn upsert_goal_test_thread(session: &Session) {
    ...
    let mut builder = anecdoct_state::ThreadMetadataBuilder::new(
...
impl anecdoct_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute<'a>(
        &'a self,
        _session_store: &'a anecdoct_extension_api::ExtensionData,
        thread_store: &'a anecdoct_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<anecdoct_extension_api::PromptFragment>>
                + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    anecdoct_extension_api::PromptFragment::developer_policy(
...
-> Arc<anecdoct_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = anecdoct_extension_api::ExtensionRegistryBuilder::new();
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let expected_saved_path = crate::stream_events_utils::image_generation_artifact_path(
        &turn_context.config.anecdoct_home,
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(anecdoct_extension_api::ExtensionData::new(
...
async fn interrupt_accounts_active_goal_before_pausing() -> anyhow::Result<()> {
    let (sess, tc, _rx, _anecdoct_home) = make_goal_session_and_context_with_rx().await;
    ...
    assert_eq!(
        anecdoct_protocol::protocol::ThreadGoalStatus::Paused,
        goal.status
    );
...
async fn active_goal_continuation_runs_again_after_no_tool_turn() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(8), async {
        loop {
            let event = test.anecdoct.next_event().await?;
...
async fn pending_request_user_input_does_not_spawn_extra_goal_continuation() -> anyhow::Result<()> {
    ...
    let request_user_input_event = wait_for_event_match(&test.anecdoct, |event| match event {
    ...
    assert!(
        timeout(Duration::from_millis(200), test.anecdoct.next_event())
            .await
            .is_err(),
...
async fn goal_test_state_db(sess: &Session) -> anyhow::Result<crate::StateDbHandle> {
    ...
    anecdoct_state::StateRuntime::init(config.sqlite_home.clone(), config.model_provider_id.clone())
        .await
}

#[tokio::test]
async fn budget_limited_accounting_steers_active_turn_without_aborting() -> anyhow::Result<()> {
    let (sess, tc, rx, _anecdoct_home) = make_goal_session_and_context_with_rx().await;
    sess.set_thread_goal(
        tc.as_ref(),
        SetGoalRequest {
            objective: Some("Keep improving the benchmark".to_string()),
    ...
    assert_eq!(anecdoct_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn external_goal_mutation_accounts_active_turn_before_status_change() -> anyhow::Result<()> {
    ...
    let updated_goal = state_db
        .update_thread_goal(
            ...
            anecdoct_state::ThreadGoalUpdate {
                objective: None,
                status: Some(anecdoct_state::ThreadGoalStatus::Complete),
                token_budget: None,
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(anecdoct_state::ThreadGoalStatus::Complete, goal.status);
    assert_eq!(70, goal.tokens_used);
...
async fn external_objective_change_steers_active_turn() -> anyhow::Result<()> {
    ...
    let old_goal = state_db
        .replace_thread_goal(
            ...
            anecdoct_state::ThreadGoalStatus::Active,
...
async fn external_active_goal_set_marks_current_turn_for_accounting() -> anyhow::Result<()> {
    ...
    let goal = state_db
        ...
        .await?
        .expect("goal should remain persisted");
    assert_eq!(anecdoct_state::ThreadGoalStatus::Active, goal.status);
    assert_eq!(25, goal.tokens_used);
...
async fn completed_goal_accounts_current_turn_tokens_before_tool_response() -> anyhow::Result<()> {
    ...
    let state_db = anecdoct_state::StateRuntime::init(
    ...
    assert_eq!(
        anecdoct_state::ThreadGoalStatus::Complete,
...
async fn create_goal_tool_rejects_existing_goal() {
    let (session, turn_context, _rx, _anecdoct_home) =
    ...
    handler
        .handle(ToolInvocation {
            ...
            tool_name: anecdoct_tools::ToolName::plain("create_goal"),
...
async fn update_goal_tool_rejects_pausing_goal() {
    ...
    let response = update_handler
        .handle(ToolInvocation {
            ...
            tool_name: anecdoct_tools::ToolName::plain("update_goal"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "status": "paused",
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use anecdoct_protocol::protocol::AskForApproval;
    use anecdoct_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: anecdoct_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: anecdoct_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let anecdoct_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_anecdoct = project_root.join(".anecdoct");
    let nested_dot_anecdoct = nested.join(".anecdoct");

    std::fs::create_dir_all(&anecdoct_home)?;
    std::fs::create_dir_all(&nested_dot_anecdoct)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_anecdoct)?;
    write_project_hooks(&nested_dot_anecdoct)?;
    write_project_trust_config(&anecdoct_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .anecdoct_home(anecdoct_home)
    ...
    let expected_source_path = anecdoct_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_anecdoct.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_anecdoct = project_root.join(".anecdoct");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_anecdoct)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let anecdoct_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&anecdoct_home)?;
        write_project_trust_config(&anecdoct_home, &trust_entries).await?;
