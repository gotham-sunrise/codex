...
use anecdoct_analytics::CompactionPhase;
use anecdoct_analytics::CompactionReason;
use anecdoct_analytics::InvocationType;
use anecdoct_analytics::TurnResolvedConfigFact;
use anecdoct_analytics::build_track_events_context;
use anecdoct_async_utils::OrCancelExt;
use anecdoct_features::Feature;
use anecdoct_git_utils::get_git_repo_root;
use anecdoct_hooks::HookEvent;
use anecdoct_hooks::HookEventAfterAgent;
use anecdoct_hooks::HookPayload;
use anecdoct_hooks::HookResult;
...
use anecdoct_protocol::items::PlanItem;
...
use anecdoct_protocol::protocol::AgentMessageContentDeltaEvent;
use anecdoct_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use anecdoct_protocol::protocol::PlanDeltaEvent;
use anecdoct_protocol::protocol::ReasoningContentDeltaEvent;
use anecdoct_protocol::protocol::ReasoningRawContentDeltaEvent;
...
use anecdoct_tools::filter_request_plugin_install_discoverable_tools_for_client;
use anecdoct_utils_stream_parser::AssistantTextChunk;
use anecdoct_utils_stream_parser::AssistantTextStreamParser;
use anecdoct_utils_stream_parser::ProposedPlanSegment;
use anecdoct_utils_stream_parser::extract_proposed_plan_text;
use anecdoct_utils_stream_parser::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<anecdoct_extension_api::ExtensionData>,
    input: Vec<UserInput>,
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = anecdoct_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
    ...
    // Although from the perspective of anecdoct.rs, TurnDiffTracker has the lifecycle of a Task which contains
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = anecdoct_hooks::StopRequest {
                    ...
                    for run in hooks.preview_stop(&stop_request) {
                        sess.send_event(
                            &turn_context,
                            EventMsg::HookStarted(anecdoct_protocol::protocol::HookStartedEvent {
            ...
            Err(AnecdoctErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(AnecdoctErr::InvalidImageRequest()) => {
                {
...
) -> AnecdoctResult<PreSamplingCompactResult> {
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = anecdoct_connectors::metadata::connector_mention_slug(connector);
...
fn connector_inserted_in_messages(
    ...
    let mention_slug = anecdoct_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
    turn_store: Arc<anecdoct_extension_api::ExtensionData>,
    ...
    cancellation_token: CancellationToken,
) -> AnecdoctResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(AnecdoctErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(AnecdoctErr::ContextWindowExceeded);
            }
            Err(AnecdoctErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(AnecdoctErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                AnecdoctErr::Stream(_, requested_delay) => {
...
fn agent_message_text(item: &anecdoct_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            anecdoct_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: anecdoct_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(anecdoct_protocol::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, AnecdoctResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> AnecdoctResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, AnecdoctResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: AnecdoctResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            anecdoct.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            anecdoct.usage.reasoning_output_tokens = field::Empty,
            anecdoct.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(anecdoct_async_utils::CancelErr::Cancelled) => break Err(AnecdoctErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![anecdoct_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(AnecdoctErr::TurnAborted);
    }
