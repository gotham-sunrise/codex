...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.anecdoct_home = temp_dir.path().join("anecdoct-home").abs();
    config.cwd = config.anecdoct_home.abs();
    std::fs::create_dir_all(&config.anecdoct_home).expect("create anecdoct home");
...
#[tokio::test]
async fn start_thread_uses_all_default_environments_from_anecdoct_home() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.anecdoct_home = temp_dir.path().join("anecdoct-home").abs();
    config.cwd = config.anecdoct_home.abs();
    std::fs::create_dir_all(&config.anecdoct_home).expect("create anecdoct home");
    std::fs::write(
        config.anecdoct_home.join("environments.toml"),
        r#"
default = "dev"
...
"#,
    ...
    .expect("runtime paths");
    let environment_manager = Arc::new(
        anecdoct_exec_server::EnvironmentManager::from_anecdoct_home(
    ...
    let prompt_items = crate::prompt_debug::build_prompt_input_from_session(
        thread.thread.anecdoct.session.as_ref(),
        Vec::<UserInput>::new(),
    )
    .await
    .expect("prompt input");
...
#[tokio::test]
async fn explicit_installation_id_skips_anecdoct_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.anecdoct_home = temp_dir.path().join("anecdoct-home").abs();
    config.cwd = config.anecdoct_home.abs();
    ...
    assert!(!config.anecdoct_home.join(INSTALLATION_ID_FILENAME).exists());
    assert_eq!(
        thread.thread.anecdoct.session.installation_id,
        installation_id
    );
...
async fn resumed_thread_keeps_paused_goal_paused() -> anyhow::Result<()> {
    ...
    assert_eq!(anecdoct_state::ThreadGoalStatus::Paused, goal.status);
    assert!(
        resumed
            .thread
            .anecdoct
