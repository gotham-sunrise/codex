...
use anecdoct_config::ConfigEditsBuilder;
use anecdoct_config::McpServerConfig;
use anecdoct_config::McpServerTransportConfig;
use anecdoct_config::load_global_mcp_servers;
use anecdoct_login::default_client::is_first_party_originator;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(anecdoct_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&anecdoct_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&anecdoct_home)
