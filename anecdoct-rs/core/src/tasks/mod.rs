...
use anecdoct_analytics::TurnTokenUsageFact;
use anecdoct_login::AuthManager;
use anecdoct_models_manager::manager::SharedModelsManager;
use anecdoct_otel::SessionTelemetry;
use anecdoct_otel::TURN_E2E_DURATION_METRIC;
use anecdoct_otel::TURN_MEMORY_METRIC;
use anecdoct_otel::TURN_NETWORK_PROXY_METRIC;
use anecdoct_otel::TURN_TOKEN_USAGE_METRIC;
use anecdoct_otel::TURN_TOOL_CALL_METRIC;
...
///
/// Implementations encapsulate a specific Anecdoct workflow (regular chat,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            anecdoct.turn.reasoning_effort = %reasoning_effort,
            anecdoct.turn.token_usage.input_tokens = field::Empty,
            anecdoct.turn.token_usage.cached_input_tokens = field::Empty,
            anecdoct.turn.token_usage.non_cached_input_tokens = field::Empty,
            anecdoct.turn.token_usage.output_tokens = field::Empty,
            anecdoct.turn.token_usage.reasoning_output_tokens = field::Empty,
            anecdoct.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Anecdoct will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub async fn on_task_finished(
        ...
        if let Some(token_usage_at_turn_start) = token_usage_at_turn_start {
            ...
            if records_turn_token_usage_on_span {
                ...
                current_span.record(
                    "anecdoct.turn.token_usage.input_tokens",
                    turn_token_usage.input_tokens,
                );
                current_span.record(
                    "anecdoct.turn.token_usage.cached_input_tokens",
                    turn_token_usage.cached_input(),
                );
                current_span.record(
                    "anecdoct.turn.token_usage.non_cached_input_tokens",
                    turn_token_usage.non_cached_input(),
                );
                current_span.record(
                    "anecdoct.turn.token_usage.output_tokens",
                    turn_token_usage.output_tokens,
                );
                current_span.record(
                    "anecdoct.turn.token_usage.reasoning_output_tokens",
                    turn_token_usage.reasoning_output_tokens,
                );
                current_span.record(
                    "anecdoct.turn.token_usage.total_tokens",
