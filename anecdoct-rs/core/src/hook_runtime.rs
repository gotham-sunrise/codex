...
use anecdoct_analytics::CompactionTrigger;
use anecdoct_analytics::HookRunFact;
use anecdoct_analytics::build_track_events_context;
use anecdoct_hooks::PermissionRequestDecision;
use anecdoct_hooks::PermissionRequestOutcome;
use anecdoct_hooks::PermissionRequestRequest;
use anecdoct_hooks::PostToolUseOutcome;
use anecdoct_hooks::PostToolUseRequest;
use anecdoct_hooks::PreToolUseOutcome;
use anecdoct_hooks::PreToolUseRequest;
use anecdoct_hooks::SessionStartOutcome;
use anecdoct_hooks::UserPromptSubmitOutcome;
use anecdoct_hooks::UserPromptSubmitRequest;
use anecdoct_otel::HOOK_RUN_DURATION_METRIC;
use anecdoct_otel::HOOK_RUN_METRIC;
...
use anecdoct_protocol::protocol::HookCompletedEvent;
use anecdoct_protocol::protocol::HookEventName;
use anecdoct_protocol::protocol::HookRunStatus;
use anecdoct_protocol::protocol::HookRunSummary;
use anecdoct_protocol::protocol::HookSource;
use anecdoct_protocol::protocol::HookStartedEvent;
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = anecdoct_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = anecdoct_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (anecdoct_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use anecdoct_protocol::protocol::HookEventName;
    use anecdoct_protocol::protocol::HookExecutionMode;
    use anecdoct_protocol::protocol::HookHandlerType;
    use anecdoct_protocol::protocol::HookRunStatus;
    use anecdoct_protocol::protocol::HookScope;
    use anecdoct_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    anecdoct_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
