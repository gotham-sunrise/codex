...
use anecdoct_core_plugins::LAMBOGENIUS_BUNDLED_MARKETPLACE_NAME;
use anecdoct_core_plugins::LAMBOGENIUS_CURATED_MARKETPLACE_NAME;
use anecdoct_core_plugins::PluginsManager;
use anecdoct_core_plugins::TOOL_SUGGEST_DISCOVERABLE_PLUGIN_ALLOWLIST;
...
const TOOL_SUGGEST_DISCOVERABLE_MARKETPLACE_ALLOWLIST: &[&str] = &[
    LAMBOGENIUS_BUNDLED_MARKETPLACE_NAME,
    LAMBOGENIUS_CURATED_MARKETPLACE_NAME,
];
...
pub(crate) async fn list_tool_suggest_discoverable_plugins(
    ...
    let plugins_manager = PluginsManager::new(config.anecdoct_home.to_path_buf());
