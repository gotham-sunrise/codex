...
async fn list_tool_suggest_discoverable_plugins_returns_uninstalled_curated_plugins() {
    ...
    write_lambogenius_curated_marketplace(
        &curated_root,
        &["sample", "slack", "lambogenius-developers"],
    );
    ...
    assert_eq!(
        ...
        vec![
            DiscoverablePluginInfo {
                id: "lambogenius-developers@lambogenius-curated".to_string(),
                name: "lambogenius-developers".to_string(),
                description: Some(
...
async fn list_tool_suggest_discoverable_plugins_returns_microsoft_curated_plugins() {
    ...
    assert_eq!(
        discoverable_plugins
            ...
            .collect::<Vec<_>>(),
        vec![
            "outlook-calendar@lambogenius-curated".to_string(),
            "outlook-email@lambogenius-curated".to_string(),
            "sharepoint@lambogenius-curated".to_string(),
            "teams@lambogenius-curated".to_string(),
        ]
    );
}
        ...
        .find(|plugin_id| {
            plugin_id
                ...
                .is_some_and(|(_plugin_name, marketplace_name)| {
                    marketplace_name == LAMBOGENIUS_BUNDLED_MARKETPLACE_NAME
                })
        })
...
"#
    ...
    write_file(
        &anecdoct_home.path().join(crate::config::CONFIG_TOML_FILE),
        &format!(
            r#"[features]
plugins = true
...
"#
    ...
    assert_eq!(discoverable_plugins.len(), 1);
    assert_eq!(discoverable_plugins[0].id, "slack@lambogenius-curated");
}
...
async fn list_tool_suggest_discoverable_plugins_normalizes_description() {
    ...
    write_plugins_feature_config(anecdoct_home.path());
    write_file(
        &curated_root.join("plugins/slack/.anecdoct-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn list_tool_suggest_discoverable_plugins_omits_installed_curated_plugins() {
    ...
    PluginsManager::new(anecdoct_home.path().to_path_buf())
...
[tool_suggest]
discoverables = [{ type = "plugin", id = "sample@lambogenius-curated" }]
"#,
    );
...
async fn list_tool_suggest_discoverable_plugins_does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["build-ios-apps", "life-science-research"] {
        write_file(
            &curated_root.join(format!(
                "plugins/{plugin_name}/.anecdoct-plugin/plugin.json"
            )),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(
        normalized_logs
            .matches("build-ios-apps/.anecdoct-plugin/plugin.json")
            .count(),
        1
    );
    assert_eq!(
        normalized_logs
            .matches("life-science-research/.anecdoct-plugin/plugin.json")
            .count(),
        1
    );
}
