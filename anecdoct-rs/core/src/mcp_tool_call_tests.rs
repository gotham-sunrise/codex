...
use anecdoct_config::types::AppConfig;
use anecdoct_config::types::AppToolConfig;
use anecdoct_config::types::AppToolsConfig;
use anecdoct_config::types::ApprovalsReviewer;
use anecdoct_config::types::AppsConfigToml;
...
use anecdoct_rollout_trace::ToolDispatchInvocation;
use anecdoct_rollout_trace::ToolDispatchPayload;
use anecdoct_rollout_trace::ToolDispatchRequester;
...
fn approval_metadata(
    ...
    McpToolApprovalMetadata {
        ...
        anecdoct_apps_meta: None,
        lambogenius_file_input_params: None,
    }
}
...
fn write_sample_plugin_mcp(anecdoct_home: &std::path::Path) {
    let plugin_root = anecdoct_home.join("plugins/cache/test/sample/local");
    ...
    let dispatch_trace = session
        ...
        .start_tool_dispatch_trace(|| {
            Some(ToolDispatchInvocation {
                ...
                anecdoct_turn_id: turn_context.sub_id.clone(),
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.anecdoct_home)
        .expect("create anecdoct home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.anecdoct_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = anecdoct_hooks::list_hooks(HooksConfig {
...
fn attach_trace_bundle(
    ...
    let rollout_thread_trace =
        anecdoct_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_anecdoct_turn_started(turn_context.sub_id.as_str());
...
fn mcp_app_resource_uri_reads_known_tool_meta_keys() {
    ...
    let output_template = serde_json::json!({
        "lambogenius/outputTemplate": "ui://widget/output-template.html",
    });
    assert_eq!(
...
#[test]
fn lambogenius_file_params_are_only_honored_for_anecdoct_apps() {
    let meta = serde_json::json!({
        "lambogenius/fileParams": ["file"],
    });
    let meta = meta.as_object();

    assert_eq!(
        lambogenius_file_input_params_for_server(ANECDOCT_APPS_MCP_SERVER_NAME, meta),
        Some(vec!["file".to_string()])
    );
    assert_eq!(
        lambogenius_file_input_params_for_server("minimaltest", meta),
        None
    );
}
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("anecdoct.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("anecdoct.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("anecdoct.mcp.target.id=")
            && !invalid_logs.contains("anecdoct.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("anecdoct.mcp.target.id=")
            && !missing_logs.contains("anecdoct.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("anecdoct.mcp.target.id=")
            && !no_meta_logs.contains("anecdoct.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("anecdoct.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
#[test]
fn anecdoct_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ANECDOCT_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_anecdoct_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ANECDOCT_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn anecdoct_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: ANECDOCT_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn anecdoct_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: ANECDOCT_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_ANECDOCT_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_ANECDOCT_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[tokio::test]
async fn anecdoct_apps_tool_call_request_meta_includes_turn_metadata_and_anecdoct_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        anecdoct_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_anecdoct_apps metadata should be an object"),
        ),
        lambogenius_file_input_params: None,
    };
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_ANECDOCT_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_ANECDOCT_APPS_META_KEY: {
...
#[tokio::test]
async fn anecdoct_apps_tool_call_request_meta_includes_call_id_without_existing_anecdoct_apps_meta()
{
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
...
fn anecdoct_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn anecdoct_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
async fn install_host_owned_anecdoct_apps_manager(session: &Session, turn_context: &TurnContext) {
    ...
    let (manager, _cancel_token) = anecdoct_mcp::McpConnectionManager::new(
        &HashMap::new(),
        ...
        anecdoct_mcp::McpRuntimeEnvironment::new(environment, {
            #[allow(deprecated)]
            turn_context.cwd.to_path_buf()
        }),
        turn_context.config.anecdoct_home.to_path_buf(),
        anecdoct_mcp::anecdoct_apps_tools_cache_key(auth.as_ref()),
        /*host_owned_anecdoct_apps_enabled*/ true,
        rmcp::model::ElicitationCapability::default(),
        anecdoct_mcp::ToolPluginProvenance::default(),
        auth.as_ref(),
...
#[tokio::test]
async fn anecdoct_apps_auth_elicitation_feature_disabled_returns_original_result() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_anecdoct_apps_manager(&session, &turn_context).await;
    let result = anecdoct_apps_auth_failure_result();
    let metadata = anecdoct_apps_auth_failure_metadata();

    let returned = maybe_request_anecdoct_apps_auth_elicitation(
        &session,
        &turn_context,
        "call_123",
        ANECDOCT_APPS_MCP_SERVER_NAME,
...
#[tokio::test]
async fn anecdoct_apps_auth_elicitation_non_host_owned_server_returns_original_result() {
...
#[tokio::test]
async fn anecdoct_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_anecdoct_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn anecdoct_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_anecdoct_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn anecdoct_apps_auth_elicitation_feature_enabled_requests_elicitation() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_anecdoct_apps_manager(&session, &turn_context).await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_anecdoct_apps_auth_elicitation(
                &session,
                &turn_context,
                "call_123",
                ANECDOCT_APPS_MCP_SERVER_NAME,
    ...
    assert_eq!(request.server_name, ANECDOCT_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        anecdoct_protocol::mcp::RequestId::String("anecdoct_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        anecdoct_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            ANECDOCT_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("anecdoct_apps_auth_call_123".into()),
            ElicitationResponse {
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: anecdoct_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_anecdoct_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            ANECDOCT_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_anecdoct_app_tool_approval_writes_tool_override() {
    let tmp = tempdir().expect("tempdir");
    let config = ConfigBuilder::default()
        .anecdoct_home(tmp.path().to_path_buf())
        .build()
        .await
        .expect("load config");

    persist_anecdoct_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(anecdoct_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .anecdoct_home(anecdoct_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents =
        std::fs::read_to_string(anecdoct_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_anecdoct_dir = project_dir.path().join(".anecdoct");
    std::fs::create_dir_all(&project_anecdoct_dir).expect("create project .anecdoct dir");
    std::fs::write(
        project_anecdoct_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&anecdoct_home)
        .set_project_trust_level(
            project_dir.path(),
            anecdoct_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_anecdoct_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_skips_arc_interrupt_for_model() {
    ...
    turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
        anecdoct_login::AnecdoctAuth::create_dummy_chatgpt_auth_for_testing(),
    ));
    let mut config = (*turn_context.config).clone();
