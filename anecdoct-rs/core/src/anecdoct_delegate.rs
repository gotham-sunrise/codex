...
use anecdoct_analytics::GuardianApprovalRequestSource;
...
/// Start an interactive sub-Anecdoct thread and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_anecdoct_thread_interactive(
    config: Config,
...
) -> Result<Anecdoct, AnecdoctErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let AnecdoctSpawnOk { anecdoct, .. } = Box::pin(Anecdoct::spawn(AnecdoctSpawnArgs {
        config,
        ...
        parent_rollout_thread_trace: anecdoct_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
    ...
    let anecdoct_for_events = Arc::clone(&anecdoct);
    ...
    let anecdoct_for_ops = Arc::clone(&anecdoct);
    tokio::spawn(async move {
        forward_ops(anecdoct_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(Anecdoct {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: anecdoct.agent_status.clone(),
        session: Arc::clone(&anecdoct.session),
        session_loop_termination: anecdoct.session_loop_termination.clone(),
    })
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_anecdoct_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let io = Box::pin(run_anecdoct_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&anecdoct).await;
                break;
            }
            event = anecdoct.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&anecdoct, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(anecdoct: &Anecdoct) {
    let _ = anecdoct.submit(Op::Interrupt).await;
    let _ = anecdoct.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = anecdoct.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = anecdoct.submit_with_id(submission).await;
    }
}
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                anecdoct_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                anecdoct_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                anecdoct_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn handle_request_user_input(
    ...
    if routes_approval_to_guardian(parent_ctx)
        ...
        .await
    {
        let _ = anecdoct.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            anecdoct_protocol::request_user_input::RequestUserInputAnswer {
...
) -> anecdoct_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = anecdoct_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, anecdoct_protocol::protocol::ReviewDecision::Abort)
                .await;
            anecdoct_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
#[cfg(test)]
#[path = "anecdoct_delegate_tests.rs"]
mod tests;
