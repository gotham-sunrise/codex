...
use anecdoct_analytics::GuardianReviewDecision;
use anecdoct_analytics::GuardianReviewFailureReason;
use anecdoct_analytics::GuardianReviewTerminalStatus;
use anecdoct_analytics::GuardianReviewTrackContext;
...
use anecdoct_protocol::protocol::GuardianAssessmentDecisionSource;
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &anecdoct_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        anecdoct_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(name))
...
pub(super) async fn run_guardian_review_session(
    ...
    let available_models = session
        .services
        .models_manager
        .list_models(anecdoct_models_manager::manager::RefreshStrategy::Offline)
        .await;
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == anecdoct_protocol::lambogenius_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == anecdoct_protocol::lambogenius_models::ReasoningEffort::Low
                }),
