...
use anecdoct_analytics::GuardianReviewAnalyticsResult;
use anecdoct_analytics::GuardianReviewSessionKind;
...
use crate::anecdoct_delegate::run_anecdoct_thread_interactive;
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            anecdoct_linux_sandbox_exe: spawn_config.anecdoct_linux_sandbox_exe.clone(),
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.anecdoct.shutdown_and_wait().await;
    }
    ...
    async fn refresh_last_committed_fork_snapshot(&self) {
        match load_rollout_items_for_fork(&self.anecdoct.session).await {
...
impl GuardianReviewSessionManager {
    pub(crate) async fn trunk_rollout_path(&self) -> Option<PathBuf> {
        ...
        trunk.anecdoct.session.ensure_rollout_materialized().await;
        match trunk.anecdoct.session.current_rollout_path().await {
            Ok(path) => path,
            Err(err) => {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, anecdoct: Anecdoct) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            anecdoct.session.get_config().await.as_ref(),
        );
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, anecdoct: Anecdoct) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            anecdoct.session.get_config().await.as_ref(),
        );
        self.state
    ...
    pub(crate) async fn send_trunk_event_raw_for_test(&self, event: Event) {
        ...
        trunk.anecdoct.session.send_event_raw(event).await;
    }
...
async fn spawn_guardian_review_session(
    ...
    let anecdoct = Box::pin(run_anecdoct_thread_interactive(
        spawn_config,
...
async fn run_review_on_session(
    ...
    let mut analytics_result = GuardianReviewAnalyticsResult::from_session(
        review_session.anecdoct.session.conversation_id.to_string(),
        guardian_session_kind,
        params.model.clone(),
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.anecdoct.session.services.network_approval,
                )
                .await;
    ...
    let submit_result = run_before_review_deadline(
        deadline,
        params.external_cancel.as_ref(),
        Box::pin(review_session.anecdoct.submit(Op::UserTurn {
            environments: None,
    ...
    if matches!(outcome.0, GuardianReviewSessionOutcome::Completed(_)) {
        if outcome.2
            && let Some(total_token_usage) =
                review_session.anecdoct.session.total_token_usage().await
        {
...
async fn append_guardian_followup_reminder(review_session: &GuardianReviewSession) {
    let turn_context = review_session.anecdoct.session.new_default_turn().await;
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            ...
            event = review_session.anecdoct.next_event() => {
                match event {
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<anecdoct_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
mod tests {
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::AnecdoctHooks));
    }
    ...
    async fn interrupt_and_drain_turn_ignores_prior_turn_completion() {
        ...
        interrupt_and_drain_turn(&review_session.anecdoct, "current-turn")
            .await
            .expect("drain current turn");

        assert!(review_session.anecdoct.rx_event.try_recv().is_err());
    }
}
