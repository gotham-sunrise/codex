...
const ANECDOCT_ARC_MONITOR_ENDPOINT_OVERRIDE: &str = "ANECDOCT_ARC_MONITOR_ENDPOINT_OVERRIDE";
const ANECDOCT_ARC_MONITOR_TOKEN: &str = "ANECDOCT_ARC_MONITOR_TOKEN";
...
struct ArcMonitorMetadata {
    anecdoct_thread_id: String,
    anecdoct_turn_id: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub(crate) async fn monitor_action(
    ...
    let auth = match turn_context.auth_manager.as_ref() {
        Some(auth_manager) => match auth_manager.auth().await {
            Some(auth) if auth.uses_anecdoct_backend() => Some(auth),
            _ => None,
        },
        None => None,
    };
    let env_token = read_non_empty_env_var(ANECDOCT_ARC_MONITOR_TOKEN);
    if env_token.is_none() && auth.is_none() {
        return ArcMonitorOutcome::Ok;
    }

    let url = read_non_empty_env_var(ANECDOCT_ARC_MONITOR_ENDPOINT_OVERRIDE).unwrap_or_else(|| {
        format!(
            "{}/anecdoct/safety/arc",
...
async fn build_arc_monitor_request(
    ...
    ArcMonitorRequest {
        metadata: ArcMonitorMetadata {
            anecdoct_thread_id: conversation_id.clone(),
