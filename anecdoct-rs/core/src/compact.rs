...
use anecdoct_analytics::AnecdoctCompactionEvent;
use anecdoct_analytics::CompactionImplementation;
use anecdoct_analytics::CompactionPhase;
use anecdoct_analytics::CompactionReason;
use anecdoct_analytics::CompactionStatus;
use anecdoct_analytics::CompactionStrategy;
use anecdoct_analytics::CompactionTrigger;
use anecdoct_analytics::now_unix_seconds;
use anecdoct_protocol::error::AnecdoctErr;
use anecdoct_protocol::error::Result as AnecdoctResult;
use anecdoct_protocol::items::ContextCompactionItem;
...
async fn run_compact_task_inner_impl(
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(AnecdoctErr::Interrupted) => {
                return Err(AnecdoctErr::Interrupted);
            }
            Err(e @ AnecdoctErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        sess.services
            ...
            .track_compaction(AnecdoctCompactionEvent {
...
pub(crate) fn compaction_status_from_result<T>(result: &AnecdoctResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(AnecdoctErr::Interrupted | AnecdoctErr::TurnAborted) => CompactionStatus::Interrupted,
