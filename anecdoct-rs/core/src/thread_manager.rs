...
use crate::anecdoct_thread::AnecdoctThread;
...
use crate::session::AnecdoctSpawnArgs;
use crate::session::AnecdoctSpawnOk;
...
use anecdoct_model_provider_info::LAMBOGENIUS_PROVIDER_ID;
...
use anecdoct_state::DirectionalThreadSpawnEdgeStatus;
use anecdoct_thread_store::InMemoryThreadStore;
use anecdoct_thread_store::LocalThreadStore;
use anecdoct_thread_store::LocalThreadStoreConfig;
use anecdoct_thread_store::ReadThreadByRolloutPathParams;
...
use anecdoct_thread_store::UpdateThreadMetadataParams;
...
struct TempAnecdoctHomeGuard {
    path: PathBuf,
}

impl Drop for TempAnecdoctHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Anecdoct thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<AnecdoctThread>,
...
pub struct ThreadManager {
    ...
    _test_anecdoct_home_guard: Option<TempAnecdoctHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<anecdoct_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<AnecdoctThread>>>>,
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        let anecdoct_home = config.anecdoct_home.clone();
        ...
        Self {
            ...
            _test_anecdoct_home_guard: None,
        }
    }

    /// Construct with a dummy AuthManager containing the provided AnecdoctAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let anecdoct_home = std::env::temp_dir().join(format!(
            "anecdoct-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&anecdoct_home)
            .unwrap_or_else(|err| panic!("temp anecdoct home dir create failed: {err}"));
        ...
        manager._test_anecdoct_home_guard = Some(TempAnecdoctHomeGuard {
            path: anecdoct_home,
        });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided AnecdoctAuth and anecdoct home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let skills_anecdoct_home = match AbsolutePathBuf::from_absolute_path_checked(&anecdoct_home)
        {
            Ok(anecdoct_home) => anecdoct_home,
            Err(err) => panic!("test anecdoct_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite_home: anecdoct_home.clone(),
                default_model_provider_id: LAMBOGENIUS_PROVIDER_ID.to_string(),
            },
            state_db.clone(),
        ));
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(anecdoct_home, /*config_model_catalog*/ None),
                environment_manager,
                skills_manager,
                plugins_manager,
                mcp_manager,
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> AnecdoctResult<Arc<AnecdoctThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `AnecdoctThread`/`LiveThread`, so metadata changes stay ordered
    ...
    ) -> AnecdoctResult<StoredThread> {
        if let Ok(thread) = self.get_thread(thread_id).await {
            if thread.config_snapshot().await.ephemeral {
                return Err(AnecdoctErr::InvalidRequest(format!(
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    AnecdoctErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn start_thread(&self, config: Config) -> AnecdoctResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<anecdoct_protocol::dynamic_tools::DynamicToolSpec>,
        persist_extended_history: bool,
    ) -> AnecdoctResult<NewThread> {
    ...
    /// as `Arc<AnecdoctThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<AnecdoctThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        &self,
        thread_id: ThreadId,
    ) -> AnecdoctResult<Arc<AnecdoctThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(AnecdoctErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => AnecdoctErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> AnecdoctResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<AnecdoctThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        let AnecdoctSpawnOk {
            ...
            thread_id,
            ..
        } = Anecdoct::spawn(AnecdoctSpawnArgs {
            config,
        ...
        .await?;
        let new_thread = self
            .finalize_thread_spawn(anecdoct, thread_id, tracked_session_source)
            .await?;
    ...
    async fn finalize_thread_spawn(
        ...
        let event = anecdoct.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(AnecdoctErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(AnecdoctThread::new(
                    anecdoct,
        ...
        if let Err(err) = anecdoct.shutdown_and_wait().await {
            warn!("failed to shut down duplicate thread {thread_id}: {err}");
        }
        Err(AnecdoctErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> anecdoct_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return anecdoct_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(anecdoct_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> AnecdoctErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => AnecdoctErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => AnecdoctErr::InvalidRequest(message),
        err => AnecdoctErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> AnecdoctErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => AnecdoctErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => AnecdoctErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => AnecdoctErr::UnsupportedOperation(format!(
            "thread metadata update is not supported by this store: {operation}"
        )),
        err => AnecdoctErr::Fatal(format!(
