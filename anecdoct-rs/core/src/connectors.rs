...
pub use anecdoct_app_server_protocol::AppBranding;
pub use anecdoct_app_server_protocol::AppInfo;
pub use anecdoct_app_server_protocol::AppMetadata;
use anecdoct_connectors::ConnectorDirectoryCacheContext;
use anecdoct_connectors::ConnectorDirectoryCacheKey;
...
use anecdoct_tools::DiscoverableTool;
...
use anecdoct_config::AppsRequirementsToml;
...
use anecdoct_mcp::ToolInfo;
use anecdoct_mcp::ToolPluginProvenance;
...
use anecdoct_mcp::with_anecdoct_apps_mcp;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = anecdoct_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        anecdoct_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config.features.apps_enabled_for_auth(
        auth.as_ref()
            .is_some_and(AnecdoctAuth::uses_anecdoct_backend),
    ) {
        return Some(Vec::new());
    }
    let cache_key = accessible_connectors_cache_key(config, auth.as_ref());
    read_cached_accessible_connectors(&cache_key).map(|connectors| {
        anecdoct_connectors::filter::filter_disallowed_connectors(
            connectors,
            originator().value.as_str(),
        )
    })
...
pub(crate) fn refresh_accessible_connectors_cache_from_mcp_tools(
    ...
    let accessible_connectors = anecdoct_connectors::filter::filter_disallowed_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_environment_manager(
    ...
    {
        let cached_connectors = anecdoct_connectors::filter::filter_disallowed_connectors(
            cached_connectors,
            originator().value.as_str(),
        );
    ...
    let mcp_servers = with_anecdoct_apps_mcp(HashMap::new(), auth.as_ref(), &mcp_config);
    let host_owned_anecdoct_apps_enabled =
        host_owned_anecdoct_apps_enabled(&mcp_config, auth.as_ref());
    ...
    let (mut mcp_connection_manager, cancel_token) = McpConnectionManager::new(
        ...
        anecdoct_apps_tools_cache_key(auth.as_ref()),
        host_owned_anecdoct_apps_enabled,
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_anecdoct_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{ANECDOCT_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let anecdoct_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(ANECDOCT_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(ANECDOCT_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(ANECDOCT_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if anecdoct_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let account_id = auth.and_then(AnecdoctAuth::get_account_id);
    let chatgpt_user_id = auth.and_then(AnecdoctAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(AnecdoctAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + anecdoct_connectors::CONNECTORS_CACHE_TTL,
...
async fn tool_suggest_connector_ids(config: &Config) -> HashSet<String> {
    ...
    let mut connector_ids = PluginsManager::new(config.anecdoct_home.to_path_buf())
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    let Some(auth) = auth.filter(|auth| auth.uses_anecdoct_backend()) else {
        return Vec::new();
    };
    ...
    anecdoct_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
pub(crate) fn accessible_connectors_from_mcp_tools(mcp_tools: &[ToolInfo]) -> Vec<AppInfo> {
    ...
    let tools = mcp_tools.iter().filter_map(|tool| {
        if tool.server_name != ANECDOCT_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(anecdoct_connectors::accessible::AccessibleConnectorTool {
    ...
    anecdoct_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn anecdoct_app_tool_is_enabled(config: &Config, tool_info: &ToolInfo) -> bool {
    if tool_info.server_name != ANECDOCT_APPS_MCP_SERVER_NAME {
        return true;
    }
