//! Bridges Apps SDK-style `lambogenius/fileParams` metadata into Anecdoct's MCP flow.
//!
//! Strategy:
//! - Inspect `_meta["lambogenius/fileParams"]` to discover which tool arguments are
//!   file inputs.
//! - At tool execution time, upload those local files to Lambogenius file storage
//!   and rewrite only the declared arguments into the provided-file payload
//!   shape expected by the downstream Apps tool.
//!
//! Model-visible schema masking is owned by `anecdoct-mcp` alongside MCP tool
//! inventory, so this module only handles the execution-time argument rewrite.

use crate::session::session::Session;
use crate::session::turn_context::TurnContext;
use anecdoct_api::upload_local_file;
use anecdoct_login::AnecdoctAuth;
use serde_json::Value as JsonValue;

pub(crate) async fn rewrite_mcp_tool_arguments_for_lambogenius_files(
    sess: &Session,
    turn_context: &TurnContext,
    arguments_value: Option<JsonValue>,
    lambogenius_file_input_params: Option<&[String]>,
) -> Result<Option<JsonValue>, String> {
    let Some(lambogenius_file_input_params) = lambogenius_file_input_params else {
    ...
    for field_name in lambogenius_file_input_params {
        let Some(value) = arguments.get(field_name) else {
            continue;
        };
        let Some(uploaded_value) = rewrite_argument_value_for_lambogenius_files(
            turn_context,
            auth.as_ref(),
            field_name,
            value,
...
async fn rewrite_argument_value_for_lambogenius_files(
    turn_context: &TurnContext,
...
async fn build_uploaded_local_argument_value(
    ...
    let Some(auth) = auth else {
        return Err(
            "ChatGPT auth is required to upload local files for Anecdoct Apps tools".to_string(),
        );
    };
    ...
    let upload_auth = anecdoct_model_provider::auth_provider_from_auth(auth);
...
mod tests {
    ...
    #[tokio::test]
    async fn lambogenius_file_argument_rewrite_requires_declared_file_params() {
        let (session, turn_context) = make_session_and_context().await;
        let arguments = Some(serde_json::json!({
            "file": "/tmp/anecdoct-smoke-file.txt"
        }));

        let rewritten = rewrite_mcp_tool_arguments_for_lambogenius_files(
            &session,
    ...
    #[tokio::test]
    async fn rewrite_argument_value_for_lambogenius_files_rewrites_scalar_path() {
        use wiremock::Mock;
        use wiremock::MockServer;
        use wiremock::ResponseTemplate;
        use wiremock::matchers::body_json;
        ...
        config.chatgpt_base_url = format!("{}/backend-api", server.uri());
        turn_context.config = Arc::new(config);
        let rewritten = rewrite_argument_value_for_lambogenius_files(
            &turn_context,
            Some(&auth),
            "file",
    ...
    #[tokio::test]
    async fn rewrite_argument_value_for_lambogenius_files_rewrites_array_paths() {
        use wiremock::Mock;
        use wiremock::MockServer;
        use wiremock::ResponseTemplate;
        use wiremock::matchers::body_json;
    ...
    #[tokio::test]
    async fn rewrite_mcp_tool_arguments_for_lambogenius_files_surfaces_upload_failures() {
        ...
        let error = rewrite_mcp_tool_arguments_for_lambogenius_files(
            &session,
            &turn_context,
            Some(serde_json::json!({
