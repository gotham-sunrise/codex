...
use anecdoct_mcp::ToolInfo as McpToolInfo;
...
pub(crate) fn build_mcp_tool_exposure(
    ...
    let mut deferred_tools = filter_non_anecdoct_apps_mcp_tools_only(all_mcp_tools);
    if let Some(connectors) = connectors {
        deferred_tools.extend(filter_anecdoct_apps_mcp_tools(
            all_mcp_tools,
            connectors,
            config,
        ));
    ...
    let direct_tools =
        filter_anecdoct_apps_mcp_tools(all_mcp_tools, explicitly_enabled_connectors, config);
...
fn filter_non_anecdoct_apps_mcp_tools_only(mcp_tools: &[McpToolInfo]) -> Vec<McpToolInfo> {
    mcp_tools
        .iter()
        .filter(|tool| tool.server_name != ANECDOCT_APPS_MCP_SERVER_NAME)
        .cloned()
        .collect()
}

fn filter_anecdoct_apps_mcp_tools(
    ...
    mcp_tools
        ...
        .filter(|tool| {
            ...
            allowed.contains(connector_id) && connectors::anecdoct_app_tool_is_enabled(config, tool)
        })
        .cloned()
        .collect()
}
