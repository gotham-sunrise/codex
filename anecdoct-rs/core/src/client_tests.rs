...
use super::X_ANECDOCT_INSTALLATION_ID_HEADER;
use super::X_ANECDOCT_PARENT_THREAD_ID_HEADER;
use super::X_ANECDOCT_TURN_METADATA_HEADER;
use super::X_ANECDOCT_WINDOW_ID_HEADER;
use super::X_LAMBOGENIUS_SUBAGENT_HEADER;
use crate::AttestationContext;
use crate::AttestationProvider;
use crate::GenerateAttestationFuture;
use anecdoct_api::ApiError;
use anecdoct_api::ResponseEvent;
use anecdoct_app_server_protocol::AuthMode;
use anecdoct_login::AnecdoctAuth;
use anecdoct_login::AuthManager;
use anecdoct_model_provider::BearerAuthProvider;
use anecdoct_model_provider_info::CHATGPT_ANECDOCT_BASE_URL;
use anecdoct_model_provider_info::ModelProviderInfo;
use anecdoct_model_provider_info::WireApi;
use anecdoct_model_provider_info::create_oss_provider_with_base_url;
...
use anecdoct_rollout_trace::ExecutionStatus;
use anecdoct_rollout_trace::InferenceTraceAttempt;
use anecdoct_rollout_trace::InferenceTraceContext;
use anecdoct_rollout_trace::RawTraceEventPayload;
use anecdoct_rollout_trace::RolloutTrace;
use anecdoct_rollout_trace::TraceWriter;
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::AnecdoctTurnStarted {
        anecdoct_turn_id: "turn-1".to_string(),
        thread_id: "thread-root".to_string(),
    })?;
...
fn build_subagent_headers_sets_other_subagent_label() {
    ...
    let value = headers
        .get(X_LAMBOGENIUS_SUBAGENT_HEADER)
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    assert_eq!(
        ...
        std::collections::HashMap::from([
            ...
            (
                X_LAMBOGENIUS_SUBAGENT_HEADER.to_string(),
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_lambogenius_provider(Some(
                CHATGPT_ANECDOCT_BASE_URL.to_string(),
            )),
        )
    } else {
        (
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_anecdoct_responses() {
...
#[tokio::test]
async fn non_chatgpt_anecdoct_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
