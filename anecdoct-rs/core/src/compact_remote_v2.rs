...
use anecdoct_rollout_trace::CompactionCheckpointTracePayload;
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped { reason } => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    anecdoct_analytics::CompactionStatus::Interrupted,
                    Some(error),
                )
                .await;
...
) -> AnecdoctResult<(ResponseItem, String)> {
...
mod tests {
    ...
    fn response_stream(events: Vec<AnecdoctResult<ResponseEvent>>) -> ResponseStream {
