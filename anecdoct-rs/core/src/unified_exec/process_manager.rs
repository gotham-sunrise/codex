...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Anecdoct sandbox network proxy.";
...
) -> anecdoct_exec_server::ExecEnvPolicy {
    anecdoct_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<anecdoct_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    request: &ExecRequest,
    tty: bool,
) -> anecdoct_exec_server::ExecParams {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Anecdoct(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        if request.sandbox == anecdoct_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let anecdoct_home = crate::config::find_anecdoct_home().map_err(|err| {
                UnifiedExecError::create_process(format!(
            ...
            let spawned = match request.windows_sandbox_level {
                anecdoct_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    anecdoct_windows_sandbox::spawn_windows_sandbox_session_elevated(
                        policy_json.as_str(),
                ...
                anecdoct_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | anecdoct_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    anecdoct_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        policy_json.as_str(),
                        request.windows_sandbox_policy_cwd.as_path(),
                        anecdoct_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            anecdoct_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
                &request.arg0,
                anecdoct_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            anecdoct_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            ANECDOCT_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Anecdoct(AnecdoctErr::Sandbox(SandboxErr::Denied {
                    output, ..
                })) => {
                    let output = *output;
