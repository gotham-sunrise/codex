...
use anecdoct_otel::GOAL_BUDGET_LIMITED_METRIC;
use anecdoct_otel::GOAL_COMPLETED_METRIC;
use anecdoct_otel::GOAL_CREATED_METRIC;
use anecdoct_otel::GOAL_DURATION_SECONDS_METRIC;
use anecdoct_otel::GOAL_TOKEN_COUNT_METRIC;
...
pub struct ExternalGoalPreviousGoal {
    goal_id: String,
    status: anecdoct_state::ThreadGoalStatus,
    objective: String,
}

impl From<&anecdoct_state::ThreadGoal> for ExternalGoalPreviousStatus {
    fn from(goal: &anecdoct_state::ThreadGoal) -> Self {
        Self::Existing(ExternalGoalPreviousGoal::from(goal))
    }
}

impl From<&anecdoct_state::ThreadGoal> for ExternalGoalPreviousGoal {
    fn from(goal: &anecdoct_state::ThreadGoal) -> Self {
        Self {
...
pub struct ExternalGoalSet {
    pub goal: anecdoct_state::ThreadGoal,
...
impl Session {
    ...
    pub(crate) async fn set_thread_goal(
                ...
                state_db
                    .replace_thread_goal(
                        ...
                        status
                            ...
                            .unwrap_or(anecdoct_state::ThreadGoalStatus::Active),
        ...
        *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        let newly_active_goal = goal_status == anecdoct_state::ThreadGoalStatus::Active
            && (replacing_goal
                || previous_status
                    .is_some_and(|status| status != anecdoct_state::ThreadGoalStatus::Active));
        if newly_active_goal {
            ...
            .await;
        } else if goal_status != anecdoct_state::ThreadGoalStatus::Active {
    ...
    async fn apply_external_thread_goal_status(self: &Arc<Self>, external_set: ExternalGoalSet) {
        ...
        match status {
            ...
            anecdoct_state::ThreadGoalStatus::BudgetLimited => {
                if self.active_turn_context().await.is_none() {
                    self.clear_stopped_thread_goal_runtime_state().await;
                }
            }
            anecdoct_state::ThreadGoalStatus::Paused
    ...
    fn emit_goal_terminal_metrics_if_status_changed(
        &self,
        previous_status: Option<anecdoct_state::ThreadGoalStatus>,
        goal: &anecdoct_state::ThreadGoal,
    ) {
        ...
        let counter = match goal.status {
            anecdoct_state::ThreadGoalStatus::BudgetLimited => GOAL_BUDGET_LIMITED_METRIC,
            anecdoct_state::ThreadGoalStatus::Complete => GOAL_COMPLETED_METRIC,
            anecdoct_state::ThreadGoalStatus::Active | anecdoct_state::ThreadGoalStatus::Paused => {
                return;
            }
        };
    ...
    ) -> anyhow::Result<Option<anecdoct_state::ThreadGoalStatus>> {
    ...
    async fn account_thread_goal_progress(
        ...
        let goal = match outcome {
            anecdoct_state::ThreadGoalAccountingOutcome::Updated(goal) => {
                let clear_active_goal = match goal.status {
                    anecdoct_state::ThreadGoalStatus::Active => false,
                    anecdoct_state::ThreadGoalStatus::BudgetLimited => {
                        matches!(budget_limit_steering, BudgetLimitSteering::Suppressed)
                    }
                    anecdoct_state::ThreadGoalStatus::Paused
                    | anecdoct_state::ThreadGoalStatus::Complete => true,
                };
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
            ...
            anecdoct_state::ThreadGoalAccountingOutcome::Unchanged(_) => return Ok(()),
        };
        let should_steer_budget_limit =
            matches!(budget_limit_steering, BudgetLimitSteering::Allowed)
                && goal.status == anecdoct_state::ThreadGoalStatus::BudgetLimited
                && !budget_limit_was_already_reported;
        let goal_status = goal.status;
        let goal_id = goal.goal_id.clone();
        if goal_status != anecdoct_state::ThreadGoalStatus::BudgetLimited {
            *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        }
        let goal = protocol_goal_from_state(goal);
        self.send_event(
    ...
    async fn account_thread_goal_wall_clock_usage(
        &self,
        state_db: &StateDbHandle,
        mode: anecdoct_state::ThreadGoalAccountingMode,
        terminal_metric_emission: TerminalMetricEmission,
            ...
            .await?
        {
            ...
            anecdoct_state::ThreadGoalAccountingOutcome::Unchanged(goal) => {
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
    ...
    async fn pause_active_thread_goal_for_interrupt(&self) -> anyhow::Result<()> {
        ...
        self.account_thread_goal_wall_clock_usage(
            &state_db,
            anecdoct_state::ThreadGoalAccountingMode::ActiveStatusOnly,
            TerminalMetricEmission::Emit,
        )
        .await?;
            ...
            Ok(Some(state_db)) => match state_db.get_thread_goal(self.conversation_id).await {
                Ok(Some(goal))
                    if goal.goal_id == candidate.goal_id
                        && goal.status == anecdoct_state::ThreadGoalStatus::Active =>
                {
                    true
                }
                Ok(Some(_)) | Ok(None) => {
    ...
    async fn goal_continuation_candidate_if_active(
        ...
        if goal.status != anecdoct_state::ThreadGoalStatus::Active {
...
pub(crate) fn protocol_goal_from_state(goal: anecdoct_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
pub(crate) fn protocol_goal_status_from_state(
    ...
    match status {
        anecdoct_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        anecdoct_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        anecdoct_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        anecdoct_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}

pub(crate) fn state_goal_status_from_protocol(
    status: ThreadGoalStatus,
) -> anecdoct_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => anecdoct_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => anecdoct_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::BudgetLimited => anecdoct_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => anecdoct_state::ThreadGoalStatus::Complete,
    }
}
