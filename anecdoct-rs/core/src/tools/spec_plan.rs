...
use anecdoct_tools::ResponsesApiNamespace;
use anecdoct_tools::ResponsesApiNamespaceTool;
use anecdoct_tools::TOOL_SEARCH_TOOL_NAME;
use anecdoct_tools::ToolCall as ExtensionToolCall;
use anecdoct_tools::ToolEnvironmentMode;
use anecdoct_tools::ToolExecutor;
use anecdoct_tools::ToolName;
use anecdoct_tools::ToolOutput;
use anecdoct_tools::ToolSpec;
use anecdoct_tools::ToolsConfig;
use anecdoct_tools::collect_code_mode_exec_prompt_tool_definitions;
use anecdoct_tools::default_namespace_description;
...
fn spec_for_model_request(
    ...
    if config.code_mode_enabled
        ...
        && anecdoct_code_mode::is_code_mode_nested_tool(spec.name())
    {
        anecdoct_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn is_hidden_by_code_mode_only(
    ...
    config.code_mode_only_enabled
        && exposure != ToolExposure::DirectModelOnly
        && anecdoct_code_mode::is_code_mode_nested_tool(
            &anecdoct_tools::code_mode_name_for_tool_name(tool_name),
        )
}
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, anecdoct_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| anecdoct_code_mode::ToolNamespaceDescription {
...
fn collect_tool_executors(
    ...
    for tool in params.dynamic_tools {
        let Some(handler) = DynamicToolHandler::new(tool).map(Arc::new) else {
            tracing::error!(
                "Failed to convert dynamic tool {:?} to Lambogenius tool",
                tool.name
            );
            continue;
        };
...
fn append_extension_tool_executors(
    ...
    let mut reserved_tool_names = registered_executors
        ...
        .collect::<HashSet<_>>();
    if config.code_mode_enabled {
        reserved_tool_names.insert(ToolName::plain(anecdoct_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(anecdoct_code_mode::WAIT_TOOL_NAME));
    }
...
impl ToolExecutor<ToolInvocation> for MultiAgentV2NamespaceOverride {
    ...
    ) -> Result<Box<dyn ToolOutput>, anecdoct_tools::FunctionCallError> {
...
fn compare_code_mode_tools(
    left: &anecdoct_code_mode::ToolDefinition,
    right: &anecdoct_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, anecdoct_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &anecdoct_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, anecdoct_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
