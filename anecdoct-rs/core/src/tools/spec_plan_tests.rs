...
use anecdoct_extension_api::ToolCall as ExtensionToolCall;
use anecdoct_extension_api::ToolExecutor;
...
use anecdoct_protocol::lambogenius_models::WebSearchToolType;
use anecdoct_protocol::models::PermissionProfile;
use anecdoct_protocol::models::VIEW_IMAGE_TOOL_NAME;
use anecdoct_protocol::protocol::SessionSource;
use anecdoct_protocol::protocol::SubAgentSource;
use anecdoct_tools::AdditionalProperties;
use anecdoct_tools::DiscoverablePluginInfo;
use anecdoct_tools::DiscoverableTool;
use anecdoct_tools::FreeformTool;
use anecdoct_tools::JsonSchema;
use anecdoct_tools::JsonSchemaPrimitiveType;
use anecdoct_tools::JsonSchemaType;
use anecdoct_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use anecdoct_tools::ResponsesApiNamespaceTool;
use anecdoct_tools::ResponsesApiTool;
use anecdoct_tools::ResponsesApiWebSearchFilters;
use anecdoct_tools::ResponsesApiWebSearchUserLocation;
use anecdoct_tools::TOOL_SEARCH_TOOL_NAME;
use anecdoct_tools::ToolEnvironmentMode;
use anecdoct_tools::ToolName;
use anecdoct_tools::ToolsConfigParams;
use anecdoct_tools::mcp_call_tool_result_output_schema;
use anecdoct_tools::request_user_input_available_modes;
...
const ANECDOCT_APPS_MCP_SERVER_NAME: &str = "anecdoct_apps";
...
fn extension_tool_executor(
    ...
    impl ToolExecutor<ExtensionToolCall> for SpecOnlyExtensionExecutor {
        ...
        ) -> Result<Box<dyn anecdoct_tools::ToolOutput>, anecdoct_tools::FunctionCallError>
        {
...
#[test]
fn test_full_toolset_specs_for_gpt5_anecdoct_unified_exec_web_search() {
    let model_info = model_info();
...
fn web_search_config_is_forwarded_to_tool_spec() {
    ...
    let web_search_config = WebSearchConfig {
        filters: Some(anecdoct_protocol::config_types::WebSearchFilters {
            allowed_domains: Some(vec!["example.com".to_string()]),
        }),
        user_location: Some(anecdoct_protocol::config_types::WebSearchUserLocation {
            r#type: anecdoct_protocol::config_types::WebSearchUserLocationType::Approximate,
            country: Some("US".to_string()),
            region: Some("California".to_string()),
            city: Some("San Francisco".to_string()),
            timezone: Some("America/Los_Angeles".to_string()),
        }),
        search_context_size: Some(anecdoct_protocol::config_types::WebSearchContextSize::High),
    };
...
fn namespaced_dynamic_specs_are_hidden_when_namespace_tools_are_disabled() {
    ...
    assert_lacks_tool_name(&tools, "anecdoct_app");
...
fn search_tool_registers_for_deferred_dynamic_tools() {
    ...
    assert!(
        description.contains("- Dynamic tools: Tools provided by the current Anecdoct thread.")
    );
    assert_contains_tool_names(&tools, &[TOOL_SEARCH_TOOL_NAME]);
    assert_lacks_tool_name(&tools, "anecdoct_app");
    assert!(registry.has_tool(&ToolName::plain(TOOL_SEARCH_TOOL_NAME)));
    assert!(registry.has_tool(&ToolName::namespaced("anecdoct_app", "automation_update")));
    assert!(registry.has_tool(&ToolName::namespaced("anecdoct_app", "automation_list")));
}
