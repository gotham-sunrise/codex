...
async fn test_network_proxy() -> anyhow::Result<NetworkProxy> {
    let state = anecdoct_network_proxy::build_config_state(
...
#[test]
fn explicit_escalation_keeps_user_proxy_env_without_anecdoct_marker() {
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_restores_anecdoct_thread_id_from_env() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport ANECDOCT_THREAD_ID='parent-thread'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"$ANECDOCT_THREAD_ID\"".to_string(),
    ];
    let rewritten = maybe_wrap_shell_lc_with_snapshot(
        &command,
        &session_shell,
        ...
        &HashMap::new(),
        &HashMap::from([(
            "ANECDOCT_THREAD_ID".to_string(),
            "nested-thread".to_string(),
        )]),
    );
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env("ANECDOCT_THREAD_ID", "nested-thread")
        .output()
        .expect("run rewritten command");
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_refreshes_anecdoct_proxy_git_ssh_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{ANECDOCT_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
    );
    let fresh_command = format!(
        "{ANECDOCT_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:48081 %h %p'"
    );
    std::fs::write(
        &snapshot_path,
        format!(
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_clears_stale_anecdoct_git_ssh_command_without_live_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{ANECDOCT_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
...
fn maybe_wrap_shell_lc_with_snapshot_does_not_embed_override_values_in_argv() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport LAMBOGENIUS_API_KEY='snapshot-value'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"$LAMBOGENIUS_API_KEY\"".to_string(),
    ];
    let explicit_env_overrides = HashMap::from([(
        "LAMBOGENIUS_API_KEY".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env("LAMBOGENIUS_API_KEY", "super-secret-value")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport ANECDOCT_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${ANECDOCT_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$ANECDOCT_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "ANECDOCT_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("ANECDOCT_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(String::from_utf8_lossy(&output.stdout), "unset");
