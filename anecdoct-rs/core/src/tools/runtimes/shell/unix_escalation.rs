...
use anecdoct_execpolicy::MatchOptions;
...
use anecdoct_protocol::protocol::NetworkPolicyRuleAction;
...
use anecdoct_shell_command::bash::parse_shell_lc_plain_commands;
use anecdoct_shell_command::bash::parse_shell_lc_single_command_prefix;
use anecdoct_shell_escalation::EscalateServer;
use anecdoct_shell_escalation::EscalationDecision;
use anecdoct_shell_escalation::EscalationExecution;
use anecdoct_shell_escalation::EscalationPermissions;
use anecdoct_shell_escalation::EscalationPolicy;
use anecdoct_shell_escalation::EscalationSession;
use anecdoct_shell_escalation::ExecParams;
use anecdoct_shell_escalation::ExecResult;
use anecdoct_shell_escalation::PreparedExec;
use anecdoct_shell_escalation::ResolvedPermissionProfile;
use anecdoct_shell_escalation::ShellCommandExecutor;
use anecdoct_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::Anecdoct(err.into()))?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        arg0,
        sandbox_policy_cwd,
        anecdoct_linux_sandbox_exe: ctx.turn.anecdoct_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    anecdoct_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<anecdoct_network_proxy::NetworkProxy>,
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["ANECDOCT_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            anecdoct_linux_sandbox_exe: self.anecdoct_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Anecdoct(AnecdoctErr::Sandbox(
            SandboxErr::Timeout {
