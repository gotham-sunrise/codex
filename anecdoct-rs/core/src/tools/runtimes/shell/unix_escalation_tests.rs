...
fn extract_shell_script_supports_wrapped_command_prefixes() {
    assert_eq!(
        extract_shell_script(&[
            "/usr/bin/env".into(),
            "ANECDOCT_EXECVE_WRAPPER=1".into(),
            "/bin/zsh".into(),
            "-lc".into(),
            "echo hello".into()
        ])
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.anecdoct_home)
        .context("recreate anecdoct home for hook fixtures")?;
    ...
    let config_toml_path = turn_context
        .config
        .anecdoct_home
        .join(anecdoct_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command = anecdoct_shell_command::parse_command::shlex_join(&[
        "/usr/bin/touch".to_string(),
        target_str,
    ]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(anecdoct_execpolicy::Policy::empty())),
        ...
        stopwatch: anecdoct_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = tokio::time::timeout(
        Duration::from_secs(5),
        anecdoct_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        anecdoct_shell_escalation::EscalationDecision::Escalate(
            anecdoct_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
