...
use anecdoct_extension_api::ResponsesApiTool;
...
impl anecdoct_extension_api::ToolContributor for ExtensionEchoContributor {
    fn tools(
        &self,
...
impl ToolExecutor<ExtensionToolCall> for ExtensionEchoExecutor {
    ...
    fn spec(&self) -> Option<ToolSpec> {
        Some(ToolSpec::Namespace(ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: true,
                parameters: anecdoct_extension_api::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
    ...
    async fn handle(
        ...
        Ok(Box::new(anecdoct_tools::JsonToolOutput::new(json!({
            "arguments": arguments,
            "callId": call.call_id,
            "ok": true,
        }))))
...
async fn build_tool_call_uses_namespace_for_registry_name() -> anyhow::Result<()> {
    ...
    let call = ToolRouter::build_tool_call(ResponseItem::FunctionCall {
        ...
        namespace: Some("mcp__anecdoct_apps__calendar".to_string()),
        arguments: "{}".to_string(),
    ...
    assert_eq!(
        call.tool_name,
        ToolName::namespaced("mcp__anecdoct_apps__calendar", tool_name)
    );
...
async fn specs_filter_deferred_dynamic_tools() -> anyhow::Result<()> {
    ...
    assert_eq!(
        namespace_function_names(&router.model_visible_specs(), "anecdoct_app"),
