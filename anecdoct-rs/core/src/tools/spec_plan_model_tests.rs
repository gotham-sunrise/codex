...
use anecdoct_models_manager::model_info::with_config_overrides;
...
use anecdoct_tools::ToolSpec;
use anecdoct_tools::ToolsConfig;
use anecdoct_tools::ToolsConfigParams;
use anecdoct_tools::UnifiedExecShellMode;
use anecdoct_tools::ZshForkConfig;
use anecdoct_tools::mcp_call_tool_result_output_schema;
use anecdoct_tools::mcp_tool_to_deferred_responses_api_tool;
...
fn deferred_responses_api_tool_serializes_with_defer_loading() {
    ...
    let serialized = serde_json::to_value(ToolSpec::Function(
        mcp_tool_to_deferred_responses_api_tool(
            &ToolName::namespaced("mcp__anecdoct_apps__", "lookup_order"),
            &tool,
        )
...
fn build_specs_with_inputs_for_test(
    ...
    extension_tool_executors: &[Arc<
        dyn anecdoct_extension_api::ToolExecutor<anecdoct_extension_api::ToolCall>,
    >],
    dynamic_tools: &[DynamicToolSpec],
) -> Vec<ToolSpec> {
...
#[tokio::test]
async fn test_build_specs_gpt5_anecdoct_default() {
...
#[tokio::test]
async fn test_build_specs_gpt51_anecdoct_default() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_build_specs_gpt5_anecdoct_unified_exec_web_search() {
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn test_build_specs_gpt51_anecdoct_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
...
#[tokio::test]
async fn test_gpt_5_1_anecdoct_max_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4",
        &features,
...
#[tokio::test]
async fn test_anecdoct_5_1_mini_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.4-mini",
        &features,
...
#[tokio::test]
async fn test_gpt_5_1_anecdoct_max_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.4",
        ...
        tools_config
            .with_unified_exec_shell_mode_for_session(
                ...
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\anecdoct\zsh"
                } else {
                    "/opt/anecdoct/zsh"
                })),
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\anecdoct\anecdoct-execve-wrapper"
                } else {
                    "/opt/anecdoct/anecdoct-execve-wrapper"
                })),
            )
            .unified_exec_shell_mode,
        if cfg!(unix) {
            UnifiedExecShellMode::ZshFork(ZshForkConfig {
                shell_zsh_path: AbsolutePathBuf::from_absolute_path("/opt/anecdoct/zsh").unwrap(),
...
async fn search_tool_description_falls_back_to_connector_name_without_description() {
    ...
    let tools = build_specs(
        ...
        Some(vec![ToolInfo {
            ...
            callable_namespace: "mcp__anecdoct_apps__calendar".to_string(),
            namespace_description: None,
            tool: mcp_tool(
...
#[tokio::test]
async fn test_get_lambogenius_tools_mcp_tools_with_additional_properties_schema() {
    let config = test_config().await;
    let model_info = construct_model_info_offline("gpt-5.4", &config);
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
