...
pub(crate) struct ExtensionToolAdapter(Arc<dyn anecdoct_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn anecdoct_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl anecdoct_extension_api::ToolExecutor<anecdoct_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> anecdoct_tools::ToolName {
            anecdoct_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> Option<anecdoct_tools::ToolSpec> {
            Some(anecdoct_tools::ToolSpec::Function(
                anecdoct_tools::ResponsesApiTool {
                    name: "extension_echo".to_string(),
                    description: "Echoes arguments.".to_string(),
                    strict: true,
                    parameters: anecdoct_tools::parse_tool_input_schema(&json!({
                        "type": "object",
                        "properties": {
                            "message": { "type": "string" },
                        },
        ...
        async fn handle(
            &self,
            _call: anecdoct_tools::ToolCall,
        ) -> Result<Box<dyn anecdoct_tools::ToolOutput>, anecdoct_tools::FunctionCallError>
        {
            Ok(Box::new(anecdoct_tools::JsonToolOutput::new(
                json!({ "ok": true }),
            )))
        }
    }
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: anecdoct_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = anecdoct_tools::JsonToolOutput::new(json!({ "ok": true }));
