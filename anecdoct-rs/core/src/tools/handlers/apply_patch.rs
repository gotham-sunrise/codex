...
use anecdoct_apply_patch::ApplyPatchAction;
use anecdoct_apply_patch::ApplyPatchFileChange;
use anecdoct_apply_patch::Hunk;
use anecdoct_apply_patch::StreamingPatchParser;
...
use anecdoct_protocol::protocol::PatchApplyUpdatedEvent;
use anecdoct_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use anecdoct_sandboxing::policy_transforms::merge_permission_profiles;
use anecdoct_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn format_update_chunks_for_progress(chunks: &[anecdoct_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &anecdoct_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    anecdoct_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ToolExecutor<ToolInvocation> for ApplyPatchHandler {
    ...
    async fn handle(
        ...
        let args = match anecdoct_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match anecdoct_apply_patch::verify_apply_patch_args(args, &cwd, fs.as_ref(), Some(&sandbox))
            .await
        {
            anecdoct_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            anecdoct_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            anecdoct_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            anecdoct_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    let sandbox = turn.file_system_sandbox_context(/*additional_permissions*/ None, cwd);
    match anecdoct_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs, Some(&sandbox))
        .await
    {
        anecdoct_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
        ...
        anecdoct_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
