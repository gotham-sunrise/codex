...
use anecdoct_tools::DiscoverableToolAction;
use anecdoct_tools::DiscoverableToolType;
use anecdoct_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use anecdoct_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use anecdoct_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use anecdoct_tools::RequestPluginInstallArgs;
use anecdoct_tools::RequestPluginInstallEntry;
use anecdoct_tools::RequestPluginInstallResult;
use anecdoct_tools::ToolName;
use anecdoct_tools::ToolSpec;
use anecdoct_tools::all_requested_connectors_picked_up;
use anecdoct_tools::build_request_plugin_install_elicitation_request;
use anecdoct_tools::collect_request_plugin_install_entries;
use anecdoct_tools::filter_request_plugin_install_discoverable_tools_for_client;
use anecdoct_tools::verified_connector_install_completed;
...
impl ToolExecutor<ToolInvocation> for RequestPluginInstallHandler {
    ...
    async fn handle(
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("anecdoct-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in anecdoct-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.anecdoct_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    anecdoct_home: &anecdoct_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    turn: &crate::session::turn_context::TurnContext,
    tool: &DiscoverableTool,
    auth: Option<&anecdoct_login::AnecdoctAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match manager.hard_refresh_anecdoct_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh anecdoct apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
