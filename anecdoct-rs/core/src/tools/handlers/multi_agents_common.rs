...
use anecdoct_protocol::protocol::CollabAgentRef;
use anecdoct_protocol::protocol::CollabAgentStatusEntry;
...
pub(crate) fn collab_spawn_error(err: AnecdoctErr) -> FunctionCallError {
    match err {
        AnecdoctErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        AnecdoctErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: AnecdoctErr) -> FunctionCallError {
    match err {
        AnecdoctErr::ThreadNotFound(id) => {
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.anecdoct_linux_sandbox_exe = turn.anecdoct_linux_sandbox_exe.clone();
    #[allow(deprecated)]
...
fn find_spawn_agent_model_name(
    available_models: &[anecdoct_protocol::lambogenius_models::ModelPreset],
    requested_model: &str,
