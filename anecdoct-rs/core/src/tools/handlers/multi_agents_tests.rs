...
use anecdoct_protocol::protocol::FileSystemAccessMode;
use anecdoct_protocol::protocol::FileSystemPath;
use anecdoct_protocol::protocol::FileSystemSandboxEntry;
use anecdoct_protocol::protocol::FileSystemSandboxPolicy;
use anecdoct_protocol::protocol::InitialHistory;
use anecdoct_protocol::protocol::InterAgentCommunication;
use anecdoct_protocol::protocol::NetworkSandboxPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.anecdoct_home)
        .await
        .expect("anecdoct home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    anecdoct_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn spawn_agent_uses_explorer_role_and_preserves_approval_policy() {
    ...
    let mut config = (*turn.config).clone();
    let provider_info = built_in_model_providers(
        /* lambogenius_base_url */ /*lambogenius_base_url*/ None,
    )["ollama"]
        .clone();
...
async fn spawn_agent_service_tier_override_validates_the_effective_child_model() {
...

    {
        ...
        assert_eq!(
            err,
            FunctionCallError::RespondToModel(
                "Service tier `priority` is not supported for model `gpt-5.3-anecdoct`. Supported service tiers: none"
                    .to_string()
            )
        );
    }
...
async fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_turn = child_thread.anecdoct.session.new_default_turn().await;
    child_thread
        .anecdoct
        .session
        .send_event(
...
async fn multi_agent_v2_followup_task_completion_notifies_parent_on_every_turn() {
    ...
    let first_turn = thread.anecdoct.session.new_default_turn().await;
    thread
        .anecdoct
        .session
        .send_event(
    ...
    let second_turn = thread.anecdoct.session.new_default_turn().await;
    thread
        .anecdoct
        .session
        .send_event(
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    ...
    let aborted_turn = thread.anecdoct.session.new_default_turn().await;
    thread
        .anecdoct
        .session
        .send_event(
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(AnecdoctAuth::from_api_key("dummy")),
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.anecdoct.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.anecdoct.session.clone();
    ...
    let operator_session = operator.thread.anecdoct.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.anecdoct.session.new_default_turn().await,
            "resume_agent",
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    turn.anecdoct_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    #[allow(deprecated)]
    ...
    expected.anecdoct_linux_sandbox_exe = turn.anecdoct_linux_sandbox_exe.clone();
    #[allow(deprecated)]
    {
