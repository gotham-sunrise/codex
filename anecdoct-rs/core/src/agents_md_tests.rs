...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".anecdoct-root"), "").unwrap();
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.anecdoct_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.anecdoct_home).unwrap();
...
fn create_skill(anecdoct_home: PathBuf, name: &str, description: &str) {
    let skill_dir = anecdoct_home.join(format!("skills/{name}"));
