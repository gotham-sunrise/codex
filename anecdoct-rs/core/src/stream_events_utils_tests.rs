...
use anecdoct_extension_api::TurnItemContributionFuture;
use anecdoct_extension_api::TurnItemContributor;
...
use anecdoct_protocol::memory_citation::MemoryCitation;
...
#[tokio::test]
async fn save_image_generation_result_saves_base64_to_png_in_anecdoct_home() {
    let anecdoct_home = tempfile::tempdir().expect("create anecdoct home");
    let anecdoct_home = anecdoct_home.path().abs();
    let expected_path =
        image_generation_artifact_path(&anecdoct_home, "session-1", "ig_save_base64");
    let _ = std::fs::remove_file(&expected_path);

    let saved_path =
        save_image_generation_result(&anecdoct_home, "session-1", "ig_save_base64", "Zm9v")
            .await
...
async fn save_image_generation_result_rejects_data_url_payload() {
    ...
    let err = save_image_generation_result(&anecdoct_home, "session-1", "ig_456", result)
        .await
...
async fn save_image_generation_result_overwrites_existing_file() {
    let anecdoct_home = tempfile::tempdir().expect("create anecdoct home");
    let anecdoct_home = anecdoct_home.path().abs();
    let existing_path = image_generation_artifact_path(&anecdoct_home, "session-1", "ig_overwrite");
    ...
    let saved_path =
        save_image_generation_result(&anecdoct_home, "session-1", "ig_overwrite", "Zm9v")
            .await
            .expect("image should be saved");
...
#[tokio::test]
async fn save_image_generation_result_sanitizes_call_id_for_anecdoct_home_output_path() {
    let anecdoct_home = tempfile::tempdir().expect("create anecdoct home");
    let anecdoct_home = anecdoct_home.path().abs();
    let expected_path = image_generation_artifact_path(&anecdoct_home, "session-1", "../ig/..");
    let _ = std::fs::remove_file(&expected_path);

    let saved_path = save_image_generation_result(&anecdoct_home, "session-1", "../ig/..", "Zm9v")
        .await
        .expect("image should be saved");
...
async fn save_image_generation_result_rejects_non_standard_base64() {
    let anecdoct_home = tempfile::tempdir().expect("create anecdoct home");
    let anecdoct_home = anecdoct_home.path().abs();
    let err = save_image_generation_result(&anecdoct_home, "session-1", "ig_urlsafe", "_-8")
        .await
