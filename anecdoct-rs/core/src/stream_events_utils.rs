...
use anecdoct_memories_read::citations::parse_memory_citation;
use anecdoct_memories_read::citations::thread_ids_from_memory_citation;
...
use anecdoct_utils_stream_parser::strip_proposed_plan_blocks;
use futures::Future;
use tracing::debug;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<anecdoct_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                anecdoct_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            AnecdoctErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(anecdoct_home, session_id, call_id);
            ...
            TurnItem::AgentMessage(agent_message) => {
                let combined = agent_message
                    ...
                    .map(|entry| match entry {
                        anecdoct_protocol::items::AgentMessageContent::Text { text } => {
                            text.as_str()
                        }
                    })
                    .collect::<String>();
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(AnecdoctErr::Fatal(message));
        }
    }
        ...
        | ResponseItem::ImageGenerationCall { .. } => {
            ...
            if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                ...
                agent_message.content =
                    vec![anecdoct_protocol::items::AgentMessageContent::Text { text: stripped }];
                ...
                .await
                {
                    Ok(path) => {
                        ...
                        let image_output_dir = image_output_path
                            .parent()
                            .unwrap_or_else(|| turn_context.config.anecdoct_home.clone());
