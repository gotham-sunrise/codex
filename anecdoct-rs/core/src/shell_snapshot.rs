...
impl ShellSnapshot {
    ...
    fn spawn_snapshot_task(
        ...
        tokio::spawn(
            async move {
                let timer =
                    session_telemetry.start_timer("anecdoct.shell_snapshot.duration_ms", &[]);
                ...
                session_telemetry.counter("anecdoct.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_new(
        ...
        let temp_path = anecdoct_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let anecdoct_home = anecdoct_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&anecdoct_home, cleanup_session_id, state_db).await
            {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            anecdoct_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(anecdoct_home, session_id, state_db.as_deref()).await?;
