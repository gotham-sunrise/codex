...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let mut builder = test_anecdoct().with_config(|config| {
        config.model_provider.name = "Non-Lambogenius Model provider".to_string();
        config.compact_prompt = Some(SUMMARIZATION_PROMPT.to_string());
    });
    let initial = builder.build(&server).await?;
    let initial_thread = Arc::clone(&initial.anecdoct);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.anecdoct, "after resume").await?;
    shutdown_thread(&resumed.anecdoct).await?;
...
async fn submit_user_turn(anecdoct: &Arc<AnecdoctThread>, text: &str) -> Result<()> {
    anecdoct
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_compact_turn(anecdoct: &Arc<AnecdoctThread>) -> Result<()> {
    anecdoct.submit(Op::Compact).await?;
    let warning_event =
        wait_for_event(anecdoct, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(anecdoct: &Arc<AnecdoctThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        .header("x-anecdoct-window-id")
        .expect("missing x-anecdoct-window-id header");
