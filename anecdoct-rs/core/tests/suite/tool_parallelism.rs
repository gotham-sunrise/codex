...
async fn run_turn(test: &TestAnecdoct, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestAnecdoct, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_anecdoct_with_test_tool(
    server: &wiremock::MockServer,
) -> anyhow::Result<TestAnecdoct> {
    let mut builder = test_anecdoct().with_model("test-gpt-5.1-anecdoct");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_anecdoct_with_test_tool(&server).await?;
