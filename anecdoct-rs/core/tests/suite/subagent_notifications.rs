...
use anecdoct_core::config::AgentRoleConfig;
...
const INHERITED_MODEL: &str = "gpt-5.3-anecdoct";
...
fn write_home_skill(anecdoct_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = anecdoct_home.join("skills").join(dir);
...
async fn wait_for_spawned_thread_id(test: &TestAnecdoct) -> Result<String> {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_anecdoct::TestAnecdoctBuilder,
    ) -> core_test_support::test_anecdoct::TestAnecdoctBuilder,
) -> Result<(TestAnecdoct, String)> {
    ...
    let mut builder = configure_test(test_anecdoct().with_config(|config| {
        config
            .features
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.anecdoct_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
