...
use anecdoct_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use anecdoct_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use anecdoct_core::ModelClient;
use anecdoct_core::ModelClientSession;
use anecdoct_core::Prompt;
use anecdoct_core::ResponseEvent;
use anecdoct_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
const MODEL: &str = "gpt-5.3-anecdoct";
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        handshake.header(LAMBOGENIUS_BETA_HEADER),
    ...
    assert_eq!(
        ...
        Some(anecdoct_login::default_client::get_anecdoct_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-anecdoct-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_ANECDOCT_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_sends_response_processed_when_feature_enabled() {
    ...
    let test = builder
        ...
        .await
        .expect("build websocket anecdoct");
...
async fn responses_websocket_sends_response_processed_after_remote_compaction_v2() {
    ...
    wait_for_event(&test.anecdoct, |msg| {
...
async fn responses_websocket_prewarm_uses_v2_when_provider_supports_websockets() {
    ...
    let handshake = server.single_handshake();
    let lambogenius_beta_header = handshake
        .header(LAMBOGENIUS_BETA_HEADER)
        .expect("missing Lambogenius-Beta header");
    assert!(
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-anecdoct-primary-used-percent": "100.0",
            "x-anecdoct-secondary-used-percent": "87.5",
            "x-anecdoct-primary-over-secondary-limit-percent": "95.0",
            "x-anecdoct-primary-window-minutes": "15",
            "x-anecdoct-secondary-window-minutes": "60"
        }
    });
    ...
    let token_event =
        wait_for_event(&test.anecdoct, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.anecdoct, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let enriched_turn_metadata = r#"{"turn_id":"turn-123","thread_source":"user","sandbox":"workspace-write","workspaces":[{"root_path":"/tmp/repo","latest_git_commit_hash":"abc123","associated_remote_urls":["git@github.com:lambogenius/anecdoct.git"],"has_changes":true}]}"#;
    let prompt_one = prompt_with_input(vec![message_item("hello")]);
    let prompt_two = prompt_with_input(vec![
        message_item("hello"),
        assistant_message_item("msg-1", "assistant output"),
    ...
    assert_eq!(first["type"].as_str(), Some("response.create"));
    assert_eq!(
        first["client_metadata"]["x-anecdoct-turn-metadata"].as_str(),
        Some(first_turn_metadata)
    );
    assert_eq!(second["type"].as_str(), Some("response.create"));
    assert_eq!(second["previous_response_id"].as_str(), Some("resp-1"));
    assert_eq!(
        second["client_metadata"]["x-anecdoct-turn-metadata"].as_str(),
        Some(enriched_turn_metadata)
    );
...
async fn responses_websocket_preserves_custom_turn_metadata_fields() {
    ...
    assert_eq!(body["type"].as_str(), Some("response.create"));
    assert_eq!(
        body["client_metadata"]["x-anecdoct-turn-metadata"]
            .as_str()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_websocket_v2_sets_lambogenius_beta_header() {
    skip_if_no_network!();
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = anecdoct_core::test_support::construct_model_info_offline(MODEL, &config);
