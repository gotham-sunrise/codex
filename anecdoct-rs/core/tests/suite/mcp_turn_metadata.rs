...
use anecdoct_protocol::protocol::ElicitationAction;
...
fn set_calendar_approval_mode(config: &mut Config, approval_mode: AppToolApproval) {
    ...
    let user_config_path = config.anecdoct_home.join("config.toml").abs();
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request() -> Result<()> {
    ...
    let EventMsg::ElicitationRequest(request) = wait_for_event(&test.anecdoct, |event| {
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-anecdoct-turn-metadata/user_input_requested_during_turn"),
        Some(&json!(true))
    );
