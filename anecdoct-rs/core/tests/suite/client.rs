...
use anecdoct_core::ModelClient;
use anecdoct_core::NewThread;
use anecdoct_core::Prompt;
use anecdoct_core::ResponseEvent;
use anecdoct_core::ThreadManager;
use anecdoct_core::resolve_installation_id;
use anecdoct_core::thread_store_from_config;
...
use anecdoct_protocol::config_types::ModelProviderAuthInfo;
...
use anecdoct_protocol::models::LocalShellAction;
use anecdoct_protocol::models::LocalShellExecAction;
use anecdoct_protocol::models::LocalShellStatus;
use anecdoct_protocol::models::MessagePhase;
use anecdoct_protocol::models::ReasoningItemContent;
use anecdoct_protocol::models::ReasoningItemReasoningSummary;
use anecdoct_protocol::models::ResponseItem;
use anecdoct_protocol::models::WebSearchAction;
...
use anecdoct_protocol::protocol::SessionMeta;
...
use core_test_support::test_anecdoct::TestAnecdoct;
...
/// Writes an `auth.json` into the provided `anecdoct_home` with the specified parameters.
/// Returns the fake JWT string written to `tokens.id_token`.
#[expect(clippy::unwrap_used)]
fn write_auth_json(
    anecdoct_home: &TempDir,
    lambogenius_api_key: Option<&str>,
    ...
    let payload = json!({
        ...
        "https://api.lambogenius.com/auth": {
    ...
    let auth_json = json!({
        "LAMBOGENIUS_API_KEY": lambogenius_api_key,
        "tokens": tokens,
    ...
    std::fs::write(
        anecdoct_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: match anecdoct_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
            {
                Ok(cwd) => cwd,
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = anecdoct_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![anecdoct_protocol::models::ContentItem::InputText {
    ...
    let prior_system = anecdoct_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![anecdoct_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = anecdoct_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Anecdoct to resume from our file
    let anecdoct_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_anecdoct()
        .with_home(anecdoct_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, anecdoct_home, session_path.clone())
        .await
        .expect("resume conversation");
    let anecdoct = test.anecdoct.clone();
    ...
    anecdoct
        ...
        .await
        .unwrap();
    wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let anecdoct_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_anecdoct().with_model("gpt-5.4");
    let test = builder
        .resume(&server, anecdoct_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_anecdoct().with_auth(AnecdoctAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.anecdoct_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_eq!(
        request_body["client_metadata"]["x-anecdoct-installation-id"].as_str(),
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let anecdoct_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&anecdoct_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort;
    let summary = config.model_reasoning_summary;
    let model = anecdoct_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        anecdoct_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(
            AnecdoctAuth::from_api_key("unused-api-key"),
        )),
        thread_id.into(),
        thread_id,
    ...
    let mut stream = client_session
        .stream(
            ...
            &anecdoct_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_anecdoct()
        .with_auth(AnecdoctAuth::from_api_key("Test API Key"))
        .with_config(|config| {
            config.base_instructions = Some("test instructions".to_string());
        });
    let anecdoct = builder
        .build(&server)
        .await
        .expect("create new conversation")
        .anecdoct;
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider = built_in_model_providers(
        /* lambogenius_base_url */ /*lambogenius_base_url*/ None,
    )["lambogenius"]
        .clone();
    model_provider.base_url = Some(format!("{}/api/anecdoct", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_anecdoct()
        .with_auth(create_dummy_anecdoct_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/anecdoct/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match AnecdoctAuth::from_auth_storage(
        anecdoct_home.path(),
    ...
    .await
    {
        Ok(Some(auth)) => anecdoct_core::test_support::auth_manager_from_auth(auth),
        Ok(None) => panic!("No AnecdoctAuth found in anecdoct_home"),
        Err(e) => panic!("Failed to load AnecdoctAuth: {e}"),
    };
    let installation_id = resolve_installation_id(&config.anecdoct_home)
        .await
...
async fn skills_append_to_developer_message() {
    ...
    let anecdoct_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = anecdoct_home.path().join("skills/demo");
    ...
    let anecdoct_home_path = anecdoct_home.path().to_path_buf();
    let mut builder = test_anecdoct()
        .with_home(anecdoct_home.clone())
        .with_auth(AnecdoctAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = anecdoct_home_path.abs();
        });
    let anecdoct = builder
        .build(&server)
        .await
    ...
    let _anecdoct_home_guard = anecdoct_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let anecdoct_home_parent = TempDir::new().unwrap();
    let long_home_parent = anecdoct_home_parent
        .path()
        .join("anecdoct-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let anecdoct_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = anecdoct_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_anecdoct()
        ...
        .with_config(move |config| {
            config.cwd = anecdoct_home_path.abs();
            let user_config_path = anecdoct_home_path.join("config.toml").abs();
    ...
    let _anecdoct_home_guard = anecdoct_home;
    let _anecdoct_home_parent_guard = anecdoct_home_parent;
}
...
async fn includes_configured_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestAnecdoct { anecdoct, .. } = test_anecdoct()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestAnecdoct { anecdoct, .. } =
        test_anecdoct().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let provider = ModelProviderInfo {
        name: "azure".into(),
        base_url: Some(format!("{}/lambogenius", server.uri())),
        env_key: None,
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
    ...
    let model_info =
        anecdoct_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let thread_id = ThreadId::new();
    let auth_manager = anecdoct_core::test_support::auth_manager_from_auth(
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/lambogenius/responses");
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-anecdoct-primary-used-percent", "12.5")
        .insert_header("x-anecdoct-secondary-used-percent", "40.0")
        .insert_header("x-anecdoct-primary-window-minutes", "10")
        .insert_header("x-anecdoct-secondary-window-minutes", "60")
        .insert_header("x-anecdoct-primary-reset-at", "1704069000")
        .insert_header("x-anecdoct-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_anecdoct()
        .with_auth(AnecdoctAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    wait_for_event(&anecdoct, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-anecdoct-primary-used-percent", "100.0")
        .insert_header("x-anecdoct-secondary-used-percent", "87.5")
        .insert_header("x-anecdoct-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-anecdoct-primary-window-minutes", "15")
        .insert_header("x-anecdoct-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_anecdoct();
    let anecdoct_fixture = builder.build(&server).await?;
    let anecdoct = anecdoct_fixture.anecdoct.clone();
    ...
    let token_event = wait_for_event(&anecdoct, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&anecdoct, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&anecdoct, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = AnecdoctErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn azure_overrides_assign_properties_used_for_responses_url() {
    ...
    // Expect POST to /lambogenius/responses with api-version query param
    Mock::given(method("POST"))
        .and(path("/lambogenius/responses"))
...
fn create_dummy_anecdoct_auth() -> AnecdoctAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Anecdoct sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
