...
const FIXED_CWD: &str = "/tmp/anecdoct_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> AnecdoctAuth {
        match self {
            AuthCase::ChatGpt => AnecdoctAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => AnecdoctAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&anecdoct, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().anecdoct_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestAnecdoctHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness_inner(
    ...
    let mut builder = test_anecdoct().with_auth(settings.auth.build());
    if hooks {
        builder = builder.with_pre_build_hook(write_manual_compact_hooks);
    }
    TestAnecdoctHarness::with_builder(builder.with_config(move |config| {
        config.cwd = anecdoct_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
fn rollout_path(harness: &TestAnecdoctHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn capture_from_requests(
    mode: Mode,
    anecdoct: &anecdoct_core::AnecdoctThread,
    rollout_path: &Path,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<ANECDOCT_HOME>");
            search_start = start_index + "<ANECDOCT_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <ANECDOCT_HOME>/skills/.system/imagegen/SKILL.md and \
         <ANECDOCT_HOME>/skills/custom/SKILL.md"
    );
}
