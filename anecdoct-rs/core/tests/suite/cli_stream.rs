...
fn repo_root() -> std::path::PathBuf {
    ...
    anecdoct_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = anecdoct_utils_cargo_bin::cargo_bin("anecdoct").unwrap();
    ...
    cmd.arg("exec")
        ...
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("ANECDOCT_HOME", home.path())
        .env("LAMBOGENIUS_API_KEY", "dummy");
    ...
    //     10,
    //     None,
    //     anecdoct_core::ThreadSortKey::UpdatedAt,
    //     &[],
...
/// Ensures `lambogenius_base_url` config override routes built-in lambogenius provider requests.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_mode_stream_cli_supports_lambogenius_base_url_config_override() {
    skip_if_no_network!();
    ...
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg(format!("lambogenius_base_url=\"{}/v1\"", server.uri()))
        .arg("-C")
        .arg(&repo_root)
        .arg("hello?");
    cmd.env("ANECDOCT_HOME", home.path())
...
/// received by a mock Lambogenius Responses endpoint.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Anecdoct to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `anecdoct exec --profile ...` preserves the active profile when it
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the anecdoct CLI and invoke `exec`, which is what records a session.
    let bin = anecdoct_utils_cargo_bin::cargo_bin("anecdoct").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.timeout(Duration::from_secs(30));
    cmd.arg("exec")
    ...
    let bin2 = anecdoct_utils_cargo_bin::cargo_bin("anecdoct").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("ANECDOCT_HOME", home.path())
        .env("LAMBOGENIUS_API_KEY", "dummy");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume anecdoct-cli run failed");
