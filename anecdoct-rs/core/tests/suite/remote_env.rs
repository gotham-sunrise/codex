...
use core_test_support::test_anecdoct::test_env;
...
async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestAnecdoct> {
...
async fn wait_for_completion_without_patch_approval(test: &TestAnecdoct) {
    let event = wait_for_event(&test.anecdoct, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/anecdoct-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    test.fs()
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/anecdoct-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    test.fs()
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/anecdoct-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/anecdoct-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    test.fs()
...
async fn remote_test_env_remove_removes_symlink_not_target() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/anecdoct-remote-remove-link-{}",
        std::process::id()
    ));
    let allowed_dir = root.join("allowed");
...
fn remote_test_file_path() -> PathBuf {
    ...
    PathBuf::from(format!(
        "/tmp/anecdoct-remote-test-env-{}-{nanos}.txt",
        std::process::id()
    ))
}
