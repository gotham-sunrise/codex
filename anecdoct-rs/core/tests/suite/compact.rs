...
use anecdoct_core::compact::SUMMARIZATION_PROMPT;
use anecdoct_core::compact::SUMMARY_PREFIX;
...
use core_test_support::test_anecdoct::turn_permission_fields;
...
fn non_lambogenius_model_provider(server: &MockServer) -> ModelProviderInfo {
    let mut provider = built_in_model_providers(
        /* lambogenius_base_url */ /*lambogenius_base_url*/ None,
    )["lambogenius"]
        .clone();
    provider.name = "Lambogenius (test)".into();
...
async fn assert_compaction_uses_turn_lifecycle_id(
    anecdoct: &std::sync::Arc<anecdoct_core::AnecdoctThread>,
) {
    ...
    while turn_completed_id.is_none() {
        let event = anecdoct.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Anecdoct.
    let model_provider = non_lambogenius_model_provider(&server);
    ...
    anecdoct.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Anecdoct to flush rollout entries before inspecting the file.
    anecdoct.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    anecdoct.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&anecdoct, |ev| match ev {
    ...
    wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let anecdoct = builder.build(&server).await.unwrap().anecdoct;
    ...
    let first = wait_for_event_match(&anecdoct, |ev| match ev {
    ...
    let last = wait_for_event_match(&anecdoct, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    {
        let event = anecdoct.next_event().await.unwrap();
        match event.msg {
            EventMsg::ItemStarted(ItemStartedEvent {
...
async fn multiple_auto_compact_per_task_runs_after_token_limit_hit() {
    ...
    let non_lambogenius_provider_name = non_lambogenius_model_provider(&server).name;
    let anecdoct = test_anecdoct()
        .with_config(move |config| {
            config.model_provider.name = non_lambogenius_provider_name;
        })
        .build(&server)
        .await
        .expect("build anecdoct")
        .anecdoct;
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        anecdoct_protocol::models::ResponseItem::Message {
            id: None,
        ...
        anecdoct_protocol::models::ResponseItem::Compaction {
    ...
    let mut resume_builder = test_anecdoct().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
    });
    ...
    wait_for_event(&resumed.anecdoct, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let previous_model = "gpt-5.3-anecdoct";
    ...
    let model_provider = non_lambogenius_model_provider(&server);
    let mut builder = test_anecdoct()
        .with_auth(AnecdoctAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
            set_test_compact_prompt(config);
        });
    let test = builder.build(&server).await.expect("build test anecdoct");
    ...
    test.anecdoct
        ...
        .await
        .expect("submit first user turn");
    wait_for_event(&test.anecdoct, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.anecdoct).await;
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    let model_provider = non_lambogenius_model_provider(&server);
    let mut initial_builder = test_anecdoct()
        .with_auth(AnecdoctAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test anecdoct");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    wait_for_event(&initial.anecdoct, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let model_provider = non_lambogenius_model_provider(&server);
    let mut resumed_builder = test_anecdoct()
        .with_auth(AnecdoctAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.anecdoct).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let mut model_provider = non_lambogenius_model_provider(&server);
    ...
    let reconnect_message = wait_for_event_match(&anecdoct, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&anecdoct, |event| match event {
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let anecdoct = builder.build(&server).await.unwrap();

    anecdoct.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    anecdoct.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_pre_turn_compaction_strips_incoming_model_switch() {
    ...
    let next_model = "gpt-5.3-anecdoct";
...
async fn snapshot_request_shape_pre_turn_compaction_context_window_exceeded() {
    ...
    let error_message = wait_for_event_match(&anecdoct, |event| match event {
        EventMsg::Error(err) => Some(err.message.clone()),
        _ => None,
    })
    .await;
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    anecdoct.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
