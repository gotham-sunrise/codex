...
async fn anecdoct_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    anecdoct_returns_json_result("gpt-5.4".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn anecdoct_returns_json_result_for_gpt5_anecdoct() -> anyhow::Result<()> {
    anecdoct_returns_json_result("gpt-5.4".to_string()).await
}

async fn anecdoct_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("anecdoct_output_schema".into()))
    ...
    let TestAnecdoct { anecdoct, cwd, .. } = test_anecdoct().build(&server).await?;
    ...
    let message = wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
