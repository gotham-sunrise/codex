...
use anecdoct_core::RolloutRecorder;
use anecdoct_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(
    ...
    let sessions = anecdoct_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(anecdoct_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(anecdoct_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(anecdoct_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = anecdoct_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_anecdoct_home_directory() {
    let repo = TempDir::new().unwrap();
    let anecdoct_home = repo.path().join(".anecdoct");
    std::fs::create_dir_all(&anecdoct_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".anecdoct/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&anecdoct_home, id);

    let found =
        find_thread_path_by_id_str(&anecdoct_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
