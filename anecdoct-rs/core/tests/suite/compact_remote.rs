...
use anecdoct_protocol::protocol::RealtimeConversationRealtimeEvent;
use anecdoct_protocol::protocol::RealtimeEvent;
use anecdoct_protocol::protocol::RealtimeOutputModality;
...
use core_test_support::test_anecdoct::TestAnecdoctBuilder;
use core_test_support::test_anecdoct::TestAnecdoctHarness;
...
fn remote_realtime_test_anecdoct_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestAnecdoctBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_anecdoct()
        .with_auth(AnecdoctAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(anecdoct: &anecdoct_core::AnecdoctThread) -> Result<()> {
    anecdoct
    ...
    wait_for_event_match(anecdoct, |msg| match msg {
...
async fn close_realtime_conversation(anecdoct: &anecdoct_core::AnecdoctThread) -> Result<()> {
    anecdoct.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(anecdoct, |msg| match msg {
...
async fn wait_for_turn_complete(anecdoct: &anecdoct_core::AnecdoctThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestAnecdoctHarness::with_builder(
        test_anecdoct().with_auth(AnecdoctAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let anecdoct = harness.test().anecdoct.clone();
    ...
    anecdoct
        ...
        .await?;
    wait_for_turn_complete(&anecdoct).await;

    anecdoct.submit(Op::Compact).await?;
    wait_for_turn_complete(&anecdoct).await;
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let mut builder = test_anecdoct().with_auth(auth);
    ...
    let harness = TestAnecdoctHarness::with_builder(builder).await?;
    let anecdoct = harness.test().anecdoct.clone();
    let image_url =
...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        AnecdoctAuth::from_api_key("dummy"),
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-anecdoct-beta-features")
            .as_deref()
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let new_thread = test
        ...
        .await?;
    test.anecdoct = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "anecdoct_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "anecdoct_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&anecdoct, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestAnecdoctHarness::with_builder(
        test_anecdoct()
            .with_auth(AnecdoctAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_anecdoct = baseline_harness.test().anecdoct.clone();
    ...
    baseline_anecdoct
        ...
        .await?;
    wait_for_event(&baseline_anecdoct, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    baseline_anecdoct.submit(Op::Compact).await?;
    wait_for_event(&baseline_anecdoct, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestAnecdoctHarness::with_builder(
        test_anecdoct()
            .with_auth(AnecdoctAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_anecdoct = override_harness.test().anecdoct.clone();
    ...
    override_anecdoct
        ...
        .await?;
    wait_for_event(&override_anecdoct, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_anecdoct.submit(Op::Compact).await?;
    wait_for_event(&override_anecdoct, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    anecdoct.submit(Op::Shutdown).await?;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.anecdoct.submit(Op::Compact).await?;
    wait_for_event(&initial.anecdoct, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    initial.anecdoct.submit(Op::Shutdown).await?;
    wait_for_event(&initial.anecdoct, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.anecdoct, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.anecdoct
        ...
        .await?;
    wait_for_event(&test.anecdoct, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    test.anecdoct.submit(Op::Compact).await?;
    wait_for_event(&test.anecdoct, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder =
        remote_realtime_test_anecdoct_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.anecdoct.as_ref()).await?;
    ...
    close_realtime_conversation(test.anecdoct.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_anecdoct_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_anecdoct_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.anecdoct.as_ref()).await?;
    ...
    close_realtime_conversation(initial.anecdoct.as_ref()).await?;
