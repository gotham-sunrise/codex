...
async fn request_body_is_zstd_compressed_for_anecdoct_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/anecdoct/v1", server.uri());
    ...
    let anecdoct = builder.build(&server).await?.anecdoct;
