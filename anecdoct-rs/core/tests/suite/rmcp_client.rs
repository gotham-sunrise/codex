...
use anecdoct_utils_cargo_bin::cargo_bin;
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/anecdoct-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn wait_for_mcp_server(fixture: &TestAnecdoct, server_name: &str) -> anyhow::Result<()> {
...
async fn call_cwd_tool(
    ...
    wait_for_event(&fixture.anecdoct, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.anecdoct, |ev| {
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.anecdoct, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.anecdoct, |ev| {
            matches!(
                ev,
...
async fn stdio_image_responses_round_trip() -> anyhow::Result<()> {
    ...
    let base64_only = LAMBOGENIUS_PNG
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: anecdoct_protocol::lambogenius_models::ReasoningEffort::Medium,
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Anecdoct should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Anecdoct can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Anecdoct will call the MCP echo tool
    ...
    // Phase 3: configure Anecdoct with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Anecdoct begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.anecdoct, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
/// This test writes to a fallback credentials file in ANECDOCT_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire ANECDOCT_HOME through the code.
#[test]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Anecdoct will call the OAuth-backed
    ...
    // Phase 3: seed an isolated ANECDOCT_HOME with fallback OAuth tokens for this
    ...
    // Phase 4: configure Anecdoct with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert Anecdoct begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.anecdoct, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Anecdoct fixture.
