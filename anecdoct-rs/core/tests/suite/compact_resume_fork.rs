...
//! request payload that Anecdoct would send to the model and assert that the
...
async fn start_test_conversation(
    ...
    (test.home, test.config, test.thread_manager, test.anecdoct)
}

async fn user_turn(conversation: &Arc<AnecdoctThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
            environments: None,
...
async fn compact_conversation(conversation: &Arc<AnecdoctThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<AnecdoctThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<AnecdoctThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = anecdoct_core::test_support::auth_manager_from_auth(
        anecdoct_login::AnecdoctAuth::from_api_key("dummy"),
    );
