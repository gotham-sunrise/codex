...
async fn build_analytics_plugin_test_anecdoct(
    server: &MockServer,
    anecdoct_home: Arc<TempDir>,
) -> Result<Arc<anecdoct_core::AnecdoctThread>> {
...
async fn build_apps_enabled_plugin_test_anecdoct(
    server: &MockServer,
    anecdoct_home: Arc<TempDir>,
...
async fn wait_for_sample_mcp_ready(anecdoct: &anecdoct_core::AnecdoctThread) -> Result<()> {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let anecdoct_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(anecdoct_home.as_ref());
    write_plugin_app_plugin(anecdoct_home.as_ref());
    let anecdoct = build_apps_enabled_plugin_test_anecdoct(
        &server,
        Arc::clone(&anecdoct_home),
    ...
    anecdoct
        .submit(Op::UserInput {
            environments: None,
            items: vec![anecdoct_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(anecdoct_home.as_ref());
    write_plugin_mcp_plugin(anecdoct_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(anecdoct_home.as_ref());
    ...
    .await?;
    wait_for_sample_mcp_ready(&anecdoct).await?;

    anecdoct
        .submit(Op::UserInput {
            environments: None,
            items: vec![anecdoct_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__anecdoct_apps__google_calendar"),
    ...
    let calendar_tool = request
        .tool_by_name("mcp__anecdoct_apps__google_calendar", "_create_event")
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let anecdoct_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(anecdoct_home.as_ref());
    let anecdoct = build_analytics_plugin_test_anecdoct(&server, anecdoct_home).await?;
    ...
    let plugin_event = loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/anecdoct/analytics-events/events")
            .find_map(|request| {
                let payload: serde_json::Value = serde_json::from_slice(&request.body).ok()?;
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "anecdoct_plugin_used")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        serde_json::json!(anecdoct_login::default_client::originator().value)
    );
