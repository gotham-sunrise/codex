...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    anecdoct.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
}
...
async fn interrupt_tool_records_history_entries() {
    ...
    let fixture = test_anecdoct()
        ...
        .await
        .unwrap();
    let anecdoct = Arc::clone(&fixture.anecdoct);
