...
use anecdoct_core::test_support::auth_manager_from_auth;
...
use anecdoct_protocol::protocol::RealtimeConversationVersion;
use anecdoct_protocol::protocol::RealtimeEvent;
use anecdoct_protocol::protocol::RealtimeNoopRequested;
...
use anecdoct_utils_output_truncation::approx_token_count;
use anyhow::Context;
use anyhow::Result;
use chrono::Utc;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Anecdoct.";
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "ANECDOCT_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in anecdoct_network_proxy::PROXY_ENV_KEYS {
        command.env_remove(key);
    }
    match lambogenius_api_key {
        Some(lambogenius_api_key) => {
            command.env(LAMBOGENIUS_API_KEY_ENV_VAR, lambogenius_api_key);
        }
        None => {
            command.env_remove(LAMBOGENIUS_API_KEY_ENV_VAR);
        }
    }
...
async fn seed_recent_thread(
    ...
    let db = test.anecdoct.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = anecdoct_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.anecdoct, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.anecdoct, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.anecdoct, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.anecdoct.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.anecdoct, |msg| match msg {
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.anecdoct, |msg| match msg {
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=anecdoct-realtime-call-boundary")
    );
    ...
    assert_eq!(
        body,
        format!(
            "--anecdoct-realtime-call-boundary\r\n\
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.anecdoct, |msg| match msg {
    ...
    let stale_event = timeout(Duration::from_millis(700), async {
        wait_for_event_match(&test.anecdoct, |msg| match msg {
            EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.anecdoct, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_start_uses_lambogenius_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    if std::env::var_os(REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR).is_none() {
        return run_realtime_conversation_test_in_subprocess(
            "suite::realtime_conversation::conversation_start_uses_lambogenius_env_key_fallback_with_chatgpt_auth",
    ...
    test.anecdoct.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.anecdoct, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.anecdoct_error_info, Some(AnecdoctErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.anecdoct
        ...
        .await?;
    let _ = wait_for_event_match(&test.anecdoct, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_experimental_realtime_ws_base_url_override() -> Result<()> {
    ...
    let mut builder = test_anecdoct().with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
...
async fn conversation_uses_explicit_start_voice() -> Result<()> {
    ...
    .await;
    let test = test_anecdoct().build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
            .await
...
async fn conversation_rejects_voice_for_wrong_realtime_version() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.anecdoct, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let history = user_turns
        ...
        .collect::<Vec<_>>();
    test.anecdoct.shutdown_and_wait().await?;
    let resumed_thread = test
        .thread_manager
        .resume_thread_with_history(
            test.config.clone(),
            InitialHistory::Forked(history),
            auth_manager_from_auth(AnecdoctAuth::from_api_key("dummy")),
            /*persist_extended_history*/ false,
            /*parent_trace*/ None,
        )
        .await?;
    let anecdoct = resumed_thread.thread;
    ...
    anecdoct.shutdown_and_wait().await?;
    realtime_server.shutdown().await;
    Ok(())
}
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("anecdoct-rs/core"))?;
    ...
    assert!(startup_context.contains("anecdoct-rs/"));
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_anecdoct().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
