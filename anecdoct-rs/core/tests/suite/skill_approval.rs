...
fn write_skill_metadata(home: &Path, name: &str, contents: &str) -> Result<()> {
    ...
    fs::write(metadata_dir.join("lambogenius.yaml"), contents)?;
    Ok(())
}
...
fn skill_script_command(test: &TestAnecdoct, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestAnecdoct) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.anecdoct.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestAnecdoct) {
    wait_for_event(test.anecdoct.as_ref(), |event| {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/anecdoct-zsh-fork-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();
