...
//! Optional smoke tests that hit the real Lambogenius /v1/responses endpoint. They are `#[ignore]` by
//! default so CI stays deterministic and free. Developers can run them locally with
//! `cargo test --test live_cli -- --ignored` provided they set a valid `LAMBOGENIUS_API_KEY`.
...
fn require_api_key() -> String {
    std::env::var("LAMBOGENIUS_API_KEY")
        .expect("LAMBOGENIUS_API_KEY env var not set — skip running live tests")
}
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let anecdoct_home = home.path().join(".anecdoct");
    ...
    let mut cmd = Command::new(anecdoct_utils_cargo_bin::cargo_bin("anecdoct-rs").unwrap());
    cmd.current_dir(dir.path());
    cmd.env("LAMBOGENIUS_API_KEY", require_api_key());
    ...
    let mut child = cmd.spawn().expect("failed to spawn anecdoct-rs");
...
fn live_create_file_hello_txt() {
    if std::env::var("LAMBOGENIUS_API_KEY").is_err() {
        eprintln!("skipping live_create_file_hello_txt – LAMBOGENIUS_API_KEY not set");
        return;
    }
...
fn live_print_working_directory() {
    if std::env::var("LAMBOGENIUS_API_KEY").is_err() {
        eprintln!("skipping live_print_working_directory – LAMBOGENIUS_API_KEY not set");
        return;
    }
