...
async fn responses_api_parent_and_subagent_requests_include_identity_headers() -> Result<()> {
    ...
    let parent_mock = mount_sse_once_match(
        ...
        |req: &wiremock::Request| {
            request_body_contains(req, PARENT_PROMPT)
                && request_header(req, "x-lambogenius-subagent").is_none()
        },
        sse(vec![
    ...
    let child_mock = mount_sse_once_match(
        ...
        |req: &wiremock::Request| {
            request_body_contains(req, CHILD_PROMPT)
                ...
                && request_header(req, "x-lambogenius-subagent") == Some("collab_spawn")
        },
        sse(vec![
    ...
    let parent = wait_for_matching_request(&parent_mock, "parent request", |request| {
        request.body_contains_text(PARENT_PROMPT)
            && request.header("x-lambogenius-subagent").is_none()
    })
    .await?;
    let child = wait_for_matching_request(&child_mock, "child request", |request| {
        request.body_contains_text(CHILD_PROMPT)
            ...
            && request.header("x-lambogenius-subagent").as_deref() == Some("collab_spawn")
    })
    .await?;

    let parent_window_id = parent
        .header("x-anecdoct-window-id")
        .ok_or_else(|| anyhow!("parent request missing x-anecdoct-window-id"))?;
    let child_window_id = child
        .header("x-anecdoct-window-id")
        .ok_or_else(|| anyhow!("child request missing x-anecdoct-window-id"))?;
    ...
    assert_eq!(parent.header("x-lambogenius-subagent"), None);
    assert_eq!(
        child.header("x-lambogenius-subagent").as_deref(),
        Some("collab_spawn")
    );
    assert_eq!(
        child.header("x-anecdoct-parent-thread-id").as_deref(),
...
async fn submit_turn_with_timeout(test: &TestAnecdoct, prompt: &str) -> Result<()> {
