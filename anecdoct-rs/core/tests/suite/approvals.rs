...
use anecdoct_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestAnecdoct) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Anecdoct helper binary
...
impl Expectation {
    fn verify(&self, test: &TestAnecdoct, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.anecdoct, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestAnecdoct) {
    let event = wait_for_event(&test.anecdoct, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestAnecdoct) {
...
async fn wait_for_spawned_thread(test: &TestAnecdoct) -> Result<Arc<AnecdoctThread>> {
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command = "touch /tmp/anecdoct-fallback-rule-test.txt && echo hello > /tmp/anecdoct-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://anecdoct-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
