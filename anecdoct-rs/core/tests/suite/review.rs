...
use anecdoct_core::REVIEW_PROMPT;
use anecdoct_core::config::Config;
use anecdoct_core::review_format::render_review_output_text;
use anecdoct_protocol::models::ContentItem;
use anecdoct_protocol::models::ResponseItem;
use anecdoct_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use anecdoct_protocol::protocol::EventMsg;
use anecdoct_protocol::protocol::ExitedReviewModeEvent;
use anecdoct_protocol::protocol::Op;
use anecdoct_protocol::protocol::ReviewCodeLocation;
use anecdoct_protocol::protocol::ReviewFinding;
use anecdoct_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Anecdoct sandbox network restrictions.
    skip_if_no_network!();
    ...
    let anecdoct = new_conversation_for_server(&server, anecdoct_home.clone(), |_| {}).await;
    ...
    let _entered =
        wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = anecdoct.rollout_path().expect("rollout path");
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let anecdoct = new_conversation_for_server(&server, anecdoct_home.clone(), |cfg| {
    ...
    let _entered =
        wait_for_event(&anecdoct, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&anecdoct, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = anecdoct_home.path().join("resume.jsonl");
    {
        ...
        let user = anecdoct_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = anecdoct_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let anecdoct = resume_conversation_for_server(
        &server,
        anecdoct_home.clone(),
        session_file.clone(),
        |_| {},
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let anecdoct = new_conversation_for_server(&server, anecdoct_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, anecdoct_home, resume_path)
        .await
