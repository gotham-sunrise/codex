...
async fn build_anecdoct(server: &StreamingSseServer) -> Arc<AnecdoctThread> {
    test_anecdoct()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .unwrap_or_else(|err| panic!("build streaming Anecdoct test session: {err}"))
        .anecdoct
}

async fn submit_user_input(anecdoct: &AnecdoctThread, text: &str) {
    anecdoct
        .submit(Op::UserInput {
            environments: None,
...
async fn submit_danger_full_access_user_turn(test: &TestAnecdoct, text: &str) {
...
async fn steer_user_input(anecdoct: &AnecdoctThread, text: &str) {
    anecdoct
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(anecdoct: &AnecdoctThread, text: &str) {
    anecdoct
...
async fn wait_for_reasoning_item_started(anecdoct: &AnecdoctThread) {
    wait_for_event(anecdoct, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(anecdoct: &AnecdoctThread, text: &str) {
...
async fn wait_for_turn_complete(anecdoct: &AnecdoctThread) {
    wait_for_event(anecdoct, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let anecdoct = build_anecdoct(&server).await;

    submit_user_input(&anecdoct, "first prompt").await;

    wait_for_reasoning_item_started(&anecdoct).await;

    submit_queue_only_agent_mail(&anecdoct, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&anecdoct, "first answer").await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&anecdoct, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    let anecdoct = test_anecdoct()
        .with_model("gpt-5.4")
        .with_config(|config| {
            config.model_provider.name = "Lambogenius (test)".to_string();
    ...
    submit_user_input(&anecdoct, "first prompt").await;
    submit_user_input(&anecdoct, "second prompt").await;

    wait_for_agent_message(&anecdoct, "resumed old task").await;
    wait_for_turn_complete(&anecdoct).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&anecdoct, "processed steered prompt").await;
    wait_for_turn_complete(&anecdoct).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&anecdoct, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&anecdoct, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
