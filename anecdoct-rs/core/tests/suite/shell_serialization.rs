...
use core_test_support::test_anecdoct::ShellModelOutput;
...
fn configure_shell_model(
    builder: TestAnecdoctBuilder,
    output_type: ShellModelOutput,
) -> TestAnecdoctBuilder {
    match output_type {
        ShellModelOutput::ShellCommand => builder.with_model("test-gpt-5-anecdoct"),
    }
}
...
async fn shell_output_is_structured_with_freeform_apply_patch(
    ...
    let mut builder = configure_shell_model(test_anecdoct(), output_type);
