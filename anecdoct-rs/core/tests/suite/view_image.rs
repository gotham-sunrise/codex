...
use anecdoct_exec_server::LOCAL_ENVIRONMENT_ID;
use anecdoct_exec_server::REMOTE_ENVIRONMENT_ID;
...
fn disabled_user_turn(test: &TestAnecdoct, items: Vec<UserInput>, model: String) -> Op {
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    match item_started.expect("view image item started event emitted") {
        anecdoct_protocol::items::TurnItem::ImageView(item) => {
...
async fn view_image_routes_to_selected_remote_environment() -> anyhow::Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/anecdoct-view-image-routing-{}",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn view_image_tool_can_preserve_original_resolution_when_requested_on_gpt5_3_anecdoct()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn view_image_tool_resizes_when_model_lacks_original_detail_support() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_anecdoct().with_model("gpt-5.2");
    let test = builder.build_with_remote_env(&server).await?;
    let TestAnecdoct {
        anecdoct,
        session_configured,
