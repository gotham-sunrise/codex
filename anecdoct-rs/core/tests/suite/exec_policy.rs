...
async fn submit_user_turn(
    test: &core_test_support::test_anecdoct::TestAnecdoct,
    prompt: &str,
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    let mut builder = test_anecdoct().with_config(|config| {
        let policy_path = config.anecdoct_home.join("rules").join("policy.rules");
        fs::create_dir_all(
            policy_path
                .parent()
    ...
    let EventMsg::ExecCommandEnd(end) = wait_for_event(&test.anecdoct, |event| {
