...
use anecdoct_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(begin) = wait_for_event(&test.anecdoct, |event| {
    ...
    let EventMsg::McpToolCallEnd(end) = wait_for_event(&test.anecdoct, |event| {
    ...
    assert_eq!(
        ...
        McpInvocation {
            server: "anecdoct_apps".to_string(),
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/_anecdoct_apps"),
        Some(&json!({
            "call_id": "calendar-call-1",
            "resource_uri": CALENDAR_CREATE_EVENT_RESOURCE_URI,
            "contains_mcp_source": true,
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-anecdoct-turn-metadata/session_id"),
        Some(&json!(test.session_configured.session_id.to_string()))
    );
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-anecdoct-turn-metadata/thread_id"),
        Some(&json!(test.session_configured.thread_id.to_string()))
    );
    assert!(
        apps_tool_call
            .pointer("/params/_meta/x-anecdoct-turn-metadata/turn_id")
            .and_then(Value::as_str)
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-anecdoct-turn-metadata/model")
            .and_then(Value::as_str),
        Some("gpt-5.4")
    );
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-anecdoct-turn-metadata/reasoning_effort")
            .and_then(Value::as_str),
        Some(first_request_reasoning_effort)
    );
    let mcp_turn_started_at_unix_ms = apps_tool_call
        .pointer("/params/_meta/x-anecdoct-turn-metadata/turn_started_at_unix_ms")
    ...
    let first_request_turn_metadata: Value = serde_json::from_str(
        &requests[0]
            .header("x-anecdoct-turn-metadata")
...
async fn tool_search_returns_deferred_dynamic_tool_and_routes_follow_up_call() -> Result<()> {
    ...
    let mut builder = test_anecdoct().with_config(configure_search_capable_model);
    ...
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.anecdoct, |event| {
    ...
    assert_eq!(request.namespace.as_deref(), Some("anecdoct_app"));
    ...
    assert_eq!(
        ...
        vec![json!({
            "type": "namespace",
            "name": "anecdoct_app",
            "description": "Tools in the anecdoct_app namespace.",
            "tools": [{
                "type": "function",
                "name": tool_name,
