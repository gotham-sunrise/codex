...
async fn websocket_test_anecdoct_shell_chain() -> Result<()> {
    ...
    let mut builder = test_anecdoct().with_windows_cmd_shell();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn websocket_v2_test_anecdoct_shell_chain() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    assert_eq!(
        handshake.header("lambogenius-beta"),
