...
use core_test_support::test_anecdoct::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestAnecdoctHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestAnecdoctBuilder) -> TestAnecdoctBuilder,
) -> Result<TestAnecdoctHarness> {
    let builder = configure(test_anecdoct());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestAnecdoctHarness::with_remote_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestAnecdoctHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_anecdoct_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let anecdoct_linux_sandbox_exe = harness
        .test()
        .config
        .anecdoct_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include anecdoct-linux-sandbox helper");
    assert_eq!(
        anecdoct_linux_sandbox_exe
            .file_name()
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&anecdoct_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
