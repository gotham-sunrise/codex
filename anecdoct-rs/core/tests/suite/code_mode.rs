...
async fn run_code_mode_turn(
    ...
    prompt: &str,
    code: &str,
) -> Result<(TestAnecdoct, ResponseMock)> {
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-anecdoct").await
}
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__anecdoct_apps__calendar_timezone_option_99"
);
if (!tool) {
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "anecdoct_app_hidden_dynamic_tool");
const out = await tools.anecdoct_app_hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    let turn_id = wait_for_event_match(&test.anecdoct, |event| match event {
        EventMsg::TurnStarted(event) => Some(event.turn_id.clone()),
        _ => None,
    })
    .await;
    let request = wait_for_event_match(&test.anecdoct, |event| match event {
    ...
    test.anecdoct
        ...
        .await?;
    wait_for_event(&test.anecdoct, |event| match event {
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String(
            "anecdoct_app_hidden_dynamic_tool".to_string()
        ))
    );
    assert_eq!(
        parsed.get("out"),
        ...
        parsed
            ...
            .is_some_and(|description| {
                description.contains("A hidden dynamic tool.")
                    ...
                    && description.contains("anecdoct_app_hidden_dynamic_tool(args:")
            })
    );
