...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let request_recorder = responses::mount_sse_once_match(
        &server,
        header("x-lambogenius-subagent", "review"),
        response_body,
    )
    .await;
    ...
    let anecdoct_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-lambogenius-subagent").as_deref(),
        Some("review")
    );
    assert_eq!(
        request.header("x-anecdoct-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-anecdoct-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-anecdoct-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(request.header("x-anecdoct-sandbox"), None);
}
...
async fn responses_stream_includes_subagent_header_on_other() {
    ...
    let request_recorder = responses::mount_sse_once_match(
        &server,
        header("x-lambogenius-subagent", "my-task"),
        response_body,
    )
    .await;
...
async fn responses_respects_model_info_overrides_from_config() {
    ...
    let thread_id = ThreadId::new();
    let auth_mode = anecdoct_core::test_support::auth_manager_from_auth(
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = first_request
        ...
        .expect("x-anecdoct-turn-metadata header should be present");
    let initial_parsed: serde_json::Value = serde_json::from_str(&initial_header)
        .expect("x-anecdoct-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-anecdoct-turn-metadata"
    );
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/lambogenius/anecdoct.git",
    ]);
