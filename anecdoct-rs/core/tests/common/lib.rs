...
use anecdoct_utils_cargo_bin::CargoBinError;
...
use anecdoct_config::NetworkRequirementsToml;
...
pub use anecdoct_utils_absolute_path::test_support::PathBufExt;
pub use anecdoct_utils_absolute_path::test_support::PathExt;
...
pub mod test_anecdoct;
pub mod test_anecdoct_exec;
pub mod tracing;
pub mod zsh_fork;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    anecdoct_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    anecdoct_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(anecdoct_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = anecdoct_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("anecdoct-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\anecdoct\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.anecdoct`.
pub async fn load_default_config_for_test(anecdoct_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        anecdoct_linux_sandbox_exe: Some(
            find_anecdoct_linux_sandbox_exe()
                .expect("should find binary for anecdoct-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_anecdoct_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    anecdoct_utils_cargo_bin::cargo_bin("anecdoct-linux-sandbox")
}

pub async fn wait_for_event<F>(
    anecdoct: &AnecdoctThread,
    predicate: F,
) -> anecdoct_protocol::protocol::EventMsg
where
    F: FnMut(&anecdoct_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(anecdoct, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(anecdoct: &AnecdoctThread, matcher: F) -> T
where
    F: Fn(&anecdoct_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(anecdoct, |ev| matcher(ev).is_some()).await;
...
pub fn sandbox_env_var() -> &'static str {
    anecdoct_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    anecdoct_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

const REMOTE_ENV_ENV_VAR: &str = "ANECDOCT_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    anecdoct_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    anecdoct_utils_cargo_bin::cargo_bin("test_stdio_server")
...
#[macro_export]
macro_rules! anecdoct_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_anecdoct_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("anecdoct-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
