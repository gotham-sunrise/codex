...
use crate::test_anecdoct::TestAnecdoct;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) =
        anecdoct_utils_cargo_bin::cargo_bin("anecdoct-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `anecdoct-execve-wrapper` binary");
        return Ok(None);
    };
