...
pub enum InstallContext {
    Standalone {
        ...
        /// `~/.anecdoct/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        release_dir: PathBuf,
    ...
    /// A Anecdoct binary launched through the npm-managed `anecdoct.js` shim.
    Npm,
    /// A Anecdoct binary launched through the bun-managed `anecdoct.js` shim.
    Bun,
    /// A Anecdoct binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Anecdoct binaries, custom
    /// internal launchers, and tests that execute Anecdoct from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let anecdoct_home = anecdoct_utils_home_dir::find_anecdoct_home().ok();
        Self::from_exe_with_anecdoct_home(
            is_macos,
            current_exe,
            managed_by_npm,
            managed_by_bun,
    ...
    fn from_exe_with_anecdoct_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        ...
        if let Some(exe_path) = current_exe
            && let Some(standalone_context) = standalone_install_context(exe_path, anecdoct_home)
        {
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("ANECDOCT_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("ANECDOCT_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
fn standalone_install_context(
    ...
    let canonical_anecdoct_home = std::fs::canonicalize(anecdoct_home?).ok()?;
    let release_dir = canonical_exe.parent()?.to_path_buf();
    let releases_root = canonical_anecdoct_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        let anecdoct_home = tempfile::tempdir()?;
        let release_dir = anecdoct_home
            .path()
        ...
        let context = InstallContext::from_exe_with_anecdoct_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*anecdoct_home*/ Some(anecdoct_home.path()),
        );
        assert_eq!(
            context,
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_anecdoct_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/anecdoct")),
            /*managed_by_npm*/ true,
            /*managed_by_bun*/ false,
            /*anecdoct_home*/ None,
        );
        ...
        let bun_context = InstallContext::from_exe_with_anecdoct_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/anecdoct")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_anecdoct_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/anecdoct")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*anecdoct_home*/ None,
        );
