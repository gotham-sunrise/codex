...
use anecdoct_otel::StatsigMetricsSettings;
use anecdoct_windows_sandbox::LOG_FILE_NAME;
use anecdoct_windows_sandbox::SETUP_VERSION;
use anecdoct_windows_sandbox::SetupErrorCode;
use anecdoct_windows_sandbox::SetupErrorReport;
use anecdoct_windows_sandbox::SetupFailure;
use anecdoct_windows_sandbox::add_deny_write_ace;
use anecdoct_windows_sandbox::canonicalize_path;
use anecdoct_windows_sandbox::convert_string_sid_to_sid;
use anecdoct_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use anecdoct_windows_sandbox::ensure_allow_write_aces;
use anecdoct_windows_sandbox::extract_setup_failure;
use anecdoct_windows_sandbox::hide_newly_created_users;
use anecdoct_windows_sandbox::install_wfp_filters;
use anecdoct_windows_sandbox::is_command_cwd_root;
use anecdoct_windows_sandbox::log_note;
use anecdoct_windows_sandbox::path_mask_allows;
use anecdoct_windows_sandbox::sandbox_bin_dir;
use anecdoct_windows_sandbox::sandbox_dir;
use anecdoct_windows_sandbox::sandbox_secrets_dir;
use anecdoct_windows_sandbox::string_from_sid_bytes;
use anecdoct_windows_sandbox::sync_persistent_deny_read_acls;
use anecdoct_windows_sandbox::to_wide;
use anecdoct_windows_sandbox::workspace_write_cap_sid_for_root;
use anecdoct_windows_sandbox::workspace_write_root_overlaps_path;
use anecdoct_windows_sandbox::write_setup_error_report;
use anyhow::Context;
use anyhow::Result;
use base64::Engine;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(anecdoct_home) = std::env::var("ANECDOCT_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&anecdoct_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.anecdoct_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.anecdoct_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_anecdoct_app_runtime_bin_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for root in &payload.write_roots {
        ...
        let root_cap_sid_str =
            workspace_write_cap_sid_for_root(&payload.anecdoct_home, &payload.command_cwd, root)?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.anecdoct`, and `.agents`.
        //
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.anecdoct_home),
        &payload.real_user,
        &sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.anecdoct_home).display()
            ),
        ))
    })?;
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.anecdoct_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.anecdoct_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.anecdoct_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.anecdoct_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.anecdoct_home).join("sandbox_users.json");
...
mod tests {
    ...
    use anecdoct_otel::StatsigMetricsSettings;
    use anecdoct_windows_sandbox::load_or_create_cap_sids;
    use anecdoct_windows_sandbox::workspace_write_cap_sid_for_root;
    ...
    fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "AnecdoctSandboxOffline",
            "online_username": "AnecdoctSandboxOnline",
            "anecdoct_home": "C:\\anecdoct-home",
    ...
    fn deny_path_under_active_root_uses_only_matching_root_sid() {
        ...
        let anecdoct_home = temp.path().join("anecdoct-home");
        ...
        let stale_sid = workspace_write_cap_sid_for_root(&anecdoct_home, &workspace, &stale_root)
            .expect("stale sid");
        let active_sid = workspace_write_cap_sid_for_root(&anecdoct_home, &workspace, &active_root)
            .expect("active sid");
        let workspace_sid =
            workspace_write_cap_sid_for_root(&anecdoct_home, &workspace, &workspace)
                .expect("workspace sid");
        let caps = load_or_create_cap_sids(&anecdoct_home).expect("load caps");
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let anecdoct_home = temp.path().join("anecdoct-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".anecdoct");
        ...
        let workspace_sid =
            workspace_write_cap_sid_for_root(&anecdoct_home, &workspace, &workspace)
                .expect("workspace sid");
        let nested_sid = workspace_write_cap_sid_for_root(&anecdoct_home, &workspace, &nested_root)
            .expect("nested sid");
