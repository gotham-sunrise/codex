...
use crate::sandbox_utils::ensure_anecdoct_home_exists;
...
use crate::workspace_acl::protect_workspace_anecdoct_dir;
use anyhow::Context;
use anyhow::Result;
...
fn prepare_spawn_context_common(
    ...
    ensure_anecdoct_home_exists(anecdoct_home)?;
    let sandbox_base = anecdoct_home.join(".sandbox");
...
pub(crate) fn prepare_legacy_session_security(
    ...
    let caps = load_or_create_cap_sids(anecdoct_home)?;
    let (h_token, readonly_sid, readonly_sid_str, write_root_sids) = unsafe {
        match policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => {
                let write_root_sids = root_capability_sids(anecdoct_home, cwd, capability_roots)?;
...
pub(crate) fn root_capability_sids(
    ...
    for root in roots {
        let sid_str = workspace_write_cap_sid_for_root(anecdoct_home, cwd, &root)?;
    ...
    unsafe {
                ...
                matching_root_capability(current_dir, acl_sids.write_root_sids)
        {
            ...
            if is_command_cwd_root(&workspace_sid.root, &canonical_cwd) {
                let _ = protect_workspace_anecdoct_dir(current_dir, workspace_sid.sid.as_ptr());
...
pub(crate) fn prepare_elevated_spawn_context(
    ...
    let (psid_to_use, cap_sids) = match &common.policy {
        ...
        SandboxPolicy::WorkspaceWrite { .. } => {
            let cap_sids = root_capability_sids(anecdoct_home, cwd, effective_write_roots)?
                .into_iter()
...
mod tests {
    ...
    fn legacy_spawn_env_applies_offline_network_rewrite() {
        let anecdoct_home = TempDir::new().expect("tempdir");
    ...
    fn legacy_deny_path_includes_nested_active_root_sid() {
        ...
        let nested_sid = workspace_write_cap_sid_for_root(&anecdoct_home, &workspace, &nested_root)
            .expect("nested sid");
        let unrelated_sid =
            workspace_write_cap_sid_for_root(&anecdoct_home, &workspace, &unrelated_root)
    ...
    fn legacy_capability_roots_use_effective_write_roots() {
        ...
        let workspace = temp.path().join("workspace");
        let active_root = temp.path().join("active-root");
        let sandbox_root = anecdoct_home.join(".sandbox");
        std::fs::create_dir_all(&anecdoct_home).expect("create anecdoct home");
        std::fs::create_dir_all(&workspace).expect("create workspace");
        std::fs::create_dir_all(&active_root).expect("create active root");
        ...
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![
                ...
                AbsolutePathBuf::try_from(anecdoct_home.as_path()).expect("anecdoct home"),
        ...
        assert!(!roots.contains(&dunce::canonicalize(&anecdoct_home).expect("anecdoct home")));
