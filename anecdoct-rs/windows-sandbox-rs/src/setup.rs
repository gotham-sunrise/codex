...
pub const OFFLINE_USERNAME: &str = "AnecdoctSandboxOffline";
pub const ONLINE_USERNAME: &str = "AnecdoctSandboxOnline";
...
pub fn sandbox_dir(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(".sandbox")
}

pub fn sandbox_bin_dir(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(anecdoct_home: &Path) -> PathBuf {
    anecdoct_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(anecdoct_home: &Path) -> PathBuf {
    sandbox_dir(anecdoct_home).join("setup_marker.json")
}

pub fn sandbox_users_path(anecdoct_home: &Path) -> PathBuf {
    sandbox_secrets_dir(anecdoct_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, anecdoct_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        anecdoct_home: request.anecdoct_home.to_path_buf(),
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.anecdoct_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(request.anecdoct_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn gather_helper_read_roots(anecdoct_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(anecdoct_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(anecdoct_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    gather_legacy_full_read_roots(command_cwd, policy, anecdoct_home)
}
...
pub(crate) fn effective_write_roots_for_setup(
    ...
    filter_sensitive_write_roots(write_roots, anecdoct_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<anecdoct_otel::StatsigMetricsSettings>,
    real_user: String,
    #[serde(default)]
    refresh_only: bool,
}
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "ANECDOCT_NETWORK_ALLOW_LOCAL_BINDING";
...
fn find_setup_exe() -> PathBuf {
    ...
    {
        let candidate = dir.join("anecdoct-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }

        // Standalone installs keep Windows helper binaries under
        // `anecdoct-resources/` next to `anecdoct.exe`, so elevation needs to probe
        // that sibling folder before falling back to PATH.
        let resource_candidate = dir
            .join("anecdoct-resources")
            .join("anecdoct-windows-sandbox-setup.exe");
        if resource_candidate.exists() {
            return resource_candidate;
        }
    }
    PathBuf::from("anecdoct-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(anecdoct_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(anecdoct_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
                ...
                Some(&sandbox_dir(anecdoct_home)),
            );
            false
        }
    };
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(anecdoct_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(request.anecdoct_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: anecdoct_otel::global_statsig_metrics_settings(),
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.anecdoct_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.anecdoct_home);
        ...
        gather_read_roots(request.command_cwd, request.policy, request.anecdoct_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, anecdoct_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to ANECDOCT_HOME or anything under ANECDOCT_HOME/.sandbox,
    // ANECDOCT_HOME/.sandbox-bin, or ANECDOCT_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let anecdoct_home_key = canonical_path_key(anecdoct_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(anecdoct_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(anecdoct_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(anecdoct_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != anecdoct_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "ANECDOCT_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_anecdoct_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let anecdoct_home = user_profile.join("AnecdoctHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&anecdoct_home).expect("create anecdoct home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &anecdoct_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let anecdoct_home = tmp.path().join("anecdoct-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &anecdoct_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&anecdoct_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&anecdoct_home);
        fs::create_dir_all(&anecdoct_home).expect("create anecdoct home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_anecdoct_home =
            dunce::canonicalize(&anecdoct_home).expect("canonical anecdoct home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_anecdoct_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_anecdoct = extra_write_root.join(".anecdoct");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_anecdoct).expect("create extra .anecdoct");
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_anecdoct).expect("canonical extra .anecdoct"),
                explicit_deny,
            ]
            .into_iter()
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &anecdoct_home);
