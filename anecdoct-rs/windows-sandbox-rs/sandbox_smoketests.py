...
# Run a suite of smoke tests against the Windows sandbox via the Anecdoct CLI
...
def _resolve_anecdoct_cmd() -> List[str]:
    """Resolve the Anecdoct CLI to invoke `anecdoct sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Anecdoct.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "anecdoct.exe",
        ws_root / "target" / "release" / "anecdoct.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "anecdoct.exe",
            cargo_base / "release" / "anecdoct.exe",
        ])
    ...
    if shutil.which("anecdoct"):
        return ["anecdoct"]

    raise FileNotFoundError(
        "Anecdoct CLI not found. Build it first, e.g.\n"
        "  cargo build -p anecdoct-cli --release\n"
        "or for debug:\n"
        "  cargo build -p anecdoct-cli\n"
    )

ANECDOCT_CMD = _resolve_anecdoct_cmd()
print(ANECDOCT_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to anecdoct CLI overrides.
    ...
    argv = [*ANECDOCT_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"ANECDOCT_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\anecdoct_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.anecdoct artifacts) denied
    anecdoct_home = Path(os.environ["USERPROFILE"]) / ".anecdoct"
    cap_sid_target = anecdoct_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .anecdoct\\policy.json"], WS_ROOT)
    add("WS: .anecdoct cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .anecdoct policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > anecdoct_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://anecdoct-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
