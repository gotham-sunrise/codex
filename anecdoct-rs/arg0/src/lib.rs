...
use anecdoct_apply_patch::ANECDOCT_CORE_APPLY_PATCH_ARG1;
use anecdoct_exec_server::ANECDOCT_FS_HELPER_ARG1;
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "anecdoct-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Anecdoct executable for child re-execs.
    ///
    ...
    /// instead of the real Anecdoct CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime
            .block_on(anecdoct_shell_escalation::run_shell_escalation_execve_wrapper(file, argv));
        match exit_code {
    ...
    if exe_name == ANECDOCT_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        anecdoct_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        anecdoct_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == ANECDOCT_FS_HELPER_ARG1 {
        anecdoct_exec_server::run_fs_helper_main();
    }
    if argv1 == ANECDOCT_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match anecdoct_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                match runtime.block_on(anecdoct_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    anecdoct_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!(
                    "Error: {ANECDOCT_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument."
                );
                1
            }
        };
    ...
    match prepend_path_entry_for_anecdoct_aliases() {
        Ok(path_entry) => Some(path_entry),
        Err(err) => {
            // It is possible that Anecdoct will proceed successfully even if
...
/// While we want to deploy the Anecdoct CLI as a single executable for simplicity,
...
/// `anecdoct-linux-sandbox` we *directly* execute
/// [`anecdoct_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.anecdoct/.env` before creating any threads.
/// 2.  Construct a Tokio multi-thread runtime.
/// 3.  Capture the current executable path and derive the
///     `anecdoct-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        anecdoct_self_exe: current_exe.clone(),
        anecdoct_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `anecdoct-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().anecdoct_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "ANECDOCT_";

/// Load env vars from ~/.anecdoct/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `ANECDOCT_`.
fn load_dotenv() {
    if let Ok(anecdoct_home) = find_anecdoct_home()
        && let Ok(iter) = dotenvy::from_path_iter(anecdoct_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `ANECDOCT_` keys.
...
///   with the hidden `--anecdoct-run-as-apply-patch` flag.
///
...
pub fn prepend_path_entry_for_anecdoct_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if anecdoct_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (anecdoct_home: {anecdoct_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a ANECDOCT_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = anecdoct_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {ANECDOCT_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        anecdoct_self_exe: std::env::current_exe().ok(),
        anecdoct_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(ANECDOCT_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_anecdoct_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("anecdoct-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                anecdoct_self_exe: Some(PathBuf::from("/usr/bin/anecdoct")),
                anecdoct_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/anecdoct"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("anecdoct-helper-alias");
        ...
        super::build_runtime()?.block_on(run_main_with_arg0_guard(
            ...
            Some(PathBuf::from("/usr/bin/anecdoct")),
            |paths| async move {
