# anecdoct-exec-server

`anecdoct-exec-server` is the library backing `anecdoct exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`anecdoct-utils-pty`.

It provides:

- a CLI entrypoint: `anecdoct exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `anecdoct` binary owns hidden helper dispatch for sandboxed
filesystem operations and `anecdoct-linux-sandbox`.

## Transport

The server speaks the shared `anecdoct-app-server-protocol` message envelope on
the wire.
...
It uses the standard Anecdoct ChatGPT sign-in state; run `anecdoct login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `ANECDOCT_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Anecdoct then registers an Agent task and sends the
...
`anecdoct.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/anecdoct.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `anecdoct-utils-pty`.
...
hidden helper process launched from the top-level `anecdoct` executable and
...
`anecdoct exec-server` command builds these paths from the `anecdoct` arg0 dispatch
state. `RemoteExecutorConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from Anecdoct auth
