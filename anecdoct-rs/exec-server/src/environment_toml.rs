...
pub(crate) fn environment_provider_from_anecdoct_home(
    anecdoct_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = anecdoct_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "anecdoct exec-server --listen stdio".to_string(),
                    ]),
                    env: Some(HashMap::from([(
                        "ANECDOCT_LOG".to_string(),
                        "debug".to_string(),
                    )])),
                    ..Default::default()
                },
        ...
        let cases = [
            ...
            (
                EnvironmentToml {
                    id: "devbox".to_string(),
                    url: Some("ws://127.0.0.1:8765".to_string()),
                    program: Some("anecdoct".to_string()),
                    ..Default::default()
                },
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = anecdoct_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "anecdoct exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
ANECDOCT_LOG = "debug"
"#,
...
"#,
    ...
    #[tokio::test]
    async fn environment_provider_from_anecdoct_home_uses_present_environments_file() {
        let anecdoct_home = tempdir().expect("tempdir");
        std::fs::write(
            anecdoct_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
"#,
        )
        ...
        let provider = environment_provider_from_anecdoct_home(anecdoct_home.path())
    ...
    #[tokio::test]
    async fn environment_provider_from_anecdoct_home_falls_back_when_file_is_missing() {
        let anecdoct_home = tempdir().expect("tempdir");
