...
use crate::environment_toml::environment_provider_from_anecdoct_home;
...
pub const ANECDOCT_EXEC_SERVER_URL_ENV_VAR: &str = "ANECDOCT_EXEC_SERVER_URL";

/// Owns the execution/filesystem environments available to the Anecdoct runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `ANECDOCT_EXEC_SERVER_URL` behavior
/// while provider-based construction accepts a provider-supplied snapshot.
///
/// Setting `ANECDOCT_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `ANECDOCT_EXEC_SERVER_URL` and local runtime paths
    /// used when creating local filesystem helpers.
    pub async fn new(args: EnvironmentManagerArgs) -> Self {
        let EnvironmentManagerArgs {
            local_runtime_paths,
        } = args;
        let exec_server_url = std::env::var(ANECDOCT_EXEC_SERVER_URL_ENV_VAR).ok();
        Self::from_default_provider_url(exec_server_url, local_runtime_paths).await
    }

    /// Builds a manager from `ANECDOCT_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `ANECDOCT_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `ANECDOCT_EXEC_SERVER_URL` behavior.
    pub async fn from_anecdoct_home(
        anecdoct_home: impl AsRef<std::path::Path>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
        let provider = environment_provider_from_anecdoct_home(anecdoct_home.as_ref())?;
    ...
    /// reading user config files from `ANECDOCT_HOME`.
...
impl Environment {
    /// Builds an environment from the raw `ANECDOCT_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `ANECDOCT_EXEC_SERVER_URL` value and
...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = anecdoct_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            anecdoct_protocol::models::PermissionProfile::from_runtime_permissions(
                &anecdoct_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                anecdoct_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
