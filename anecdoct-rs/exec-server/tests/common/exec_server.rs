...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestAnecdoctHelperPaths,
    child: Child,
...
pub(crate) struct TestAnecdoctHelperPaths {
    pub(crate) anecdoct_exe: PathBuf,
    pub(crate) anecdoct_linux_sandbox_exe: Option<PathBuf>,
}

pub(crate) fn test_anecdoct_helper_paths() -> anyhow::Result<TestAnecdoctHelperPaths> {
    let (helper_binary, anecdoct_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestAnecdoctHelperPaths {
        anecdoct_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.anecdoct_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("ANECDOCT_HOME", anecdoct_home.path());
    child.envs(env);
