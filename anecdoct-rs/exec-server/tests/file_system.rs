...
use anecdoct_exec_server::LocalFileSystem;
use anecdoct_exec_server::ReadDirectoryEntry;
...
use common::exec_server::TestAnecdoctHelperPaths;
use common::exec_server::exec_server;
#[cfg(target_os = "linux")]
use common::exec_server::exec_server_with_env;
use common::exec_server::test_anecdoct_helper_paths;

struct FileSystemContext {
    file_system: Arc<dyn ExecutorFileSystem>,
    _helper_paths: Option<TestAnecdoctHelperPaths>,
...
async fn create_file_system_context(use_remote: bool) -> Result<FileSystemContext> {
    if use_remote {
        ...
        let helper_paths = test_anecdoct_helper_paths()?;
        let runtime_paths = ExecServerRuntimePaths::new(
            helper_paths.anecdoct_exe.clone(),
            helper_paths.anecdoct_linux_sandbox_exe.clone(),
        )?;
        Ok(FileSystemContext {
...
async fn file_system_sandboxed_write_allows_explicit_alias_roots(use_remote: bool) -> Result<()> {
    ...
    let tmp = tempfile::Builder::new()
        .prefix("anecdoct-fs-sandbox-alias-")
