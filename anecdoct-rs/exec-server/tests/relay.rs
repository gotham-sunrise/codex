...
#[path = "../src/proto/anecdoct.exec_server.relay.v1.rs"]
mod relay_proto;
...
use anecdoct_exec_server::InitializeParams;
use anecdoct_exec_server::InitializeResponse;
use anecdoct_exec_server::RemoteExecutorConfig;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use anyhow::bail;
...
fn static_registry_auth_provider() -> anecdoct_api::SharedAuthProvider {
...
async fn multiplexed_remote_executor_routes_independent_virtual_streams() -> Result<()> {
    ...
    let (anecdoct_exe, anecdoct_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(anecdoct_exe, anecdoct_linux_sandbox_exe)?;
    let config = RemoteExecutorConfig::new(
        registry.uri(),
        EXECUTOR_ID.to_string(),
        static_registry_auth_provider(),
    )?;
    let remote_executor = tokio::spawn(anecdoct_exec_server::run_remote_executor(
        config,
        runtime_paths,
    ));
