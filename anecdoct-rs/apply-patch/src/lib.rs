...
/// Special argv[1] flag used when the Anecdoct executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `anecdoct-apply-patch` (to avoid forcing
/// `anecdoct-arg0` to depend on `anecdoct-core`), it remains part of the "anecdoct core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const ANECDOCT_CORE_APPLY_PATCH_ARG1: &str = "--anecdoct-run-as-apply-patch";
