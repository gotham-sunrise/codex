...
/// Anecdoct conversation/session UUID.
...
pub type AnecdoctTurnId = String;
...
/// Process/session ID returned by Anecdoct's terminal runtime.
...
/// Canonical reduced graph for one Anecdoct rollout.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct RolloutTrace {
    pub schema_version: u32,
    /// Unique identity for this trace capture.
    ///
    /// `rollout_id` names the Anecdoct rollout/session being observed. `trace_id`
    ...
    pub anecdoct_turns: BTreeMap<AnecdoctTurnId, AnecdoctTurn>,
