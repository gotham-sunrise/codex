...
use crate::model::AnecdoctTurnId;
...
const INFERENCE_CALL_ID_HEADER: &str = "x-anecdoct-inference-call-id";
...
///
/// A Anecdoct turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Anecdoct turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        anecdoct_turn_id: AnecdoctTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                anecdoct_turn_id: attempt.context.anecdoct_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps anecdoct-core focused on
    ...
    /// Records a provider stream that Anecdoct intentionally stopped consuming.
    ///
...
/// Anecdoct received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        anecdoct_turn_id: Some(context.anecdoct_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.anecdoct_turn_id, "turn-1");
