...
impl TraceReducer {
    ...
    pub(super) fn start_inference_call(
        ...
        let anecdoct_turn_id = started.anecdoct_turn_id.clone();
        let request_payload = started.request_payload.clone();
        let Some(turn) = self.rollout.anecdoct_turns.get(&anecdoct_turn_id) else {
            bail!(
                "inference start {inference_call_id} referenced unknown anecdoct turn {anecdoct_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "inference start {inference_call_id} used thread {thread_id}, \
                 but anecdoct turn {anecdoct_turn_id} belongs to {}",
                turn.thread_id
            );
        }
    ...
    /// If a call is still `Running`, Anecdoct stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.anecdoct_turn_id == anecdoct_turn_id
