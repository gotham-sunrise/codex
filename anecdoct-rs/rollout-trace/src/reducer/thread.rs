...
impl TraceReducer {
    ...
    /// Starts a Anecdoct turn inside an existing thread.
    pub(super) fn start_anecdoct_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.anecdoct_turns.contains_key(&anecdoct_turn_id) {
            bail!("duplicate anecdoct turn start for {anecdoct_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.anecdoct_turns.insert(
    ...
    /// Marks a Anecdoct turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_anecdoct_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.anecdoct_turns.get(&anecdoct_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "anecdoct turn end for {anecdoct_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.anecdoct_turns.get_mut(&anecdoct_turn_id) else {
            bail!("anecdoct turn end referenced unknown turn {anecdoct_turn_id}");
        };
