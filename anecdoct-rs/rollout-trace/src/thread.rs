...
use crate::protocol_event::anecdoct_turn_trace_event;
...
pub const ANECDOCT_ROLLOUT_TRACE_ROOT_ENV: &str = "ANECDOCT_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `ANECDOCT_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Anecdoct
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(ANECDOCT_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Anecdoct turn lifecycle events from protocol lifecycle events.
    pub fn record_anecdoct_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            anecdoct_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Anecdoct did while executing that boundary.
    pub fn record_tool_call_event(
        &self,
        anecdoct_turn_id: impl Into<AnecdoctTurnId>,
        event: &EventMsg,
    ) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        ...
        context.append_with_context_best_effort(anecdoct_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_anecdoct_turn_id: impl Into<AnecdoctTurnId>,
    ...
    pub fn record_anecdoct_turn_started(&self, anecdoct_turn_id: impl Into<AnecdoctTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let anecdoct_turn_id = anecdoct_turn_id.into();
        context.append_with_context_best_effort(
            anecdoct_turn_id.clone(),
            RawTraceEventPayload::AnecdoctTurnStarted {
                anecdoct_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    pub fn start_code_cell_trace(
        ...
        let context = self.code_cell_trace_context(anecdoct_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one Anecdoct turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        anecdoct_turn_id: AnecdoctTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            anecdoct_turn_id: Some(anecdoct_turn_id),
        };
