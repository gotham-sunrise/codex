//! Mapping from Anecdoct protocol events into raw rollout-trace events.
//!
...
//! `anecdoct-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `anecdoct-rollout-trace`.
//!
...
use anecdoct_protocol::protocol::PatchApplyStatus;
use anecdoct_protocol::protocol::TurnAbortReason;
use serde::Serialize;

use crate::AgentThreadId;
use crate::AnecdoctTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct AnecdoctTurnTraceEvent {
    pub context_turn_id: AnecdoctTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn anecdoct_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<AnecdoctTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let anecdoct_turn_id = event.turn_id.clone();
            Some(AnecdoctTurnTraceEvent {
                context_turn_id: anecdoct_turn_id.clone(),
                payload: RawTraceEventPayload::AnecdoctTurnStarted {
                    anecdoct_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(AnecdoctTurnTraceEvent {
                context_turn_id: anecdoct_turn_id.clone(),
                payload: RawTraceEventPayload::AnecdoctTurnEnded {
                    anecdoct_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a anecdoct_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a anecdoct_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a anecdoct_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a anecdoct_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a anecdoct_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a anecdoct_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a anecdoct_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a anecdoct_protocol::protocol::CollabCloseEndEvent),
}
