...
remote_path='~/code/anecdoct-sync'
local_exec_server_port="${ANECDOCT_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${ANECDOCT_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/anecdoct-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/anecdoct-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/anecdoct-sync"
remote_anecdoct_rs="$remote_repo_root/anecdoct-rs"

cd "${remote_anecdoct_rs}"
cargo build -p anecdoct-cli --bin anecdoct

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/anecdoct exec-server --listen ws://127.0.0.1:0 \
...
echo "Start anecdoct via: "
printf '  ANECDOCT_EXEC_SERVER_URL=ws://127.0.0.1:%s anecdoct -C %q\n' \
